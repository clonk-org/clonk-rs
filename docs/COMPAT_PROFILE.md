...
| `sim-shared-bases` | reverted | With the Shared Bases rule (`SHBS`) in play, `FindBase` falls back to a non-hostile player's base after the asking player's own bases, so "back to base" and the wormhole reach an ally's base instead of failing when no own base remains. TeamAccount (`TACC`) continues to share wealth only. `C4Game::FindBase` matches `Base == iPlayer` exactly, with no team, alliance or rule test (`C4Game.cpp:3732-3745`), so this widens a lookup the oracle keeps strict. Own bases keep their existing priority and master-list order; allied bases follow in master-list order, and the relation is mutual `!Hostile`, matching the friendly relation `C4Game::FindFriendlyBase` applies to buying. Base lookup drives command AI, so the profile clears it and restores the exact owner match. **Product decision approved 2026-08-25**, recorded at `crates/clonk-engine/src/compat/objects.rs`. |
| `sim-navigation-ai` | reverted | Command AI for crew and Pathfinder walkers plans its own routes. A MoveTo goal is planned over the positions the actor's collision vertices can stand or swim at, joined by the moves the engine performs for it — walking over small steps, walking off a ledge and letting go of any wall grabbed on the way down, the ObjectComJump arc, scaling a wall to KneelUp, and swimming: in by a fall or a walk down a beach, out up a wall or onto a shore it corner-scales onto, within the actor's breath and never in a liquid that harms it (`C4Object.cpp:880-938,4375-4379,4938-4980`) — where C++ follows C4PathFinder's single-pixel route (`C4PathFinder.cpp:403-406`), which squeezes through gaps no body fits, crosses open air and runs up faces the actor cannot climb (clonk-org/clonk-rs#209). Planned waypoints and goals that expire before arrival fail, where C++ counts every expiry as success (`C4Command.cpp:1544-1552`), and a goal with no planned route tries the C4PathFinder route once before failing, so a parent command cannot retry an impossible route forever. Acquire takes the nearest candidates in order of planned route cost, skipping the one its last Get failed to reach and preferring any no other object is fetching, where C++ fetches the nearest (`C4Command.cpp:2108-2130`). A walking scaler under an intermediate fallback waypoint more than 40 pixels overhead also plans a side run and wall jump toward it; C++ leaves that waypoint inert — DFA_WALK steers only horizontally and JumpControl's high-angle arm accepts at most a 40-pixel gap (`C4Command.cpp:219-220,319-327,1874-1893`). All of it issues MoveTo, Jump and Get commands C++ never would, which is synchronized command and movement state. The profile reverts it through the `navigation_ai` session switch, which scenario activation installs on the engine that runs the round; an engine no session configured defaults to the oracle. **Product decision approved 2026-09-22**, recorded at `crates/clonk-app-core/src/config.rs`. |
...
| `ctrl-ready-check-toast-body` | kept | Clicking the ready-check notification *body* answers neither yes nor no: the port raises the window and leaves the prompt open and answerable. This is the one divergence here with nothing to revert *to*, because C++ disagrees with itself — libnotify registers the reserved `"default"` action against `Activated()`, which is `Close(true)` and broadcasts `C4PacketReadyCheck::Ready` (`C4ToastLibNotify.cpp:45`, `C4Network2.cpp:185-188`), while a WinRT body click still carries `IToastActivatedEventArgs` and so reaches `OnAction("")`, which is `Close(action == IDS_DLG_YES)` and broadcasts `NotReady` (`C4ToastWinRT.cpp:113-121`, `C4Network2.cpp:190-193`). The same gesture submits opposite answers on the two platforms, and the port declines to guess which one a player meant by a click that says only "come back to the game". The Yes and No buttons are untouched and answer exactly as they do natively. Owned by clonk-org/clonk-rs#1210. |
...
| `pres-client-scenario-art` | kept | Network clients replace the startup backdrop when the admitted scenario becomes available, using its embedded artwork or installed Origin folders, and retain it from the lobby through loading. C++ initializes the client loader before joining and leaves that selection unchanged. Only the image changes; loader state, font and GUI refresh timing, and synchronized simulation are unchanged. Owned by clonk-org/clonk-rs#1602. |
