...
| `sim-navigation-ai` | reverted | Command AI for crew and Pathfinder objects gets the port's navigation extensions. A walking scaler aligned under an intermediate pathfinder MoveTo waypoint more than 40 pixels overhead plans one native-height side run and wall jump toward it (clonk-org/clonk-rs#209). C++ leaves that waypoint inert — DFA_WALK steers only horizontally and JumpControl's high-angle arm accepts at most a 40-pixel gap (`C4Command.cpp:219-220,319-327,1874-1893`) — so the extension issues Jump and MoveTo commands C++ never would, which is synchronized command and movement state. The profile reverts it through the `navigation_ai` session switch, which scenario activation installs on the engine that runs the round; an engine no session configured defaults to the oracle. **Product decision approved 2026-09-22**, recorded at `crates/clonk-app-core/src/config.rs`. |
