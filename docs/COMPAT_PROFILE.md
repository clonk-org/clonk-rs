...
| `pres-render-inactive` | reverted | `Graphics.RenderInactive` defaults to both bits so an Alt-Tabbed game keeps drawing; C++ adapts `Console` alone. Only the default diverges — a written value is honoured verbatim, and `RenderInactive=2` restores C++ exactly. Owned by clonk-org/clonk-rs#57. |
| `pres-first-run-scale` | reverted | The first-run application scale is seeded from the monitor's pixel density; C++ starts every install at `Scale=100`, which on a 2x panel is an 800x600 device-pixel window with a 14px font. The profile reverts the seeding, so a compatibility install opens the window a native one does. The profile consulted there is the launch override alone, which is exact rather than a shortcut: the seeding runs only when no configuration file exists, so no persisted `General/CompatProfile` can be in play. |
