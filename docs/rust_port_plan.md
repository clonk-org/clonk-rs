...
- **P0 BLOCKERS:** AI command system still partial (Acquire/Buy persist across saves; Take/Take2 still missing)
- **Scenario Discovery:** ✅ Official content repo wired; menu now lists full scenario catalog
- **Current State:** Build compiles; workspace tests green; Build/Construct/Exit/Grab/UnGrab/Throw/Wait/Jump/Chop/Get/Sell/Context commands implemented in command stack
...
2. Implement remaining command types (Context/Sell/Take/Take2) and associated movement heuristics
...
| **AI Commands** | ⚠️ Partial | MoveTo/Follow/Enter/Exit/Build/Attack/Wait/Retry/Throw/Chop/Jump/Home + Acquire + Grab/UnGrab handle loose/container/cross-container pickups; Buy spawns base purchases and targeted hand-offs; Activate covers container release parity; PushTo mirrors C++ queue chaining with parity tests; Transfer now mirrors zone entry + ControlTransfer script hand-off with unit tests; remaining commands (Context/Sell/Take/Take2) still pending | TBD |
