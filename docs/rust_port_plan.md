...
3. **Frontend & IO**
  - Progress: `lc-frontend` renders the HUD overlay, keeps the camera locked to the focus object, now drives crew command input through the shared `PlayerInputState`, and the software renderer plots object silhouettes from engine vertex data; next up is replacing remaining SDL/GDI surfaces.
  - Implement rendering (software or wgpu/winit) to match C4 graphics, HUD, GUI widgets, and text output; unify font pipelines. Software overlay now respects engine vertex meshes; sprite atlas parity, lighting, and font layout still pending.
