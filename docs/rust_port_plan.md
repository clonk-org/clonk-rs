...
- Scenario manifests now provide per-scenario environment settings; configuring wind (with optional oscillation) feeds into the physics loop so horizontal drift matches manifest expectations, applying a scenario resets the environment to avoid bleed-through between runs, and manifests can now set ambient temperature for playback metadata.
...
- AUL-compatible host helpers now include `GetEffect`, allowing scripts to probe effect stacks (name, priority, interval, timer, and priority-filtered lookups) without reimplementing stack scans in script code.
- `AddEffect` and `RemoveEffect` host helpers now queue effect commands directly from scripts so effect lifecycles can be managed without manually returning effect descriptors; the helpers now capture command target objects and command IDs for compatibility with C4 scripts, though global effect scopes are still unsupported.
...
- Recreate the complete C4 object lifecycle, including the full action procedure system, physics integration beyond the new baseline gravity/velocity caps, scenario/environment management that extends past the new wind oscillation/temperature fields in the JSON loader, and player control/GUI pathways that match the shipping UI; cross-runtime savegame formats and animation graphs remain open. Owner ids and crew membership flags now propagate through the Rust engine with per-owner enumeration, but higher-level crew orchestration (selection UX, role assignment, elimination rules, and command routing) that the C++ runtime provides is still missing. (Action name/phase tracking, manifest-driven action maps with length/next transitions, procedure tags now flow through manifests to scripts while the engine applies baseline float/flight/hang physics adjustments, environment manifests can drive basic wind oscillation plus ambient temperature, effect stacks now exist as minimal scaffolds, scenario manifests can seed initial action phases and effect stacks per spawn, the command queue now covers deferred state updates and queued spawn/destroy requests, effect lifecycle callbacks now fire for start/timer/stop events, the physics layer now clamps horizontal velocity through the configurable `max_horizontal_speed`, and the new `EngineState` serialization restores deterministic engine state even though a production-ready save pipeline and animation graphs are still missing.)
- Port AUL runtime features: continue rounding out effect handlers and compatibility behaviors relied upon by shipped scripts (host helpers now cover `GetEffect`/`AddEffect`/`RemoveEffect` for object-local stacks, command target plumbing and ID metadata now mirror the C4 API, but global effect support and broader compatibility shims remain incomplete).
