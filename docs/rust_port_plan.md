...
1. **Platform Bootstrap**
  - Status: `lc-game` (Rust) now owns path discovery, prepares user directories, migrates legacy configs into `LC_CONFIG_FILE`, captures runtime stdout/stderr into timestamped logs, launches the shipping C++ runtime so `cargo run -p lc-game` opens the Startup Menu with live input, validates the bundled `c4group` updater, syncs fresh `Clonk*.log` files into the Rust logs directory, records runtime exit status breakdowns, copies crash dumps into the Rust logs, mirrors updater telemetry into the launcher log, emits a structured `launcher-summary.json`, produces `support-bundle-*.zip` archives with launcher/runtime logs, crash artifacts, telemetry, and a bundled `support-bundle-report.txt` that mirrors the CLI diagnostics; the new `lc-launcher` crate now feeds the graphical shell with summary/support bundle metadata, manual regeneration support, copy/share helpers, and upload artifact listings while the deprecated `lc-app` demo harness remains retired. `lc-launcher-ui` renders that shell state (summary, logs, telemetry) and wires regeneration, copy/share, and upload staging actions for the support bundle artifacts, and the `lc-launcher-shell` winit host now embeds that diagnostics screen with live regeneration/reveal handling. The diagnostics shell now uses OS-native pickers to stage copies/uploads, surfaces in-window success or error feedback for each action, automatically stages support bundles/artifacts into first-party share/upload drop targets, remembers the last picker destinations for fast follow-up transfers, surfaces first-party provider configuration/status directly in the diagnostics UI, emits submission-request payloads so staged bundles/artifacts drive automated provider workflows, records provider submission outcomes into `launcher-summary.json` so later regenerations and exported bundles retain automation history, hydrates persisted provider automation snapshots on launcher boot so the diagnostics shell restores the last recorded submission outcomes before any new staging, and now marks submissions as stale whenever staging directories go missing or move so the shell explains when past requests must be regenerated. Stale share/upload providers now surface in-line Restage and Retarget actions that re-stage support bundles or update provider drop locations, persist overrides in launcher preferences, and refresh automation snapshots without leaving the diagnostics shell. `launcher-summary.json` now captures provider override provenance (default paths plus retarget timestamps) so exported diagnostics preserve retargeted staging context alongside automation history. The diagnostics UI and the support bundle report now surface those default paths and override histories directly, so operators can reconcile staging directories without cracking open raw JSON. The support bundle report now mirrors the bulk retarget history-cleared annotation so exported diagnostics flag when staging bases were reset. Provider diagnostics now expose per-target "Restore default path"/"Clear saved overrides" controls that drop persisted overrides from launcher preferences, retarget providers back to their defaults, refresh automation snapshots, and log the reset so support bundles capture the revert. Bulk "Restore defaults" controls now reset every share/upload provider in one click, clear stored overrides, log aggregate outcomes, and refresh summaries/UI without manual intervention. Bulk retarget flow now repoints every share/upload directory in one action, updates launcher preferences, snapshots, and logs granular outcomes. The summary now records bulk retarget base directories with timestamped counts so exported diagnostics reflect staging bases without log diffing, those bases now surface in the diagnostics UI and support bundle report so operators can confirm staging expectations at a glance, bulk retarget resets now stamp a "history cleared" marker into `launcher-summary.json`, surface that annotation in the diagnostics UI, and log the event so support staff can see when staging bases were reset, bulk retarget history now prunes share/upload entries once every first-party provider returns to default paths so diagnostics never advertise stale staging bases, and the diagnostics UI now renders the support bundle report inline so operators can review the exported summary without extracting archives. The inline report preview now supports scrolling plus clipboard and file export so large diagnostics stay accessible inside the launcher, and it now includes inline search with error/warning presets, keyboard navigation, and highlighted matches so support can stay in-window while triaging failures. CLI automation consumers can now invoke `lc-game --automation-report` to receive the annotated triage summary as structured JSON.
  - The report search now retains the active filter across launcher refreshes and app restarts, exported reports append annotated match summaries so offline triage keeps the same cues, `launcher-summary.json` captures the active search query/highlight whenever filters change so automation hydrates identical views, automated support bundle reports consume those presets to append the same annotated triage summary, and the automation payload now mirrors those annotations for headless regression tooling.
  - Next: convert the automation payload into actionable exit codes/CI signals so regressions fail fast when triage summaries flag blocking errors without manual parsing.
