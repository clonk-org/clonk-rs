...
- Port AUL runtime features: round out effect handlers and compatibility behaviors relied upon by shipped scripts (`GetEffect` now mirrors the common lookup semantics, but AddEffect/RemoveEffect host helpers, command target plumbing, and broader compatibility shims remain incomplete).
