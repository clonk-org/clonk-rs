...
## Parity Gaps
- **Boot & Platform:** patcher/updater now probed by `lc-game` via a no-args `c4group` dry run, localization, logging/crash handling, launcher integration; configuration migration now handled by `lc-game` (copies legacy configs and honours `LC_CONFIG_FILE`).
...
- **Frontend & UI:** renderer, HUD, menus, cutscenes, text layout, input (mouse/keyboard/touch parity; gamepad wired in Rust harness), accessibility, editor dialogs.
...
- Headless parity harness validates the `basic_movement`, `queued_commands`, and `environment_cycle` recordings via `cargo xtask engine-snapshots verify`, and the same `fixtures::SNAPSHOT_SCENARIOS` suite now runs automatically under `cargo test -p lc-engine`, locking down environment drift/wind/time-of-day parity inside the default CI lane.
- **Gate:** headless Rust loop reproduces the deterministic regression pack byte-for-byte against C++ recordings.

3. **Frontend & IO**
  - Progress: `lc-frontend` renders the HUD overlay, keeps the camera locked to the focus object, drives crew command input through the shared `PlayerInputState`, plots object silhouettes from engine vertex data, feeds GUI text through the bitmap font pipeline, drives lighting from `EnvironmentSettings::time_of_day`, and mirrors liquid columns with temperature-aware blending. The `lc-app` harness opens a winit/pixels window, ticks `lc-engine`, and mirrors frames via `lc-frontend`; the Startup Menu is fully Rust, surfaces `lc-resources` scenarios, supports mouse/keyboard/touch/gamepad navigation with a Rust back-stack, launches real scenarios when manifests are present, and now maps gamepad D-pad/analog + confirm/back into both menu actions and live crew control through `InputDispatcher`. Escape and gamepad Back return directly to the Startup Menu without losing the folder stack, so scenario runs can round-trip through the Rust UI. Next up is replacing the remaining SDL/GDI surfaces.
  ...
  - Port the full input stack and GUI/dialog system (lobbies, editor, in-game menus) beyond the preview harness; connect audio backends for music/effects parity.
