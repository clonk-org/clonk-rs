...
- Script and engine API coverage: Hundreds of AUL functions, proplist operations, callback hooks, effect priorities, overlay rendering, and global state mutations need Rust equivalents with identical call ordering and edge cases. (Rust host layer now forwards `GetOwner`/`SetOwner`, `GetX`/`GetY`, and `SetXDir`/`SetYDir`/`GetXDir`/`GetYDir`, wiring owner queries, assignments, positional reads, and velocity control through `lc-engine` for parity.)
