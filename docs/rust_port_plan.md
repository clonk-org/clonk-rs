...
2. **Runtime Authority**
  ...
  - Mirror every AUL call/effect hook into Rust; `ObjectCount` and `ObjectDistance` now execute fully on the Rust side with tests guarding owner/definition filtering and positional parity, and we continue to extend coverage driven by replay fixtures.
