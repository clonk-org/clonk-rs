...
- **Phase 0 · Parity Harness Expansion**
  - Drive the shipping client through `LC_RUST_ENGINE_RUNTIME` for full matches, capturing snapshots, I/O, particles, and HUD state. (Snapshots now embed per-frame particle state alongside I/O, HUD capture records per-owner focus/crew panels, the Rust parity harness now fails when control logs diverge, runtime comparisons include back-buffer hashes, the runtime now applies player control packets to update crew command directions before each simulated frame, and parity runs can rehydrate state from `LC_RUST_ENGINE_RUNTIME_STATE_IMPORT` before kickoff to resume full matches.)
