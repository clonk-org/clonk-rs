...
- `AddEffect` and `RemoveEffect` host helpers now queue effect commands directly from scripts so effect lifecycles can be managed without manually returning effect descriptors; command target/id parameters remain placeholders and global effect scopes are still unsupported.
