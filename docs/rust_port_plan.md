...
1. **Platform Bootstrap**
  ...
  - The report search now retains the active filter across launcher refreshes and app restarts, exported reports append annotated match summaries so offline triage keeps the same cues, `launcher-summary.json` captures the active search query/highlight whenever filters change so automation hydrates identical views, and automated support bundle reports consume those presets to append the same annotated triage summary.
  - Next: surface the annotated triage summary through the CLI automation endpoints so regression bots can flag failing logs without launching the UI.
