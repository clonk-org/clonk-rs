...
1. **Platform Bootstrap**
  ...
  - The report search now retains the active filter across launcher refreshes and app restarts, exported reports append annotated match summaries so offline triage keeps the same cues, and `launcher-summary.json` captures the active search query/highlight whenever filters change so automation hydrates identical views.
  - Next: teach the diagnostics tooling to consume the stored report-search presets when generating automated triage views.
