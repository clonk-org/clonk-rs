...
enum SurfaceAcquisitionIssue {
    Suboptimal,
    Outdated,
}

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
enum SurfaceAcquisitionAction {
    Reconfigure,
    UseFrame,
    SkipFrame,
}

const fn surface_acquisition_action(
    issue: SurfaceAcquisitionIssue,
    reconfigured: bool,
) -> SurfaceAcquisitionAction {
    match (issue, reconfigured) {
        (_, false) => SurfaceAcquisitionAction::Reconfigure,
        (SurfaceAcquisitionIssue::Suboptimal, true) => SurfaceAcquisitionAction::UseFrame,
        (SurfaceAcquisitionIssue::Outdated, true) => SurfaceAcquisitionAction::SkipFrame,
    }
}

fn acquire_surface_texture(
    mut acquire: impl FnMut() -> CurrentSurfaceTexture,
    ...
    loop {
        match acquire() {
            CurrentSurfaceTexture::Success(surface_texture) => {
                return Ok(Some(surface_texture));
            }
            CurrentSurfaceTexture::Suboptimal(surface_texture) => {
                match surface_acquisition_action(SurfaceAcquisitionIssue::Suboptimal, reconfigured)
                {
                    SurfaceAcquisitionAction::Reconfigure => {
                        // Drop the acquired texture before reconfiguration; keeping it alive makes
                        // wgpu reject the configure call. See
                        // https://github.com/parasyte/pixels/issues/450.
                        drop(surface_texture);
                        reconfigure();
                        reconfigured = true;
                    }
                    SurfaceAcquisitionAction::UseFrame => {
                        return Ok(Some(surface_texture));
                    }
                    SurfaceAcquisitionAction::SkipFrame => return Ok(None),
                }
            }
            CurrentSurfaceTexture::Outdated => {
                match surface_acquisition_action(SurfaceAcquisitionIssue::Outdated, reconfigured) {
                    SurfaceAcquisitionAction::Reconfigure => {
                        reconfigure();
                        reconfigured = true;
                    }
                    SurfaceAcquisitionAction::UseFrame | SurfaceAcquisitionAction::SkipFrame => {
                        return Ok(None);
                    }
                }
            }
            CurrentSurfaceTexture::Lost => return Err(Error::SurfaceLost),
            CurrentSurfaceTexture::Occluded | CurrentSurfaceTexture::Timeout => return Ok(None),
...
impl<'win> Pixels<'win> {
    ...
    {
        let frame = acquire_surface_texture(
...
mod tests {
    ...
    #[test]
    fn outdated_surface_acquisition_reconfigures_only_once() {
        ...
        .expect("an outdated surface skips after its bounded retry");
    ...
    #[test]
    fn lost_surface_is_returned_without_reconfiguration() {
        let acquisitions = Cell::new(0);
        let reconfigurations = Cell::new(0);

        let error = acquire_surface_texture(
            || {
                acquisitions.set(acquisitions.get() + 1);
        ...
        .expect_err("a lost surface must be recreated by the caller");

        assert!(matches!(error, Error::SurfaceLost));
        assert_eq!(acquisitions.get(), 1);
        assert_eq!(reconfigurations.get(), 0);
    }

    #[test]
    fn suboptimal_surface_is_used_after_one_reconfiguration() {
        assert_eq!(
            surface_acquisition_action(SurfaceAcquisitionIssue::Suboptimal, false),
            SurfaceAcquisitionAction::Reconfigure
        );
        assert_eq!(
            surface_acquisition_action(SurfaceAcquisitionIssue::Suboptimal, true),
            SurfaceAcquisitionAction::UseFrame
        );
    }
}
