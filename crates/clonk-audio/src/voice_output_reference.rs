...
impl OutputReference {
    pub(crate) fn new(sample_rate: u32) -> Arc<Self> {
        Arc::new(Self {
            ...
            coefficients: reference_coefficients(sample_rate),
...
fn reference_coefficients(sample_rate: u32) -> Box<[[f32; TAPS]]> {
    let cutoff = if sample_rate <= crate::VOICE_SAMPLE_RATE {
        0.5
    } else {
        0.48 * f64::from(crate::VOICE_SAMPLE_RATE) / f64::from(sample_rate)
    };
    (0..PHASES)
        .map(|phase| {
