...
use clonk_script::{c4_string_from_bytes, Engine, Value};
...
eval_test! { string_comparison_is_case_sensitive_and_uses_raw_text {
    "func Test() { return \"a\" S= \"a\"; }" => Value::Bool(true);
    "func Test() { return \"a\" eq \"A\"; }" => Value::Bool(false);
} }
...
eval_cases! {
    strict_two_treats_adjacent_s_operators_as_identifier_s:
        "#strict 2\nfunc Test() { var S; S=1; return [S, S!=2, S<5]; }" =>
        Value::Array(vec![Value::Int(1), Value::Bool(true), Value::Bool(true)]);
}

eval_test! { textual_ne_compares_nil_as_empty_string {
    "func Test() { var empty; return empty ne \"x\"; }" => Value::Bool(true);
    "func Test() { var empty; return empty ne \"\"; }" => Value::Bool(false);
} }
