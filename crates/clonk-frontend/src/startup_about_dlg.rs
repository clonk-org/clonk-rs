...
/// The notices compiled into Clonk Rust: this source tree's own, then the
/// terms of every package linked into the shipped binaries.
///
/// The MIT entry applies to source only; bundled game content retains its
/// separate CC BY-NC notices, and the file-type icons recovered from the C++
/// engine keep the ISC notice in `clonk-icon/res/COPYING`.
///
/// The second entry is the dependency corpus C++ builds from
/// `deps/licenses.cmake`. It is generated from the release dependency graph by
/// `scripts/generate_dependency_licenses.py` rather than maintained by hand,
/// because it has to follow that graph exactly -- a dependency added, removed
/// or bumped changes what must be attributed, and a hand-written list is wrong
/// long before anyone notices. Packages that ship identical text share one
/// section, which is what keeps 549 packages' 4.3 MiB of text down to the
/// distinct 909 KiB that actually has to travel with the binary.
pub static ABOUT_LICENSES: [AboutLicense; 2] = [
    ...
    AboutLicense {
        ...
        license_title: "",
        text: include_str!("dependency_licenses.txt"),
    },
];
...
mod tests {
    ...
    fn baseline_licenses_preserve_legal_text_under_the_product_name() {
        ...
        // The port asserts no trademark of its own on this page. Scoped to the
        // port's own notice rather than every entry: the dependency corpus
        // reproduces third-party texts verbatim, and Apache-2.0 carries its own
        // Trademarks clause, which is theirs to state and not ours to edit.
        assert!(!ABOUT_LICENSES[0].text.contains("trademark"));
    }

    #[test]
    fn the_dependency_corpus_attributes_the_shipped_graph_verbatim() {
        // Generated by scripts/generate_dependency_licenses.py; the Python
        // gate checks it against Cargo.lock. What matters here is that it
        // reaches the dialog at all, and reaches it whole.
        let corpus = ABOUT_LICENSES[1];
        assert_eq!(license_display_title(&corpus), "Third-party packages");
        assert!(corpus.text.starts_with("Third-party licenses"));

        // Grouping identical texts is what keeps this shippable: without it the
        // same Apache-2.0 text would appear hundreds of times.
        let sections = corpus
            .text
            .lines()
            .filter(|line| line.starts_with("Applies to: "))
            .count();
        assert!(sections > 0, "the corpus has to carry license sections");
        let packages = corpus
            .text
            .lines()
            .find_map(|line| line.strip_prefix("Packages: "))
            .and_then(|count| count.parse::<usize>().ok())
            .expect("the corpus states how many packages it covers");
        assert!(
            packages > sections,
            "packages sharing a text must share a section: {packages} packages, \
             {sections} sections"
        );

        // Verbatim means verbatim: the Apache-2.0 clause that the port's own
        // notice must not contain is reproduced here because its authors wrote
        // it.
        assert!(corpus.text.contains("6. Trademarks."));
        assert!(corpus
            .text
            .contains("Permission is hereby granted, free of charge"));
    }
