...
/// Ordinary text stays at scale one, so this small facet path is kept local
/// instead of changing the shared font renderer.
fn draw_scaled_caret(
    ...
    if glyph.pixels.len() != width as usize * height as usize {
        return;
    }
    ...
    for y in 0..height as usize {
        for x in 0..width as usize {
            let pixel = glyph.pixels[y * width as usize + x];
            let destination = (y * atlas_width as usize + x) * 4;
