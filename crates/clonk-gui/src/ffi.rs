...
/// The font the bridge measures text with; the differential's Rust side lays out
/// with the same one so both views agree on every rectangle.
pub fn default_font() -> Arc<dyn TextFont> {
...
fn read_widget(gui: &Gui, raw_id: u32) -> Option<WidgetId> {
    let widget = WidgetId::new(raw_id as usize);
