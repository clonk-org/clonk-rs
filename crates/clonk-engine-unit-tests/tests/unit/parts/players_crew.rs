fn players_test_engine(seed: u64) -> Engine {
    let mut engine = Engine::with_seed(seed);
    engine.register_test_definition(build_definition());
    engine
}

fn test_crew_definition(mut definition: Definition) -> Definition {
    definition.set_crew_member(true);
    definition
}

fn players_crew_test_engine(seed: u64) -> Engine {
    let mut engine = Engine::with_seed(seed);
    let definition = test_crew_definition(build_definition());
    engine.register_test_definition(definition);
    engine
}

fn test_owned_config(definition: &str, owner: i32) -> SpawnConfig {
    SpawnConfig::new(definition).with_owner(owner)
}

fn test_object_config(definition: &str) -> SpawnConfig {
    SpawnConfig::new(definition).with_category(CATEGORY_OBJECT)
}

fn test_contained_object_config(definition: &str, container: ObjectId) -> SpawnConfig {
    test_object_config(definition).with_container(container)
}

fn test_crew_config(definition: &str, owner: i32) -> SpawnConfig {
    test_owned_config(definition, owner)
        .with_alive(true)
...
fn crew_members_enumerates_owned_crew() {
    ...
    let crew_owner_one = engine.spawn_test_object(test_crew_config("Test", 1));
    let crew_owner_two = engine.spawn_test_object(test_crew_config("Test", 2));
    engine.spawn_test_object(test_owned_config("Test", 1).with_crew_member(false));
...
fn select_crew_tracks_selection_and_cursor() {
    ...
    let first = engine.spawn_test_object(test_crew_config("Test", 1));
    let second = engine.spawn_test_object(test_crew_config("Test", 1));
...
fn selected_is_persisted_on_the_object_like_cpp() -> Result<(), EngineError> {
    ...
    let mut engine = players_crew_test_engine(0);
    engine.register_test_player(PlayerConfig::new(1, "Selector"));
    let crew = engine.spawn_test_object(test_crew_config("Test", 1));
...
"#;
    ...
    let definition =
        test_crew_definition(Definition::from_script("SELC", "Select callback", script)?);
    engine.register_test_definition(definition);
    engine.register_test_player(PlayerConfig::new(1, "Selector"));
    let a = engine.spawn_test_object(test_crew_config("SELC", 1));
    let b = engine.spawn_test_object(test_crew_config("SELC", 1));
...
"#;
    let mut engine = Engine::with_seed(0);
    let crew_definition = test_crew_definition(Definition::from_script("MAGE", "Mage", script)?);
    engine.register_test_definition(crew_definition);
    engine.register_test_definition(Definition::from_script("HELP", "Combo helper", script)?);
    engine.register_test_player(PlayerConfig::new(1, "Mage"));
    let mage = engine.spawn_test_object(test_crew_config("MAGE", 1));
    let helper = engine.spawn_test_object(test_owned_config("HELP", 1).with_alive(true));
    ...
    let helper_index = engine.test_object_index(helper);
...
"#;
    let mut engine = Engine::with_seed(0);
    let definition = test_crew_definition(Definition::from_script("DSBL", "Disable crew", script)?);
    engine.register_test_definition(definition);
    engine.register_test_player(PlayerConfig::new(1, "Selector"));
    let a = engine.spawn_test_object(test_crew_config("DSBL", 1));
    let b = engine.spawn_test_object(test_crew_config("DSBL", 1));
    engine.select_crew(1, [a])?;
    engine.set_crew_cursor(1, Some(a))?;
    let call = |engine: &mut Engine, id: ObjectId, name: &str, args: Vec<Value>| {
        let index = engine.test_object_index(id);
...
"#;
    let mut engine = Engine::with_seed(0);
    let definition = test_crew_definition(Definition::from_script(
        "SAFE",
        "Fail-safe selection",
        script,
    )?);
...
fn register_player_populates_snapshot_state() -> Result<(), EngineError> {
    ...
    engine.register_test_player(PlayerConfig::new(1, "Alice").with_wealth(75));
    let definition = test_crew_definition(Definition::from_script(
        "Walker",
        "Walker",
        PASSIVE_PLAYER_SCRIPT,
    )?);
    engine.register_test_definition(definition);
    let crew =
...
fn player_asset_value_accounts_for_owned_objects() -> Result<(), EngineError> {
    ...
    engine.spawn_test_object(test_owned_config("Ore", 1));
...
fn player_asset_value_uses_cpp_get_value_chain_on_tick35() -> Result<(), EngineError> {
    ...
    let crew = test_crew_definition(Definition::from_script("Crew", "Crew", "#strict 2\n")?);
    engine.register_test_definition(crew);
    engine.spawn_test_object(test_crew_config("Crew", 1));

    let mut half_built = Definition::from_script("HalfBuilt", "Half-built", "#strict 2\n")?;
    half_built.set_value(101);
    half_built.set_category(CATEGORY_STRUCTURE);
    engine.register_test_definition(half_built);
    engine.spawn_test_object(test_owned_config("HalfBuilt", 1).with_construction(FULL_CON / 2));
...
"#,
    ...
    engine.register_test_definition(object_override);
    let value_probe = engine.spawn_test_object(test_owned_config("ObjectOverride", 1));
...
"#,
    ...
    engine.register_test_definition(definition_override);
    engine.spawn_test_object(test_owned_config("DefinitionOverride", 1));
...
"#,
    ...
    engine.register_test_definition(mirror);
    engine.spawn_test_object(test_owned_config("ValueMirror", 1));
...
#[test]
fn calc_value_created_later_master_link_is_valued_in_the_same_pass() -> Result<(), EngineError> {
    let mut engine = Engine::new();
...
"#,
    ...
    engine.spawn_test_object(test_owned_config("PARN", 1));
...
fn player_file_loads_player_and_crew_extra_data_into_join_state() -> Result<(), EngineError> {
    ...
    std::fs::create_dir_all(&profile_path).test_value();

    let mut engine = Engine::with_seed(0);
    engine.register_test_definition(simple_definition("MARK"));
    let marker = engine.spawn_test_object(SpawnConfig::new("MARK"));
    ...
    std::fs::create_dir_all(&crew_path).test_value();
    std::fs::write(
    ...
    std::fs::create_dir_all(&malformed_path).test_value();
    std::fs::write(
    ...
    let group = clonk_resources::Group::open(&profile_path).test_value();
    ...
    std::fs::create_dir_all(&malformed_player_path).test_value();
    std::fs::write(
...
fn player_lifecycle_restore_reapplies_autostop_to_inactive_crew() -> Result<(), EngineError> {
    ...
    let mut engine = Engine::new();
    let definition = test_crew_definition(simple_definition("REST"));
    engine.register_test_definition(definition);
    let crew = engine.spawn_test_object(
        test_owned_config("REST", 1)
            .with_status(ObjectStatus::Inactive)
            .with_command_direction(CommandDirection::Right),
    );
    engine.register_test_player(PlayerConfig::new(1, "Saved"));
    {
...
#[test]
fn player_lifecycle_repeated_surrender_does_not_restart_retire_delay() -> Result<(), EngineError> {
...
fn player_lifecycle_fresh_autostop_clears_only_owned_inactive_crew_definitions(
    ...
    let crew_definition = test_crew_definition(simple_definition("AUTO"));
    join_engine.register_test_definition(crew_definition);
    join_engine.register_test_definition(simple_definition("ITEM"));

    let owned_inactive_crew = join_engine.spawn_test_object(
        test_owned_config("AUTO", 0)
            .with_status(ObjectStatus::Inactive)
            .with_command_direction(CommandDirection::Right),
    );
    let owned_active_crew = join_engine.spawn_test_object(
        test_owned_config("AUTO", 0).with_command_direction(CommandDirection::Right),
    );
    let foreign_inactive_crew = join_engine.spawn_test_object(
        test_owned_config("AUTO", 9)
            .with_status(ObjectStatus::Inactive)
            .with_command_direction(CommandDirection::Right),
    );
    let owned_inactive_noncrew = join_engine.spawn_test_object(
        test_owned_config("ITEM", 0)
            .with_status(ObjectStatus::Inactive)
            .with_command_direction(CommandDirection::Right),
    );
    ...
    let crew_definition = test_crew_definition(simple_definition("AUTO"));
    register_engine.register_test_definition(crew_definition);
    let registered_inactive_crew = register_engine.spawn_test_object(
        test_owned_config("AUTO", 7)
            .with_status(ObjectStatus::Inactive)
...
fn player_lifecycle_review_final_init_preserves_or_derives_cursor_in_cpp_order(
    ...
    let join = |initialize_body: &str| -> Result<(Engine, i32, ObjectId, ObjectId), EngineError> {
        let mut engine = Engine::with_seed(0);
        for id in ["HIGH", "LOWR"] {
            let definition = test_crew_definition(simple_definition(id));
            engine.register_test_definition(definition);
        }
        let mut start = PlayerStart::default();
        ...
        let player = engine
            ...
            .number();
        let crew = engine.player(player).test_value().crew();
        let low = crew
            .iter()
            .copied()
            .find(|id| {
...
#[test]
fn player_lifecycle_review_team_homebase_sync_follows_initialize_player() -> Result<(), EngineError>
{
...
"#;
    let mut engine = Engine::with_seed(0);
    engine.register_test_definition(simple_definition("BRCK"));
    engine.register_test_definition(simple_definition("ORE1"));
...
fn player_lifecycle_view_delays_arm_only_at_cpp_boundaries_and_decay_in_the_same_execute(
    ...
    engine.register_test_definition(valuable);
    engine.install_scenario_script_with_convention(
        "Score delay",
        "#strict 2\nglobal func AwardScore(int player) { return DoScore(player, 0); }\n",
        true,
    )?;
    engine.register_test_player(PlayerConfig::new(1, "Valuer"));
    engine.spawn_test_object(test_owned_config("VALU", 1));
...
"#,
    ...
    crew_definition.set_crew_member(true);
    engine.register_test_definition(crew_definition);
    engine.register_test_player(PlayerConfig::new(1, "Buffered input"));
    let crew = engine.spawn_test_object(test_crew_config("LCOM", 1));
    engine.select_crew(1, [crew])?;
...
fn player_lifecycle_select_count_is_a_saved_cache_refreshed_at_player_execute(
    ...
    let mut engine = Engine::with_seed(0);
    let crew_definition = test_crew_definition(simple_definition("SCNT"));
    engine.register_test_definition(crew_definition.clone());
    engine.register_test_player(PlayerConfig::new(1, "Selector"));
...
fn player_lifecycle_startup_hint_clears_through_object_com_and_object_command(
    ...
    let mut engine = Engine::with_seed(0);
    let crew_definition = test_crew_definition(simple_definition("HINT"));
    engine.register_test_definition(crew_definition);
    engine.register_test_player(PlayerConfig::new(1, "Hints"));
...
fn player_lifecycle_captain_assigns_in_final_init_round_trips_and_clears_with_object(
    ...
    let mut engine = Engine::with_seed(0);
    let kilc_definition = test_crew_definition(simple_definition("KILC"));
    engine.register_test_definition(kilc_definition.clone());
    let kilc = engine.spawn_test_object(test_crew_config("KILC", 0));
    let joined = engine
...
fn player_lifecycle_crew_created_counts_new_info_but_not_loaded_info_reuse(
    ...
    let setup = |engine: &mut Engine| -> Result<(), EngineError> {
        let crew_definition = test_crew_definition(simple_definition("CRNW"));
        engine.register_test_definition(crew_definition);
        let mut start = PlayerStart::default();
...
#[test]
fn shipped_hazard_shuttle_scores_both_driver_owner_transfers_in_order() -> Result<(), EngineError> {
    ...
    let group = clonk_resources::Group::open(content.join("Hazard.c4d/Vehicles.c4d/Shuttle.c4d"))
        .test_value();
    let resource = ResourceDefinitionData::load(&group).test_value();

    let mut engine = Engine::new();
    engine.register_test_player(PlayerConfig::new(0, "Owner").with_points(99_950));
    engine.register_test_definition(simple_definition("WEPN"));
    engine.register_test_definition(Definition::from_resource(&resource)?);
    engine.register_test_definition(simple_definition("CLNK"));
    engine.resolve_includes()?;

    let shuttle = engine.spawn_test_object(test_owned_config("SHTL", 0).with_loaded(true));
    let driver = engine.spawn_test_object(test_owned_config("CLNK", 0));
    let shuttle_index = engine.test_object_index(shuttle);
    engine.call_object_function(
        shuttle_index,
        "DriverIsOwner",
...
fn player_cursor_tracks_selection_changes() -> Result<(), EngineError> {
    let mut engine = Engine::new();
    engine.register_test_player(PlayerConfig::new(1, "Cursor"));
    let mut definition =
        Definition::from_script("CursorCrew", "CursorCrew", PASSIVE_PLAYER_SCRIPT)?;
    definition.set_crew_member(true);
    engine.register_test_definition(definition);
    let crew = engine.spawn_test_object(
        test_owned_config("CursorCrew", 1)
            .with_crew_member(true)
...
fn select_crew_rejects_wrong_owner() {
    ...
    let owned = engine.spawn_test_object(test_crew_config("Test", 1));
    let other_owner = engine.spawn_test_object(test_crew_config("Test", 2));

    engine.select_crew(1, vec![owned]).test_value();
...
fn apply_command_uses_engine_order_for_selection() {
            ...
            if name == "OnIdleAbort" || name == "OnWalkStart" {
                if let Some(Value::Proplist(state)) = args.first() {
                    if let Some(Value::Int(id)) = state.get("id") {
                        call_log.lock().test_value().push((name.to_string(), *id));
                    }
                }
            }
        });
    ...
    let mut definition = Definition::from_script("Crew", "Crew", script).test_value();
    ...
    let first = engine.spawn_test_object(test_crew_config("Crew", 1).with_id(ObjectId::new(200)));
    let second = engine.spawn_test_object(test_crew_config("Crew", 1).with_id(ObjectId::new(100)));
    ...
    let log = call_log.lock().test_value().clone();
...
fn capture_state_preserves_crew_roles() {
    ...
    let restored_definition = test_crew_definition(build_definition());
...
fn shipped_hazard_teams_do_evaluation_records_both_player_lines() {
    ...
    let content = std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("../../content");
    let group =
        clonk_resources::Group::open(content.join("Hazard.c4d/Goals.c4d/Teams.c4d")).test_value();
    let resource = ResourceDefinitionData::load(&group).test_value();
...
fn shipped_hazard_chooser_selects_the_lowest_client_id() {
    ...
    let get_x_val = system_group.read_file("GetXVal.c").test_value();
    let get_x_val = String::from_utf8_lossy(&get_x_val).into_owned();
    let chooser_group =
        clonk_resources::Group::open(repository.join("content/Hazard.c4d/Rules.c4d/Chooser.c4d"))
            .test_value();
    let chooser_resource = ResourceDefinitionData::load(&chooser_group).test_value();
...
fn legacy_state_without_round_results_restores_cpp_defaults() {
    ...
    let mut value =
        serde_json::to_value(state).unwrap_or_else(|error| panic!("state serializes: {error}"));
    value
        .as_object_mut()
...
fn engine_state_from_snapshot_allows_resuming_simulation() {
    ...
    let mut restored = players_test_engine(1234);
    restored.restore_state(&state).test_value();
...
fn crew_elimination_marks_owner_after_last_crew_destroyed() {
    let mut engine = Engine::with_seed(0);
    let mut definition = test_crew_definition(build_definition());
...
fn crew_elimination_is_one_way_like_cpp() {
    ...
    let owner = engine.spawn_test_object(test_crew_config("Test", 1));
...
fn capture_state_preserves_crew_selection() {
    ...
    let mut restored = players_crew_test_engine(5);
    restored.restore_state(&state).test_value();
...
fn capture_state_preserves_elimination_status() {
    ...
    let eliminated = engine.spawn_test_object(test_crew_config("Test", 1));
    engine.spawn_test_object(test_crew_config("Test", 2));
...
fn transfer_zone_set_for_a_vanished_owner_drops_instead_of_aborting() {
    ...
    // regression class).
    let mut engine = players_test_engine(0);
    engine
...
fn capture_state_preserves_transfer_zones() {
    let mut engine = players_test_engine(0);
    let object_id = engine.spawn_test_object(SpawnConfig::new("Test"));
...
fn tracks_action_state_changes() {
    ...
    let mut definition = Definition::from_script("Actor", "Actor", source).test_value();
    let mut actions = HashMap::new();
...
fn spawns_additional_objects_from_step() {
    let source = r#"#strict 3
        ...
        "#;
    let mut engine = Engine::with_seed(0);
    engine.register_test_definition(Definition::from_script("Test", "Test", source).test_value());
...
fn produces_deterministic_snapshots() {
    let source = r#"#strict 3
        ...
        "#;
    let definition = Definition::from_script("Mover", "Mover", source).test_value();

    let mut engine_a = Engine::with_seed(7);
    engine_a.register_test_definition(definition);
...
fn pixel_less_landscape_does_not_clamp_objects_to_surface() {
    ...
    let script = r#"
        ...
        "#;
    let mut definition = Definition::from_script("Static", "Static", script).test_value();
...
fn applies_effect_stack_operations() {
    ...
    let mut engine = Engine::with_seed(0);
    let definition = Definition::from_script("Actor", "Actor", source).test_value();
    engine.register_test_definition(definition);
...
"#;
    let mut engine = Engine::with_seed(0);
    let mut hud = Definition::from_script("AHUD", "Hud", script).test_value();
    hud.configure_actions(
        None,
        HashMap::from([("AmmoHud".to_string(), ActionSpec::default())]),
    );
    engine.register_test_definition(hud);
...
"#;
    let mut engine = Engine::with_seed(0);
    let mut living = Definition::from_script("CLNK", "Clonk", script).test_value();
    living.set_physical(PhysicalInfo {
        energy: 50_000,
        ..PhysicalInfo::default()
    });
...
"#;
    let mut definition = Definition::from_script("CLNK", "Clonk", script).test_value();
    ...
    let mut engine = Engine::with_seed(0);
    engine.register_test_definition(definition);
    let clonk = engine.spawn_test_object(test_object_config("CLNK"));
...
"#;
    let mut victim = Definition::from_script("VCTM", "Victim", "#strict\n").test_value();
    ...
    let actor = engine.spawn_test_object(test_object_config("ACTR"));
    let victim = engine.spawn_test_object(test_object_config("VCTM"));
...
fn check_energy_need_chain_follows_power_lines_and_breaks_cycles_like_cpp() {
    ...
    .test_value();
    engine.register_test_definition(plant);
    engine.register_test_definition(simple_definition("RELY"));
    ...
    let connect = |definition: &str, from, to| {
        ...
        test_object_config(definition).with_action(action)
    };
    engine.spawn_test_object(connect("PWRL", plant, relay));
    engine.spawn_test_object(connect("PWRL", relay, plant));
    engine.spawn_test_object(connect("PWRL", plant, consumer).with_status(ObjectStatus::Inactive));
...
"#;
    ...
    engine.register_test_script_definition("ZONE", "Zone owner", "#strict 2\n");
...
"#;
    ...
    for (id, name, script) in [
        ...
        let mut definition = Definition::from_script(id, name, script).test_value();
        definition.set_c4_callback_convention(true);
        engine.register_test_definition(definition);
    }

    let outer = engine.spawn_test_object(SpawnConfig::new("L82O"));
...
"#;
    ...
    let mut zone = Definition::from_script("ZONE", "Zone owner", zone_script).test_value();
...
"#;
    let mut engine = Engine::with_seed(0);
    let parent = Definition::from_script("PRNT", "Parent", script).test_value();
    engine.register_test_definition(parent);
    let child = Definition::from_script("CHLD", "Child", "#strict\n").test_value();
    engine.register_test_definition(child);
    let parent_id = engine.spawn_test_object(test_object_config("PRNT"));
    let snapshot = engine.snapshot();
...
"#;
    ...
    let mut engine = Engine::with_seed(0);
    engine.register_test_script_definition("PRNT", "Parent", parent_script);
    ...
    engine.spawn_test_object(test_object_config("PRNT"));
...
"#;
    ...
    let marker_id = engine.spawn_test_object(test_object_config("MARK"));
    ...
    engine
        .process_spawn_queue_with_outcomes(vec![test_object_config("CHLD")], vec![parent_outcome])
        .test_value();
...
"#;
    ...
    let mut engine = Engine::with_seed(0);
    engine.register_test_script_definition("CHLD", "Child", child_script);
    engine.register_test_script_definition("OBSV", "Observer", observer_script);
...
"#;
    let mut engine = Engine::with_seed(0);
    let mut parent = Definition::from_script("PRNT", "Parent", parent_script).test_value();
    parent.set_category(CATEGORY_OBJECT);
    engine.register_test_definition(parent);
    for id in ["ROCK", "GOLD"] {
    ...
    let parent = engine.spawn_test_object(test_object_config("PRNT"));
    let contents = engine
        .test_object_snapshot(parent)
        .contents
        .into_iter()
...
"#;
    ...
    let mut mage = Definition::from_script("MAGE", "Mage", script).test_value();
...
"#;
    let mut engine = Engine::with_seed(0);
    let npc =
        test_crew_definition(Definition::from_script("NPCX", "Npc", "#strict\n").test_value());
    engine.register_test_definition(npc);
    let caller = Definition::from_script("CALL", "Caller", script).test_value();
    engine.register_test_definition(caller);

    for x in [10, 40] {
        engine.spawn_test_object(
            test_object_config("NPCX")
...
"#;
    let mut engine = Engine::with_seed(0);
    let talker = Definition::from_script("TALK", "Talker", talker_script).test_value();
    engine.register_test_definition(talker);
...
fn object_property_and_index_write_declared_foreign_named_locals() {
    ...
    let bank = engine.spawn_test_object(SpawnConfig::new("BANK"));
...
"#;
    let mut engine = Engine::with_seed(0);
    let cross = Definition::from_script("WCHR", "Cross", cross_script).test_value();
    engine.register_test_definition(cross);
    let rider = Definition::from_script("RIDR", "Rider", rider_script).test_value();
    engine.register_test_definition(rider);

    let cross_id = engine.spawn_test_object(test_object_config("WCHR"));
    let rider_id = engine.spawn_test_object(test_object_config("RIDR"));
...
"#;
    let mut engine = Engine::with_seed(0);
    let horse = Definition::from_script("HRSE", "Horse", horse_script).test_value();
    engine.register_test_definition(horse);
    let rider = Definition::from_script("RIDR", "Rider", rider_script).test_value();
    engine.register_test_definition(rider);

    let horse_id = engine.spawn_test_object(test_object_config("HRSE"));
    let rider_id = engine.spawn_test_object(test_object_config("RIDR"));
...
"#;
    let mut engine = Engine::with_seed(0);
    let caller = Definition::from_script("CALR", "Caller", caller_script).test_value();
    engine.register_test_definition(caller);
    let mut animal_def = simple_definition("ANML");
...
"#;
    let mut engine = Engine::with_seed(0);
    engine.register_test_script_definition("BASE", "Basement", script);
    engine.register_test_definition(simple_definition("CLNK"));
    // Solid ground from y >= 20 in every column.
    engine.set_landscape(Landscape::new(100, vec![20; 100]).test_value());

    let base =
        engine.spawn_test_object(test_object_config("BASE").with_position(Vector2::new(50, 28)));
    let stuck_clonk = engine.spawn_test_object(
        test_object_config("CLNK")
...
"#;
    let mut engine = Engine::with_seed(0);
    engine.register_test_script_definition("CONT", "Container", script);
    engine.register_test_definition(simple_definition("ITEM"));
    let container =
        engine.spawn_test_object(test_object_config("CONT").with_position(Vector2::new(40, 40)));
    let item_a = engine.spawn_test_object(test_contained_object_config("ITEM", container));
    let item_b = engine.spawn_test_object(test_contained_object_config("ITEM", container));
...
"#;
    let mut engine = Engine::with_seed(0);
    engine.register_test_script_definition("Prob", "Prober", prober_script);
...
"#;
    ...
    engine.register_test_definition(item_def);
    let container =
        engine.spawn_test_object(test_object_config("CONT").with_position(Vector2::new(40, 40)));
    let item = engine.spawn_test_object(test_contained_object_config("ITEM", container));
...
#[test]
fn script_exit_runs_bounds_check_contacts_before_ejection_and_departure() -> Result<(), EngineError>
{
...
"#;
    ...
    engine.register_test_script_definition("CONT", "Container", container_script);
    engine.register_test_script_definition("ITEM", "Item", item_script);
    let container = engine.spawn_test_object(test_object_config("CONT"));
    let item = engine.spawn_test_object(test_contained_object_config("ITEM", container));
...
"#;
    ...
    engine.register_test_script_definition("DRVR", "Driver", driver_script);
    let mut item_definition = Definition::from_script("ITEM", "Item", item_script).test_value();
...
"#;
    ...
    let container =
        engine.spawn_test_object(test_object_config("CONT").with_position(Vector2::new(40, 40)));
    let item =
        engine.spawn_test_object(test_object_config("ITEM").with_position(Vector2::new(10, 10)));
...
"#;
    let mut engine = Engine::with_seed(0);
    engine.register_test_script_definition("OLDK", "Old Knight", old_script);
...
"#;
    let mut engine = Engine::with_seed(0);
    engine.register_test_script_definition("SRCE", "Source", source_script);
    engine.register_test_definition(simple_definition("DEST"));
    let mut rejected = Definition::from_script("NOPE", "Rejected", rejected_script).test_value();
    rejected.set_c4_callback_convention(true);
    engine.register_test_definition(rejected);

    let source = engine.spawn_test_object(test_object_config("SRCE"));
    let destination = engine.spawn_test_object(test_object_config("DEST"));
    let rejected = engine.spawn_test_object(test_contained_object_config("NOPE", source));
...
#[test]
fn collect_and_grab_contents_transfers_run_full_exit_bounds_before_enter() -> Result<(), EngineError>
{
...
"#;
    ...
    let mut destination = Definition::from_script("DEST", "Destination", destination_script)?;
...
#[test]
fn allied_teammates_let_the_no_friendly_fire_rule_decline_a_hit() -> Result<(), EngineError> {
