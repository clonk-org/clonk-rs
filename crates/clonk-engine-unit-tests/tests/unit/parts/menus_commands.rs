...
fn attach_one_pixel_portrait(engine: &mut Engine, definition: &mut Definition, name: &str) {
    let mut portrait_image = test_definition("PIMG", "Portrait image", "");
    ...
    engine.register_test_definition(portrait_image);
    let image = engine
...
fn add_menu_text_spec_uses_the_shared_cpp_grammar() {
    ...
    engine.register_test_definition(portrait_definition);
    engine.register_test_script_definition("CLNK", "Clonk", script);
    let clonk = engine.spawn_test_object(SpawnConfig::new("CLNK"));
    engine.tick_without_snapshot().test_value();
...
fn menu_definition_picture_phase_preserves_index_and_clips_out_of_bounds() {
    ...
    definition.set_sprite_image(Some(DefinitionSpriteImage {
        width: 3,
        height: 1,
        pixels: std::sync::Arc::from([0xff, 0, 0, 0xff, 0, 0xff, 0, 0xff, 0, 0, 0xff, 0xff]),
        color_mask: None,
    }));
...
fn real_clonk_category_agrees_across_definition_object_and_reflection() {
    ...
    let group =
        clonk_resources::Group::open(repository.join("content/Objects.c4d/Crew.c4d/Clonk.c4d"))
            .test_value();
...
fn knight_include_chain_inherits_clonk_rank_strip_and_base_count() {
    ...
    let mut engine = Engine::new();
    for path in paths {
        let group = clonk_resources::Group::open(repository.join(path)).test_value();
        let resource = ResourceDefinitionData::load(&group).test_value();
        engine.register_test_definition(Definition::from_resource(&resource).test_value());
    }
    engine.resolve_includes().test_value();
...
fn object_menu_picture_snapshot_captures_same_call_mutation_and_round_trips() {
    let script = r#"
        ...
        "#;
    let mut engine = Engine::new();
    engine.register_test_script_definition("NEWW", "New", "");
    engine.register_test_script_definition("CLNK", "Clonk", script);
    let clonk = engine.spawn_test_object(SpawnConfig::new("CLNK"));
    engine
        .apply_object_update(
...
fn object_menu_picture_caches_foreign_temporary_overlay_before_removal() {
    ...
    for (id, definition_script) in [("TEMP", ""), ("PICT", ""), ("CTRL", script)] {
        engine.register_test_script_definition(id, id, definition_script);
    }
...
fn menu_item_caches_custom_components_from_the_menu_target_builder_like_cpp() {
    ...
    for id in ["WOOD", "METL", "GOLD", "ROCK"] {
        engine.register_test_script_definition(id, id, "");
    }
    engine.register_test_script_definition("BULD", "Builder", builder_script);
    engine.register_test_script_definition("CMND", "Command", command_script);
    let mut item = test_definition("DYNA", "Dynamic", item_script);
    ...
    let call = |engine: &mut Engine, object: ObjectId, function: &str, args: Vec<Value>| {
...
fn menu_item_custom_component_fallbacks_and_force_no_desc_match_cpp() {
    ...
    engine.register_test_script_definition("BULD", "Builder", "");
    engine.register_test_script_definition("CMND", "Command", command_script);
    ...
    let register_item = |engine: &mut Engine,
                         ...
                         count: i32,
                         description: &str| {
        let mut item = test_definition(id, id, script);
        item.set_components(vec![DefinitionComponent {
...
fn punch_follows_object_com_punch_semantics_like_cpp() {
    ...
    let victim_script = r#"
        ...
        "#;
    let mut attacker_def = test_definition("SNKE", "Snake", attacker_script);
    ...
    let snake_idx = engine.test_object_index(snake);
    ...
    let bite = |engine: &mut Engine, target: ObjectId, strength: Option<i32>| {
        let idx = engine.test_object_index(snake);
        let args = vec![
            Value::Object(target.as_u64()),
            strength.map(Value::Int).unwrap_or(Value::Nil),
        ];
        engine.call_test_object_function(idx, "Bite", args)
    };
...
fn player_object_command_fixture() -> (Engine, ObjectId, ObjectId, ObjectId) {
    ...
    engine.register_test_script_definition("ITEM", "Item", "");
    let mut container_definition = test_definition("CONT", "Container", "");
...
fn player_object_command_call_warns_below_strict3_and_errors_at_strict3() {
    ...
    for (definition, strict_directive) in [("NSCL", ""), ("S1CL", "#strict"), ("S2CL", "#strict 2")]
    {
        let script = format!(
            r#"{strict_directive}
        ...
        engine.register_test_script_definition(definition, definition, &script);
        let caller = engine.spawn_test_object(SpawnConfig::new(definition));
    ...
    let error = tracing::subscriber::with_default(subscriber, || {
        call_player_object_command_fixture(&mut engine, strict3_caller, "BadCall", Vec::new())
    })
...
"#;
    ...
    for (id, name, script) in [
        ...
        let mut target = test_definition(id, name, script);
...
fn grab_calls_reject_grabbed_before_push_and_honors_veto() {
    ...
    let scale_actor_index = engine.test_object_index(scale_actor);
    engine.objects[scale_actor_index].state.direction = Direction::Left;
    let hangle_actor_index = engine.test_object_index(hangle_actor);
    ...
    let (zero_id_actor, zero_id_target) = spawn_grab_probe(&mut engine, "RGNO", "Walk", 600);
    let (clear_target_actor, _) = spawn_grab_probe(&mut engine, "RGCP", "Walk", 700);
    let (clear_then_replace_actor, _) = spawn_grab_probe(&mut engine, "RGCR", "Walk", 800);
    ...
    let removed_actor_index = engine.test_object_index(removed_actor);
    engine.call_test_object_function(removed_actor_index, "RemoveOnJump", Vec::new());
...
fn execute_command_runs_reject_grabbed_before_returning_to_script() {
    ...
    let (mutating_actor, mutating_target) = spawn_grab_probe(&mut engine, "RGAD", "Walk", 300);
    let (zero_id_actor, zero_id_target) = spawn_grab_probe(&mut engine, "RGNO", "Walk", 400);
    let (clear_target_actor, clear_target) = spawn_grab_probe(&mut engine, "RGCP", "Walk", 500);
    let (far_scale_actor, far_scale_target) = spawn_grab_probe(&mut engine, "RGVT", "Scale", 600);
    let far_scale_target_index = engine.test_object_index(far_scale_target);
...
"#;
    let mut target = test_definition("OGTG", "ObjectComGrab target", target_script);
    target.set_c4_callback_convention(true);
...
"#;
    let mut no_reject = test_definition("OGNR", "Grabbed-only target", no_reject_script);
...
fn spawn_object_com_grab_probe(engine: &mut Engine, action: &str, x: i32) -> (ObjectId, ObjectId) {
...
fn object_com_grab_matches_cpp_callbacks_controller_walk_and_stop_gates() {
    ...
    let (walk_actor, walk_target) = spawn_object_com_grab_probe(&mut engine, "Walk", 0);
    let stopped = [
    ...
    let (flight_actor, flight_target) = spawn_object_com_grab_probe(&mut engine, "Flight", 400);
    let (swim_actor, swim_target) = spawn_object_com_grab_probe(&mut engine, "Swim", 450);
    let (removed_actor, removed_actor_target) =
        spawn_object_com_grab_probe(&mut engine, "Walk", 500);
    let removed_actor_index = engine.test_object_index(removed_actor);
    engine.call_test_object_function(removed_actor_index, "ArmGrabDeletion", vec![Value::Int(1)]);
    let (target_remover, removed_target) = spawn_object_com_grab_probe(&mut engine, "Walk", 600);
    let target_remover_index = engine.test_object_index(target_remover);
    engine.call_test_object_function(target_remover_index, "ArmGrabDeletion", vec![Value::Int(2)]);
    let (far_builder, far_target) = spawn_object_com_grab_probe(&mut engine, "Build", 700);
    let far_target_index = engine.test_object_index(far_target);
    ...
    let command_replacer_index = engine.test_object_index(command_replacer);
    engine.call_test_object_function(
        command_replacer_index,
        "ArmGrabDeletion",
        vec![Value::Int(3)],
    );
    let (cleared_builder, cleared_target) = spawn_object_com_grab_probe(&mut engine, "Build", 900);
    let cleared_builder_index = engine.test_object_index(cleared_builder);
    engine.call_test_object_function(
    ...
    let sequential_builder_index = engine.test_object_index(sequential_builder);
    engine.call_test_object_function(sequential_builder_index, "ArmStopToDig", Vec::new());
    let (mutated_actor, mutated_target) = spawn_object_com_grab_probe(&mut engine, "Walk", 1_100);
    let mutated_actor_index = engine.test_object_index(mutated_actor);
    engine.call_test_object_function(mutated_actor_index, "ArmGrabDeletion", vec![Value::Int(4)]);
    ...
    let (inactive_actor, inactive_target) = spawn_object_com_grab_probe(&mut engine, "Walk", 1_300);
    ...
    let clear_then_detach_index = engine.test_object_index(clear_then_detach);
    engine.call_test_object_function(
        clear_then_detach_index,
        "ArmStopPointerOrder",
        vec![
            object_reference_value(clear_then_detach_target),
            Value::Int(1),
        ],
    );
    let (detach_then_remove, detach_then_remove_target) =
        spawn_object_com_grab_probe(&mut engine, "Build", 1_500);
    let detach_then_remove_index = engine.test_object_index(detach_then_remove);
    engine.call_test_object_function(
        detach_then_remove_index,
        "ArmStopPointerOrder",
        vec![
            object_reference_value(detach_then_remove_target),
            Value::Int(2),
        ],
    );
...
"#
    ...
    for (id, silent, crew) in [
        ...
        let mut definition = test_definition(id, id, actor_script);
        definition.set_crew_member(crew);
        definition.set_silent_commands(silent);
        engine.register_test_definition(definition);
    }
    engine.register_test_script_definition("TGTF", "Falsy target", &target_script("0"));
    engine.register_test_script_definition("TGTT", "Truthy target", &target_script("WOOD"));
    engine.register_test_script_definition("MARK", "Marker", "#strict");

    let falsy_target = engine.spawn_test_object(SpawnConfig::new("TGTF"));
    let truthy_target = engine.spawn_test_object(SpawnConfig::new("TGTT"));
...
"#;
    let register = |engine: &mut Engine| {
        let mut actor = test_definition("CTXA", "Call actor", actor_script);
        actor.set_crew_member(true);
        engine.register_test_definition(actor);
        engine.register_test_script_definition("CTXT", "Call target", target_script);
        engine.register_test_definition(simple_definition("CTXM"));
        engine.register_test_definition(simple_definition("CTXD"));
    };
    ...
    let read_tx = |engine: &mut Engine| {
        let actor_index = engine.test_object_index(actor);
        engine.call_test_object_function(actor_index, "ReadTx", Vec::new())
    };
    ...
    state.objects.retain(|object| object.snapshot.id != doomed);
    let json = state.to_json_string().test_value();
    let state = EngineState::from_json_str(&json).test_value();
    let mut restored = Engine::with_seed(0);
    register(&mut restored);
    restored.restore_state(&state).test_value();
...
"#;
    let mut builder = test_definition("BLDR", "Builder", script);
...
fn completed_builders_queue_only_one_energy_command_in_same_tick() {
    let mut builder = test_definition("BLDR", "Builder", "#strict");
    builder.set_physical(PhysicalInfo {
        can_construct: 1,
        ..PhysicalInfo::default()
    });
    ...
    engine.register_test_definition(builder);
    engine.register_test_definition(site);
    let target = engine.spawn_test_object(SpawnConfig::new("SITE"));
    let builders = [
        engine.spawn_test_object(SpawnConfig::new("BLDR")),
        engine.spawn_test_object(SpawnConfig::new("BLDR")),
    ];
    for builder_id in builders {
        let index = engine.test_object_index(builder_id);
        engine.objects[index]
            .commands
            .push_front(
                CommandRequest::new(CommandId::Build)
...
"#;
    let mut builder = test_definition("BLDR", "Builder", actor_script);
    builder.set_crew_member(true);
    let mut site = test_definition("SITE", "Site", "#strict");
...
"#;
    let mut floater = test_definition("FLTR", "Floater", script);
...
"#;
    let mut definition = test_definition("MVST", "MoveTo stopper", script);
    let mut actions = HashMap::new();
    actions.insert(
        "Walk".to_string(),
        ActionSpec::default()
...
fn do_damage_asks_effects_for_non_living_and_fires_callback() {
    ...
    let mut hooks = DebuggerHooks::new();
    {
        ...
        hooks.set_on_call(move |name, _args| {
            call_log.lock().test_value().push(name.to_string());
        });
    }
...
"#;
    let mut definition = test_definition("PING", "Zero-hit amplifier", script);
...
fn do_energy_clamps_to_physical_energy_ceiling() {
    ...
    let mut definition = test_definition("Clonk", "Clonk", script);
