...
fn walk_procedure_uses_walk_physical_limit_and_const_accel() {
    ...
    let mut definition = test_definition("Walker", "Walker", script);
    ...
    let id = spawn_fixture!(engine, "Walker", with_position: Vector2::new(0, 0), with_command_direction: CommandDirection::Right);
    ...
    unit_assert_eq!(engine.objects[idx].fixed_velocity.x.val() => 32768);
    engine.tick_without_snapshot().test_value();
    unit_assert_eq!(engine.objects[idx].fixed_velocity.x.val() => 64225);
}
...
"#;
    let mut definition = test_definition("KTRC", "Kill trace walker", script);
    ...
    let id = spawn_fixture!(engine, "KTRC", with_category: CATEGORY_LIVING, with_alive: true, with_action: ActionState::new("Walk"));
    let idx = engine.test_object_index(id);
    engine.objects[idx].last_energy_loss_cause = 7;

    engine.apply_physics_at_index(idx).test_value();
    unit_assert_eq!(engine.objects[idx].last_energy_loss_cause => OWNER_NONE, "a controllable action clears the stale attacker");

    engine.assign_death(idx, false).test_value();
    let idx = engine.test_object_index(id);
    unit_assert_eq!(engine.objects[idx].state.local_vars.get("death_by") => Some(&Value::Int(OWNER_NONE)), "a later environmental death is credited to NO_OWNER");
}
...
fn exec_action_retains_kill_trace_for_idle_flight_swim_disabled_and_fire() {
    ...
    for (action, on_fire) in [
        ...
        let id = spawn_fixture!(engine, "KTRC", with_category: CATEGORY_LIVING, with_alive: true, with_action: ActionState::new(action));
        let idx = engine.test_object_index(id);
        engine.objects[idx].last_energy_loss_cause = 7;
        engine.objects[idx].state.on_fire = on_fire;

        engine.apply_physics_at_index(idx).test_value();
        unit_assert_eq!(engine.objects[idx].last_energy_loss_cause => 7, "{action} (on_fire={on_fire}) must retain the attacker");
    }
}
...
fn contained_flight_queues_replacing_exit_only_on_tick10() {
    let mut engine = Engine::with_seed(5);
    engine.register_test_definition(contained_flight_definition("FALL", ""));
    engine.register_test_definition(simple_definition("CONT"));
    ...
    for action in ["Jump", "Tumble"] {
        let actor = spawn_fixture!(engine, "FALL", with_category: CATEGORY_LIVING, with_alive: true, with_container: container, with_action: ActionState::new(action));
        let idx = engine.test_object_index(actor);
    ...
    for &(actor, action) in &actors {
        let idx = engine.test_object_index(actor);
        engine.apply_physics_at_index(idx).test_value();
        let snapshot = engine.test_object_snapshot(actor);
        unit_assert_eq!(snapshot.action.name => action);
        unit_assert_eq!(engine.objects[idx].state.no_collect_delay => 2);
        unit_assert_eq!(snapshot.command_stack.command_names() => vec!["MoveTo"]);
    }
    ...
    for &(actor, _) in &actors {
        ...
        unit_assert_eq!(snapshot.command_direction => CommandDirection::Stop);
        unit_assert_eq!(snapshot.container => Some(container));
        unit_assert_eq!(engine.objects[idx].state.no_collect_delay => 1);
        unit_assert_eq!(engine.objects[idx].fixed_velocity.y => math::fixed100(20), "captured FLIGHT still applies gravity after stopping");
        unit_assert!(engine.objects[idx].state.mobile);
        unit_assert_eq!(snapshot.command_stack.command_names() => vec!["Exit"], "SetCommand clears both the old command and delayed Wait");
    }
    ...
    engine.tick_without_snapshot().test_value();
    engine.tick_without_snapshot().test_value();
    unit_assert_eq!(engine.frame => 12);
    for &(actor, _) in &actors {
        let snapshot = engine.test_object_snapshot(actor);
        unit_assert_eq!(snapshot.container => None);
        unit_assert!(snapshot.command_stack.command_names().is_empty());
    }
}
...
fn exit_init_evaluation_cancels_live_attach_before_exit() -> Result<(), EngineError> {
    ...
    let mut actor = test_definition("EXAT", "Attached exiter", "#strict 2\n");
    ...
    let container_id =
        engine.spawn_object(SpawnConfig::new("EXAC").with_position(Vector2::new(40, 70)))?;
    let container_index = engine.test_object_index(container_id);
    ...
    let snapshot = engine.test_object_snapshot(actor_id);
    unit_assert_eq!(snapshot.action.name => "Idle");
    unit_assert_eq!(snapshot.container => Some(container_id));
    unit_assert_eq!(snapshot.command_stack.command_names() => vec!["Exit".to_string()]);
...
fn collection_exit_runs_live_jump_with_preserved_comdir() -> Result<(), EngineError> {
    ...
    let mut actor = test_definition("EXJP", "Collection exit jumper", "#strict 2\n");
    ...
    let container_id =
        engine.spawn_object(SpawnConfig::new("EXCT").with_position(Vector2::new(120, 180)))?;
    let container_index = engine.test_object_index(container_id);
    engine.objects[container_index].state.entrance_status = true;
    unit_assert_eq!(engine.objects[container_index].state.ocf & ocf::ENTRANCE => 0, "an actual entrance area would take priority over Collection");
    ...
    unit_assert_eq!(actor.state.container => None);
    unit_assert_eq!(actor.state.position => Vector2::new(120, 162));
    unit_assert_eq!(actor.fixed_position => FixedVec2::from_ints(120, 162), "Exit installs the collection-area position before jumping");
    unit_assert_eq!(actor.state.action.name => "Jump");
    unit_assert_eq!(actor.state.command_direction => CommandDirection::Right, "Exit must not replace the ComDir consumed by ObjectComJump");
    unit_assert_eq!(
    ...
    unit_assert!(actor.state.mobile);
    unit_assert!(actor.commands.command_names().is_empty());
    Ok(())
}
...
"#;
    let mut engine = Engine::with_seed(5);
    engine.register_test_definition(contained_flight_definition("FALL", actor_script));
    let mut container_definition = test_definition("CONT", "Control vehicle", container_script);
    container_definition.set_vehicle_control(VEHICLE_CONTROL_INSIDE);
    engine.register_test_definition(container_definition);
    engine.set_physics(PhysicsSettings::new(0, 0, 0));
    let container = engine.spawn_test_object(SpawnConfig::new("CONT"));
    let actor = spawn_fixture!(engine, "FALL", with_category: CATEGORY_LIVING, with_alive: true, with_controller: 7, with_container: container, with_action: ActionState::new("Jump"));
    ...
    engine.frame = 10;
    engine.apply_physics_at_index(actor_idx).test_value();
    ...
    unit_assert_eq!(container_state.controller => 7);
    unit_assert_eq!(container_state.local_vars.get("control_calls") => Some(&Value::Int(1)));
    unit_assert_eq!(container_state.local_vars.get("control_command") => Some(&Value::String("Exit".to_string().into())));
    unit_assert_eq!(container_state.local_vars.get("control_by") => Some(&compat::object_reference_value(actor)));
}
...
fn walk_procedure_automatically_steers_rotation_to_floor_slope() {
    ...
    let mut definition = test_definition("Walker", "Walker", "");
    let mut actions = HashMap::new();
    actions.insert("Walk".to_string(), ActionSpec::for_procedure("walk"));
    definition.configure_actions(Some("Walk".to_string()), actions);
    ...
    let mut engine = Engine::with_seed(5);
    engine.register_definition(definition).test_value();
    let mut surface = vec![25; 32];
    surface.extend(vec![5; 32]);
    engine.set_landscape(Landscape::new(64, surface).test_value());
    ...
    let id = engine
        ...
        .test_value();
    let idx = engine.find_object_index(id).test_value();
    ...
    unit_assert_eq!(engine.objects[idx].rotation_velocity => C4Fixed::from_raw(-9 * 65536 / 100));
}
...
fn stationary_walk_on_offset_vertex_still_adjusts_rotation() {
    ...
    unit_assert_eq!(engine.objects[idx].rotation_velocity => C4Fixed::from_raw(-15 * 65536 / 100));
}
...
fn scale_procedure_uses_scale_physical_limit_and_trains_at_limit() {
    ...
    let id = spawn_fixture!(engine, "Climber", with_position: Vector2::new(0, 0), with_command_direction: CommandDirection::Up);
    let idx = engine.test_object_index(id);
    ...
    engine.tick_without_snapshot().test_value();
    unit_assert_eq!(engine.objects[idx].fixed_velocity.y.val() => -32768);
    engine.tick_without_snapshot().test_value();
    unit_assert_eq!(engine.objects[idx].fixed_velocity.y.val() => -39321);
    unit_assert_eq!(engine.objects[idx].state.info_physical => None);
    ...
    engine.tick_without_snapshot().test_value();
    engine.tick_without_snapshot().test_value();
    let trained = engine.objects[idx].state.temporary_physical.test_value();
    unit_assert_eq!(trained.scale => 30_001);
}
...
fn attached_scaler_hangles_when_up_hits_the_ceiling_like_cpp() {
    ...
    let mut climber = test_definition("Climber", "Climber", "#strict\n");
    ...
    let mut landscape = Landscape::new(20, vec![0; 20]).test_value();
    ...
    engine.set_physics(PhysicsSettings::new(0, 20, -20));
    engine.register_test_definition(climber);
    ...
    unit_assert_eq!(object.state.action.name => "Hangle");
    unit_assert_eq!(object.state.direction => Direction::Right);
    unit_assert_eq!(object.state.command_direction => CommandDirection::Up);
    unit_assert_eq!(object.state.position => Vector2::new(10, 10));
    unit_assert_eq!(object.fixed_position => FixedVec2::from_ints(10, 10));
...
fn hangle_procedure_uses_hangle_physical_limit_and_trains_at_limit() {
    ...
    let mut definition = test_definition("Hangler", "Hangler", script);
    let mut actions = HashMap::new();
    actions.insert(
        "Hangle".to_string(),
    ...
    let id = spawn_fixture!(engine, "Hangler", with_position: Vector2::new(0, 0), with_command_direction: CommandDirection::Right);
    let idx = engine.test_object_index(id);
    engine.objects[idx].state.temporary_physical = Some(PhysicalInfo {
        hangle: 40_000,
        ..PhysicalInfo::default()
    ...
    unit_assert_eq!(engine.objects[idx].fixed_velocity.x.val() => 32768);
    engine.tick_without_snapshot().test_value();
    unit_assert_eq!(engine.objects[idx].fixed_velocity.x.val() => 41943);
    unit_assert_eq!(engine.objects[idx].fixed_velocity.y => C4Fixed::ZERO);
    unit_assert_eq!(engine.objects[idx].state.direction => Direction::Right);
    unit_assert_eq!(engine.objects[idx].state.info_physical => None);
    ...
    engine.tick_without_snapshot().test_value();
    let trained = engine.objects[idx].state.temporary_physical.test_value();
    unit_assert_eq!(trained.hangle => 40_001);
}
...
fn swim_phase_advances_by_the_physical_limit_like_cpp() {
    let mut fish = test_definition("Fish", "Fish", "#strict\n");
    ...
    let id = spawn_fixture!(engine, "Fish", with_category: CATEGORY_OBJECT, with_position: Vector2::new(10, 10), with_action: ActionState::new("Turn"));
    ...
    engine.tick_without_snapshot().test_value();
    let idx = engine.test_object_index(id);
    unit_assert_eq!(engine.objects[idx].state.action.phase => 1, "PhaseDelay += fixtoi(ValByPhysical(160,100000)*10)=16 >= Delay 3 on the first exec");
}
...
fn swim_stop_decays_fish_xdir_to_exact_zero_on_the_cpp_schedule() {
    let mut definition = test_definition("FISH", "Fish", "#strict\n");
    let mut actions = HashMap::new();
    ...
    let id = spawn_fixture!(engine, "FISH", with_position: Vector2::new(0, 0), with_command_direction: CommandDirection::Stop);
    let idx = engine.test_object_index(id);
    engine.objects[idx].state.in_liquid = true;
    engine.objects[idx].fixed_velocity.x = C4Fixed::from_raw(104857);
    engine.objects[idx].state.mobile = true;

    let ladder = [91750, 78643, 65536, 52429, 39322, 26215, 13108, 0, 0];
    for (tick, expected) in ladder.iter().enumerate() {
        engine.tick_without_snapshot().test_value();
        let idx = engine.test_object_index(id);
        unit_assert_eq!(engine.objects[idx].fixed_velocity.x.val() => *expected, "COMD_Stop decay after tick {}", tick + 1);
        unit_assert_eq!(engine.objects[idx].fixed_velocity.y => C4Fixed::ZERO);
    }
}
...
"#;
    ...
    let mut swim_spec = actions.get("Swim").cloned().test_value();
    ...
    engine.set_physics(PhysicsSettings::new(2, 20, -20));
    engine.register_test_definition(fish);
    let id = spawn_fixture!(engine, "FISH", with_position: Vector2::new(100, 50), with_command_direction: CommandDirection::Right);
    let idx = engine.test_object_index(id);
    engine.objects[idx].state.in_liquid = true;
    ...
    unit_assert_eq!(object.state.action.name => "Turn", "TurnAction fired");
    unit_assert_eq!(object.state.command_direction => CommandDirection::Left);
}
...
"#;
    let mut horse = test_definition("HRSE", "Horse", script);
    ...
    let coach = spawn_fixture!(engine, "HRSE", with_position: Vector2::new(50, 50));
    let id = spawn_fixture!(engine, "HRSE", with_position: Vector2::new(100, 50));
    let idx = engine.test_object_index(id);
    ...
    engine.tick_without_snapshot().test_value();
    let idx = engine.test_object_index(id);
    unit_assert_eq!(engine.objects[idx].state.action.target => None, "SetActionTargets() nil-fills pTarget1 -> Action.Target cleared");
    unit_assert_eq!(engine.objects[idx].state.action.target2 => None, "SetActionTargets() nil-fills pTarget2 -> Action.Target2 cleared");
}
...
fn swim_procedure_uses_swim_physical_limit_and_trains_on_tick10() {
    ...
    let mut definition = test_definition("Swimmer", "Swimmer", script);
    let mut actions = HashMap::new();
    actions.insert(
        "Swim".to_string(),
    ...
    let id = spawn_fixture!(engine, "Swimmer", with_position: Vector2::new(0, 0), with_command_direction: CommandDirection::Right);
    let idx = engine.test_object_index(id);
    engine.objects[idx].state.temporary_physical = Some(PhysicalInfo {
        swim: 50_000,
        ..PhysicalInfo::default()
    ...
    engine.tick_without_snapshot().test_value();
    unit_assert_eq!(engine.objects[idx].fixed_velocity.x.val() => 13107);
    unit_assert_eq!(engine.objects[idx].fixed_velocity.y => C4Fixed::ZERO);
    for _ in 0..4 {
        engine.tick_without_snapshot().test_value();
    }
    unit_assert_eq!(engine.objects[idx].fixed_velocity.x.val() => 52428);
    unit_assert_eq!(engine.objects[idx].fixed_velocity.y => C4Fixed::ZERO);
    unit_assert_eq!(engine.objects[idx].state.direction => Direction::Right);
    unit_assert_eq!(engine.objects[idx].state.info_physical => None);
    ...
    let trained = engine.objects[idx].state.temporary_physical.test_value();
    unit_assert_eq!(trained.swim => 50_001);
}
...
fn pxs_wind_drift_dies_in_tunnel_background() {
    ...
    unit_assert_eq!(pixel.len() => 1);
    unit_assert_eq!(pixel[0].xdir => expected_xdir, "tunnel reads wind 0");
    unit_assert_eq!(tunnel_engine.rng => mirror, "jitter draws happen either way");
    ...
    unit_assert_eq!(open_pixel[0].xdir => open_xdir);
    unit_assert_ne!(open_pixel[0].xdir => pixel[0].xdir);
}
...
fn pxs_wind_drift_uses_the_grid_ift_bit() {
    ...
    let build_engine = |ift: bool| {
        ...
        let grid = landscape::PixelGrid::new(8, 100, bytes, vec![0], vec![None], vec![None]);
        let mut landscape = Landscape::flat(8, 100);
        landscape.set_pixel_grid(grid);
        engine.set_landscape(landscape);
        engine.set_environment(EnvironmentSettings::new(80));
    ...
    let r1 = mirror.random(1200);
    let _ = mirror.random(1200);
    let expected_ift_xdir = math::fixed256(r1 - 600) * 10 * math::itofix_prec(1, 800);
    ift_engine.tick_pxs();
    let ift_pixel: Vec<pxs::Pxs> = ift_engine.pxs_system.iter().copied().collect();
    unit_assert_eq!(ift_pixel.len() => 1);
    unit_assert_eq!(ift_pixel[0].xdir => expected_ift_xdir, "IFT grid byte reads wind 0");
    ...
    unit_assert_eq!(open_pixel[0].xdir => expected_open_xdir);
    unit_assert_ne!(open_pixel[0].xdir => ift_pixel[0].xdir);
    unit_assert_eq!(open_engine.rng => mirror, "both paths consume two draws");
}
...
fn dig_procedure_uses_dig_physical_speed() {
    ...
    let mut definition = test_definition("Digger", "Digger", script);
    let mut actions = HashMap::new();
    actions.insert(
        "Dig".to_string(),
    ...
    let id = spawn_fixture!(engine, "Digger", with_position: Vector2::new(0, 0), with_command_direction: CommandDirection::UpRight);
    let idx = engine.test_object_index(id);
    ...
    engine.tick_without_snapshot().test_value();
    unit_assert_eq!(engine.objects[idx].fixed_velocity.x.val() => 32768);
    unit_assert_eq!(engine.objects[idx].fixed_velocity.y.val() => -16384);
    unit_assert_eq!(engine.objects[idx].state.direction => Direction::Right);
}

#[test]
fn stopped_dig_freezes_phase_but_counts_action_time_like_cpp() {
    let mut definition = test_definition("Digger", "Digger", PROCEDURE_MOVEMENT_SCRIPT);
    let mut actions = HashMap::new();
    actions.insert(
        "Dig".to_string(),
        ActionSpec::default()
    ...
    let mut engine = Engine::with_seed(5);
    engine.register_test_definition(definition);
    let id = spawn_fixture!(engine, "Digger", with_action: ActionState::new("Dig"), with_command_direction: CommandDirection::Stop);
    ...
    let action = &engine.test_object_snapshot(id).action;
    unit_assert_eq!(action.phase => 0);
    unit_assert_eq!(action.ticks => 0);
    unit_assert_eq!(action.time => 1);
}
...
fn float_procedure_uses_float_physical_limit_and_const_accel() {
    ...
    let mut definition = test_definition("Boat", "Boat", script);
    let mut actions = HashMap::new();
    ...
    let id = spawn_fixture!(engine, "Boat", with_position: Vector2::new(0, 0), with_command_direction: CommandDirection::Right);
    let idx = engine.test_object_index(id);
    ...
    engine.tick_without_snapshot().test_value();
    unit_assert_eq!(engine.objects[idx].fixed_velocity.x.val() => 6553);
    unit_assert_eq!(engine.objects[idx].fixed_velocity.y => C4Fixed::ZERO);
    engine.tick_without_snapshot().test_value();
    unit_assert_eq!(engine.objects[idx].fixed_velocity.x.val() => 13106);
    engine.tick_without_snapshot().test_value();
    unit_assert_eq!(engine.objects[idx].fixed_velocity.x.val() => 13107);
    unit_assert_eq!(engine.objects[idx].fixed_velocity.y => C4Fixed::ZERO);
}
...
fn grab_lost_push_fixture(
    ...
    let mut pusher = test_definition("PSHR", "Pusher", "#strict");
...
"#;
    let mut vehicle = test_definition("VEHI", "Vehicle", vehicle_script);
    ...
    engine.set_physics(PhysicsSettings::new(0, 20, -20));
    engine.register_test_definition(pusher);
    engine.register_test_definition(vehicle);
    let vehicle_id = spawn_fixture!(engine, "VEHI", with_category: CATEGORY_VEHICLE, with_position: vehicle_position, with_fixed_position: FixedVec2::from_ints(vehicle_position.x, vehicle_position.y), with_loaded: true);
    let mut push = ActionState::new("Push");
    push.target = Some(vehicle_id);
    let pusher_id = spawn_fixture!(engine, "PSHR", with_category: CATEGORY_OBJECT, with_position: pusher_position, with_fixed_position: FixedVec2::from_ints(pusher_position.x, pusher_position.y), with_action: push, with_command_direction: CommandDirection::Right, with_mobile: true, with_loaded: true);

    let vehicle_idx = engine.test_object_index(vehicle_id);
    engine.call_test_object_function(vehicle_idx, "Arm", vec![object_reference_value(pusher_id)]);
    ...
    commands
        ...
        .test_value();
    unit_assert_eq!(commands.command_names() => vec!["MoveTo", "PushTo", "Wait"]);

    (engine, pusher_id, vehicle_id)
}

fn no_attach_fighter_definition() -> Definition {
    let mut fighter = test_definition(
        "FGTR",
        "Fighter",
        r#"#strict
        local death_by;
...
fn fight_no_attach_credits_opponent_controller_through_fall_death() {
    let mut engine = Engine::with_seed(7);
    engine.set_physics(PhysicsSettings::new(0, 20, -20));
    engine.register_test_definition(no_attach_fighter_definition());
    engine.register_test_definition(simple_definition("OPPN"));

    let opponent = spawn_fixture!(engine, "OPPN", with_owner: 2, with_controller: 9, with_position: Vector2::new(8, 5));
    let mut fight = ActionState::new("Fight");
    fight.target = Some(opponent);
    let fighter = engine.spawn_test_object(
    ...
    engine.objects[fighter_idx].set_fixed_velocity(FixedVec2::new(itofix(1), C4Fixed::ZERO));
    ...
    engine.assign_death(fighter_idx, true).test_value();
    let fighter_idx = engine.test_object_index(fighter);
    unit_assert_eq!(
...
fn non_fight_no_attach_leaves_the_kill_trace_untouched() {
    let mut engine = Engine::with_seed(7);
    engine.register_test_definition(no_attach_fighter_definition());
    let actor = spawn_fixture!(engine, "FGTR", with_category: CATEGORY_LIVING, with_action: ActionState::new("Walk"));
    let actor_idx = engine.test_object_index(actor);
    ...
    engine.objects[actor_idx].set_fixed_velocity(FixedVec2::new(itofix(-4), itofix(-5)));
    ...
    engine
        ...
        .test_value();
    unit_assert_eq!(engine.objects[actor_idx].state.action.name => "Jump");
    unit_assert_eq!(engine.objects[actor_idx].fixed_velocity => FixedVec2::new(itofix(-4), itofix(-5)));
    unit_assert!(engine.objects[actor_idx].state.mobile);
    unit_assert_eq!(engine.objects[actor_idx].state.t_attach => CNAT_RIGHT);
    unit_assert_eq!(engine.objects[actor_idx].frame_t_attach => CNAT_RIGHT);
    unit_assert_eq!(engine.objects[actor_idx].last_energy_loss_cause => 7, "the NoAttachAction path only rewrites Fight kill tracing");
}
...
fn push_no_attach_calls_grab_lost_before_jump_and_restores_push_to() {
    ...
    engine.objects[pusher_idx].set_fixed_velocity(FixedVec2::new(itofix(1), C4Fixed::ZERO));
    ...
    unit_assert_eq!(pusher.action.name => "Jump");
    unit_assert_eq!(pusher.command_stack.command_names() => vec!["PushTo", "Wait"]);
    let vehicle = engine.test_object_snapshot(vehicle_id);
    unit_assert_eq!(vehicle.local_vars.get("grab_lost_calls") => Some(&Value::Int(1)));
    unit_assert_eq!(
...
fn push_got_hold_loss_clears_delay_and_approach_above_push_to() {
    ...
    unit_assert_eq!(pusher.command_direction => CommandDirection::Stop);
    unit_assert_eq!(pusher.command_stack.command_names() => vec!["PushTo", "Wait"], "GrabLost clears the new delay and MoveTo but preserves PushTo's tail");
    let vehicle = engine.test_object_snapshot(vehicle_id);
    unit_assert_eq!(vehicle.local_vars.get("grab_lost_calls") => Some(&Value::Int(1)));
    unit_assert_eq!(vehicle.local_vars.get("action_seen") => Some(&Value::String("Walk".to_owned().into())), "StopActionDelayCommand precedes GrabLost");
}
...
fn command_enter_push_target_replaces_vehicle_stack_without_controller_transfer() {
    ...
    let mut actor = test_definition("ACTR", "Actor", "#strict");
    actor.configure_actions(
        Some("Push".to_string()),
        HashMap::from([(
            "Push".to_string(),
    ...
    let entrance_id = spawn_fixture!(engine, "ENTR", with_category: CATEGORY_STRUCTURE, with_position: Vector2::new(100, 100), with_entrance_status: true, with_loaded: true);
    // Keep ExecAction from performing a real Push and
    // independently transferring the actor's controller.
    let vehicle_id = spawn_fixture!(
        engine,
        "VEHI",
        with_category: CATEGORY_STATIC_BACK,
    ...
    let vehicle_index = engine.test_object_index(vehicle_id);
    ...
    let mut push = ActionState::new("Push");
    push.target = Some(vehicle_id);
    let actor_id = spawn_fixture!(engine, "ACTR", with_position: Vector2::new(0, 100), with_controller: 7, with_action: push, with_loaded: true);
    let actor_index = engine.test_object_index(actor_id);
    engine.objects[actor_index]
        .commands
        .push_back(
    ...
    unit_assert_eq!(commands[0].name => "Enter");
    unit_assert_eq!(commands[0].target => Some(entrance_id));
    unit_assert_eq!(vehicle.controller => 9, "SetCommand preserves Controller");
    unit_assert_eq!(vehicle.container => None, "vehicle executes on its next turn");

    engine.tick_without_snapshot().test_value();
    unit_assert_eq!(engine.object_snapshot(vehicle_id).expect("vehicle remains").container => Some(entrance_id));
}
...
fn push_pull_fixture() -> (Engine, ObjectId, ObjectId) {
    ...
    let mut crate_definition = test_definition("Crate", "Crate", script);
    crate_definition.set_grab(1);
    crate_definition.set_mass(200);

    let mut engine = Engine::with_seed(18);
    engine.register_test_definition(pusher_definition);
    engine.register_test_definition(crate_definition);
    engine.set_physics(
        PhysicsSettings::new(0, 20, -20)
            .with_max_horizontal_speed(20)
            .test_value(),
...
"#;
    ...
    let vehicle_id = spawn_fixture!(engine, "VEHI", with_category: CATEGORY_VEHICLE, with_position: Vector2::new(8, 10), with_fixed_position: FixedVec2::from_ints(8, 10), with_loaded: true);
    let mut push = ActionState::new("Push");
    push.target = Some(vehicle_id);
    let pusher_id = spawn_fixture!(engine, "PSHR", with_category: CATEGORY_OBJECT, with_position: Vector2::new(8, 10), with_fixed_position: FixedVec2::from_ints(8, 10), with_action: push, with_command_direction: command_direction, with_mobile: true, with_loaded: true);
...
fn push_tick35_refreshes_contact_and_reports_stuck() {
    ...
    let (mut engine, pusher_id, vehicle_id) = stuck_push_fixture(CommandDirection::Right, 0);
    let pusher_idx = engine.test_object_index(pusher_id);
    let vehicle_idx = engine.test_object_index(vehicle_id);
    ...
    unit_assert_eq!(engine.objects[vehicle_idx].frame_t_contact => CNAT_LEFT);
    unit_assert_eq!(engine.objects[vehicle_idx].state.local_vars.get("stuck_calls") => None);
    unit_assert!(engine.snapshot().hud.messages.is_empty());

    engine.frame = 35;
    let _ = engine.apply_physics_at_index(pusher_idx).test_value();
    unit_assert_eq!(engine.objects[vehicle_idx].frame_t_contact => CNAT_RIGHT, "ContactCheck replaces the previous t_contact latch");
    unit_assert_eq!(engine.objects[vehicle_idx].state.local_vars.get("stuck_calls") => Some(&Value::Int(1)));
    let first_message = engine
        ...
        .next()
        .test_value();
    unit_assert_eq!(first_message.kind => MessageKind::Target);
    unit_assert_eq!(first_message.target => Some(vehicle_id));
    unit_assert_eq!(first_message.player => None);
    unit_assert_eq!(first_message.lines => vec!["Vehicle is stuck!"]);

    for frame in 36..=69 {
        engine.frame = frame;
        let _ = engine.apply_physics_at_index(pusher_idx).test_value();
    }
    unit_assert_eq!(engine.objects[vehicle_idx].state.local_vars.get("stuck_calls") => Some(&Value::Int(1)), "no second callback before the next Tick35");
    ...
    unit_assert_eq!(messages.len() => 1, "same-target messages replace each other");
    unit_assert_ne!(messages[0].id => first_message.id, "the Tick70 check emitted a fresh stuck message");
}
...
fn push_procedure_uses_walk_and_push_physicals() {
    ...
    let pusher_id = spawn_fixture!(engine, "Pusher", with_position: Vector2::new(0, 0), with_controller: 7, with_vertices: vertices, with_action: push_state, with_command_direction: CommandDirection::Right);
    ...
    unit_assert_eq!(engine.objects[crate_idx].fixed_velocity.x.val() => 36864);
    // Pusher: follow-x BoundBy(0, 2, 17) = 2 → xdir = +lLimit
    // = ValByPhysical(280, 35000) = raw 64225 (C4Object.cpp:5085-5087).
    unit_assert_eq!(engine.objects[pusher_idx].fixed_velocity.x.val() => 64225);
    unit_assert_eq!(engine.objects[pusher_idx].fixed_velocity.y => C4Fixed::ZERO);
    unit_assert_eq!(engine.objects[pusher_idx].state.direction => Direction::Right);
    ...
    unit_assert_eq!(engine.objects[crate_idx].state.controller => 7, "a successful PUSH copies the pusher Controller before range checks");
}
...
fn push_procedure_resets_without_grab_ocf() {
    ...
    let plain = test_definition("Rock", "Rock", script);
    engine.register_test_definition(plain);
    let rock_id = spawn_fixture!(engine, "Rock", with_position: Vector2::new(10, 0));
    let mut push_state = ActionState::new("Push");
    push_state.target = Some(rock_id);
    let pusher_id = spawn_fixture!(engine, "Pusher", with_position: Vector2::new(0, 0), with_action: push_state, with_command_direction: CommandDirection::Right);

    engine.tick_without_snapshot().test_value();
    let pusher_idx = engine.test_object_index(pusher_id);
    let rock_idx = engine.test_object_index(rock_id);
    unit_assert_ne!(engine.objects[pusher_idx].state.action.name => "Push");
    unit_assert_eq!(engine.objects[rock_idx].fixed_velocity.x => C4Fixed::ZERO);
}
...
fn pull_procedure_uses_walk_and_push_physicals() {
    ...
    let puller_id = spawn_fixture!(engine, "Pusher", with_position: Vector2::new(0, 0), with_controller: 9, with_vertices: vertices, with_action: pull_state, with_command_direction: CommandDirection::Right);
    ...
    unit_assert_eq!(engine.objects[crate_idx].fixed_velocity.x => C4Fixed::ZERO);
    // Own move: iTargetX = 12+16 = 28; xdir = 64225 +
    // 64225*BoundBy(28,-10,10)/10 = raw 128450 (C4Object.cpp:5164).
    unit_assert_eq!(engine.objects[puller_idx].fixed_velocity.x.val() => 128450);
    unit_assert_eq!(engine.objects[puller_idx].fixed_velocity.y => C4Fixed::ZERO);
    unit_assert_eq!(engine.objects[puller_idx].state.direction => Direction::Right);
    unit_assert_eq!(engine.objects[puller_idx].state.action.name => "Pull");
    unit_assert_eq!(
        (
    ...
    unit_assert_eq!(engine.objects[crate_idx].state.controller => 9, "a successful PULL copies the puller Controller before range checks");
}
...
fn stationary_push_retains_one_phase_step_but_pull_freezes() {
    ...
    for (action, stationary_advance) in [("Push", 1_i32), ("Pull", 0_i32)] {
        ...
        for frame in 1_u32..=20 {
            ...
            unit_assert_eq!(object.fixed_velocity.x => C4Fixed::ZERO, "{action} is stationary");
            unit_assert_eq!(object.state.action.name => action);

            let accumulated = frame as i32 * stationary_advance;
            let expected_phase = accumulated / 13;
            let expected_delay = accumulated % 13;
            unit_assert_eq!((object.state.action.phase, object.state.action.ticks) => (expected_phase, expected_delay), "{action} phase state after frame {frame}");
        }
    }
}
...
fn pinned_push_phase_trajectory_matches_cpp_for_one_hundred_frames() {
    ...
    let pusher = spawn_fixture!(engine, "Pusher", with_position: Vector2::ZERO, with_action: push, with_command_direction: CommandDirection::Right);

    for frame in 1_u32..=100 {
        let pusher_index = engine.test_object_index(pusher);
        engine.objects[pusher_index].set_position(Vector2::ZERO);
        engine.objects[pusher_index].set_fixed_velocity(FixedVec2::ZERO);
        let crate_index = engine.test_object_index(crate_id);
        ...
        unit_assert_eq!(object.fixed_velocity.x.val() => 64225);
        unit_assert_eq!(
...
fn pinned_horse_pull_wraps_and_refires_start_call_after_twenty_frames() {
    ...
    let mut hooks = DebuggerHooks::new();
    {
        ...
        hooks.set_on_call(move |name, _args| {
            if name == "Pulling" {
                calls.lock().test_value().push(name.to_string());
            }
        });
    }

    let mut horse = test_definition("HRSE", "Horse", "#strict 2\nfunc Pulling() { return 1; }\n");
    ...
    let wagon = spawn_fixture!(engine, "WAGN", with_position: Vector2::new(12, 0));
    let mut pull = ActionState::new("Pull");
    pull.target = Some(wagon);
    let horse = spawn_fixture!(engine, "HRSE", with_position: Vector2::ZERO, with_action: pull, with_command_direction: CommandDirection::Right);
    ...
    for frame in 1_i32..=20 {
        ...
        unit_assert_eq!(engine.objects[horse_index].fixed_velocity.x.val() => 183500);
        unit_assert_eq!(action.phase => frame % 20, "PULL phase at frame {frame}");
        unit_assert_eq!(action.ticks => 0, "advance 28 crosses Delay 13 each frame");
        unit_assert_eq!(calls.lock().unwrap().len() => usize::from(frame == 20), "Pulling StartCall count at frame {frame}");
    }
}
...
fn train_physical_crew_fixture(use_fair_crew: bool) -> (Engine, ObjectId, usize) {
    ...
    engine
        .join_player(JoinPlayerConfig {
            ...
            ..join_player_config("Training owner")
        })
        .test_value();
...
fn persisted_crew_scale(engine: &Engine, crew: ObjectId) -> i32 {
    ...
    let link = state.crew_info_links.get(&crew).test_value();
    state
        .crew_info_rosters
        .get(&link.player_id)
...
fn train_physical_under_fair_crew_persists_without_changing_live_physicals() {
    ...
    unit_assert_eq!(engine.object_physical(crew_index).scale => 33_500);
    ...
    unit_assert_eq!(engine.object_physical(crew_index).scale => 33_500);
    unit_assert_eq!(engine.objects[crew_index].state.info_physical.expect("crew carries raw info physicals").scale => 80_005);
    unit_assert_eq!(persisted_crew_scale(&engine, crew) => 80_005);

    engine.set_use_fair_crew(false);
    unit_assert_eq!(engine.object_physical(crew_index).scale => 80_005, "disabling fair crew reveals the training instead of losing it");
}
...
fn train_physical_trains_temporary_and_stacked_values_under_fair_crew() {
    ...
    unit_assert_eq!(engine.object_physical(crew_index).scale => 50_005);
    unit_assert_eq!(engine.objects[crew_index].state.temporary_physical.expect("temporary mode stays active").scale => 50_005);
    unit_assert_eq!(
    ...
    unit_assert_eq!(engine.objects[crew_index].state.info_physical.expect("raw info is trained alongside temporary state").scale => 80_005);
    unit_assert_eq!(persisted_crew_scale(&engine, crew) => 80_005);
}
...
fn train_physical_is_live_and_persistent_when_fair_crew_is_off() {
    ...
    unit_assert_eq!(engine.object_physical(crew_index).scale => 80_000);
    unit_assert!(engine.train_physical(crew_index, "Scale", 5, C4_MAX_PHYSICAL));
    unit_assert_eq!(engine.object_physical(crew_index).scale => 80_005);
    unit_assert_eq!(persisted_crew_scale(&engine, crew) => 80_005);
}
...
fn train_physical_requires_info_or_temporary_physicals() {
    ...
    let definition = test_definition("Sheep", "Sheep", script);
    ...
    let crew_id = spawn_fixture!(engine, "Sheep", with_crew_member: true);
    let crew_idx = engine.test_object_index(crew_id);
    unit_assert!(!engine.train_physical(crew_idx, "Fight", 1, C4_MAX_PHYSICAL));
    unit_assert_eq!(engine.objects[crew_idx].state.info_physical => None);
    ...
    unit_assert!(engine.train_physical(crew_idx, "Fight", 1, C4_MAX_PHYSICAL));
    unit_assert_eq!(engine.objects[crew_idx].state.temporary_physical.expect("temporary set remains active").fight => 101);
    unit_assert_eq!(engine.objects[crew_idx].state.info_physical => None);
}
...
fn object_physical_uses_promoted_info_when_fair_crew_is_off() {
    ...
    engine
        .join_player(JoinPlayerConfig {
            ...
            ..join_player_config("Info owner")
        })
        .test_value();
    let crew = engine.player(0).test_value().crew()[0];
    let crew_index = engine.test_object_index(crew);
    unit_assert_eq!(engine.fair_crew_strength() => 1_000);
    unit_assert!(!engine.use_fair_crew());

    unit_assert_eq!(engine.object_physical(crew_index).walk => 80_000);
}
...
fn object_physical_trains_fair_crew_from_live_strength() {
    ...
    let mut engine = Engine::new();
    // Fair crew is this assertion's subject, so state it rather than lean
    // on the engine default: `C4GameParameters` compiles `UseFairCrew`
    // false absent a scenario forcing it (C4GameParameters.cpp:560), and
    // the port's fresh-engine default disagrees (clonk-org/clonk-rs#1071).
    engine.set_use_fair_crew(true);
    engine.set_fair_crew_strength(5_000);
    engine.register_test_definition(definition);
    let mut changed = test_definition("NEWP", "Changed crew", "#strict\npublic func ReadFair() { return([GetPhysical(\"Scale\"), GetPhysical(\"Scale\", 1)]); }\n");
    ...
    engine.register_test_definition(changed);
    let mut start = PlayerStart::default();
    ...
    unit_assert_eq!(physical.scale => 37_000);
    unit_assert_eq!(physical.hangle => 46_000);
    unit_assert_eq!(physical.swim => 55_000);
    unit_assert_eq!(physical.fight => 64_000);
    unit_assert_eq!(
        engine
    ...
    engine.call_test_object_function(crew_index, "Swap", Vec::new());
    let crew_index = engine.test_object_index(crew);
    unit_assert_eq!(engine.objects[crew_index].definition_id => "NEWP");
    unit_assert_eq!(engine.object_physical(crew_index) => physical, "fair crew remains sourced from Info->pDef after ChangeDef");
    unit_assert_eq!(
        engine
            .call_object_function(crew_index, "ReadFair", Vec::new())
...
fn fair_crew_projection_is_cached_per_definition_until_synchronize() {
    ...
    engine
        .join_player(JoinPlayerConfig {
            ...
            ..join_player_config("Cache owner")
        })
        .test_value();

    let crew = engine.player(0).test_value().crew().to_vec();
    unit_assert_eq!(crew.len() => 2);
    let first_index = engine.test_object_index(crew[0]);
    let second_index = engine.test_object_index(crew[1]);
    ...
    unit_assert_eq!(engine.object_physical(second_index) => initial);
    unit_assert_eq!(engine.object_physical(first_index) => initial);
    unit_assert_eq!(engine.call_object_function(second_index, "ReadEnergy", Vec::new()).expect("script reads the shared projection") => Value::Int(initial.energy));
    unit_assert_eq!(engine.rng => rng_after_join, "repeated reads and a second same-definition crew consume no Random calls");
    ...
    unit_assert_eq!(engine.rng => rng_after_synchronize);
    unit_assert_eq!(engine.call_object_function(first_index, "FailMagicUnderload", Vec::new()).expect("failed magic underload returns") => Value::Bool(false));
    unit_assert_eq!(
        engine
            .call_object_function(first_index, "HookState", Vec::new())
    ...
    unit_assert_eq!(engine.call_object_function(first_index, "ArmProbe", Vec::new()).expect("post-sync compat-path probe arms") => Value::Bool(true));
    unit_assert_eq!(
        engine
    ...
    unit_assert_eq!(engine.rng => rng_after_refill);
    unit_assert_eq!(
        engine
            .call_object_function(first_index, "HookState", Vec::new())
...
fn fair_crew_uses_custom_rank_base_and_definition_script_override() {
    ...
    unit_assert_eq!(base_boundary.magic => 46_007);
    unit_assert_eq!(base_boundary.energy => 55_000);
    ...
    let physical = engine.object_physical(crew_index);
    unit_assert_eq!(physical.magic => 46_007);
    unit_assert_eq!(physical.energy => 55_000);
    unit_assert_eq!(physical.breath => 123);
    unit_assert_eq!(
        engine
            .call_object_function(crew_index, "ReadFair", Vec::new())
...
fn engine_state_retains_forced_control_style_for_later_joins() {
    ...
    unit_assert_eq!(restored.capture_state().forced_control_style => Some(true));
}
...
fn engine_state_retains_fair_crew_parameters_with_legacy_defaults() {
    ...
    let mut restored = Engine::new();
    restored.restore_state(&decoded).test_value();
    unit_assert!(!restored.use_fair_crew());
    unit_assert_eq!(restored.fair_crew_strength() => 5_000);

    let mut legacy: serde_json::Value = serde_json::from_str(&encoded).test_value();
    let object = legacy.as_object_mut().test_value();
    object.remove("use_fair_crew");
    object.remove("fair_crew_strength");
    let legacy = EngineState::from_json_str(&legacy.to_string()).test_value();
    let mut restored = Engine::new();
    restored.restore_state(&legacy).test_value();
    unit_assert!(restored.use_fair_crew());
    unit_assert_eq!(restored.fair_crew_strength() => 1_000);
}
...
fn engine_state_retains_forced_auto_context_menu_for_later_joins() {
    ...
    unit_assert_eq!(restored.capture_state().forced_auto_context_menu => Some(true));
}
    ...
    for (forced, preference) in [(true, false), (false, true)] {
        ...
        let joined = engine
            .join_player(JoinPlayerConfig {
                ...
                ..join_player_config("Tester")
            })
            .test_value();
        unit_assert_eq!(engine.player(joined.number()).expect("joined player").control_style() => forced);
    }
}
...
fn forced_auto_context_menu_overrides_player_preference_both_ways() {
    ...
    for (forced, preference) in [(true, false), (false, true)] {
        ...
        let joined = engine
            ...
            .test_value();
        unit_assert_eq!(engine.player(joined.number()).expect("joined player").control.auto_context_menu => forced);
    }
}
...
fn state_round_trip_preserves_physicals_and_energy_loss_cause() {
    ...
    let definition = test_definition("Clonk", "Clonk", script);
    let mut engine = Engine::with_seed(9);
    engine.register_test_definition(definition);
    let id = spawn_fixture!(engine, "Clonk", with_energy: 40);
    let idx = engine.test_object_index(id);
    ...
    let idx = restored.test_object_index(id);
    unit_assert_eq!(restored.objects[idx].state.info_physical => Some(PhysicalInfo {fight: 12_345, walk: 35_000,..PhysicalInfo::default()}));
    unit_assert_eq!(restored.objects[idx].state.temporary_physical => Some(PhysicalInfo {walk: 99_000,..PhysicalInfo::default()}));
    unit_assert_eq!(restored.objects[idx].state.physical_changes => vec![("Walk".to_string(), 35_000)]);
    unit_assert_eq!(restored.objects[idx].last_energy_loss_cause => 3);
}

#[test]
fn fire_effect_executes_once_per_tick() {
    let script = NOOP_DEFINITION_SCRIPT;
    let mut definition = test_definition("Torch", "Torch", script);
    definition.set_fire_properties(1, true, true);
    let mut engine = Engine::with_seed(1);
    engine.register_test_definition(definition);
    let id = engine.spawn_test_object(SpawnConfig::new("Torch"));
    let idx = engine.test_object_index(id);
...
fn exec_life_breath_depletes_then_asphyxiates_in_semisolid() {
    ...
    let id = spawn_fixture!(engine, "Diver", with_alive: true, with_position: Vector2::new(2, 26), with_vertices: vertices, with_energy: 50_000);
    let idx = engine.test_object_index(id);
    unit_assert_eq!(engine.objects[idx].state.breath => 50_000, "breath fills from the physicals at birth (C4Object.cpp:193)");
    ...
    unit_assert_eq!(engine.objects[idx].state.breath => 50_000, "Tick5 gate");
    engine.tick_without_snapshot().test_value();
    unit_assert_eq!(engine.objects[idx].state.breath => 50_000 - 2 * C4_MAX_PHYSICAL / 100);
    unit_assert_eq!(engine.objects[idx].state.energy => 50_000, "breath before energy");
    let _ = mirror.random(5); // the BubbleOut x argument (C4Object.cpp:905)
    let _ = mirror.random(i32::MAX); // the per-object Step draw
    unit_assert_eq!(engine.rng => mirror, "exactly one extra synced draw");
    ...
    unit_assert_eq!(engine.objects[idx].last_energy_loss_cause => 7);
}
...
fn exec_life_vehicle_material_is_breathable_like_cpp() {
    ...
    let grid = landscape::PixelGrid::new(8, 40, bytes, densities, material_names, vec![None; 128]);
    let mut landscape = Landscape::new(8, vec![40; 8]).test_value();
...
fn exec_life_material_landscape(
    ...
    let grid = landscape::PixelGrid::new(width, height, pixels, densities, names, vec![None; 128]);
...
fn exec_life_periodic_base_energy_buys_and_transfers_exact_units() -> Result<(), EngineError> {
    ...
    let base_idx = engine.find_object_index(base_id).test_value();
    engine.objects[base_idx]
        .state
        .effects
        .push(EffectState::new("OwnerSwap").with_command_target(Some(base_id.as_u64() as i32)));
    ...
    let crew_idx = engine.find_object_index(crew_id).test_value();
    engine.objects[crew_idx]
        .state
        .effects
        .push(EffectState::new("CreditProbe").with_command_target(Some(crew_id.as_u64() as i32)));

    engine.tick_without_snapshot()?;
    engine.tick_without_snapshot()?;
    unit_assert_eq!(engine.object_snapshot(crew_id).unwrap().energy => 10_000);
    engine.tick_without_snapshot()?;
    unit_assert_eq!(engine.object_snapshot(crew_id).unwrap().energy => 12_000);
    unit_assert_eq!(engine.object_snapshot(base_id).unwrap().energy => 48_000);
    unit_assert_eq!(engine.player(1).unwrap().wealth() => 5);
    unit_assert_eq!(engine.objects[base_idx].state.local_vars.get("debit_by") => Some(&Value::Int(1)));
    unit_assert_eq!(
    ...
    unit_assert_eq!(engine.object_snapshot(crew_id).unwrap().energy => 50_000);
    unit_assert_eq!(engine.object_snapshot(base_id).unwrap().energy => 700);

    engine.objects[crew_idx].state.energy = 48_000;
    for _ in 0..3 {
        engine.tick_without_snapshot()?;
    }
    unit_assert_eq!(engine.object_snapshot(crew_id).unwrap().energy => 48_700);
    unit_assert_eq!(engine.object_snapshot(base_id).unwrap().energy => 0);
    Ok(())
}
...
fn exec_life_periodic_magic_uses_global_debit_gate_and_whole_points() -> Result<(), EngineError> {
    fn run(override_source: &str) -> Result<(i32, i32), EngineError> {
        let mut donor = test_definition("DONR", "Donor", override_source);
        donor.set_physical(PhysicalInfo {
            magic: 50_000,
            ..PhysicalInfo::default()
        });
        let mut recipient = test_definition("RCPT", "Recipient", "");
        ...
        Ok((
            engine.object_snapshot(donor_id).test_value().magic_energy,
            engine
                .object_snapshot(recipient_id)
                .test_value()
                .magic_energy,
        ))
    }

    unit_assert_eq!(run("")? => (500, 1_000));
    unit_assert_eq!(
...
#[test]
fn exec_life_periodic_corrosion_and_closed_container_use_cached_in_mat() -> Result<(), EngineError>
{
    let library = MaterialLibrary::parse("[Material Acid]\nName=Acid\nDensity=0\nCorrosive=31\n")
        .test_value();
    ...
    let mut victim = test_definition(
        "VCTM",
        "Victim",
        r#"
    ...
    let mut resistant = test_definition("RSST", "Resistant", "");
    ...
    let open = test_definition("OPEN", "Open", "");
    let mut closed = test_definition("CLSD", "Closed", "");
    ...
    let direct_idx = engine.find_object_index(direct).test_value();
    engine.objects[direct_idx]
        .state
        .effects
        .push(EffectState::new("CorrosionProbe").with_command_target(Some(direct.as_u64() as i32)));
    ...
    unit_assert_eq!(engine.object_snapshot(direct).unwrap().energy => 50_000);
    engine.tick_without_snapshot()?;
    unit_assert_eq!(engine.object_snapshot(direct).unwrap().energy => 48_000);
    unit_assert_eq!(engine.objects[direct_idx].last_energy_loss_cause => 7);
    unit_assert_eq!(engine.objects[direct_idx].state.local_vars.get("corrosion_change") => Some(&Value::Int(-2_000)));
    unit_assert_eq!(engine.objects[direct_idx].state.local_vars.get("corrosion_cause") => Some(&Value::Int(C4FX_CALL_ENG_CORROSION)));
    unit_assert_eq!(engine.objects[direct_idx].state.local_vars.get("corrosion_by") => Some(&Value::Int(7)));
    unit_assert_eq!(engine.object_snapshot(resist).unwrap().energy => 50_000);
    unit_assert_eq!(engine.object_snapshot(open_child).unwrap().energy => 48_000);
    unit_assert_eq!(engine.object_snapshot(closed_child).unwrap().energy => 50_000);
    ...
    let mut mild_definition = test_definition(
        "MILD",
    ...
    let mild_idx = mild_engine.find_object_index(mild).test_value();
    mild_engine.objects[mild_idx]
        .state
        .effects
        .push(EffectState::new("CorrosionProbe").with_command_target(Some(mild.as_u64() as i32)));
    mild_engine.objects[mild_idx].last_energy_loss_cause = 9;
    mild_engine.frame = 9;
    mild_engine.tick_without_snapshot()?;
    unit_assert_eq!(mild_engine.object_snapshot(mild).unwrap().energy => 49_000);
    unit_assert_eq!(
        mild_engine.objects[mild_idx]
            .state
            .local_vars
    ...
    unit_assert_eq!(mild_engine.objects[mild_idx].state.local_vars.get("corrosion_cause") => Some(&Value::Int(C4FX_CALL_ENG_CORROSION)));
    unit_assert_eq!(mild_engine.objects[mild_idx].state.local_vars.get("corrosion_by") => Some(&Value::Int(9)));
    Ok(())
}
...
fn exec_life_periodic_lava_uses_pre_movement_cache_and_standard_fire_draw(
    ...
    let library = MaterialLibrary::parse("[Material Lava]\nName=Lava\nDensity=0\nIncindiary=1\n")
        .test_value();
    ...
    let state = engine.object_snapshot(id).test_value();
    unit_assert_ne!(state.position.x => 1, "movement left the lava pixel");
    unit_assert!(state.on_fire, "cached frame-start Lava still ignites");
    unit_assert_eq!(state.fire_phase => expected_phase);
    unit_assert_eq!(state.local_vars.get("cause") => Some(&Value::Int(7)));
    unit_assert_eq!(engine.rng.count => expected_rng.count);
    unit_assert_eq!(engine.rng.hold => expected_rng.hold);
    Ok(())
}

#[test]
fn exec_life_periodic_nonlife_decay_respects_base_and_energy_holder() -> Result<(), EngineError> {
    let mut normal = test_definition("NORM", "Normal", "");
    normal.set_physical(PhysicalInfo {
        energy: 10_000,
        ..PhysicalInfo::default()
    });
    let mut holder = test_definition("HOLD", "Holder", "");
    ...
    let protected_idx = engine.find_object_index(protected).test_value();
    engine.objects[protected_idx].state.base = 1;

    for _ in 0..10 {
        engine.tick_without_snapshot()?;
    }
    unit_assert_eq!(engine.object_snapshot(ordinary).unwrap().energy => 4_000);
    unit_assert_eq!(engine.object_snapshot(held).unwrap().energy => 5_000);
    unit_assert_eq!(engine.object_snapshot(protected).unwrap().energy => 5_000);

    engine.set_base_regenerate_energy_enabled(false);
    for _ in 0..10 {
        engine.tick_without_snapshot()?;
    }
    unit_assert_eq!(engine.object_snapshot(protected).unwrap().energy => 4_000);
    Ok(())
}

#[test]
fn exec_life_periodic_growth_stays_suppressed_while_on_fire() -> Result<(), EngineError> {
    let mut definition = test_definition("TREE", "Tree", "");
    ...
    engine.frame = 34;
    engine.tick_without_snapshot()?;
    unit_assert_eq!(engine.object_snapshot(id).unwrap().construction => 50_000);
    Ok(())
}

#[test]
fn exec_life_periodic_birthday_updates_info_and_presents_once() -> Result<(), EngineError> {
    let mut crew = test_definition("BDAY", "Birthday crew", "");
    ...
    engine.join_player(JoinPlayerConfig {
        ...
        ..join_player_config("Birthday player")
    })?;
    let crew_id = *engine.player(0).unwrap().crew().first().test_value();
    ...
    let live_info = engine.crew_object_info(crew_id).test_value();
    unit_assert_eq!(
        (
            live_info.rounds,
    ...
    let restored_info = engine.crew_object_info(crew_id).test_value();
    unit_assert_eq!(
        (
...
fn set_physical_host_fn_applies_to_engine_state() {
    ...
    let definition = test_definition("Mutant", "Mutant", script);
    let mut engine = Engine::with_seed(3);
    engine.register_test_definition(definition);
    let id = engine.spawn_test_object(SpawnConfig::new("Mutant"));

    engine.tick_without_snapshot().test_value();
    let idx = engine.test_object_index(id);
    unit_assert_eq!(engine.objects[idx].state.temporary_physical => Some(PhysicalInfo {walk: 42_000,..PhysicalInfo::default()}));
    unit_assert_eq!(engine.object_physical(idx).walk => 42_000);
}
...
fn chop_procedure_zeroes_velocity_and_damages_on_aligned_tick() {
    ...
    let mut definition = test_definition("Chopper", "Chopper", script);
    let mut actions = HashMap::new();
    ...
    let mut hooks = DebuggerHooks::new();
    {
        ...
        hooks.set_on_call(move |name, args| {
            if name == "Damage" {
                damage_events.lock().test_value().push(args.to_vec());
            }
        });
    }
    let mut tree = test_definition(
        "Tree",
        "Tree",
    ...
    let target = spawn_fixture!(engine, "Tree", with_position: Vector2::new(0, 0), with_category: CATEGORY_STATIC_BACK, with_loaded: true);
    let mut action = ActionState::new("Chop");
    action.target = Some(target);
    let id = spawn_fixture!(engine, "Chopper", with_position: Vector2::new(0, 0), with_velocity: Vector2::new(5, -3), with_owner: 7, with_action: action);
    let target_idx = engine.test_object_index(target);
    unit_assert_ne!(engine.objects[target_idx].state.ocf & ocf::CHOP => 0);
    ...
    for frame in 1..30 {
        ...
        unit_assert_eq!(object.action.name => "Chop");
        unit_assert_eq!(object.action.target => Some(target));
        unit_assert_eq!(object.velocity => Vector2::ZERO);
        unit_assert_eq!(object.position => Vector2::new(0, 0));
        unit_assert_eq!(snapshot.object(target).expect("tree present").damage => 0, "frame {frame} is not both Tick3 and Tick10");
    }
    unit_assert!(damage_events.lock().unwrap().is_empty());

    let snapshot = engine.test_tick();
    unit_assert_eq!(snapshot.object(target).expect("tree present").damage => 10);
    unit_assert_eq!(
...
fn chop_procedure_stands_in_walk_when_target_stops_being_choppable() {
    ...
    let mut chopper = test_definition("CLNK", "Clonk", script);
    let mut actions = HashMap::new();
    ...
    let target = test_definition("TREE", "Tree", script);
    let mut engine = Engine::with_seed(21);
    engine.register_definition(chopper).test_value();
    engine.register_definition(target).test_value();
    let target = engine
    ...
    unit_assert_eq!(snapshot.object(clonk).unwrap().action.name => "Walk");
}
...
fn wind_force_respects_variation_and_period() {
    ...
    for frame in 0..5 {
        unit_assert_eq!(settings.wind_force(frame) => 2);
    }

    let default_period = EnvironmentSettings::new(1).with_wind_variation(3, 0);
    unit_assert_eq!(default_period.wind_variation => 3);
    unit_assert_eq!(default_period.wind_period => 2);
}
...
fn ambient_temperature_cycles_with_climate() {
    ...
    unit_assert_eq!(settings.ambient_temperature(0) => 15);
    unit_assert_eq!(settings.ambient_temperature(3) => 23);
    unit_assert_eq!(settings.ambient_temperature(9) => 7);
}
...
fn ambient_temperature_resets_when_cycle_disabled() {
    ...
    unit_assert_ne!(settings.ambient_temperature(1) => 3);

    settings = settings.with_temperature_cycle(0, 6, 0);
    unit_assert_eq!(settings.temperature_variation => 0);
    unit_assert_eq!(settings.temperature_period => 0);
    unit_assert_eq!(settings.temperature_phase => 0);
    unit_assert_eq!(settings.ambient_temperature(1) => 3);
}
...
fn temperature_rises_towards_target_without_year_speed() {
    ...
    unit_assert_eq!(settings.temperature => -39);
}
...
fn season_gamma_winter_start_is_the_exact_winter_ramp() {
    ...
    let settings = EnvironmentSettings::new(0)
        .with_season(0)
        .with_gamma_enabled();
    unit_assert_eq!(settings.season_gamma() => Some((RgbColor::new(0x00, 0x00, 0x00), RgbColor::new(0x7f, 0x7f, 0x90), RgbColor::new(0xef, 0xef, 0xff),)));
}
...
fn season_gamma_blends_winter_into_spring_with_truncating_division() {
    ...
    let settings = EnvironmentSettings::new(0)
        .with_season(12)
        .with_gamma_enabled();
    unit_assert_eq!(settings.season_gamma() => Some((RgbColor::new(3, 7, 0), RgbColor::new(134, 142, 136), RgbColor::new(246, 246, 240),)));
}
...
fn season_gamma_negative_temperature_shifts_blue_up_red_green_down() {
    ...
    let settings = EnvironmentSettings::new(0)
        ...
        .with_temperature(-25)
        .with_gamma_enabled();
    unit_assert_eq!(settings.season_gamma() => Some((RgbColor::new(0, 0, 12), RgbColor::new(115, 115, 156), RgbColor::new(227, 227, 255),)));
}
...
fn season_gamma_season_hundred_wraps_to_winter() {
    ...
    let at_zero = EnvironmentSettings::new(0)
        .with_season(0)
        .with_gamma_enabled();
    unit_assert_eq!(at_hundred.season_gamma() => at_zero.season_gamma());
}
...
fn season_gamma_uses_cpp_truncating_remainder_for_small_negative_season() {
    ...
    unit_assert_eq!(negative.season_gamma() => zero.season_gamma());
}
...
fn season_gamma_suppressed_by_no_gamma() {
    ...
    unit_assert!(settings.no_gamma);
    unit_assert_eq!(settings.season_gamma() => None);
}
...
fn season_advance_resets_delay_to_zero() {
    ...
    let mut rng = LcgRng::seed_from_u64(0);
    settings.advance_frame(&mut rng, 35);
    unit_assert_eq!((settings.season, settings.season_delay) => (10, 150));
    settings.advance_frame(&mut rng, 70);
    // 300 >= 200: advance once, delay = 0 (NOT 300 - 200 = 100).
    unit_assert_eq!((settings.season, settings.season_delay) => (11, 0));
}
...
fn season_reaches_the_default_max_of_one_hundred_before_wrapping() {
    ...
    let mut rng = LcgRng::seed_from_u64(0);
    settings.advance_frame(&mut rng, 35);
    unit_assert_eq!(settings.season => 100);
    settings.advance_frame(&mut rng, 70);
    unit_assert_eq!(settings.season => 0);
}
...
fn season_wraps_to_scenario_min_beyond_scenario_max() {
    ...
    let mut rng = LcgRng::seed_from_u64(0);
    settings.advance_frame(&mut rng, 35);
    unit_assert_eq!(settings.season => 20);
}
...
fn season_pins_at_min_when_max_below_min() {
    ...
    let mut rng = LcgRng::seed_from_u64(0);
    settings.advance_frame(&mut rng, 35);
    unit_assert_eq!(settings.season => 6, "below Max the advance is plain ++");
    settings.season = 10;
    settings.advance_frame(&mut rng, 70);
    unit_assert_eq!(settings.season => 50, "11 > Max(10) wraps to Min(50)");
    settings.advance_frame(&mut rng, 105);
    unit_assert_eq!(settings.season => 50, "51 > Max(10) re-pins to Min(50)");
}
...
fn preloaded_season_delay_advances_even_with_zero_year_speed() {
    ...
    let mut rng = LcgRng::seed_from_u64(0);
    settings.advance_frame(&mut rng, 35);
    unit_assert_eq!((settings.season, settings.season_delay) => (4, 0));
}
...
fn negative_year_speed_never_advances_or_regresses_season() {
    ...
    settings.advance_frame(&mut rng, 35);
    settings.advance_frame(&mut rng, 70);
    unit_assert_eq!((settings.season, settings.season_delay) => (40, -600));
}
...
fn temperature_drifts_toward_climate_when_range_is_zero() {
    ...
    let mut rng = LcgRng::seed_from_u64(0);
    settings.advance_frame(&mut rng, 35);
    unit_assert_eq!(settings.temperature => 1);
}
...
fn temperature_target_truncates_the_cpp_cos_product() {
    ...
    let mut rng = LcgRng::seed_from_u64(0);
    settings.advance_frame(&mut rng, 35);
    unit_assert_eq!(settings.temperature => -21);
}
...
fn temperature_target_truncates_toward_zero_past_half_year() {
    ...
    let mut rng = LcgRng::seed_from_u64(0);
    settings.advance_frame(&mut rng, 35);
    unit_assert_eq!(settings.temperature => 9);
}
...
fn weather_init_ingests_start_season_bounds_without_extra_clamp() {
    ...
    engine.apply_weather_init(&init).test_value();
    unit_assert_eq!(engine.environment.season => 120);
    unit_assert_eq!((engine.environment.season_min, engine.environment.season_max) => (110, 130));
}
...
"#;
    let mut definition = test_definition("FXP1", "Precipitation", script);
...
fn get_temperature_returns_weather_temperature_after_init_and_season_drift() {
    ...
    engine.apply_weather_init(&init).test_value();
    let probe = engine.spawn_test_object(SpawnConfig::new("WTMP"));
    let probe_index = engine.test_object_index(probe);

    unit_assert_eq!(engine.environment().climate => 20);
    unit_assert_eq!(engine.environment().temperature => 20);
    unit_assert_eq!(engine.call_object_function(probe_index, "ReadTemperature", Vec::new()).expect("GetTemperature succeeds after weather init") => Value::Int(20));

    for _ in 0..35 {
        engine.tick_without_snapshot().test_value();
    }
    unit_assert_eq!(engine.environment().temperature => 19);
    let probe_index = engine.test_object_index(probe);
    unit_assert_eq!(
        engine
...
fn weather_init_launches_cpp_precipitation_objects_in_draw_order() {
    ...
    unit_assert_eq!(water.index() => 0, "fixture exercises the falsy material id");
    let mut engine = Engine::with_seed(17);
    engine.set_materials(materials);
    engine.set_landscape(Landscape::flat(1_100, 100));
    engine.register_test_definition(test_precipitation_definition());
    ...
    unit_assert_eq!(engine.rng => mirror, "LaunchCloud adds no RNG draws");
    let clouds: Vec<_> = engine
        .objects
        .iter()
        .filter(|object| object.definition_id == "FXP1")
        .collect();
    unit_assert_eq!(clouds.len() => 2, "GBackWdt 1100 launches two clouds");
    for (cloud, (x, width, strength)) in clouds.into_iter().zip(expected) {
        unit_assert_eq!(cloud.state.position => Vector2::new(x, -2));
        unit_assert_eq!(cloud.state.owner => OWNER_NONE);
        unit_assert_eq!(cloud.state.controller => OWNER_NONE);
        unit_assert_eq!(cloud.state.category => CATEGORY_VEHICLE);
        unit_assert_eq!(cloud.state.construction => FULL_CON);
        unit_assert_eq!(cloud.state.status => ObjectStatus::Normal);
        unit_assert_eq!(cloud.state.action.name => "Process");
        unit_assert_eq!((cloud.state.action.phase, cloud.state.action.ticks) => (0, 0));
        unit_assert_eq!(cloud.state.local_vars.get("iMat") => Some(&Value::Nil), "material index zero is normalized at the pre-strict-3 Activate boundary");
        unit_assert_eq!(cloud.state.local_vars.get("iLength") => Some(&Value::Int(width)));
        unit_assert_eq!(cloud.state.local_vars.get("iStrength") => Some(&Value::Int(strength)));
        unit_assert_eq!(cloud.state.local_vars.get("iMovement") => Some(&Value::Int(1)));
        unit_assert_eq!(cloud.fixed_velocity.x => math::fixed10(37));
        unit_assert_eq!(cloud.fixed_velocity.y => C4Fixed::ZERO);
        unit_assert!(cloud.state.mobile, "SetXDir mobilizes the cloud");
    }
}
...
fn weather_init_missing_precipitation_material_consumes_draws_without_spawning() {
    ...
    unit_assert!(engine.objects.is_empty());
    unit_assert_eq!(engine.next_object_id => before_number);
}
...
fn snapshot_reports_environment_metrics() {
    ...
    unit_assert_eq!(snapshot.environment.settings => environment);
    unit_assert_eq!(snapshot.environment.wind_force => environment.wind_force(0));
    unit_assert_eq!(snapshot.environment.ambient_temperature => environment.ambient_temperature(0));
    unit_assert_eq!(snapshot.environment.precipitation => environment.precipitation());
    unit_assert_eq!(snapshot.environment.sky_color => environment.sky_color());
}
...
fn environment_sky_color_can_be_configured_and_cleared() {
    ...
    unit_assert_eq!(configured.sky_color() => Some(RgbColor::new(5, 10, 15)));

    let cleared = configured.without_sky_color();
    unit_assert!(cleared.sky_color().is_none());
}
...
fn environment_time_advances_each_tick() {
    ...
    unit_assert_eq!(engine.environment().time_of_day => 2300);

    engine.tick_without_snapshot().test_value();
    unit_assert_eq!(engine.environment().time_of_day => 2375);

    engine.tick_without_snapshot().test_value();
    unit_assert_eq!(engine.environment().time_of_day => 50);
}

#[test]
fn precipitation_clamps_to_range() {
    let wet = EnvironmentSettings::new(0).with_precipitation(140);
    unit_assert_eq!(wet.precipitation() => 100);

    let balanced = EnvironmentSettings::new(0).with_precipitation(42);
    unit_assert_eq!(balanced.precipitation() => 42);

    let dry = EnvironmentSettings::new(0).with_precipitation(-180);
    unit_assert_eq!(dry.precipitation() => -100);
}
...
fn lightning_event_initializes_at_cpp_default_position_before_activate() {
    ...
    let index = engine
        ...
        .test_value();
    unit_assert_eq!(engine.objects[index].state.position => Vector2::new(50, 50), "C++ creates FXL1 at the native default position before Activate");
}
...
"#;
    ...
    let mut engine = Engine::with_seed(9);
    engine.register_test_script_definition("LAYR", "Layer", "#strict\n");
    engine.register_test_script_definition("CALL", "Caller", caller_script);
    engine.register_test_script_definition("FXL1", "Lightning", lightning_script);
    let layer = engine.spawn_test_object(SpawnConfig::new("LAYR"));
    let caller = spawn_fixture!(engine, "CALL", with_position: Vector2::new(91, 82), with_owner: 3, with_controller: 7, with_layer: layer);
    let rng_count_before = engine.debug_rng_clone().count;
    let caller_index = engine.test_object_index(caller);
    unit_assert_eq!(engine.call_object_function(caller_index, "Trigger", Vec::new()).expect("LaunchLightning remains fail-safe") => Value::Bool(true));
    unit_assert_eq!(engine.debug_rng_clone().count => rng_count_before, "native LaunchLightning draws no synchronized RNG itself");
    let caller_state = engine.test_object_snapshot(caller);
    unit_assert_eq!(caller_state.local_vars.get("first") => Some(&Value::Int(1)));
    unit_assert_eq!(caller_state.local_vars.get("second") => Some(&Value::Int(1)));
    ...
    let lightning = engine
        ...
        .collect::<Vec<_>>();
    unit_assert_eq!(lightning.len() => 2);
    for object in &lightning {
        unit_assert_eq!(object.state.position => Vector2::new(50, 50));
        unit_assert_eq!(object.state.owner => OWNER_NONE);
        unit_assert_eq!(object.state.controller => OWNER_NONE);
        unit_assert_eq!(object.state.layer => None);
        unit_assert_eq!(object.state.local_vars.get("construction_creator") => Some(&Value::Nil));
        unit_assert_eq!(object.state.local_vars.get("construction_x") => Some(&Value::Int(50)));
        unit_assert_eq!(object.state.local_vars.get("construction_y") => Some(&Value::Int(50)));
        unit_assert_eq!(object.state.local_vars.get("touched") => Some(&Value::Int(1)));
    }
    ...
    for (name, value) in [
        ...
        unit_assert_eq!(first.state.local_vars.get(name) => Some(&Value::Int(value)));
    }
    unit_assert_eq!(first.state.local_vars.get("seen_gamma") => Some(&Value::Bool(true)));
    let second = lightning[1];
    unit_assert_eq!(second.state.local_vars.get("seen_x") => Some(&Value::Int(1)));
    unit_assert_eq!(second.state.local_vars.get("seen_gamma") => Some(&Value::Nil));
}

#[test]
fn launch_lightning_returns_true_without_fxl1() {
    let caller_script = "#strict\nfunc Trigger() { return(LaunchLightning()); }\n";
    let mut engine = Engine::with_seed(9);
    engine.register_test_script_definition("CALL", "Caller", caller_script);
    let caller = engine.spawn_test_object(SpawnConfig::new("CALL"));
    let rng_count_before = engine.debug_rng_clone().count;
    let caller_index = engine.test_object_index(caller);
    unit_assert_eq!(engine.call_object_function(caller_index, "Trigger", Vec::new()).expect("missing FXL1 remains successful") => Value::Int(1));
    unit_assert_eq!(engine.debug_rng_clone().count => rng_count_before);
...
fn volcano_event_initializes_at_cpp_default_position_before_activate() {
    ...
    let mut engine = Engine::with_seed(7);
    engine.register_test_script_definition("FXV1", "Volcano", script);
    engine.set_landscape(Landscape::flat(64, 40));
    let mut environment = engine.environment();
    ...
    for frame in (10..=20_000).step_by(10) {
        ...
        {
            unit_assert_eq!(volcano.state.position => Vector2::new(50, 50), "C++ creates FXV1 at the native default position before Activate");
            return;
        }
    }
...
"#;
    ...
    .test_value();
    let materials = MaterialSet::from_resource_library(&library);
    let lava = materials.id_of("Lava").test_value();
    let mut engine = Engine::with_seed(7);
    engine.set_materials(materials);
    let mut landscape = Landscape::flat(64, 40);
    landscape.set_world_height(500);
    engine.set_landscape(landscape);
    engine.register_test_script_definition("FXV1", "Volcano", volcano_script);
    engine.register_test_script_definition("CALL", "Caller", caller_script);

    let caller = spawn_fixture!(engine, "CALL", with_category: CATEGORY_OBJECT, with_position: Vector2::new(91, 82), with_owner: 3);
    let mut expected_rng = engine.debug_rng_clone();
    let expected_size = (15 + expected_rng.random(10)).clamp(10, 60);
    let rng_count_before = engine.debug_rng_clone().count;
    let caller_index = engine.test_object_index(caller);
    let result = engine.call_test_object_function(caller_index, "Trigger", Vec::new());

    unit_assert_eq!(result => Value::Int(1), "Activate(false) is ignored");
    unit_assert_eq!(engine.debug_rng_clone().count => rng_count_before + 1, "the bounded size consumes exactly Random(10)");
    let volcano = engine
        .objects
        .iter()
        .find(|object| object.definition_id == "FXV1")
        .test_value();
    unit_assert_eq!(volcano.state.position => Vector2::new(50, 50));
    unit_assert_eq!(volcano.state.owner => OWNER_NONE);
    unit_assert_eq!(volcano.state.local_vars.get("construction_creator") => Some(&Value::Nil), "C4Weather creates FXV1 without a creator");
    unit_assert_eq!(volcano.state.local_vars.get("seen_x") => Some(&Value::Int(37)));
    unit_assert_eq!(volcano.state.local_vars.get("seen_y") => Some(&Value::Int(499)), "the extra y argument is ignored in favor of GBackHgt - 1");
    unit_assert_eq!(volcano.state.local_vars.get("seen_size") => Some(&Value::Int(expected_size)), "the extra thickness argument is ignored");
    unit_assert_eq!(
...
"#;
    ...
    unit_assert_eq!(engine.call_object_function(caller_index, "Trigger", Vec::new()).expect("missing FXV1 is fail-safe") => Value::Int(1));
    unit_assert_eq!(engine.debug_rng_clone().count => rng_count_before + 1);
    unit_assert_eq!(engine.objects.len() => 1, "missing FXV1 creates no object");
}
...
"#;
    ...
    let mut engine = Engine::with_seed(9);
    engine.register_test_definition(simple_definition("LAYR"));
    engine.register_test_script_definition("CALL", "Caller", caller_script);
    engine.register_test_script_definition("FXQ1", "Earthquake", earthquake_script);
    let layer = engine.spawn_test_object(SpawnConfig::new("LAYR"));
    let caller = spawn_fixture!(engine, "CALL", with_position: Vector2::new(91, 82), with_owner: 3, with_controller: 7, with_layer: layer);
    let rng_count_before = engine.debug_rng_clone().count;
    let caller_index = engine.test_object_index(caller);

    unit_assert_eq!(engine.call_object_function(caller_index, "Trigger", Vec::new()).expect("Activate errors remain fail-safe") => Value::Nil);
    unit_assert_eq!(engine.debug_rng_clone().count => rng_count_before);
    unit_assert!(engine.object_snapshot(caller).is_some(), "caller survives");
    ...
    let quake = engine
        ...
        .test_value();
    unit_assert_eq!(quake.state.position => Vector2::new(-7, 83));
    unit_assert_eq!(quake.state.owner => OWNER_NONE);
    unit_assert_eq!(quake.state.controller => OWNER_NONE);
    unit_assert_eq!(quake.state.layer => None);
    unit_assert_eq!(quake.state.local_vars.get("construction_creator") => Some(&Value::Nil));
    unit_assert_eq!(quake.state.local_vars.get("construction_x") => Some(&Value::Int(-7)));
    unit_assert_eq!(quake.state.local_vars.get("construction_y") => Some(&Value::Int(83)));
    unit_assert_eq!(quake.state.local_vars.get("activated") => Some(&Value::Int(1)));
    unit_assert_eq!(quake.state.local_vars.get("activate_argument") => Some(&Value::Nil), "native Activate receives no arguments");
}
...
"#;
    ...
    unit_assert_eq!(engine.debug_rng_clone().count => rng_count_before);
    unit_assert_eq!(engine.objects.len() => 1, "missing FXQ1 creates nothing");
}
...
fn earthquake_event_requires_truthy_activate_like_cpp() {
    ...
    let mut engine = Engine::with_seed(7);
    engine.register_test_script_definition("FXQ1", "Earthquake", script);
    engine.set_landscape(Landscape::flat(64, 40));
    let mut environment = engine.environment();
