...
"#;
    ...
    let coach =
        engine.spawn_test_object(SpawnConfig::new("HRSE").with_position(Vector2::new(50, 50)));
    let id =
        engine.spawn_test_object(SpawnConfig::new("HRSE").with_position(Vector2::new(100, 50)));
    let idx = engine.test_object_index(id);
...
fn pxs_wind_drift_uses_the_grid_ift_bit() {
    ...
    assert_eq!(
        ift_pixel[0].xdir, expected_ift_xdir,
        "IFT grid byte reads wind 0"
    );
...
"#;
    ...
    let vehicle_id = engine.spawn_test_object(
        SpawnConfig::new("VEHI")
            .with_category(CATEGORY_VEHICLE)
            .with_position(vehicle_position)
            .with_fixed_position(FixedVec2::from_ints(vehicle_position.x, vehicle_position.y))
            .with_loaded(true),
    );
    ...
    let pusher_id = engine.spawn_test_object(
        SpawnConfig::new("PSHR")
            ...
            .with_fixed_position(FixedVec2::from_ints(pusher_position.x, pusher_position.y))
            .with_action(push)
            .with_command_direction(CommandDirection::Right)
            .with_mobile(true)
            .with_loaded(true),
    ...
    commands
        ...
        .test_value();
    assert_eq!(commands.command_names(), vec!["MoveTo", "PushTo", "Wait"]);
...
fn fight_no_attach_credits_opponent_controller_through_fall_death() {
    ...
    let opponent = engine.spawn_test_object(
...
fn push_no_attach_calls_grab_lost_before_jump_and_restores_push_to() {
    ...
    assert_eq!(pusher.command_stack.command_names(), vec!["PushTo", "Wait"]);
...
fn command_enter_push_target_replaces_vehicle_stack_without_controller_transfer() {
    ...
    let entrance_id = engine.spawn_test_object(
...
fn push_pull_fixture() -> (Engine, ObjectId, ObjectId) {
    ...
    let crate_id = engine.spawn_test_object(
...
fn push_procedure_resets_without_grab_ocf() {
    ...
    let rock_id =
        engine.spawn_test_object(SpawnConfig::new("Rock").with_position(Vector2::new(10, 0)));
    let mut push_state = ActionState::new("Push");
...
fn pinned_push_phase_trajectory_matches_cpp_for_one_hundred_frames() {
    ...
    let pusher = engine.spawn_test_object(
        SpawnConfig::new("Pusher")
...
fn pinned_horse_pull_wraps_and_refires_start_call_after_twenty_frames() {
    ...
    let wagon =
        engine.spawn_test_object(SpawnConfig::new("WAGN").with_position(Vector2::new(12, 0)));
...
fn train_physical_requires_info_or_temporary_physicals() {
    ...
    let crew_id = engine.spawn_test_object(SpawnConfig::new("Sheep").with_crew_member(true));
...
fn state_round_trip_preserves_physicals_and_energy_loss_cause() {
    ...
    let mut engine = Engine::with_seed(9);
    engine.register_test_definition(definition);
    let id = engine.spawn_test_object(SpawnConfig::new("Clonk").with_energy(40));
    let idx = engine.test_object_index(id);
...
fn exec_life_breath_restores_with_supply() {
    ...
    let id = engine.spawn_test_object(SpawnConfig::new("Diver").with_alive(true).with_energy(50));
    let idx = engine.test_object_index(id);
    engine.objects[idx].state.breath = 10_000;
...
fn weather_init_ingests_start_season_bounds_without_extra_clamp() {
    ...
    assert_eq!(
        (engine.environment.season_min, engine.environment.season_max),
        (110, 130)
    );
}
...
"#;
    ...
    assert_eq!(second.state.local_vars.get("seen_gamma"), Some(&Value::Nil));
}
...
"#;
    ...
    assert_eq!(
        volcano.state.local_vars.get("seen_x"),
        Some(&Value::Int(37))
    );
    assert_eq!(
