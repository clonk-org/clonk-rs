...
fn multi_attach_excludes_own_rect_solid_mask_after_first_motion() {
    ...
    for (id, attach) in [
        ...
        let actions = engine
            ...
            .clone();
        engine
            .exec_object_movement(index, &actions, &definition_id, &solid_mask_indices)
            .test_value();
    }
...
fn rotation_redirect_fixture_at_con(
    ...
    definition.set_rotateable(360);
    definition.set_shape_vertices(vec![ObjectVertex::new(40, 0).with_cnat(CNAT_TOP)]);
    definition.set_contact_density(50);
...
"#;
    let mut definition = test_definition("RailMover", "RailMover", script);
...
fn non_rotateable_static_movement_zeroes_rotation_without_resetting_fixed_state() {
    let mut engine = Engine::with_seed(2);
    let mut definition = test_definition("FixedStatic", "Fixed static", "");
    definition.set_no_stabilize(true);
    engine.register_test_definition(definition);
    engine.set_physics(PhysicsSettings::new(0, 0, 0));
...
fn ocf_entrance_rotation_thresholds_match_cpp() {
    ...
    engine.register_test_definition(any_rotation);
...
fn ocf_fight_ready_respects_no_fight_and_disabled_actions() {
    ...
    engine.register_test_definition(pacifist);
...
fn queued_commands_lower_landscape_columns() {
    ...
    let mut definition = test_definition("Miner", "Miner", script);
    let mut actions = HashMap::new();
    actions.insert("Idle".to_string(), ActionSpec::default());
    definition.configure_actions(Some("Idle".to_string()), actions);
    ...
    engine.spawn_test_object(SpawnConfig::new("Miner"));
...
fn no_scenario_init_script_player_skips_broadcasts_and_runs_its_extra_callback(
    ...
    engine.register_test_script_definition("__AI", "AI", AI);
    engine.install_scenario_script_with_convention("Scenario", SCENARIO, true)?;

    let info = ControlPlayerInfoEntry {
        name: LegacyCString::from_bytes(b"Bot".to_vec()).test_value(),
...
"#;
    ...
    engine.set_landscape(Landscape::flat(64, 48));
    engine.register_test_script_definition("PROB", "Probe", PROBE);
...
"#;
    ...
    engine
        .register_definition(test_definition("TEAM", "Teams", script))
        .test_value();
...
#[test]
fn scoreboard_nil_prunes_but_empty_string_persists_through_save_restore() -> Result<(), EngineError>
{
...
#[test]
fn runtime_scoreboard_requests_capture_call_time_dimensions_and_order() -> Result<(), EngineError> {
...
"#;
    let mut engine = Engine::new();
    engine.register_definition(test_definition("TEAM", "Team reader", script))?;
...
"#;
    let mut engine = Engine::new();
    engine.register_definition(test_definition("TEAM", "Team initializer", script))?;
...
"#;
    let mut engine = Engine::new();
    engine.register_definition(test_definition("SURR", "Surrender probe", script))?;
    let mut crew_definition = test_definition("CREW", "Departing crew", crew_script);
    crew_definition.set_crew_member(true);
    engine.register_definition(crew_definition)?;
    engine.register_test_script_definition("ITEM", "Ejected item", item_script);
    engine.install_scenario_script_with_convention("Scenario", scenario_script, true)?;
...
"#;
    ...
    let mut engine = Engine::new();
    engine
        .register_player(PlayerConfig::new(1, "Old").with_color(Some(RgbColor::new(0xaa, 0, 0))))?;
    engine.register_player(
        PlayerConfig::new(2, "New").with_color(Some(RgbColor::new(0x44, 0x55, 0x66))),
    )?;
    ...
    let mut flag = test_definition("FLAG", "Flag", FLAG_SCRIPT);
    ...
    engine.register_test_script_definition("BORN", "Construction owner probe", BIRTH_SCRIPT);
    ...
    let flag = engine.spawn_object(SpawnConfig::new("FLAG").with_owner(1).with_action(fly_base))?;
...
"#;
    ...
    let mut movable = test_definition("MOVE", "Movable", OWNED_SCRIPT);
    movable.set_category(CATEGORY_OBJECT);
    movable.set_color_by_owner(true);
    engine.register_definition(movable)?;
    let mut flag = test_definition("FLAG", "Flag", OWNED_SCRIPT);
    flag.set_category(CATEGORY_STATIC_BACK);
    engine.register_definition(flag)?;
    let mut background = test_definition("BACK", "Background", OWNED_SCRIPT);
    background.set_category(CATEGORY_STATIC_BACK);
    engine.register_definition(background)?;
    let mut crew_definition = test_definition("CREW", "Crew", OWNED_SCRIPT);
    ...
    engine.load_scenario_script_with_convention("Script.c", SCENARIO_SCRIPT, true)?;
...
"#;
    ...
    let mut definition = test_definition("MOVE", "Movable", SCRIPT);
...
#[test]
fn remove_player_uses_last_eligible_non_hostile_fallback_owner() -> Result<(), EngineError> {
...
fn remove_player_clears_invalid_static_back_owner_and_controller() -> Result<(), EngineError> {
    ...
    let object = engine.spawn_object(SpawnConfig::new("BACK").with_owner(1).with_controller(1))?;
    engine.register_player(PlayerConfig::new(1, "Departing"))?;
...
fn script_game_over_triggers_on_game_over() -> Result<(), EngineError> {
    ...
    engine.register_definition(crew_definition)?;
    let mut goal_definition = test_definition(
        "GOAL",
        "Goal",
        r#"
        local calls;
    ...
    engine.replace_player_info_league_progress_data([(41, Some(vec![b'P', 0xff]))]);
    engine.game_time = 19;
...
"#;
    let mut caller = test_definition("CALL", "Caller", script);
...
fn flight_slide_free_uses_live_position_for_each_contact() {
    ...
    engine
        .exec_contact_action(idx, CNAT_RIGHT | CNAT_LEFT | CNAT_TOP, &definition_id)
        .test_value();
...
"#,
    ...
    let stalled = engine.spawn_test_object(
    ...
    let powered = engine.spawn_test_object(
        SpawnConfig::new("PWRD")
...
"#;
    ...
    engine.set_structures_need_energy(true);
    engine.set_physics(PhysicsSettings::new(100, 200, -200));
    engine.register_test_definition(make_definition("RST0", false));
    engine.register_test_definition(make_definition("KEEP", true));
    let walk_action = || {
    ...
    let reset = engine.spawn_test_object(
    ...
    let keep = engine.spawn_test_object(
...
fn resting_object_freezes_until_tick10_mobilization_like_cpp() {
    ...
    engine.register_test_definition(simple_definition("Test"));
    engine.set_physics(PhysicsSettings::new(100, 200, -200));
...
fn stabilize_contact_probe_dispatches_contact_callbacks_like_cpp() {
    ...
    definition.set_shape_vertices(vec![ObjectVertex::new(0, 6)
...
"#;
    ...
    let mut definition = test_definition("SACC", "Accepted stabilizer", script);
    definition.set_c4_callback_convention(true);
    definition.set_contact_function_calls(true);
    definition.set_rotateable(1);
    definition.set_contact_density(50);
...
"#;
    ...
    let mut victim_definition = test_definition("TOMB", "Movement tombstone", script);
...
fn upright_attached_vehicle_on_ground_keeps_its_action_like_cpp() {
    let mut coach = test_definition("Coch", "Coach", "#strict\n");
