...
"#;
    ...
    let talker = test_definition("TALK", "Talker", talker_script);
...
"#;
    let mut engine = Engine::with_seed(0);
    let bandit = test_definition("BNDT", "Bandit", bandit_script);
...
"#;
    ...
    let mut spark = test_definition("SPRK", "Spark", spark_script);
...
"#;
    ...
    engine.register_test_script_definition("CALL", "Caller", script);
    let mut grow = test_definition("GROW", "Growing", "");
...
"#;
    ...
    let near_idx = engine.test_object_index(engine.objects[fast_idx + 1].id);
...
"#;
    let mut remover = test_definition("RMVR", "Remover", remover_script);
    remover.set_timer(3);
    ...
    let layer = engine.spawn_object(SpawnConfig::new("LAYR").with_position(Vector2::new(5, 5)))?;
    ...
    let item = engine.spawn_object(SpawnConfig::new("ITEM").with_position(Vector2::new(50, 50)))?;
...
"#;
    let mut observer = test_definition("OBSV", "Observer", observer_script);
...
"#;
    ...
    engine.call_test_object_function(caller_index, "ProbeMaterializedCaches", Vec::new());
...
"#;
    ...
    let other_id = engine.spawn_test_object(SpawnConfig::new("OTHR"));
...
"#;
    let mut engine = Engine::with_seed(0);
    engine.register_test_script_definition("CALL", "Caller", script);
    let id = engine.spawn_test_object(SpawnConfig::new("CALL"));
    engine
        .apply_object_update(
            id,
            ObjectUpdate {
...
"#;
    let mut old = test_definition("OLDD", "Old", script);
    old.set_blit_mode(2);
    let mut new = test_definition("NEWD", "New", "#strict\n");
    new.set_blit_mode(4);
    let mut engine = Engine::with_seed(0);
...
"#;
    ...
    let mut old = test_definition("OLDD", "Old", old_script);
    old.set_category(CATEGORY_OBJECT);
    old.set_c4_callback_convention(true);
    let mut new = test_definition("NEWD", "New", new_script);
...
"#
        ...
        engine.register_definition(new)?;
        let container =
            engine.spawn_object(SpawnConfig::new("CONT").with_position(Vector2::new(30, 40)))?;
        let changed = engine.spawn_object(
            SpawnConfig::new("OLDD")
                .with_container(container)
                .with_action(ActionState::new("Work")),
...
fn change_def_waits_for_a_later_global_unsorted_sweep_then_uses_the_new_def_cluster(
    ...
    let changed = engine.spawn_object(SpawnConfig::new("OLDD").with_category(CATEGORY_OBJECT))?;
    let separator = engine.spawn_object(SpawnConfig::new("SEPR").with_category(CATEGORY_OBJECT))?;
    let new_peer = engine.spawn_object(SpawnConfig::new("NEWD").with_category(CATEGORY_OBJECT))?;
    let trigger =
...
#[test]
fn change_def_unsorted_link_survives_a_frame_until_an_explicit_resort() -> Result<(), EngineError> {
    ...
    let trigger =
        engine.spawn_object(SpawnConfig::new("TRIG").with_category(CATEGORY_STATIC_BACK))?;
    let anchor = engine.spawn_object(SpawnConfig::new("ANCH").with_category(CATEGORY_OBJECT))?;
...
fn change_def_self_arrow_resolves_the_new_definition_inline() {
    ...
    let new = test_definition("NEWD", "New", "#strict\nfunc Probe() { return(2); }\n");
    let mut engine = Engine::with_seed(0);
    engine.register_test_definition(old);
    engine.register_test_definition(new);
    let id = engine.spawn_test_object(SpawnConfig::new("OLDD"));
    let index = engine.test_object_index(id);
...
fn self_arrow_world_dispatch_keeps_object_locals_live() {
    ...
    let id = engine.spawn_test_object(SpawnConfig::new("LIVE"));
    let index = engine.test_object_index(id);
...
"#;
    let mut live = test_definition("HRSX", "Horse", live_script);
    ...
    engine.register_test_definition(live);
    let mut corpse = test_definition("CRPS", "Corpse", "#strict\n");
...
"#;
    let mut horse = test_definition("HRSX", "Horse", script);
...
fn shake_objects_flings_attached_living_object_with_cpp_rng_order() {
    ...
    let mut target = test_definition("CLNK", "Clonk", "#strict\n");
...
"#;
    let mut target = test_definition("FLTG", "Fling target", target_script);
...
fn fling_add_speed_uses_cpp_bool_coercion() {
    ...
    let mut target = test_definition("FLBT", "Fling bool target", "#strict\n");
    target.set_category(CATEGORY_OBJECT);
...
"#;
    let mut caller = test_definition("FLOR", "Fling order", caller_script);
    caller.set_category(CATEGORY_OBJECT);

    let mut target = test_definition("FLOT", "Fling order target", "#strict\n");
    target.set_category(CATEGORY_OBJECT);
    target.configure_actions(
        Some("Walk".to_string()),
        HashMap::from([
...
"#;
    let mut target = test_definition("FIIT", "Incomplete fling target", target_script);
    target.set_category(CATEGORY_OBJECT);
...
"#;
    let mut old = test_definition("FCDO", "Old fling definition", old_script);
    old.set_category(CATEGORY_OBJECT);
    old.set_c4_callback_convention(true);
    ...
    let mut new = test_definition("FCDN", "New fling definition", "#strict\n");
...
"#;
    let mut target = test_definition("FOCT", "OCF fling target", target_script);
    target.set_category(CATEGORY_LIVING | CATEGORY_OBJECT);
...
fn shake_objects_matches_cpp_master_order_gate_ledger() {
    ...
    caller.set_category(CATEGORY_OBJECT);
    let mut target = test_definition("SHKT", "Shake target", "#strict\n");
    target.set_category(CATEGORY_LIVING | CATEGORY_OBJECT);
...
fn change_def_swaps_definition_in_place_like_cpp() {
    ...
    engine.register_test_definition(old_def);
    ...
    engine.register_test_definition(new_def);
...
fn foreign_target_change_def_cannot_be_shadowed_by_target_script() {
    ...
    let target = engine.spawn_test_object(SpawnConfig::new("OLDD"));
...
#[test]
fn change_def_from_idle_preserves_time_but_resets_phase_on_both_paths() -> Result<(), EngineError> {
...
#[test]
fn change_def_veto_after_abort_enter_keeps_the_pre_swap_contents_link() -> Result<(), EngineError> {
...
fn connect_lines_track_their_targets_like_cpp() {
    ...
    engine.register_test_definition(beam);
    let mut anchor_def = test_definition("ANCR", "Anchor", "#strict\n");
...
fn resource_definition_preserves_line_core_fields() {
    ...
    let group = clonk_resources::Group::open(
        repository.join("content/Objects.c4d/Structures.c4d/Lines.c4d/PowerLine.c4d"),
    )
    .test_value();
    let resource = ResourceDefinitionData::load(&group).test_value();
    ...
    let mut legacy_json = serde_json::to_value(&snapshot).test_value();
    legacy_json
        .as_object_mut()
        .test_value()
...
fn shipped_power_line_inserts_first_cpp_bend_around_solid_pixel() {
    ...
    let mut engine = Engine::with_seed(0);
    let mut power_line = Definition::from_resource(&resource).test_value();
    power_line.set_line(resource.core.line);
    power_line.set_line_intersect(resource.core.line_intersect);
    engine.register_test_definition(power_line);
    engine.register_test_script_definition("ANCR", "Line anchor", "#strict\n");

    let mut pixels = vec![0_u8; 12 * 10];
    pixels[5 * 12 + 5] = 1;
    let grid =
        landscape::PixelGrid::new(12, 10, pixels, vec![0, 100], vec![None; 2], vec![None; 2]);
...
fn shipped_power_line_old_endpoint_fallback_uses_cpp_material_index_quirk() {
    ...
    fn run_case(
        ...
        let mut landscape = Landscape::new(20, vec![20; 20]).test_value();
        landscape.set_pixel_grid(grid);
...
fn connect_lines_reduce_redundant_bends_on_tick35_like_cpp() {
    ...
    let mut line = test_definition("LINE", "Line", "#strict\n");
    line.set_line(8); // C4D_Line_Vertex
    ...
    let mut anchor = test_definition("ANCR", "Anchor", "#strict\n");
...
fn walkers_snap_to_one_pixel_above_ground_like_cpp() {
    ...
    let mut walker = test_definition("WLKR", "Walker", "#strict\n");
