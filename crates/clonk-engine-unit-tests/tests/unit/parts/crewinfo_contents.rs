...
fn float_procedure_clamps_loaded_velocity_to_the_physical_limit() {
    let mut engine = Engine::with_seed(0);
    let mut bird = test_definition("BIRD", "Bird", "#strict\n");
...
"#;
    ...
    let id = engine.spawn_test_object(SpawnConfig::new("AHUD").with_category(CATEGORY_OBJECT));
    engine.spawn_test_object(SpawnConfig::new("AHUD").with_category(CATEGORY_OBJECT));
    let idx = engine.test_object_index(id);
...
fn sibling_includes_give_the_last_declaration_priority() {
    ...
    let mut engine = Engine::with_seed(0);
    engine.register_test_script_definition("INCA", "A", "public func Foo() { return(1); }");
    ...
    engine.resolve_includes().test_value();
    let id = engine.spawn_test_object(SpawnConfig::new("CHLD").with_category(CATEGORY_OBJECT));
...
fn magic_workshop_uses_bas8_basement_functions() {
    ...
    engine.resolve_includes().test_value();
    let id = engine.spawn_test_object(SpawnConfig::new("MWKS").with_category(CATEGORY_OBJECT));
    let index = engine.test_object_index(id);
...
"#;
    let mut engine = Engine::with_seed(0);
    engine.register_test_definition(test_definition("BASE", "Base", base));
    engine.register_test_definition(test_definition("MIDD", "Mid", mid));
    engine.register_test_definition(test_definition("TOPP", "Top", top));
    engine.resolve_includes().test_value();

    let id = engine.spawn_test_object(SpawnConfig::new("TOPP").with_category(CATEGORY_OBJECT));
    let idx = engine.test_object_index(id);
...
"#;
    ...
    let receiver = engine.spawn_test_object(SpawnConfig::new("RCVR").with_owner(0));
...
"#;
    ...
    let newer = engine.spawn_test_object(
        SpawnConfig::new("CREW")
            .with_owner(0)
            .with_crew_member(false),
    );
    ...
    let rock = engine.spawn_test_object(SpawnConfig::new("ROCK").with_owner(0));
    assert_eq!(
        engine
            .call_object_function(
                target_index,
...
fn shipped_hazard_tutorial_script1_raises_limit_before_creating_drones() {
    ...
    assert_eq!(
        (drone.color, drone.original_color),
        (0x0001_0101, 0x0001_0101)
    );
...
"#;
    ...
    assert_eq!(
        read(i32::MIN),
        Value::Nil,
        "signed-overflow input is hardened"
    );
}
...
"#;
    ...
    for rank in [137, 143] {
        ...
        assert_eq!(
            before[2],
            Value::Int(777),
            "load must not recompute progression"
        );
    }
    ...
    for object in [rank_137, rank_143] {
        ...
        assert_eq!(
            roster.core, live.core,
            "save refresh mirrors the shared C++ pointer"
        );
    }
}
...
"#;
    ...
    assert_eq!(
        promoted_object.energy, 12_000,
        "promotion does not heal live Energy"
    );
    ...
    assert_eq!(
        ...
        vec![
            "Rookie is promoted".to_string(),
            "to Custom One!".to_string()
        ]
    );
    ...
    let info_less = engine.spawn_test_object(
        SpawnConfig::new("CREW")
            .with_owner(0)
            .with_crew_member(false),
    );
...
"#;
    ...
    let donor = engine.spawn_test_object(
        SpawnConfig::new("CREW")
            .with_owner(0)
            .with_crew_member(false),
    );
    let receiver = engine.spawn_test_object(
        SpawnConfig::new("CREW")
            .with_owner(0)
            .with_crew_member(false),
    );
    ...
    let readded = engine.spawn_test_object(
        SpawnConfig::new("CREW")
            .with_owner(0)
            .with_crew_member(false),
    );
...
"#;
    ...
    assert!(
        !engine
            .object_snapshot(donor)
            .expect("donor remains")
            .crew_member
    );
    assert!(
        engine
...
"#;
    ...
    let dead_receiver = engine.spawn_test_object(
        SpawnConfig::new("RCVR")
            .with_owner(1)
            .with_crew_member(false)
            .with_alive(false),
...
"#;
    ...
    assert_eq!(
        restored
            .call_object_function(restored_index, "RemoveFrom", vec![Value::Int(1)],)
    ...
    assert_eq!(
        restored
            .call_object_function(restored_index, "RemoveFrom", vec![Value::Int(0)],)
...
fn object_phase_created_crew_exists_for_the_same_tick35_player_check() {
    ...
    let mut engine = Engine::with_seed(0);
    engine.register_test_definition(creator);
    engine.register_test_definition(crew);
...
"#;
    ...
    let driver = engine.spawn_test_object(SpawnConfig::new("DRVR").with_owner(OWNER_NONE));
...
"#;
    ...
    assert_ne!(
        ...
        "the exact infos are distinguishable"
    );
    for object in &objects[..2] {
        let index = engine.test_object_index(*object);
        assert_eq!(
...
"#;
    ...
    let earlier_replacement = engine.spawn_test_object(
        SpawnConfig::new("CREW")
            .with_owner(0)
            .with_crew_member(false),
    );
    let later_replacement = engine.spawn_test_object(
        SpawnConfig::new("CREW")
            .with_owner(1)
            .with_crew_member(false),
    );
...
"#;
    ...
    let temporary_only = engine.spawn_test_object(
        SpawnConfig::new("PORC")
            .with_category(CATEGORY_OBJECT)
            .with_owner(0),
    );
    ...
    let noncrew = engine.spawn_test_object(SpawnConfig::new("PORC").with_category(CATEGORY_OBJECT));
...
"#;
    ...
    let grabber = engine.spawn_test_object(
        SpawnConfig::new("TRAP")
            .with_category(CATEGORY_OBJECT)
            .with_owner(0)
            .with_crew_member(true),
...
"#;
    ...
    let id = engine.spawn_test_object(SpawnConfig::new("Actor").with_category(CATEGORY_OBJECT));
    let idx = engine.test_object_index(id);
...
fn effect_callbacks_fire_across_lifecycle_events() {
    ...
    let fourth = engine.test_tick();
    let object = fourth.object(id).test_value();
    assert!(object.effects.is_empty());
...
fn runtime_contents_enter_inserts_before_equal_category_like_cpp() {
    ...
    let chest = engine.spawn_test_object(SpawnConfig::new("Ches").with_category(CATEGORY_OBJECT));
    let ammo_a = engine.spawn_test_object(
        SpawnConfig::new("AMBO")
            .with_category(CATEGORY_OBJECT)
            .with_container(chest),
    );
    let ammo_b = engine.spawn_test_object(
        SpawnConfig::new("AMBO")
            .with_category(CATEGORY_OBJECT)
            .with_container(chest),
    );
    let rifle = engine.spawn_test_object(
...
fn scroll_contents_moves_first_to_back_and_returns_new_front_like_cpp() {
    let script = r#"#strict 3
        ...
        "#;
    let mut engine = Engine::with_seed(3);
    engine.register_test_script_definition("CHES", "Chest", script);
    engine.register_test_definition(simple_definition("ITEM"));

    let filled = engine.spawn_test_object(SpawnConfig::new("CHES").with_category(CATEGORY_OBJECT));
    let empty = engine.spawn_test_object(SpawnConfig::new("CHES").with_category(CATEGORY_OBJECT));
    let c = engine.spawn_test_object(
...
fn scroll_contents_rotates_an_explicit_foreign_container_like_cpp() {
    ...
    let actor = engine.spawn_test_object(SpawnConfig::new("ACTR").with_category(CATEGORY_OBJECT));
    let chest = engine.spawn_test_object(SpawnConfig::new("CHES").with_category(CATEGORY_OBJECT));
    let b = engine.spawn_test_object(
        SpawnConfig::new("ITEM")
            .with_category(CATEGORY_OBJECT)
...
fn shift_contents_rotates_to_next_different_item_like_cpp() {
    let script = r#"
        ...
        "#;
    let mut engine = Engine::with_seed(3);
    engine.register_test_script_definition("Ches", "Ches", script);
    engine.register_test_definition(simple_definition("SWRD"));
    engine.register_test_definition(simple_definition("REVR"));

    let chest = engine.spawn_test_object(SpawnConfig::new("Ches").with_category(CATEGORY_OBJECT));
    let sword = engine.spawn_test_object(
        SpawnConfig::new("SWRD")
            .with_category(CATEGORY_OBJECT)
            .with_container(chest),
    );
    let revolver_a = engine.spawn_test_object(
        SpawnConfig::new("REVR")
            .with_category(CATEGORY_OBJECT)
            .with_container(chest),
    );
    let revolver_b = engine.spawn_test_object(
        SpawnConfig::new("REVR")
            .with_category(CATEGORY_OBJECT)
            .with_container(chest),
    );
...
fn shift_contents_skips_only_cpp_picture_concat_equivalents() {
    ...
    engine.register_test_script_definition("CHES", "Chest", chest_script);
    ...
    let visually_distinct =
        engine.spawn_test_object(SpawnConfig::new("ITEM").with_container(chest));
    let equivalent = engine.spawn_test_object(SpawnConfig::new("ITEM").with_container(chest));
    let front = engine.spawn_test_object(SpawnConfig::new("ITEM").with_container(chest));
    ...
    let picture_distinct = engine.spawn_test_object(
        SpawnConfig::new("PASS")
            .with_picture_rect(DefinitionRect::new(0, 76, 64, 64))
            .with_container(color_chest),
    );
    let tint_ignored = engine.spawn_test_object(
        SpawnConfig::new("PASS")
            .with_color_modulation(0x0040_4040)
            .with_container(color_chest),
    );
    let color_front =
        engine.spawn_test_object(SpawnConfig::new("PASS").with_container(color_chest));
    ...
    assert_eq!(
        engine
            .call_object_function(color_chest_index, "Cycle", vec![Value::Bool(false)],)
...
fn shift_contents_operates_on_a_foreign_container_like_cpp() {
    ...
    let revolver = engine.spawn_test_object(
        SpawnConfig::new("REVR")
            .with_category(CATEGORY_OBJECT)
            .with_container(chest),
    );
...
"#;
    ...
    engine.register_test_definition(simple_definition("Targ"));
    let actor = engine.spawn_test_object(SpawnConfig::new("Actr").with_category(CATEGORY_OBJECT));
    let target = engine.spawn_test_object(SpawnConfig::new("Targ").with_category(CATEGORY_OBJECT));
...
"#;
    let mut engine = Engine::with_seed(0);
    engine.register_test_script_definition("Opnr", "Opener", opener_script);
    engine.register_test_script_definition("Prob", "Prober", prober_script);
    let opener = engine.spawn_test_object(SpawnConfig::new("Opnr").with_category(CATEGORY_OBJECT));
    let prober = engine.spawn_test_object(SpawnConfig::new("Prob").with_category(CATEGORY_OBJECT));
...
"#;
    let mut engine = Engine::with_seed(0);
    engine.register_test_script_definition("Actr", "Actor", script);
    let id = engine.spawn_test_object(SpawnConfig::new("Actr").with_category(CATEGORY_OBJECT));
    let idx = engine.test_object_index(id);
...
"#;
    ...
    let actor = engine.spawn_test_object(SpawnConfig::new("ACTR").with_category(CATEGORY_OBJECT));
    let victim = engine.spawn_test_object(SpawnConfig::new("VCTM").with_category(CATEGORY_OBJECT));
    ...
    let victim_idx = engine.test_object_index(victim);
    assert_eq!(
        engine.objects[victim_idx].state.energy, 40_000,
...
"#;
    ...
    let bomb = engine.spawn_test_object(
    ...
    assert!(
        engine.objects[bomb_index].destroyed,
        "Explode removes its target"
    );
    ...
    assert!(
        engine
            .objects
            .iter()
            .all(|object| object.definition_id != "FXID"),
...
"#;
    ...
    let layer =
        engine.spawn_test_object(SpawnConfig::new("LAYR").with_position(Vector2::new(500, 500)));
    let caller =
        engine.spawn_test_object(SpawnConfig::new("CALL").with_position(Vector2::new(300, 300)));
    let target = engine.spawn_test_object(
    ...
    assert!(
        !engine.objects[caller_index].destroyed,
...
fn native_explode_initial_partial_effect_idles_after_full_construction() {
    ...
    engine.register_test_definition(simple_definition("TARG"));
    let mut effect = test_definition(
        "FXID",
        "Explosion effect",
        r#"#strict 3
...
fn native_explode_script_overload_shadows_and_inherited_reaches_the_host() {
    ...
    let bomb = engine.spawn_test_object(SpawnConfig::new("BOOM").with_category(CATEGORY_OBJECT));
    let bomb_index = engine.test_object_index(bomb);
...
fn native_explode_contain_blast_suppresses_visuals_and_blasts_contents() {
    ...
    let caller =
        engine.spawn_test_object(SpawnConfig::new("CALL").with_position(Vector2::new(300, 300)));
    let shield = engine.spawn_test_object(
    ...
    let sibling = engine.spawn_test_object(
...
fn system_blast_objects_overload_shadows_the_native_host() {
    ...
    let caller = engine.spawn_test_object(SpawnConfig::new("BLST").with_category(CATEGORY_OBJECT));
    let caller_idx = engine.test_object_index(caller);
...
"#;
    ...
    let burning =
        engine.spawn_test_object(SpawnConfig::new("TREE").with_position(Vector2::new(10, 10)));
    ...
    let soaked =
        engine.spawn_test_object(SpawnConfig::new("TREE").with_position(Vector2::new(30, 8)));
    let mirror = engine.rng.clone();
    engine.call_test_object_function(
        actor_idx,
        "Zap",
    ...
    let chest =
        engine.spawn_test_object(SpawnConfig::new("CHST").with_position(Vector2::new(12, 12)));
...
"#;
    ...
    let actor = engine.spawn_test_object(SpawnConfig::new("ACTR").with_category(CATEGORY_OBJECT));
    let warded = engine.spawn_test_object(
        SpawnConfig::new("WARD")
            .with_category(CATEGORY_OBJECT)
            .with_alive(true),
    );
    let nulled = engine.spawn_test_object(
...
"#;
    ...
    let victim_idx = engine.test_object_index(victim);
    assert_eq!(
        engine.objects[victim_idx].state.damage, 3,
...
"#;
    ...
    let victim_idx = engine.test_object_index(victim);
    assert_eq!(
        engine.objects[victim_idx].state.energy, 45_000,
