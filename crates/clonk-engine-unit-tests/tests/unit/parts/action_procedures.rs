...
fn flight_procedure_suppresses_gravity_and_wind() {
    let mut definition = test_definition("Glider", "Glider", PROCEDURE_MOVEMENT_SCRIPT);
...
fn resource_float_without_float_physical_clamps_raw_velocity() {
    ...
    let definition_path = temp.path().join("Sub.c4d");
    ...
    assert_eq!(definition.physical().float, 0);
...
fn float_physical_preserves_hazard_bullet_velocity_above_synthetic_limit() {
    ...
    let mut definition = test_definition("SHT1", "Shot", &script);
    definition.configure_actions(
...
fn swim_procedure_reduces_gravity_and_blocks_wind() {
    let mut definition = test_definition("Swimmer", "Swimmer", PROCEDURE_MOVEMENT_SCRIPT);
    let mut actions = HashMap::new();
...
fn lift_procedure_matches_cpp_mass_scaled_force_and_terminal_speeds() {
    ...
    let lifter_id = engine.spawn_test_object(
    ...
    let heavy_lifter_id = engine.spawn_test_object(
        SpawnConfig::new("Lifter")
...
fn lift_procedure_resets_without_target() {
    ...
    let snapshot = engine.test_tick();
    let lifter = snapshot.object(lifter_id).test_value();
...
fn hang_procedure_locks_vertical_velocity() {
    let mut definition = test_definition("Clinger", "Clinger", PROCEDURE_MOVEMENT_SCRIPT);
    let mut actions = HashMap::new();
...
fn set_bridge_action_data_updates_action_data() {
    let mut definition = test_definition("Bridger", "Bridger", SET_BRIDGE_ACTION_DATA_SCRIPT);
    let mut actions = HashMap::new();
...
fn set_bridge_action_data_returns_false_when_not_in_bridge_procedure() {
    ...
    let id = engine.spawn_test_object(SpawnConfig::new("IdleActor"));
...
fn connect_procedure_freezes_velocity_and_ignores_wind() {
    let mut definition = test_definition("Connector", "Connector", PROCEDURE_MOVEMENT_SCRIPT);
    let mut actions = HashMap::new();
...
fn kneel_procedure_locks_vertical_velocity_and_blocks_wind() {
    let mut definition = test_definition("Kneeler", "Kneeler", PROCEDURE_MOVEMENT_SCRIPT);
    let mut actions = HashMap::new();
...
"#;
    ...
    let clonk_id = engine.spawn_test_object(
        SpawnConfig::new("CLNK")
            .with_owner(1)
            .with_crew_member(true)
            .with_action(ActionState::new("Idle")),
...
"#;
    ...
    assert!(
        handled,
        "the container consumed the com (DirectCom returns)"
    );
    let snapshot = engine.snapshot();
    assert_eq!(
...
"#;
    let mut definition = test_definition("WSKI", "Waterskin", script);
    let mut actions = HashMap::new();
    actions.insert("Idle".to_string(), ActionSpec::default());
    definition.configure_actions(Some("Idle".to_string()), actions);
...
fn dig_procedure_carves_diggable_material() {
    ...
    let material_source = r#"
            ...
            DigFree=1
        "#;
    let library = clonk_resources::MaterialLibrary::parse(material_source).test_value();
    let materials = MaterialSet::from_resource_library(&library);
    let earth = materials.id_of("Earth").test_value();
...
fn dig_procedure_spawns_dig2object_when_ratio_reached() {
    let mut digger = test_definition("DGRR", "Digger", PROCEDURE_MOVEMENT_SCRIPT);
    let mut actions = HashMap::new();
    actions.insert(
        "Dig".to_string(),
        ActionSpec::default().with_procedure("dig").with_dig_free(6),
...
fn dig2object_rotation_uses_one_cpp_random_draw() {
    ...
    engine.register_test_definition(gem_definition);
    ...
    let digger = engine.spawn_test_object(
        SpawnConfig::new("DGRR")
...
fn dig2object_request_only_requires_explicit_request() {
    ...
    {
        let mut engine = Engine::with_seed(19);
        engine.register_test_definition(build_digger_definition());
...
fn throw_procedure_zeroes_velocity() {
    let mut definition = test_definition("Thrower", "Thrower", PROCEDURE_MOVEMENT_SCRIPT);
    let mut actions = HashMap::new();
    actions.insert("Idle".to_string(), ActionSpec::default());
...
fn scale_procedure_zeroes_horizontal_velocity() {
    let mut definition = test_definition("Scaler", "Scaler", PROCEDURE_MOVEMENT_SCRIPT);
    let mut actions = HashMap::new();
...
fn hangle_command_direction_updates_velocity_and_direction() {
    let mut definition = test_definition("Hangler", "Hangler", PROCEDURE_MOVEMENT_SCRIPT);
    let mut actions = HashMap::new();
...
fn build_contained_target_requires_live_powered_build_container() {
    ...
    for (label, container_action, need_energy, inactive, construction, expected_delta) in [
        ...
        engine.register_test_definition(construction_builder_definition("BLDR", "", false));
        engine.register_test_definition(construction_builder_definition("BCON", "", false));
        engine.register_test_definition(construction_target_definition(""));

        let container = engine.spawn_test_object(
        ...
        let builder = engine.spawn_test_object(
        ...
        if expected_delta == 0 {
            ...
            assert_eq!(
                builder_state.state.command_direction,
...
fn build_area_uses_live_shape_and_inclusive_vertical_margin() {
    ...
    for (label, position_case, should_build, no_other_action, inactive) in [
        ...
        let mut engine = Engine::with_seed(67);
        engine.register_test_definition(construction_builder_definition(
            "BLDR",
            "",
            no_other_action,
        ));
...
"#;
    let mut engine = Engine::with_seed(67);
    engine.register_test_definition(construction_builder_definition("BLDR", "", false));
    engine.register_test_definition(construction_target_definition(target_script));
    let builder = engine.spawn_test_object(
        SpawnConfig::new("BLDR")
            .with_position(Vector2::new(100, 200))
            .with_action(ActionState::new("Build")),
...
"#;
    ...
    let mut engine = Engine::with_seed(67);
    engine.register_test_definition(builder_definition);
    engine.register_test_definition(construction_target_definition(target_script));
    let builder = engine.spawn_test_object(
        SpawnConfig::new("BLDR")
            .with_position(Vector2::new(100, 200))
    ...
    assert_eq!(
        builder_state.local_vars.get("control_calls"),
        Some(&Value::Int(1))
    );
    assert_eq!(
...
fn build_procedure_requires_components_before_progress() -> Result<(), EngineError> {
    ...
    let mut builder_definition = test_definition("Builder", "Builder", script);
    ...
    let mut structure_definition = test_definition("Structure", "Structure", script);
...
fn build_procedure_noncrew_reports_material_without_acquire() -> Result<(), EngineError> {
    ...
    let mut builder_definition = test_definition("Machine", "Machine", script);
    ...
    let mut structure_definition = test_definition("Site", "Site", script);
    structure_definition.set_constructable(true);
    structure_definition.set_category(CATEGORY_STRUCTURE);
    structure_definition.set_components(vec![DefinitionComponent {
        id: "Wood".to_string(),
...
"#;
    ...
    let mut structure_definition = test_definition("Site", "Structure", "#strict");
    structure_definition.set_constructable(true);
    structure_definition.set_category(CATEGORY_STRUCTURE);
    structure_definition.set_mass(100);
    structure_definition.set_components(vec![DefinitionComponent {
...
"#;
    ...
    let mut builder = test_definition("BLDR", "Builder", builder_script);
    ...
    assert_eq!(
        valid_builder.local_vars.get("removal_order"),
        Some(&Value::Int(12))
    );
    assert_eq!(
        valid_builder.local_vars.get("contents_seen"),
        Some(&Value::Int(0))
    );
    assert_eq!(
        valid_builder.local_vars.get("component_seen"),
        Some(&Value::Int(1))
    );
    assert_eq!(
        valid_builder.local_vars.get("contained_seen"),
...
fn push_procedure_without_target_resets_to_default() {
    ...
    let mut definition = test_definition("Pusher", "Pusher", script);
    let mut actions = HashMap::new();
    actions.insert("Idle".to_string(), ActionSpec::for_procedure("walk"));
...
fn failed_push_stands_in_walk_and_adds_cpp_delay_command() {
    ...
    let mut definition = test_definition("Pusher", "Pusher", "");
    definition.configure_actions(
        Some("Idle".to_string()),
        HashMap::from([
            ("Idle".to_string(), ActionSpec::default()),
...
fn push_from_unrelated_container_still_applies_force_to_inactive_target() {
    ...
    assert_eq!(
        engine.objects[pusher_idx].state.container,
        Some(unrelated_id)
    );
...
fn push_procedure_moves_target_and_pusher() {
    ...
    let mut target_definition = test_definition("Crate", "Crate", script);
    ...
    let snapshot = engine.test_tick();
    let target_after = snapshot.object(target_id).test_value();
    assert!(
...
fn pull_without_target_stops_in_walk_with_silent_wait() {
    ...
    let mut definition = test_definition("Puller", "Puller", script);
    let mut actions = HashMap::new();
    actions.insert("Idle".to_string(), ActionSpec::for_procedure("walk"));
    actions.insert("Walk".to_string(), ActionSpec::for_procedure("walk"));
...
fn pull_procedure_moves_target_and_puller() {
    ...
    let mut engine = Engine::with_seed(5);
    engine.register_test_definition(puller_definition);
    engine.register_test_definition(target_definition);
    engine.set_physics(
        PhysicsSettings::new(0, 20, -20)
            .with_max_horizontal_speed(20)
...
fn fight_procedure_retains_inactive_action_target_like_cpp() {
    ...
    assert_eq!(
        engine.objects[fighter_index].state.action.name, "Fight",
...
"#;
    ...
    engine.register_test_definition(attach_actor_definition("L75A", script, Some("AttachAbort")));
...
"#;
    ...
    let mut engine = Engine::with_seed(75);
    engine.register_test_definition(attach_actor_definition("L75I", actor_script, None));
    engine.register_test_definition(blocked);
    engine.register_test_definition(allowed);
...
"#;
    let mut engine = Engine::with_seed(75);
    engine.register_test_definition(attach_actor_definition("L75E", actor_script, None));
    engine.register_test_definition(point_definition("L75C", container_script));
    engine.register_test_definition(point_definition("L75T", "#strict 2"));
...
"#;
    let mut engine = Engine::with_seed(75);
    engine.register_test_definition(attach_actor_definition("L75X", actor_script, None));
    engine.register_test_definition(point_definition("L75O", container_script));
    engine.register_test_definition(point_definition("L75U", "#strict 2"));
...
fn fight_procedure_moves_toward_target() {
    ...
    engine.register_test_definition(fighter_definition);
    engine.register_test_definition(opponent_definition);
    engine.set_physics(
        PhysicsSettings::new(0, 20, -20)
            .with_max_horizontal_speed(20)
            .test_value(),
    ...
    let opponent_id = engine.spawn_test_object(
    ...
    let fighter_id = engine.spawn_test_object(
    ...
    let snapshot = engine.test_tick();
    let fighter = snapshot.object(fighter_id).test_value();
...
fn fight_procedure_stands_when_target_not_fighting() {
    ...
    let mut passive_definition = test_definition("Passive", "Passive", script);
    ...
    let mut engine = Engine::with_seed(41);
    engine.register_test_definition(fighter_definition);
    engine.register_test_definition(passive_definition);
    ...
    let passive_id = engine.spawn_test_object(
...
fn actmap_sound_loops_while_its_action_slot_stays_selected() {
    ...
    let mut definition = test_definition("Uzi", "Uzi", "func Initialize() { }");
    definition.configure_actions(
        Some("Idle".to_string()),
        HashMap::from([
            ("Idle".to_string(), ActionSpec::default()),
    ...
    let burst = engine.spawn_test_object(
        SpawnConfig::new("Uzi")
            .with_category(CATEGORY_OBJECT)
