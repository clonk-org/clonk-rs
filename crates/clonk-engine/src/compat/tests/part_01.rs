...
fn cpp_add_func_adapter_runs_after_debugger_without_changing_generic_hosts() {
    ...
    fn run_probe(wrapped: bool, values: &[Value]) -> (Value, Vec<Value>) {
        ...
        engine.set_debugger_hooks(clonk_script::DebuggerHooks::new().with_on_call(
            move |name, args| {
                if name == "Probe" {
                    *observed_by_hook.lock().test_value() = args.to_vec();
                }
            },
        ));
        let result = engine.call("Run", values).test_value();
        let debugger_args = observed.lock().test_value().clone();
...
fn host_raster_preview_reuses_the_landscape_allocation() {
    ...
    let source = world.landscape_shared().test_value();
    let preview = world.host_raster_preview();
    assert!(!preview.inherit_landscape);
    let preview_landscape = preview.landscape.as_ref().test_value();
    ...
    let replayed = replay.landscape_shared().test_value();
...
const EXPECTED_HOST_FUNCTIONS: &str = "AbortMessageBoard Abs ActIdle ActivateGameGoalMenu AddCommand AddEffect AddEvaluationData AddMenuItem AddMessage AddMsgBoardCmd AddVertex AdjustWalkRotation And Angle AnyContainer AppendCommand ArcCos ArcSin AssignVar AsyncRandom BitAnd BlastFree BlastObject BlastObjects BoundBy Bubble Buy C4Id Call CallMessageBoard CastAny CastBackParticles CastBool CastC4ID CastInt CastObjects CastPXS CastParticles ChangeDef ChangeEffect CheckEffect CheckEnergyNeedChain ClearLastPlrCom ClearMenuItems ClearParticles CloseMenu Collect ComponentAll ComposeContents Contained Contents ContentsCount Cos CreateArray CreateConstruction CreateContents CreateMenu CreateObject CreateParticle CreateScriptPlayer CrewMember CustomMessage DeathAnnounce DebugLog Dec DecVar DefinitionCall DigFree DigFreeRect Distance Div DoBreath DoCon DoCrewExp DoDamage DoEnergy DoHomebaseMaterial DoHomebaseProduction DoMagicEnergy DoScore DoScoreboardShow DrawDefMap DrawMap DrawMatChunks DrawMaterialQuad DrawVolcanoBranch EditCursor EffectCall EffectVar EliminatePlayer EnergyCheck Enter Equal ExecuteCommand Exit Explode Extinguish ExtractLiquid ExtractMaterialAmount FatalError FightWith FindBase FindConstructionSite FindContents FindObject FindObject2 FindObjectOwner FindObjects FindOtherContents Find_AtPoint Find_Category Find_ID FinishCommand FlameConsumeMaterial Fling Format FrameCounter FreeRect FxFireInfo FxFireStart FxFireStop FxFireTimer GBackLiquid GBackSemiSolid GBackSky GBackSolid GainMissionAccess GameCall GameCallEx GameOver GetActMapVal GetActTime GetAction GetActionData GetActionTarget GetAlive GetBase GetBreath GetCaptain GetCategory GetChar GetClimate GetClrModulation GetColor GetColorDw GetComDir GetCommand GetComponent GetCon GetContact GetController GetCrew GetCrewCount GetCrewEnabled GetCrewExtraData GetCursor GetDamage GetDefBottom GetDefCoreVal GetDefinition GetDesc GetDir GetEffect GetEffectCount GetEnergy GetEntrance GetGravity GetHiRank GetHomebaseMaterial GetHomebaseProduction GetID GetIndexOf GetKeys GetKiller GetLeague GetLeagueProgressData GetLeagueScore GetLength GetMagicEnergy GetMass GetMatAdjust GetMaterial GetMaterialColor GetMaterialCount GetMaterialVal GetMaxPlayer GetMenu GetMenuSelection GetMissionAccess GetName GetNeededMatStr GetOCF GetObjHeight GetObjWidth GetObjectBlitMode GetObjectInfoCoreVal GetObjectLayer GetObjectStatus GetObjectVal GetOwner GetPath GetPhase GetPhysical GetPlayerByIndex GetPlayerCount GetPlayerID GetPlayerInfoCoreVal GetPlayerName GetPlayerTeam GetPlayerType GetPlayerVal GetPlrColorDw GetPlrControlName GetPlrDownDouble GetPlrExtraData GetPlrJumpAndRunControl GetPlrKnowledge GetPlrMagic GetPlrValue GetPlrValueGain GetPlrView GetPlrViewMode GetPortrait GetProcedure GetR GetRDir GetRank GetScenarioVal GetScore GetScoreboardData GetScoreboardString GetSeason GetSelectCount GetSkyAdjust GetSkyColor GetSystemTime GetTaggedPlayerName GetTeamByIndex GetTeamColor GetTeamConfig GetTeamCount GetTeamName GetTemperature GetTexture GetTime GetType GetUnusedOverlayID GetValue GetValues GetVertex GetVertexContact GetVertexNum GetViewCursor GetVisibility GetWealth GetWind GetX GetXDir GetY GetYDir GrabContents GrabObjectInfo GreaterThan HideSettlementScoreInEvaluation Hostile InLiquid Inc IncVar Incinerate IncinerateLandscape InitScenarioPlayer InsertMaterial Inside IsNetwork IsNewgfx IsRef Jump Kill LandscapeHeight LandscapeWidth LaunchEarthquake LaunchLightning LaunchVolcano LessThan LoadScenarioSection LocateFunc Log MakeCrewMember Material MaterialName Max Message Min Mod ModulateColor Mul Music MusicLevel NoContainer Not Object ObjectCall ObjectCount ObjectCount2 ObjectDistance ObjectNumber ObjectSetAction OnFire OnMessageBoardAnswer Or PathFree PathFree2 PauseGame PlaceAnimal PlaceVegetation PlayerMessage PlayerObjectCommand PlrMessage Pow PrivateCall ProtectedCall Punch PushParticles Random ReloadDef ReloadParticle RemoveEffect RemoveObject RemoveUnusedTexMapEntries RemoveVertex ResetGamma ResetPhysical Resort ResortObject ResortObjects SEqual ScoreboardCol ScriptCounter ScriptGo ScrollContents SelectCrew SelectMenuItem Sell Set SetAction SetActionData SetActionTargets SetAlive SetBridgeActionData SetCategory SetClimate SetClrModulation SetColor SetColorDw SetComDir SetCommand SetComponent SetContactDensity SetController SetCrewEnabled SetCrewExtraData SetCrewStatus SetCursor SetDir SetEntrance SetFilmView SetFoW SetGameSpeed SetGamma SetGraphics SetGravity SetHostility SetKiller SetLandscapePixel SetLeaguePerformance SetLeagueProgressData SetLength SetMass SetMatAdjust SetMaterialColor SetMaxPlayer SetMenuDecoration SetMenuSize SetMenuTextProgress SetName SetNextMission SetObjDrawTransform SetObjDrawTransform2 SetObjectBlitMode SetObjectLayer SetObjectOrder SetObjectStatus SetOwner SetPhase SetPhysical SetPicture SetPlayList SetPlayerTeam SetPlrExtraData SetPlrKnowledge SetPlrMagic SetPlrShowCommand SetPlrShowControl SetPlrShowControlPos SetPlrView SetPlrViewRange SetPortrait SetPosition SetPreSend SetR SetRDir SetRestoreInfos SetScoreboardData SetSeason SetShape SetSkyAdjust SetSkyColor SetSkyFade SetSkyParallax SetSolidMask SetTemperature SetTextureIndex SetTransferZone SetVar SetVertex SetViewCursor SetViewOffset SetVisibility SetWealth SetWind SetXDir SetYDir ShakeFree ShakeObjects ShiftContents ShowInfo SimFlight Sin Smoke SortScoreboard Sound SoundLevel Split2Components Sqrt StartCallTrace StartScriptProfiler StopScriptProfiler Stuck Sub Sum SurrenderPlayer TestMessageBoard TrainPhysical UnselectCrew Value WildcardMatch goto";
...
fn scenario_section_world_object(id: u64, status: ObjectStatus) -> HostWorldObject {
    fixture_world_object(ObjectId::new(id), "TEST").with_status(status)
}
...
fn find_at_point_adds_the_calling_object_position() {
    ...
    let (local, _) = with_effect_context(Some(object), &[], HostWorldContext::default(), 1, || {
...
"#;
    let mut engine = crate::Engine::with_seed(0);
    engine.register_test_player(crate::PlayerConfig::new(7, "Active").with_player_info_id(41));
    engine.register_test_player(crate::PlayerConfig::new(8, "Zero score").with_player_info_id(43));
    engine
        .round_results
        .players
        .push(crate::RoundResultsPlayerState {
...
"#;
    ...
    engine.register_test_player(crate::PlayerConfig::new(7, "Player").with_player_info_id(41));
    engine.register_test_definition(test_definition("CALL", "Strict caller", strict_script));
    engine.register_test_definition(test_definition("OLDC", "Legacy caller", legacy_script));
    let strict = engine.spawn_test_object(crate::SpawnConfig::new("CALL"));
    let legacy = engine.spawn_test_object(crate::SpawnConfig::new("OLDC"));
    let strict_index = engine.find_object_index(strict).test_value();
    ...
    let legacy_index = engine.find_object_index(legacy).test_value();
    assert_eq!(
        engine
...
"#;
    ...
    engine.register_test_player(crate::PlayerConfig::new(7, "Active").with_player_info_id(41));
    engine.register_test_player(crate::PlayerConfig::new(8, "Empty").with_player_info_id(43));
    engine.register_test_player(crate::PlayerConfig::new(9, "Binary").with_player_info_id(59));
    ...
    let mut restored = crate::Engine::with_seed(1);
    restored.restore_state(&state).test_value();
    restored.register_test_definition(test_definition("CALL", "Caller", script));
    let caller = restored.spawn_test_object(crate::SpawnConfig::new("CALL"));
    let caller_index = restored.find_object_index(caller).test_value();
    assert_eq!(
        restored
...
"#;
    ...
    engine.set_league_name(b"LeagueName".to_vec());
    engine.register_test_definition(test_definition("MARK", "Marker", ""));
    engine
        .install_scenario_script_with_convention("Scenario", scenario, true)
        .test_value();

    let info = crate::ControlPlayerInfoEntry {
        name: crate::LegacyCString::from_bytes(b"Player".to_vec()).test_value(),
        id: 41,
        league_score: 123,
        league_progress_data: crate::LegacyCString::from_bytes(b"join-data".to_vec()).test_value(),
        ..Default::default()
    };
...
fn set_object_order_queues_cpp_pairs_and_rejects_invalid_pairs() {
    ...
    let world =
        HostWorldContext::from_objects(vec![fixture_world_object(ObjectId::new(2), "Dummy")]);
    let (result, outcome) = with_object_host_context_with_world(world, || {
        assert_eq!(
