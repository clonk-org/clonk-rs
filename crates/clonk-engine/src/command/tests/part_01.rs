    ...
    static EMPTY_COMMAND_PLAYERS: Lazy<HashMap<i32, CommandPlayerSnapshot>> =
    ...
    fn command_objects<const N: usize>(
        objects: [CommandObjectSnapshot; N],
    ...
    fn command_ctx<'a>(
        ...
        command_ctx_at_frame(
            object,
            objects,
            &EMPTY_COMMAND_PLAYERS,
            &EMPTY_COMMAND_DEFINITIONS,
            frame,
        )
    }

    fn command_ctx_with_players<'a>(
        object: &'a CommandObjectSnapshot,
        objects: &'a CommandObjectSnapshots,
        players: &'a HashMap<i32, CommandPlayerSnapshot>,
        frame: u64,
    ) -> CommandRuntimeContext<'a> {
        command_ctx_at_frame(object, objects, players, &EMPTY_COMMAND_DEFINITIONS, frame)
    }

    fn command_ctx_with_definitions<'a>(
        object: &'a CommandObjectSnapshot,
        objects: &'a CommandObjectSnapshots,
        definitions: &'a HashMap<DefinitionId, CommandDefinitionSnapshot>,
        frame: u64,
    ) -> CommandRuntimeContext<'a> {
        command_ctx_at_frame(object, objects, &EMPTY_COMMAND_PLAYERS, definitions, frame)
    }

    fn empty_command_ctx(object: &CommandObjectSnapshot, frame: u64) -> CommandRuntimeContext<'_> {
        command_ctx(object, &EMPTY_COMMAND_OBJECTS, frame)
    }
    ...
    fn move_to_swim_steers_horizontal_on_tick2_and_vertical_otherwise() {
        ...
        let ctx = empty_command_ctx(&swimmer, 2);
        let mut state = evaluated_move_to(&request);
        let result = state.step(&ctx);
        assert_eq!(result.status, CommandStatus::Running);
        assert_eq!(
    ...
    fn move_to_noncrew_pathfinder_flight_control_and_disabled_gate() {
        ...
        let mut ctx = empty_command_ctx(&flyer, 1);
    ...
    fn move_to_init_evaluation_adjusts_target_unless_no_pos_adjust() {
        ...
        let mut ctx = empty_command_ctx(&walker, 1);
        ctx.landscape = Some(&landscape);
        ...
        let mut ctx = empty_command_ctx(&walker, 2);
        ctx.landscape = Some(&landscape);
    ...
    fn move_to_absorbs_target_position_once() {
        ...
        let ctx = command_ctx(&walker, &objects, 2);
        let result = state.step(&ctx);
        assert_eq!(
            result.update.and_then(|update| update.command_direction),
            Some(CommandDirection::Right),
        ...
        let ctx = command_ctx(&walker, &objects, 3);
        let result = state.step(&ctx);
        assert_ne!(
            result.update.and_then(|update| update.command_direction),
    ...
    fn move_to_update_interval_is_cpp_lifetime_not_step_throttle() {
        ...
        let mut walker = walking_jumper(Vector2::new(100, 100));
        let ctx = empty_command_ctx(&walker, 0);
        assert_eq!(
        ...
        let ctx = empty_command_ctx(&walker, 2);
        ...
        let ctx = empty_command_ctx(&walker, 3);
        assert_eq!(
    ...
    fn move_to_push_intermediate_waypoint_steers_from_actor_position() {
        ...
        let waypoint_ctx =
            command_ctx_with_definitions(&pusher_at_waypoint, &objects, &definitions, 2);
        let waypoint = stack
            .step(&waypoint_ctx)
        ...
        let final_ctx = command_ctx_with_definitions(&pusher, &objects, &definitions, 3);
    ...
    fn move_to_flight_takeoff_defers_walk_jump_until_after_callbacks() {
        ...
        let mutated_ctx = jump_ctx(&callback_mutated, &landscape);
    ...
    fn follow_enters_the_targets_container() {
        ...
        let contained_follower = contained_objects
            .get(&follower_id)
            .expect("follower present");
        let contained_ctx = command_ctx(contained_follower, &contained_objects, 1);
    ...
    fn follow_grabs_copies_and_releases_the_targets_vehicle() {
        ...
        let follower = objects.get(&follower_id).expect("follower present");
        let ctx = command_ctx(follower, &objects, 1);
        ...
        objects
            ...
            .command_direction = CommandDirection::DownRight;
        let follower = objects.get(&follower_id).expect("follower present");
        let ctx = command_ctx(follower, &objects, 2);
        ...
        let follower = objects.get(&follower_id).expect("follower present");
        let ctx = command_ctx(follower, &objects, 3);
    ...
    fn follow_requests_move_when_out_of_range() {
        ...
        let follower = objects.get(&follower_id).expect("follower present");
        let removed_ctx = command_ctx(follower, &objects, 1);
