...
fn scenario_tracks_crew_member_flags() {
    ...
    assert_eq!(created.len(), 2);
    let first = engine.object_snapshot(created[0]).test_value();
    assert!(first.crew_member);
    assert_eq!(first.owner, 1);

    let second = engine.object_snapshot(created[1]).test_value();
...
fn missing_round_results_component_uses_cpp_melee_default() {
    let dir = test_tempdir();
    let scenario_dir = write_resilience_fixture(dir.path(), None, "// no script\n");
    let source = std::fs::read_to_string(scenario_dir.join("Scenario.txt")).test_value();
    write_test_file(
        scenario_dir.join("Scenario.txt"),
        format!("{source}\n[Game]\nGoals=MELE=1\n"),
    );
...
fn legacy_group_loading_reports_monotonic_nonempty_decode_status() {
    ...
    assert!(reported
        .iter()
        .all(|(progress, log)| { !log.trim().is_empty() || matches!(progress, 11 | 35) }));
}
...
fn explicit_definition_error_precedes_folder_local_scan_failure() {
    ...
    assert!(matches!(
        error,
        ScenarioError::LegacyDefinitionNotFound { .. }
    ));
}
...
fn failed_landscape_decode_does_not_report_a_prepared_source_map() {
    ...
    let result = Scenario::load_from_group_with_languages_and_definition_selection_and_progress(
        &group,
        &resolver,
        &["US"],
        &[] as &[String],
...
fn network_group_loading_reports_the_shared_initgame_milestones() {
    ...
    let scenario_dir = write_resilience_fixture(dir.path(), None, "// scenario script\n");
    let group = Group::open(&scenario_dir).test_value();
    let definitions = Group::open(dir.path().join("Defs.c4d")).test_value();
    let mut reported = Vec::new();
...
fn definition_clonk_names_cross_load_only_after_local_marker() {
    ...
    load(&["DE", "US"]).apply(&mut without_marker).test_value();
    assert_eq!(
        without_marker
            .definitions
    ...
    load(&["US", "DE"]).apply(&mut us_first).test_value();
    assert_eq!(
        us_first
            .definitions
            .get(&DefinitionId::from("GOOD"))
    ...
    load(&["DE", "US"]).apply(&mut de_first).test_value();
    assert_eq!(
        de_first
            .definitions
            .get(&DefinitionId::from("GOOD"))
...
fn legacy_scenario_script_respects_language_order() {
    ...
    let scenario = Scenario::load_from_path_with_languages(&scenario_dir, &resolver, &["DE", "US"])
        .test_value();
    let script = &scenario.script.as_ref().test_value().source;
...
fn join_test_player(engine: &mut Engine) -> Vec<ObjectId> {
    let before: std::collections::HashSet<ObjectId> = engine
        ...
        .collect();
    engine
        .join_player(scenario_join_player_config("Tester"))
        .test_value();
    engine
        .snapshot()
        .objects
...
fn crew_infos_promote_physicals_by_rank_like_cpp() {
    ...
    // The live fair-crew projection below is this test's subject, so state
    // the parameter rather than lean on the engine default:
    // `C4GameParameters` compiles `UseFairCrew` false absent a scenario
    // forcing it (C4GameParameters.cpp:560), and the port's fresh-engine
    // default disagrees (clonk-org/clonk-rs#1071).
    engine.set_use_fair_crew(true);
    let mut clonk = Definition::from_script("CLNK", "Clonk", "#strict\n").test_value();
