...
// by `include!` from the parent module so every test id is unchanged.
    ...
    fn base_functionality_numeric_tail_round_trips_cpp_decompiler_output() {
        let serialized = format_base_functionality(65).test_value();
    ...
    fn definition_collection_truncates_and_skips_invalid_c4ids() {
        fn write_definition(path: &Path, id: &str) {
            ...
            write_test_file(path.join("Script.c"), "// definition\n");
        ...
        write_test_file(root.join("Lowercase.c4d/ActMap.txt"), "not an action map");
    ...
    fn definition_sound_groups_follow_native_load_effects_gates_and_preorder() {
        fn write_definition(path: &Path, id: &str, needed_gfx_mode: Option<i32>) {
            ...
            write_test_file(path.join("DefCore.txt"), core);
            write_test_file(path.join("Script.c"), "// definition\n");
            write_test_definition_graphics(path);
        }
        ...
        let dir = test_tempdir();
        let root = dir.path().join("Defs.c4d");
        std::fs::create_dir_all(&root).test_value();

        let pure = root.join("Pure.c4d");
        std::fs::create_dir_all(&pure).test_value();
        write_test_file(pure.join("Pure.wav"), b"pure");

        let good = root.join("Good.c4d");
        write_definition(&good, "GOOD", None);
        let nested = good.join("Nested.c4d");
        std::fs::create_dir_all(&nested).test_value();
        write_test_file(nested.join("Nested.wav"), b"nested");

        let particle = root.join("Particle.c4d");
        std::fs::create_dir_all(&particle).test_value();
        write_test_file(particle.join("Particle.txt"), b"invalid particle data");
        ...
        std::fs::create_dir_all(&rejected_core).test_value();
        write_test_file(
        ...
        std::fs::create_dir_all(&broken_graphics).test_value();
        write_test_file(
        ...
        write_test_file(broken_act_map.join("ActMap.txt"), "not an action map");

        let ordinary = root.join("Ordinary");
        std::fs::create_dir_all(&ordinary).test_value();
        write_test_file(ordinary.join("Hidden.wav"), b"hidden");
        ...
        let good_index = roots.iter().position(|path| path == &good).test_value();
        let nested_index = roots.iter().position(|path| path == &nested).test_value();
        assert!(
    ...
    fn child_definition_enumeration_preserves_distinct_native_names_in_group_order() {
        ...
        let definition = |id: &str| {
            ...
            child
                .add_file("Script.c", format!("// {id}\n").into_bytes()).test_value();
            child
                .add_file("Graphics.bmp", graphics.clone()).test_value();
            child
        };
        ...
        packed
            .add_child_bytes_with_metadata(U_CHILD_NAME.to_vec(), definition("UDEF"), 1, false).test_value();
        packed
            .add_child_bytes_with_metadata(O_CHILD_NAME.to_vec(), definition("ODEF"), 1, false).test_value();
    ...
    fn scenario_local_definition_children_load_and_override_packs() {
        ...
        write_test_file(local.join("Script.c"), "func Tag() { return 5; }\n");
        ...
        std::fs::create_dir_all(&shadow).test_value();
        write_test_file(
            shadow.join("DefCore.txt"),
            "[DefCore]\nid=GOOD\nName=LocalGood\nCategory=0\nCrewMember=0\n",
        );
        write_test_file(shadow.join("Script.c"), "// local override\n");
    ...
    fn folder_local_definitions_resolve_for_scenarios_like_cpp() {
        ...
        std::fs::create_dir_all(&shared).test_value();
        write_test_file(
            shared.join("DefCore.txt"),
            "[DefCore]\nid=SHRD\nName=Shared\nCategory=0\nCrewMember=0\n",
        );
        write_test_file(shared.join("Script.c"), "// shared\n");
    ...
    fn folder_local_scan_checks_all_c4f_prefixes_outer_to_inner() {
        fn write_definition(path: &Path, id: &str) {
            ...
            write_test_file(path.join("Script.c"), format!("// {id}\n"));
            write_test_definition_graphics(path);
        }
    ...
    fn packed_group_path_reopens_native_byte_child_exactly() {
        ...
        child
            .add_file("marker.txt", b"exact child".to_vec()).test_value();
        let mut outer = clonk_resources::MutableGroup::new("Outer.c4f");
        outer
            .add_child_bytes_with_metadata(CHILD_NAME.to_vec(), child, 1, false).test_value();

        let directory = test_tempdir();
        let outer_path = directory.path().join("Outer.c4f");
        write_test_file(&outer_path, outer.pack().test_value());
        let outer = Group::open(&outer_path).test_value();
        let entry = outer
            .entries()
            .expect("enumerate native-byte parent")
            .into_iter()
            .find(|entry| entry.name_bytes == CHILD_NAME).test_value();
        let child = outer
            .open_child_entry_exact(&entry).test_value();
        assert_eq!(
            child
                .root()
                .file_name()
        ...
        let reopened = open_group_path(child.root()).test_value();
    ...
    fn packed_nested_folder_locals_load_outer_to_inner_and_retained_paths_reload() {
        ...
        std::fs::create_dir_all(&global).test_value();
        write_test_file(
            global.join("DefCore.txt"),
            "[DefCore]\nid=GLOB\nName=GLOB\nCategory=0\n",
        );
        write_test_file(global.join("Script.c"), "// global definition\n");
        ...
        write_test_file(&outer_path, outer);

        let outer_group = Group::open(&outer_path).test_value();
        let inner_group = outer_group
            .open_child("Inner.c4f").test_value();
        let scenario_group = inner_group
            .open_child("Nested.c4s").test_value();
        let resolver = test_resolver(vec![dir.path().to_path_buf()]);

        let scenario = Scenario::load_from_group_with(&scenario_group, &resolver).test_value();
        assert_eq!(
            scenario
                .definitions
                .iter()
    ...
    fn objects_txt_tolerates_windows_1252_like_cpp() {
        ...
        write_test_file(scenario_dir.join("Objects.txt"), objects);
        let (engine, _created) = apply_resilience_fixture(&dir, &scenario_dir);
    ...
    fn unknown_defcore_bit_names_do_not_abort_scenario_definition_scan() {
        ...
        std::fs::create_dir(&bits).test_value();
        write_test_file(
        ...
        std::fs::create_dir(&tail).test_value();
        write_test_file(
    ...
    fn network_savegame_runtime_state_precedes_initialize_def_and_skips_fresh_weather() {
        ...
        let mut game_source = crate::serialize_initial_network_game(&scalar_game_data, None)
            .expect("savegame runtime serializes").test_value();
        let counter = b"Counter=9\r\n";
        let insertion = game_source
            .windows(counter.len())
            .position(|window| window == counter)
            .map(|position| position + counter.len()).test_value();
        game_source.splice(
            insertion..insertion,
...
[Scoreboard]\r\nRows=2\r\nCols=2\r\nDlgShow=1\r\nCell0_0String=\"Scores\"\r\nCell0_0Value=-1\r\nCell1_0String=\"Round\"\r\nCell1_0Value=1234\r\nCell0_1String=\"Alice\"\r\nCell0_1Value=7\r\nCell1_1String=\"42\"\r\nCell1_1Value=42\r\n",
        ...
        write_test_file(scenario_dir.join("Game.txt"), &game_source);
    ...
    fn deferred_apply_synchronizes_before_initialize_and_player_join() {
        ...
        engine
            .initialize_scenario_script().test_value();
        assert_eq!(
            engine
                .script_globals
                .borrow()
    ...
    fn legacy_value_overloads_replace_definition_value_before_objects_load_like_cpp() {
        ...
        std::fs::create_dir_all(&fish).test_value();
        write_test_file(
            fish.join("DefCore.txt"),
            "[DefCore]\nid=FISH\nName=Fish\nCategory=0\nValue=10\n",
        );
        write_test_file(fish.join("Script.c"), "// fish\n");
    ...
    fn loads_legacy_scenario_with_definitions() {
        ...
        write_test_file(foo_core.join("Script.c"), "// empty definition script\n");
        RgbaImage::from_pixel(1, 1, Rgba([255, 255, 255, 255]))
            .save(foo_core.join("Graphics.png")).test_value();

        assert!(foo_core.join("DefCore.txt").exists(), "defcore exists");
        assert!(foo_core.join("Script.c").exists(), "script exists");

        let foo_group = Group::open(&foo_core).test_value();
        ResourceDefinitionData::load(&foo_group).test_value();
        ...
        resolver
            .resolve_definition_groups(&scenario_group, "Defs.c4d").test_value();
        ...
        for id in &joined {
            let object = engine.object_snapshot(*id).test_value();
        ...
        let id = engine.spawn_test_object(SpawnConfig::new("FOOO"));
        let object = engine
            .object_snapshot(id).test_value();
        assert_eq!(object.definition_id, "FOOO");
        assert_eq!(
    ...
    fn legacy_definitions_prune_future_versions_and_unmet_requirements_before_objects() {
        fn write_definition(
            ...
            write_test_file(path.join("DefCore.txt"), core);
            write_test_file(path.join("Script.c"), "// definition script\n");
            write_test_definition_graphics(&path);
        }
    ...
    fn fixed_definition_modules_replace_preset_and_keep_selected_order() {
        ...
        for (module, id) in [("First.c4d", "FIRS"), ("Second.c4d", "SCND")] {
            ...
            write_test_file(definition.join("Script.c"), "// fixed module\n");
    ...
    fn network_game_resources_replace_local_definition_and_material_discovery() {
        ...
        std::fs::create_dir_all(&local_definition).test_value();
        write_test_file(
        ...
        write_test_file(local_materials.join("TexMap.txt"), "3=Rock-Smooth\n");
        write_test_file(
        ...
        std::fs::create_dir_all(&host_definition).test_value();
        write_test_file(
        ...
        std::fs::create_dir_all(&folder_definition).test_value();
        write_test_file(
        ...
        std::fs::create_dir_all(&map_materials).test_value();
        write_test_file(
        ...
        write_test_file(global_materials.join("TexMap.txt"), "# global\n");
        write_test_file(
        ...
        std::fs::create_dir_all(&graphics).test_value();
        let mut sky_png = Vec::new();
        {
            use image::ImageEncoder as _;
            image::codecs::png::PngEncoder::new(&mut sky_png)
                .write_image(&[11, 22, 33, 255], 1, 1, ColorType::Rgba8.into()).test_value();
        }
        write_test_file(graphics.join("ClientSky.png"), sky_png);

        let definition_groups = [
            Group::open(&host_definitions).test_value(),
            Group::open(&folder_definitions).test_value(),
        ];
        let material_groups = [
            Group::open(&map_materials).test_value(),
            Group::open(&global_materials).test_value(),
        ];
        let graphics_groups = [Group::open(&graphics).test_value()];
    ...
    fn non_fixed_definition_seed_is_replaced_by_non_local_preset() {
        ...
        let scenario = Scenario::load_from_path_with_languages_and_definition_seed(
            &scenario_dir,
            &test_resolver(vec![dir.path().to_path_buf()]),
            &["US"],
            &["Objects.c4d"],
        ).test_value();
        assert_eq!(
        ...
        std::fs::create_dir_all(&empty_dir).test_value();
        write_test_file(
    ...
    fn shipped_objects_collision_prefers_the_global_external_group() {
        ...
        let scenario = Group::open(content.join("Tutorial.c4f/Tutorial01.c4s")).test_value();
        let local = Group::open(content.join("Tutorial.c4f/Objects.c4d")).test_value();
        let global =
            Group::open(content.join("Objects.c4d")).test_value();
        let resolver = CollisionResolver {
            local,
            global: global.clone(),
        };

        let selected = resolve_one_definition_group(&scenario, &resolver, "Objects.c4d").test_value();
    ...
    fn definition_group_paths_restore_projected_non_ascii_bytes() {
        ...
        std::fs::create_dir_all(&rooted_path).test_value();
        let relative_spec = clonk_script::c4_string_from_bytes(relative_name.as_bytes());
        let resolved_rooted = resolve_rooted_definition_group(dir.path(), &relative_spec).test_value();
    ...
    fn literal_definition_prefix_preserves_separatorless_and_dot_spellings() {
        fn write_pack(path: &Path, id: &str) {
            ...
            write_test_file(path.join("Script.c"), "// prefix fixture\n");
            write_test_definition_graphics(path);
        }

        let dir = test_tempdir();
        let prefix = dir.path().join("Defs");
        std::fs::create_dir_all(&prefix).test_value();
    ...
    fn non_fixed_module_loads_one_external_then_folder_local_once() {
        ...
        let scenario = load_test_scenario(
            &scenario_dir,
            &CollisionResolver {
                local: local_module,
    ...
    fn fixed_modules_expand_rooted_block_then_original_block() {
        fn write_pack(root: &Path, id: &str) {
            ...
            write_test_file(definition.join("Script.c"), format!("// {id}\n"));
            write_test_definition_graphics(&definition);
        }
        ...
        let resolver = test_resolver(vec![normal_first.clone(), normal_second]);
