...
fn landscape_push_pull_routes_insert_material_and_matches_cpp_surface8_fixture() {
    ...
    let (mut disabled, water) =
        engine_with_push_pull_plane(false, WIDTH, HEIGHT, initial.clone());
    assert!(
    ...
    let (mut enabled, water) =
        engine_with_push_pull_plane(true, WIDTH, HEIGHT, initial.clone());
