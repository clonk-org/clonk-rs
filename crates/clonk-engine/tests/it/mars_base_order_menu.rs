...
fn items(engine: &Engine, clonk: ObjectId) -> Vec<clonk_engine::ObjectMenuItem> {
    crate::support::TestValueExt::test_value(engine.debug_object_menu(clonk.as_u64()).flatten())
        .items
}
...
fn open_order_page(engine: &mut Engine) -> (i32, ObjectId) {
    ...
    crate::support::TestValueExt::test_value(engine.select_crew(player, vec![clonk]));
    crate::support::TestValueExt::test_value(engine.set_crew_cursor(player, Some(clonk)));
    ...
    let sat = engine.spawn_test_object(
    ...
    crate::support::TestValueExt::test_value(engine.objects[index].state.menu.as_mut()).selection =
        ORDER_SUBMENU_ROW;
...
fn mars_order_row_adds_on_a_left_enter_and_takes_back_on_a_right_one(
    ...
    crate::support::TestValueExt::test_value(engine.menu_user_enter(clonk, true));
...
fn mars_order_row_steps_with_the_left_and_right_controls(
    ...
    for _ in 0..3 {
        crate::support::TestValueExt::test_value(engine.player_in_com(
            player,
            clonk_engine::COM_MENU_RIGHT,
            0,
        ));
    }
    crate::support::TestValueExt::test_value(engine.player_in_com(
        player,
        clonk_engine::COM_MENU_LEFT,
        0,
    ));

    let menu = crate::support::TestValueExt::test_value(
        engine.debug_object_menu(clonk.as_u64()).flatten(),
    );
...
fn order_and_commit(engine: &mut Engine, player: i32, clonk: ObjectId, quantity: usize) {
    ...
    let select = |engine: &mut Engine, row: i32| {
        let index = engine.test_object_index(clonk);
        crate::support::TestValueExt::test_value(engine.objects[index].state.menu.as_mut())
            .selection = row;
    };
    let last_row = |engine: &Engine| {
...
fn an_order_over_the_players_wealth_is_refused_whole() {
    ...
    let wealth = crate::support::TestValueExt::test_value(engine.player(player)).wealth();
...
fn the_order_page_shows_what_it_will_cost(engine: &mut Engine, player: i32, clonk: ObjectId) {
    ...
    let priced = crate::support::TestValueExt::test_value(
        engine.debug_object_menu(clonk.as_u64()).flatten(),
    );
    assert!(
...
fn a_menu_whose_owner_prices_nothing_keeps_the_shipped_footer(engine: &Engine, cursor: ObjectId) {
    ...
    let menu = crate::support::TestValueExt::test_value(
        engine.debug_object_menu(cursor.as_u64()).flatten(),
    );
    assert_eq!(
        menu.extra,
...
fn the_order_page_offers_undo_only_once_there_is_something_to_undo(
    ...
    let index = engine.test_object_index(clonk);
    crate::support::TestValueExt::test_value(engine.objects[index].state.menu.as_mut()).selection =
        crate::support::TestValueExt::test_value(i32::try_from(undo_row));
    crate::support::TestValueExt::test_value(engine.menu_user_enter(clonk, false));
...
fn the_closing_row_says_which_of_its_two_jobs_it_is_doing(engine: &mut Engine, clonk: ObjectId) {
    ...
    let closing_row = crate::support::TestValueExt::test_value(i32::try_from(order_page.len())) - 1;
    let index = engine.test_object_index(clonk);
    crate::support::TestValueExt::test_value(engine.objects[index].state.menu.as_mut()).selection =
        closing_row;
    crate::support::TestValueExt::test_value(engine.menu_user_enter(clonk, false));
...
fn open_viewport_menu(engine: &mut Engine) -> ObjectId {
    ...
    let cursor = crate::support::TestValueExt::test_value(
        engine.player(player).and_then(|player| player.cursor()),
    );
    let viewport = engine.spawn_test_object(SpawnConfig::new("VWPT").with_owner(player));
...
fn a_range_whose_condition_fails_collapses_to_one_inert_row(engine: &mut Engine, cursor: ObjectId) {
    ...
    crate::support::TestValueExt::test_value(engine.menu_user_enter(cursor, false));
...
fn collapsing_the_rows_spends_no_synchronized_draw() {
    ...
    let spawned = [&mut collapsed, &mut shipped]
        .map(|engine| engine.spawn_test_object(SpawnConfig::new("METL")));
    assert_ne!(
        spawned[0], spawned[1],
...
fn the_row_hint_is_localized_from_the_system_group() {
    ...
    let system = crate::support::TestValueExt::test_value(Group::open(
        repository_root().join("planet/System.c4g"),
    ));
    let source = clonk_script::c4_string_from_bytes(&crate::support::TestValueExt::test_value(
        system.read_file(APPEND),
    ));
    ...
    for (language, expected) in [
        ...
        let localized = crate::support::TestValueExt::test_value(
            clonk_resources::localize_script_source(&system, &source, &[language]),
        );
        assert!(
