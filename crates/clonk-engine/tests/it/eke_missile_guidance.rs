...
fn join_guiding_player(engine: &mut Engine, auto_stop: bool) -> i32 {
    engine
        ...
        .initialized()
        .expect("MissileMatch needs no runtime team selection")
        .number
}
...
fn launch_guided_missile(engine: &mut Engine, auto_stop: bool) -> (i32, ObjectId, ObjectId) {
    ...
    let sft = engine
        .crew_cursor(owner)
        .expect("MissileMatch joins a selected SFT");
    ...
    let launcher = engine
        ...
        .expect("the real Eke rocket launcher enters the SFT");
    engine
        .spawn_object(
        ...
        .expect("the launcher holds one real Eke missile");
    ...
    for _ in 0..4 {
        ...
        engine
            .call_object_function(sft_index, "CheckArmed", Vec::new())
            .expect("the real Walk StartCall runs");
        if engine
            .object_snapshot(sft)
            .expect("the SFT remains live")
            .action
    ...
    engine
        ...
        .expect("Throw fires the launcher");
    ...
    engine
        ...
        .expect("Dig switches on the remote guidance");
    assert_eq!(
...
fn eke_missile_stops_turning_when_the_turn_key_is_released() {
    for auto_stop in [false, true] {
        ...
        engine
            ...
            .expect("Left steers the guided missile");
        assert_eq!(
            steering_command(&engine, missile),
            Value::String("Left".into()),
            "holding Left turns the missile (auto_stop={auto_stop})"
        );
        engine.tick_without_snapshot().expect("the missile flies");
        assert_eq!(
            engine
                .object_snapshot(missile)
                .expect("the guided missile remains live")
                .rotation,
            85,
        ...
        engine
            ...
            .expect("releasing Left reaches the launcher");
        assert_eq!(
            steering_command(&engine, missile),
            Value::String("Straight".into()),
        ...
        engine
            ...
            .expect("Right steers the guided missile");
        engine
            .player_in_com(owner, COM_RIGHT + COM_RELEASE_OFFSET, 0)
            .expect("releasing Right reaches the launcher");
        assert_eq!(
            steering_command(&engine, missile),
            Value::String("Straight".into()),
...
fn eke_missile_keeps_the_newer_turn_when_the_previous_key_is_released() {
    ...
    engine
        .player_in_com(owner, COM_RIGHT, 0)
        .expect("Right takes over while Left is still held");
    assert_eq!(
        steering_command(&engine, missile),
    ...
    engine
        .player_in_com(owner, COM_LEFT + COM_RELEASE_OFFSET, 0)
        .expect("releasing the older key reaches the launcher");
    assert_eq!(
        steering_command(&engine, missile),
        Value::String("Right".into()),
    ...
    engine
        .player_in_com(owner, COM_RIGHT + COM_RELEASE_OFFSET, 0)
        .expect("releasing the held key reaches the launcher");
    assert_eq!(
        steering_command(&engine, missile),
        Value::String("Straight".into()),
...
fn eke_missile_down_and_up_still_straighten_the_guided_missile() {
    for auto_stop in [false, true] {
        ...
        engine
            ...
            .expect("Down straightens the guided missile");
        assert_eq!(
            steering_command(&engine, missile),
            Value::String("Straight".into()),
