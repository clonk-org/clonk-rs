...
/// (C4FullScreen.cpp:474-480; C4Version.h:19,24). The port is a different
/// product, so its `STD_PRODUCT` is `PRODUCT_NAME`: the window, the console
/// and every caption built from the engine name say "Clonk Rust", never the
/// C++ engine's "LegacyClonk".
#[test]
fn startup_window_builder_uses_clonk_rust_product_title() {
    assert_eq!(native_window_title(false), "Clonk Rust");
    assert_eq!(native_window_title(true), "Clonk Rust Console");
    assert_eq!(clonk_platform::ENGINE_CAPTION, clonk_platform::PRODUCT_NAME);
    assert!(!clonk_platform::ENGINE_CAPTION.contains("Legacy"));
    assert!(!clonk_platform::CONSOLE_CAPTION.contains("Legacy"));
