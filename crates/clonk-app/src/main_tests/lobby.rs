...
fn joined_lobby_tab_point(app: &mut GameApp, sheet: LobbySheet) -> GuiPoint {
    let layout = app_lobby_mut(&mut app.lobby.session)
...
fn app_lobby(app: &GameApp) -> &NetworkLobbyState {
    app.lobby.session.as_ref().test_value()
}
...
fn app_classic_lobby(app: &GameApp) -> &ClassicHostLobbyState {
    app.lobby.classic_host.as_ref().test_value()
}
...
fn classic_command_line_lobby_timeout_starts_the_host_countdown() {
    ...
    main_assert_eq!(app.lobby.host_countdown => Some(HostLobbyCountdown::with_seconds(120)));
}
...
fn classic_command_line_network_scenario_skips_unrequested_lobby() {
    ...
    app.lobby.preload_task = Some(LobbyPreloadTask {
    ...
    main_assert!(some(&app.lobby.preload_task).start_host_when_ready);
...
fn already_failed_client_start_resource_never_opens_a_stale_progress_wait() {
    ...
    app.netplay.admission_resources.mark_failed(core.id);
...
fn scenario_join_places_ready_crew_and_selects_cursor() {
    ...
    let selection = snapshot.crew_selection.get(&app.players.local_owner).test_value();
    main_assert!(
...
fn network_too_few_warning_then_profile_ack_stages_and_enters_exact_lobby() {
    ...
    let staged = some(&app.netplay.staged_host_scenario);
...
fn joined_lobby_tooltips_survive_frames_and_use_shared_delay() {
    ...
    app.lobby.session = Some(client_lobby_state().with_preloading(false, LobbyLabels::default()));
    app.startup.view = StartupView::NetworkLobby;
    ...
    let tooltip_text = |app: &GameApp| {
        app.lobby.session.as_ref().and_then(|lobby| {
            lobby
                .controller
    ...
    app_lobby_mut(&mut app.lobby.session)
        .render_classic(
            &mut surface,
            assets.as_ref(),
            &app.scenario_game_options,
...
fn captured_classic_lobby_wheel_releases_tooltip_hover_ownership() {
    ...
    let rows = (0..32)
        ...
        .collect();
    app_classic_lobby_mut(&mut app.lobby.classic_host)
        .controller
        .set_rows(rows);
...
fn staged_host_completion_enters_exact_lobby_over_loader_background() {
    ...
    main_assert_ne!(staged.lobby.local_name => app.players.local_name);
    ...
    main_assert_eq!(app.mode => AppMode::Menu);
    main_assert!(app.netplay.client_activity.last_frame.is_empty());
    main_assert_eq!(app.startup.view => StartupView::NetworkLobby);
    main_assert!(
    ...
    main_assert!(app.netplay.control_clients.is_activated(0));
}
...
fn staged_host_installs_activated_participant_before_building_lobby_roster() {
    ...
    main_assert!(app.lobby.session.is_none());
    main_assert_eq!(
        app.players.host_local_alternate_colors => expected_alternate_colors,
        "the prepared host retains its process-local AlternateColorDw sidecar"
    );
    main_assert_eq!(app.players.host_local_info_ids.len() => 1);
    let lobby = &app_classic_lobby(&app).controller;
    ...
    main_assert!(
        app.netplay.admission_resources.complete_path(resource.id).is_some(),
...
fn lobby_options_refresh_only_while_the_sheet_is_active() {
    ...
    select_classic_lobby_sheet(&mut app, LobbySheet::Players);
    some_mut(&mut app.netplay.control_clock).set_control_rate(7);
    ...
    some_mut(&mut app.netplay.control_clock).set_control_rate(8);
    app.sec1_timer().test_value();
...
fn aborting_live_internet_signup_keeps_the_prior_off_state() {
    ...
    process_lobby_game_option(&mut app, LobbyGameOptionInput::Hotkey('I'));
    main_assert!(!app.scenario_game_options.values().master_server_signup);
    main_assert!(app.netplay.pending_masterserver_signup.is_some());
    main_assert!(
    ...
    main_assert!(app.netplay.pending_masterserver_signup.is_none());
    main_assert!(!app.scenario_game_options.values().master_server_signup);
...
fn live_signup_applies_every_start_response_field() {
    ...
    let parameters = &some(&app.netplay.host_join_snapshot).parameters;
    ...
    main_assert_eq!(app.netplay.stream_address => stream_to);
    main_assert_eq!(app.netplay.league_name => b"Cup");
    main_assert_eq!(app.netplay.max_players => 4);
    main_assert_eq!(app.engine.max_players() => Some(4));
    let reference = some(&app.netplay.advertised_game_reference);
    ...
    main_assert_eq!(parameters.max_players => 4);
    main_assert_eq!(app.netplay.stream_address => stream_to);
    main_assert!(app.netplay.league_name.is_empty());
    let reference = some(&app.netplay.advertised_game_reference);
...
fn committed_start_apply_failure_tears_down_when_cleanup_cannot_start() {
    ...
    app.netplay.advertised_game_reference = None;
...
fn leaving_lobby_during_compensating_end_preserves_worker_cleanup() {
    ...
    app.poll_live_masterserver_signup().test_value();
    main_assert!(
        app.netplay.pending_masterserver_signup.is_some(),
...
fn harpoonrace_invalid_live_seed_is_ended_before_the_host_can_launch() {
    ...
    let mut app = new_menu_app(640, 480);
    let (_events, mut commands) = install_classic_host_network_stub(&mut app);
    app.netplay.host_join_snapshot = Some(initial_snapshot);
    app.netplay.advertised_game_reference = Some(reference);
    ...
    main_assert_eq!(some(&app.netplay.host_join_snapshot).parameters.random_seed => 1_784_903_471, "the rejected response must not partially replace the local seed");
    ...
    main_assert!(app.netplay.pending_masterserver_signup.is_none());
    main_assert!(!app.scenario_game_options.values().master_server_signup);
    main_assert_eq!(some(&app.netplay.host_join_snapshot).parameters.random_seed => 1_784_903_471);
    main_assert!(
        app.status_text.contains("1784903470"),
        "successful cleanup preserves the actionable rejection"
    );
    let scenario = match app.netplay.mode.as_ref() {
...
fn classic_lobby_password_button_clears_then_presets_and_sets_live_password() {
    ...
    let (_snapshot, reference) = default_exact_host_reference();
    app.netplay.advertised_game_reference = Some(
        reference
    ...
    main_assert!(
        some(&app.netplay.advertised_game_reference)
            .summary()
            .password_needed
    );
...
fn classic_lobby_comment_updates_and_invalidates_the_advertised_reference() {
    ...
    main_assert_eq!(some(&app.netplay.advertised_game_reference).metadata().comment.as_bytes() => b"old comment");

    process_lobby_game_option(&mut app, LobbyGameOptionInput::Hotkey('M'));
    accept_game_option_input(&mut app, "new comment");

    main_assert_eq!(commands.take_league_update_effects().1 => 1);
    main_assert_eq!(some(&app.netplay.advertised_game_reference).metadata().comment.as_bytes() => b"new comment");
    main_assert_eq!(
        app_classic_lobby(&app)
            .controller
            .logs()
...
fn countdown_zero_quits_a_dialogless_host_that_is_short_of_min_players() {
    let short_host = |dialogless: bool| {
        ...
        app.lobby.host_countdown = Some(HostLobbyCountdown::with_seconds(1));
        (app, events, commands)
    };
    ...
    main_assert!(server.lobby.host_countdown.is_none());
    ...
    ready.lobby.min_players = Some(1);
...
fn classic_host_zero_countdown_enters_go_without_a_countdown_packet() {
    ...
    app.netplay.host_join_snapshot = prepared.host_config().initial_join_snapshot.clone();
    app.netplay.mode = Some(NetworkMode::Host(
        lobby_fixture!(host: 11112, "Exact Host".to_string(), Some(prepared)),
    ));
    let (_events, mut commands) = install_network_commands(&mut app);
    ...
    app.lobby.session = Some(generic_lobby);
    install_test_classic_host_lobby(&mut app);
...
fn atomic_go_worker_failure_is_reported_before_lobby_teardown() {
    ...
    main_assert!(app.lobby.classic_host.is_some());
    main_assert!(app.lobby.session.is_some());
...
fn prepared_host_rebind_leaves_the_classic_lobby_presentable() {
    ...
    app.netplay.pending_host_preparation = Some(receiver);
...
fn classic_host_chat_start_abort_and_readycheck_use_live_lobby_actions() {
    ...
    if let Some(lobby) = app.lobby.classic_host.as_mut() {
...
fn reached_network_start_wait_uses_host_roster_and_client_abort_dialog() {
    ...
    main_assert!(client.lobby.start_wait.is_none());
...
fn joined_lobby_chat_routes_pointer_context_and_log_scroll() {
    let mut app = new_menu_app(640, 480);
    app.startup.view = StartupView::NetworkLobby;
    app.lobby.session = Some(client_lobby_state());
    {
        let lobby = app_lobby_mut(&mut app.lobby.session);
    ...
    let assets = Arc::clone(&app.assets);
    let (layout, max_scroll) = app_lobby_mut(&mut app.lobby.session)
        .with_classic_controller_input(
            app.rendering.graphics.surface(),
            assets.as_ref(),
            &app.scenario_game_options,
    ...
    app_lobby_mut(&mut app.lobby.session).pointer = None;
...
fn joined_lobby_options_tab_click_opens_the_read_only_sheet() {
    ...
    let assets = Arc::clone(&app.assets);
    let options_tab = app_lobby_mut(&mut app.lobby.session)
        .with_classic_controller_input(
            app.rendering.graphics.surface(),
            assets.as_ref(),
            &app.scenario_game_options,
    ...
    let combo = app_lobby_mut(&mut app.lobby.session)
        .with_classic_controller_input(
            app.rendering.graphics.surface(),
            assets.as_ref(),
            &app.scenario_game_options,
...
fn client_lobby_resource_sheet_tracks_hidden_transfer_progress() {
    ...
    let (controller, _) = {
        let surface = app.rendering.graphics.surface();
        app_lobby_mut(&mut app.lobby.session).classic_render_state(
            surface,
            app.assets.as_ref(),
            &app.scenario_game_options,
        )
...
fn classic_lobby_client_removal_evicts_its_resource_namespace() {
    ...
    for resource_id in [host_resource, remote_resource] {
        ...
        app.netplay.admission_resources.resource_cores.insert(
            resource_id,
            lobby_fixture!(resource {
                id: resource_id,
                resource_type: clonk_network::HostResourceType::Player as u8,
...
fn joined_lobby_chrome_routes_exit_and_right_tab_context() {
    ...
    app_lobby_mut(&mut app.lobby.session).handle_panel_pointer_move(caption);
    app.handle_network_lobby_secondary_button(ElementState::Pressed)
        .test_value();
    ...
    app_lobby_mut(&mut app.lobby.session).has_teams = true;
    let caption = right_caption_context_point(&mut app);
    app_lobby_mut(&mut app.lobby.session).handle_panel_pointer_move(caption);
    app.handle_network_lobby_secondary_button(ElementState::Pressed)
        .test_value();
    ...
    app_lobby_mut(&mut app.lobby.session).handle_panel_pointer_move(exit);
    ...
    main_assert!(escape.lobby.session.is_none());
    main_assert!(escape.netplay.manager.is_none());
    main_assert!(escape.netplay.mode.is_none());
    ...
    main_assert!(hotkey.lobby.session.is_none());
    main_assert!(hotkey.netplay.manager.is_none());
    main_assert!(hotkey.netplay.mode.is_none());
...
fn joined_client_roster_context_reaches_mute_and_info_without_host_actions() {
    ...
    app_lobby_mut(&mut app.lobby.session).handle_panel_pointer_move(point);
    app.handle_network_lobby_secondary_button(ElementState::Pressed)
        .test_value();
...
fn joined_lobby_roster_routes_and_retains_classic_interactions() {
    ...
    app.netplay.control_clients.replace_snapshot(clients.clone());
    ...
    app.netplay.pending_join_data = Some(
    ...
    for modifiers in [ModifiersState::empty(), ModifiersState::SHIFT] {
        app_lobby_mut(&mut app.lobby.session).chat_edit = LobbyChatEditView::default();
        app.joined_lobby_layouts().test_value();
        app.input_routing.live.modifiers = modifiers;
    ...
    app_lobby_mut(&mut app.lobby.session).chat_edit = LobbyChatEditView {
        text: "draft".into(),
        caret: 5,
        cursor_visible: true,
        ..LobbyChatEditView::default()
    ...
    app.netplay.control_clients.replace_snapshot(clients);
    app.sync_classic_lobby_roster();
    app.joined_lobby_layouts().test_value();
    let lobby = app_lobby(&app);
    ...
    app.input_routing.live.modifiers = ModifiersState::empty();
    some_mut(&mut app.netplay.pending_join_data)
        .parameters
        .teams
        .team_distribution = 1;
    app.sync_classic_lobby_roster();
    ...
    main_assert_eq!(some(&app.lobby.pending_player_selection).client_id => 7);
}
...
fn unstaged_host_retained_roster_routes_script_player_add() {
    let mut app = new_menu_app(640, 480);
    app.startup.view = StartupView::NetworkLobby;
    app.lobby.session = Some(host_lobby_state());
...
fn network_lobby_game_option_state_matches_role_and_render_focus() {
    ...
    let surface = app.rendering.graphics.surface();
    let (controller, options) = app_lobby_mut(&mut app.lobby.session)
        .classic_render_state(surface, app.assets.as_ref(), &app.scenario_game_options)
        .test_value();
    ...
    host.lobby.session = Some(host_lobby_state());
    ...
    classic.lobby.session = Some(host_lobby_state());
    let before = classic.scenario_game_options.values().clone();
    classic.netplay.is_league = true;
...
fn client_info_dialog_shows_unknown_id_and_host_unacknowledged_marker() {
    ...
    host.netplay.mode = Some(NetworkMode::Host(
        lobby_fixture!(host: 0, "Host".to_string(), None),
    ));
    host.netplay.control_clients
        .replace_snapshot([message_client(0, b"Host")]);
    main_assert!(host
        .open_classic_lobby_client_info(0)
...
fn classic_lobby_add_player_picker_publishes_relative_file_and_projects_echo() {
    ...
    main_assert_eq!(
        some(&app.lobby.pending_player_selection).candidates[&selected].wire_filename =>
        "Players/Alice.c4p",
        "the physical picker path must not leak onto the wire"
    );
    main_assert_eq!(
        some(&app.lobby.pending_player_selection).candidates[&selected].source_path =>
        player_path,
...
fn classic_lobby_team_combo_filters_teams_and_submits_the_full_player_packet() {
    ...
    app.lobby.session = Some(NetworkLobbyState::new(0, "Peer view".to_string(), false));
...
fn classic_lobby_client_telemetry_refreshes_on_the_one_second_timer() {
    ...
    let lobby = app_classic_lobby_mut(&mut app.lobby.classic_host);
    ...
    some(&app.netplay.manager).set_test_lobby_client_telemetry(
...
fn classic_host_lobby_exit_directly_tears_down_and_returns_to_startup() {
    ...
    app.netplay.staged_host_scenario = Some(staged);
    install_test_classic_host_lobby(&mut app);
    let _ = app_classic_lobby_mut(&mut app.lobby.classic_host)
        .controller
    ...
    app.lobby.session = Some(NetworkLobbyState::new(0, "Exact Host".to_string(), true));
    app.netplay.ticks.queue(0, 4, Vec::new());
    app.netplay.sync.queue(0, 5, Vec::new());
    ...
    app.netplay.sync_checks.local.insert(local_check.frame, local_check);
    app.netplay.sync_checks
        .remote
    ...
    app.netplay.executing_ready_tick = Some(6);
    ...
    main_assert!(app.netplay.sync_checks.local.is_empty() && app.netplay.sync_checks.remote.is_empty());
    main_assert!(app.netplay.admission_resources.resources.is_empty());
    main_assert!(app.netplay.executing_ready_tick.is_none());
...
fn connected_client_enters_exact_classic_lobby() {
    ...
    main_assert!(app.status_text.is_empty());
    main_assert!(!app.netplay.control_running);
    main_assert!(app.netplay.control_clients.contains(7));
...
fn network_lobby_renders_classic_base_without_enabling_generic_fallback() {
    ...
    assetless.lobby.session = Some(client_lobby_state());
...
fn options_program_round_trips_bound_values_and_raw_fair_crew_strength() {
    ...
    main_assert!(!app.lobby.white_chat);
...
fn initial_network_game_join_fully_loads_the_client_lobby_within_500ms() {
    ...
    host.netplay.staged_host_scenario = Some(prepare_minimal_host_lobby(&host, scenario.clone()));
    ...
    let host_reference = some(&host.netplay.advertised_game_reference).summary().clone();
    ...
    loop {
        ...
        let roster_propagated = local_client_id.is_some_and(|local_client_id| {
            ...
            client.netplay.control_clients.contains(0)
                && client.netplay.control_clients.contains(local_client_id)
                && client.netplay.control_clients.is_activated(0)
                && client.netplay.control_clients.is_activated(local_client_id)
                && host.netplay.control_clients.contains(local_client_id)
                && host.netplay.control_clients.is_activated(local_client_id)
                && client.lobby.session.as_ref().is_some_and(|lobby| {
                    lobby.roster_rows_authoritative
                        && client_ids(lobby.controller.rows()) == [0, local_client_id]
                })
                && host.lobby.classic_host.as_ref().is_some_and(|lobby| {
                    client_ids(lobby.controller.rows()) == [0, local_client_id]
                })
        });
                        ...
                        && app.players.infos.get(ids[0]).is_some_and(|player| {
                            player.name.as_bytes() == expected_name
                                && player.resource.as_ref().is_some_and(|core| {
                                    app.netplay.admission_resources.complete_path(core.id).is_some()
                                })
                        })
                };
        ...
        let resources_available = local_client_id.is_some_and(|local_client_id| {
            let every_loadable_resource_is_complete = |app: &GameApp| {
                !app.netplay.admission_resources.resource_cores.is_empty()
                    && app.netplay.admission_resources.resource_cores.values().all(|core| {
                        !core.loadable || app.netplay.admission_resources.complete_path(core.id).is_some()
                    })
            };
            ...
            client
                ...
                .is_some_and(|join| {
                    std::iter::once(&join.parameters.scenario)
                        ...
                        .all(|core| {
                            !core.loadable
                                || client.netplay.admission_resources.complete_path(core.id).is_some()
                        })
                        && client.netplay.admission_resources.lobby_ready_available()
                })
        ...
        let status_acknowledged =
            client.netplay.pending_join_data.is_some() && !client.netplay.initial_lobby_status_ack_pending;
        let startup_connection_finished =
            client.startup_network.connection.is_none() && client.netplay.manager.is_some();
        let state_ready = [
            lobby_visible,
            scenario_identity,
            roster_propagated,
        ...
        main_assert!(
            ...
            client.status_text,
            host.netplay.pending_host_preparation.is_some(),
            host.netplay.pending_masterserver_signup.is_some(),
...
fn selected_clonkmars_host_reference_is_queryable_within_one_second() {
    ...
    while app.netplay.advertised_game_reference.is_none() {
        app.test_update();
        main_assert!(
...
fn prepared_host_transport_swap_retains_automatic_preload_state() {
    ...
    while app.netplay.manager.is_none() {
        app.poll_startup_network_connection().test_value();
        main_assert!(
            Instant::now() < deadline,
    ...
    main_assert!(app.lobby.preload_task.is_some() || app.lobby.preload_artifact.is_some());
...
fn selected_network_scenario_installs_prepared_host_before_admission() {
    ...
    for _ in 0..3_000 {
        app.poll_startup_network_connection().test_value();
        if app.netplay.manager.is_some() {
            break;
        }
        std::thread::sleep(Duration::from_millis(10));
    }

    main_assert!(app.netplay.manager.is_some(), "{}", app.status_text);
    let NetworkMode::Host(preparing) = some(&app.netplay.mode) else {
        panic!("selected network scenario must install a host");
    };
    main_assert!(preparing.prepared.is_none());
    main_assert!(app.netplay.pending_host_preparation.is_some());
    ...
    for _ in 0..3_000 {
        app.test_update();
        if app.netplay.pending_host_preparation.is_none()
    ...
    let NetworkMode::Host(settings) = some(&app.netplay.mode) else {
    ...
    let _prepared_resource_owner = app.netplay.manager.replace(manager).test_value();
    install_test_classic_host_lobby(&mut app);
    ...
    main_assert!(
        app.lobby.host_countdown.is_none(),
    ...
    main_assert!(
        app.netplay.game_advertiser.is_some(),
        "native keeps the reference listener alive during play"
    );
    let running_reference = some(&app.netplay.advertised_game_reference);
    ...
    let final_reference = game_over_host_reference(
        some(&app.netplay.advertised_game_reference),
        some(&app.netplay.host_join_snapshot).parameters.clone(),
        &app.netplay.control_clients,
        &app.players.infos,
        app.engine.teams(),
    ...
    app.netplay.advertised_game_reference = Some(final_reference);
    app.snapshot.game_over = true;
    main_assert!(app.netplay.control_clients.set_lobby_ready(7, false));
    app.publish_updated_host_join_snapshot();
    let republished_player = &some(&app.netplay.advertised_game_reference)
        .parameters()
        .player_infos
        .clients[0]
        .players[0];
...
fn cancelling_compatibility_profile_notice_fully_tears_down_the_visible_lobby() {
    ...
    main_assert_eq!(host.startup.view => StartupView::NetworkLobby);
    main_assert!(host.netplay.manager.is_some());
    host.finish_message_dialog(clonk_frontend::message_dialog::MessageDialogResult::Cancel)
        .test_value();

    main_assert!(host.netplay.manager.is_none());
    main_assert!(host.netplay.mode.is_none());
    main_assert!(host.lobby.session.is_none());
    main_assert!(host.lobby.classic_host.is_none());
    main_assert!(host.dialogs.messages.is_empty());
}
...
fn host_reports_the_session_profile_while_a_client_reports_only_its_own_refusal() {
    ...
    for app in [&client, &host] {
        main_assert!(
            !app.lobby.session.test_ref().logs.iter().any(|line| line
                .text
                .to_ascii_lowercase()
...
fn a_joining_client_is_told_its_own_requested_profile_is_unavailable() {
    ...
    main_assert_eq!(client.startup.view => StartupView::NetworkLobby);
    main_assert!(
        !client.lobby.session.test_ref().logs.iter().any(|line| line
            .text
            .to_ascii_lowercase()
            .contains("compatibility profile")),
...
fn a_blocked_compatibility_profile_is_reported_and_not_claimed_to_peers() {
    ...
    host.finish_message_dialog(clonk_frontend::message_dialog::MessageDialogResult::Ok)
        .test_value();
    main_assert!(
        !host.lobby.session.test_ref().logs.iter().any(|line| line
            .text
            .to_ascii_lowercase()
            .contains("compatibility profile")),
...
fn runtime_status_reach_uses_preflight_ready_not_raw_packet_presence() {
    ...
    main_assert_eq!(app.netplay.admission_resources.status(resource_id) => Some(&AdmissionResourceState::Loading { removed: false }));
...
fn client_join_data_replaces_the_lobby_participant_snapshot() {
    ...
    main_assert_eq!(app.netplay.control_clients.state(0).expect("raw host core remains registered").name.as_bytes() => b"Exact Andr\xe9");
    main_assert_eq!(some(&app.netplay.pending_join_data).parameters.title.as_bytes() => b"Caf\xe9 Arena");
}
...
fn clearing_client_lobby_preload_removes_only_its_committed_combined_file() {
    ...
    let mut app = new_state_only_menu_app(320, 200);
    app.netplay.client_combined_scenario_path = Some(owned_path.clone());
    app.netplay.client_combined_preload_file.replace(owned_path.clone());
    app.scenario_lifecycle.network_material_resource_groups = Some(Vec::new());

    app.clear_lobby_preload();

    main_assert!(!owned_path.exists());
    main_assert!(app.netplay.client_combined_scenario_path.is_none());
    main_assert!(!app.netplay.client_combined_preload_file.is_owned());
    ...
    app.netplay.client_combined_scenario_path = Some(existing_path.clone());
    app.clear_lobby_preload();
    ...
    fs::write(&dropped_path, b"drop-owned scenario").test_value();
    {
        ...
        dropped_app.netplay.client_combined_scenario_path = Some(dropped_path.clone());
        dropped_app
            .netplay.client_combined_preload_file
...
fn configured_automatic_lobby_preload_runs_off_thread_and_activation_reuses_it() {
    ...
    app.lobby.classic_host = Some(lobby);
    app.scenario_game_options = options;
    app.netplay.mode = Some(mode);
    app.netplay.manager = Some(manager);
    ...
    main_assert!(app.lobby.preload_task.is_some());
    ...
    let artifact = some(&app.lobby.preload_artifact);
    ...
    let staged = app.netplay.staged_host_scenario.take().test_value();
...
fn join_data_resources_keep_lobby_ready_disabled_until_completion_events() {
    ...
    app.test_network_events();
    main_assert!(!app.netplay.admission_resources.lobby_ready_available());
    main_assert_eq!(app.netplay.admission_resources.status(player.id) => Some(&AdmissionResourceState::Loading { removed: false }));
    ...
    main_assert!(app.netplay.admission_resources.lobby_ready_available());
    main_assert_eq!(
        app.netplay.admission_resources.status(player.id) =>
...
fn host_ready_check_request_aborts_countdown_and_clears_only_nonhosts() {
    ...
    app.lobby.session = Some(lobby);
    app.netplay.mode = Some(NetworkMode::Host(host_network_settings()));
    app.lobby.host_countdown = Some(HostLobbyCountdown::new());
    for client_id in [0, 7, 9] {
        app.netplay.control_clients.register(client_id, true, false);
        main_assert!(app.netplay.control_clients.set_lobby_ready(client_id, true));
    }
    let (_events, mut commands) = install_network_commands(&mut app);
    ...
    main_assert!(app.netplay.control_clients.state(0).unwrap().lobby_ready);
    main_assert!(!app.netplay.control_clients.state(7).unwrap().lobby_ready);
    main_assert!(!app.netplay.control_clients.state(9).unwrap().lobby_ready);
...
fn host_ready_check_uses_cpp_ten_second_default_cooldown() {
    ...
    main_assert!(app.lobby.host_countdown.is_some());
    main_assert!(commands.take_submitted_ready_checks().is_empty());
...
fn ready_check_toast_follows_the_config_flag_rather_than_window_focus() {
    fn client_app(window_active: bool, toasts_enabled: bool) -> GameApp {
        ...
        app.lobby.ready_check_toasts_enabled = toasts_enabled;
        app
    }
...
fn answering_a_ready_check_dismisses_its_notification_exactly_once() {
    ...
    app.lobby.ready_check_toasts_enabled = true;
...
fn an_actionable_desktop_shows_the_ready_check_toast_through_its_watcher() {
    ...
    app.lobby.ready_check_toast_backend = ReadyCheckToastBackend::with_sink(actionable);
...
fn ready_check_notification_actions_submit_the_answer_and_close_the_prompt() {
    ...
    for (action, expected) in [
        ...
        let continuation = app.lobby.ready_check_continuation.clone().test_value();
        main_assert!(continuation.activate(
            NotificationActivation::Chosen(action),
            app.lobby.ready_check_sink.as_ref()
        ));
...
fn a_ready_check_notification_body_click_raises_without_answering() {
    ...
    let continuation = app.lobby.ready_check_continuation.clone().test_value();
    main_assert!(
        !continuation.activate(NotificationActivation::Default, app.lobby.ready_check_sink.as_ref()),
        "a body click resolves nothing"
    );
    app.poll_lobby_ready_check_notification().test_value();

    main_assert!(
        app.lobby.ready_check_continuation.is_some(),
...
fn lobby_teardown_closes_the_ready_check_notification_and_makes_it_inert() {
    ...
    main_assert!(app.dialogs.messages.is_empty());
    main_assert!(app.lobby.ready_check_continuation.is_none());
...
fn go_status_request_deletes_client_lobby_and_suppresses_stale_ready_reply() {
    ...
    app.test_network_events();
    main_assert!(matches!(app.mode, AppMode::Loading));
    main_assert_eq!(app.netplay.pending_client_start_status => Some(go));
    main_assert!(
        app.lobby.session.is_none(),
...
fn joined_lobby_long_countdown_keeps_game_options_unlocked_and_renderable() {
    ...
    let assets = Arc::clone(&app.assets);
    let (_, options) = app_lobby_mut(&mut app.lobby.session)
        .classic_render_state(
            app.rendering.graphics.surface(),
            assets.as_ref(),
            &app.scenario_game_options,
...
fn inbound_lobby_countdown_updates_cpp_countdown_start_and_abort_states() {
    ...
    for (countdown, expected, expected_controller, expected_logs) in [
        ...
        let assets = Arc::clone(&app.assets);
        let (projection, _) = app_lobby_mut(&mut app.lobby.session)
            .classic_render_state(
                app.rendering.graphics.surface(),
                assets.as_ref(),
                &app.scenario_game_options,
...
fn host_start_begins_default_cpp_lobby_countdown_without_leaving_lobby() {
    ...
    main_assert_eq!(app.lobby.host_countdown => Some(HostLobbyCountdown {remaining: DEFAULT_LOBBY_COUNTDOWN_SECONDS,}));
    main_assert!(matches!(app.mode, AppMode::Menu));
}
...
fn host_start_preserves_scenario_selection_guards_before_arming_countdown() {
    ...
    app_lobby_mut(&mut app.lobby.session).select_scenario("missing.c4s", "Missing");
    process_joined_lobby_action(&mut app, LobbyAction::StartGame);
...
fn connected_observer_starts_not_ready_and_retains_explicit_ready_state() {
    ...
    let mut app = new_state_only_menu_app(320, 200);
    let event_tx = install_client_network_stub(&mut app, 7);
    app.lobby.session = Some(NetworkLobbyState::new(7, "Local".to_string(), false));
    ...
    event_tx
        ...
        .test_value();
    app.test_network_events();
    app_lobby_mut(&mut app.lobby.session).register_peer(
        9,
...
fn inbound_ready_check_updates_the_claimed_lobby_participant() {
    ...
    app.lobby.session = Some(lobby);
    app.netplay.control_clients.register(9, true, false);
...
fn host_countdown_disconnect_fixture(
    ...
    app.lobby.session = Some(lobby);
    app.netplay.mode = Some(NetworkMode::Host(host_network_settings()));
    app.lobby.host_countdown = Some(HostLobbyCountdown { remaining });
...
fn host_disconnect_during_long_countdown_keeps_native_timer() {
    ...
    main_assert_eq!(app.lobby.host_countdown => Some(HostLobbyCountdown { remaining: 11 }));
...
fn team_selection_execute_queues_the_only_non_full_team() {
    ...
    // C4Teams.cpp:876-914).
    let mut app = new_running_sandbox_app();
    let occupant = app.players.local_owner;
...
fn client_join_publishes_selected_players_before_info_and_lobby_ack() {
    ...
    for (core, path) in expected_cores.iter().zip(&configured_paths) {
        main_assert_eq!(app.netplay.admission_resources.complete_path(core.id) => Some(path.as_path()), "the publishing client keeps each initial player resource complete");
    }
...
fn client_lobby_acknowledges_join_status_at_the_initialized_control_tick_once() {
    ...
    main_assert_eq!(app.netplay.control_clock.map(NetworkControlClock::current_tick) => Some(23), "PID_StatusAck uses Game.Control.ControlTick");
...
fn later_admission_reuses_shifted_initial_hosts_persisted_alternate_color() {
    ...
    app.players.host_local_alternate_colors = HashMap::from([(11, 0), (22, 0x0000_00e8)]);
    app.players.host_local_info_ids = HashSet::from([1, 2]);
    ...
    main_assert_eq!(
        host_runtime_alternate_color(
            &app.players.host_local_alternate_colors,
            &app.players.host_local_info_ids,
            &aliased_remote,
        ) =>
        Some(0),
...
fn host_remote_lobby_player_info_assigns_the_unique_least_used_free_team() {
    ...
    let teams = some_mut(&mut app.players.team_assignment).teams_mut();
...
fn loading_host_with_retained_lobby_does_not_assign_a_free_team() {
    ...
    let teams = some(&app.players.team_assignment).teams();
...
fn offline_set_max_player_raise_reaches_admission_before_queued_script_player() {
    ...
    main_assert_eq!(app.netplay.max_players => 2, "the app admission cap follows Game.Parameters.MaxPlayers");
    let admitted = app
        .players.infos
        .client_info_ids(0)
        .into_iter()
...
fn ready_ticks_follow_plus_and_minus_one_control_rate_changes() {
    let mut app = new_running_sandbox_app();
    let event_tx = install_network_stub(&mut app);
    app.netplay.control_clock = Some(NetworkControlClock::new(9, 2));
    ...
    main_assert_eq!(app.netplay.control_clock.map(NetworkControlClock::current_tick) => Some(10));
    ...
    main_assert_eq!(app.netplay.control_clock.map(NetworkControlClock::current_tick) => Some(11));
}
...
fn host_lobby_sets_update_published_join_data_and_obey_fair_crew_lock() {
    ...
    some_mut(&mut app.netplay.host_join_snapshot)
        .parameters
...
fn host_deactivates_playerless_remote_before_the_ready_control_gate() {
    ...
    main_assert!(
        app.netplay.control_clients.is_activated(3),
...
fn frozen_lobby_executes_synchronized_activation_immediately() {
    ...
    let event_tx = install_network_stub(&mut app);
    app.netplay.mode = Some(NetworkMode::Host(host_network_settings()));
    app.lobby.session = Some(NetworkLobbyState::new(0, "Host".to_string(), false));
    app.startup.view = StartupView::NetworkLobby;
...
fn network_tick_waits_for_exact_ready_batch_and_ignores_duplicate() {
    ...
    let mut app = new_running_sandbox_app();
    let remote_owner = app.players.local_owner + 1;
    app.engine
        .register_player(PlayerConfig::new(remote_owner, "Remote"))
        .test_value();
    app.engine
        .player_mut(app.players.local_owner)
        .test_value()
        .control
        .control_style = true;
    app.engine
...
fn synchronized_player_select_executes_at_the_ready_tick() {
    ...
    let stats = some(&app.netplay.stats);
...
fn ready_tick_follow_on_uses_next_open_tick_and_clears_marker() {
    ...
    main_assert_eq!(app.netplay.executing_ready_tick => None, "the ready marker is cleared after application");
}
