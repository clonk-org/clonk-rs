...
fn event_loop_uses_cpp_startup_and_ingame_tick_delays() {
    ...
    assert_eq!(
        frame_schedule_for_mode(AppMode::Loading, 28, 1, 16),
        startup
    );
    assert_eq!(
...
fn undecodable_speech_sample_does_not_suppress_text_fallback() {
    ...
    assert!(
        advertised.is_empty(),
        "undecodable speech is not advertised"
    );
...
fn sound_search_terms_preserves_cpp_literal_whitespace_and_dotfile_extensions() {
    ...
    assert_eq!(SoundSearchTerms::new(&nested).search_names, [nested_wav]);
}

#[test]
fn extensionless_sound_names_resolve_only_wav_across_libraries() {
    assert_eq!(SoundSearchTerms::new("Boom").search_names, ["boom.wav"]);
    assert_eq!(SoundSearchTerms::new("Boom.ogg").search_names, ["boom.ogg"]);
    assert_eq!(SoundSearchTerms::new("Boom.mp3").search_names, ["boom.mp3"]);
    let dir = tempdir();
...
fn wildcard_sound_resolution_spans_scenario_global_and_definitions() {
    ...
    let exact = resolver
        ...
        .test_value();
    assert_eq!(
        exact.load_audio().expect("exact sound bytes"),
        b"global blast"
    );
}
...
fn definition_sound_overrides_global_while_scenario_remains_first() {
    ...
    let mut resolver = SoundResolver {
        ...
        registered_definitions: HashSet::new(),
        definition_library_count: 0,
        base_sample_loads: direct_sound_sample_loads(&Group::open(&global).test_value()),
        definition_sample_loads: Vec::new(),
        scenario_sample_loads: Vec::new(),
        sample_ranks: HashMap::new(),
        sample_ranks_prebuilt: false,
...
fn sound_sample_rank_tracks_cpp_last_load_order_separately_from_precedence() {
    ...
    assert!(resolver
        .configure_scenario_with_sound_effect_groups(Some(&scenario), &sound_effect_groups,));

    for definition_sample in ["a.wav", "c.wav"] {
        for scenario_sample in ["b.wav", "d.wav"] {
            assert!(
                resolver.sample_order(definition_sample) < resolver.sample_order(scenario_sample),
...
fn running_gui_sound_requires_fe_samples_and_rx_sound() {
    ...
    assert!(!muted.non_looping_past_half_duration(muted.started_at + Duration::from_millis(500)));
...
fn object_removal_detach_freezes_one_shot_at_last_positional_mix() {
    ...
    assert!(!audio
        .try_start_sound("Impact", None, 100, false, true, None, &moved, &[],)
...
fn more_music_directory_after_clear_replaces_global_catalog() {
    ...
    assert_eq!(catalog.filenames(), ["Added.ogg", "Loose.mod", "Loose.mod"]);
    assert_eq!(
        catalog
            .resolve("Added.ogg")
...
fn music_playlist_filter_uses_raw_semicolon_patterns_and_basename_matching() {
    ...
    assert!(music_playlist_matches(b"NoMatch;Ambient.*", b"Ambient.ogg"));
    assert!(
...
fn options_sound_sheet_seeds_from_live_audio_and_applies_typed_actions() {
    ...
    assert_eq!(audio.options.voice_volume_percent(), 160);
    assert_eq!(
...
fn options_sound_modifier_tabs_and_raw_gamepad_buttons_keep_classic_ownership() {
    ...
    back.test_gamepad_events([GamepadEvent::Direction {
        slot: GamepadSlot::new(0),
...
fn ingame_options_sound_and_music_toggles_persist_to_config_file() {
    ...
    assert_eq!(after_sound.get_in(Some("Sound"), "Sound"), Some("false"));
    assert_eq!(
    ...
    assert_eq!(after_music.get_in(Some("Sound"), "Sound"), Some("false"));
    assert_eq!(after_music.get_in(Some("Sound"), "Music"), Some("false"));
    assert_eq!(
