...
fn app_navigate_object_menu_to(
    ...
    for _ in 0..item_count {
        ...
        AppVirtualKeyboard::new(app).tap(VirtualKeyCode::KeyX);
    }
...
fn app_collect_one_gold_around_blast_debris(app: &mut GameApp, clonk: ObjectId) {
    ...
    for _ in 0..8 {
        ...
        let carried_action = app.engine.test_object_snapshot(clonk).action.name;
        if !app.engine.test_object_snapshot(clonk).contents.is_empty() && carried_action != "Walk" {
            if carried_action == "Hangle" {
                AppVirtualKeyboard::new(app).tap(VirtualKeyCode::KeyX);
            } else if carried_action.starts_with("Scale") {
                let direction = app.engine.test_object_snapshot(clonk).direction;
                let detach = if direction == Direction::Left {
                    VirtualKeyCode::KeyC
                } else {
                    VirtualKeyCode::KeyZ
                };
                AppVirtualKeyboard::new(app).tap(detach);
            }
        ...
        {
            let start_x = app.engine.test_object_snapshot(clonk).position.x;
        ...
        if action.starts_with("Scale") {
            ...
            AppVirtualKeyboard::new(app).press(let_go);
            ...
            AppVirtualKeyboard::new(app).release(let_go);
            continue;
        }
        if action != "Walk" {
...
fn app_tutorial07_climb_right_out_of_blast_pocket(
    ...
    AppVirtualKeyboard::new(app).press(VirtualKeyCode::KeyC);
    ...
    for _ in 0..300 {
        ...
        if entered_scale {
            ...
            AppVirtualKeyboard::new(app).tap(VirtualKeyCode::KeyS);
        }
        previous_action = action;
        app.update()
            .unwrap_or_else(|error| panic!("advance {milestone}: {error}"));
    }
    AppVirtualKeyboard::new(app).release(VirtualKeyCode::KeyC);
    assert!(
        app.engine
            .object_snapshot(clonk)
...
fn app_tutorial07_return_to_hut(
    ...
    if settle_in_shaft {
        let mut previous_action = String::new();
        for _ in 0..160 {
            let action = app.engine.test_object_snapshot(clonk).action.name;
            if action == "Walk" {
                break;
            }
            if action == "Hangle" && previous_action != "Hangle" {
                ...
                AppVirtualKeyboard::new(app).press(VirtualKeyCode::KeyX);
                for _ in 0..4 {
                    app.test_update();
                    if app
                        .engine
                ...
                AppVirtualKeyboard::new(app).release(VirtualKeyCode::KeyX);
                previous_action = action;
                continue;
            }
            previous_action = action;
    ...
    let elevator_start_y = app.engine.test_object_snapshot(elevator_case).position.y;
    hold_app_key_until(
        app,
        VirtualKeyCode::KeyS,
    ...
    AppVirtualKeyboard::new(app).press(VirtualKeyCode::KeyZ);
    let mut previous_action = String::new();
    for _ in 0..300 {
        let clonk_now = app.engine.test_object_snapshot(clonk);
    ...
    AppVirtualKeyboard::new(app).release(VirtualKeyCode::KeyZ);
    assert!(
        app.engine
            .object_snapshot(clonk)
...
fn app_tutorial07_exit_hut_and_descend_to_gold(
    ...
    AppVirtualKeyboard::new(app).press(VirtualKeyCode::KeyC);
    let mut previous_action = app.engine.test_object_snapshot(clonk).action.name;
    for _ in 0..300 {
        let clonk_now = app.engine.test_object_snapshot(clonk);
...
fn app_climb_from_tutorial04_gold_pocket(
        ...
        .is_some_and(|object| !object.action.name.starts_with("Scale"))
    {
        ...
        AppVirtualKeyboard::new(app).press(VirtualKeyCode::KeyS);
    }
    advance_app_until(
        app,
    ...
    AppVirtualKeyboard::new(app).release(VirtualKeyCode::KeyS);
    let throat = app.engine.test_object_snapshot(clonk);
        ...
        .is_some_and(|object| object.position.y > 403)
    {
        if deepened_pocket {
            ...
            {
                let mut keyboard = AppVirtualKeyboard::new(app);
                keyboard.tap(VirtualKeyCode::KeyZ);
                keyboard.tap(VirtualKeyCode::KeyX);
                keyboard.press(VirtualKeyCode::KeyS);
            }
            advance_app_until(
            ...
            for _ in 0..360 {
                ...
                if entered_scale {
                    ...
                    AppVirtualKeyboard::new(app).tap(away);
                } else if landed || left_scale_in_flight {
                    if !right_held {
                        AppVirtualKeyboard::new(app).press(VirtualKeyCode::KeyC);
                        right_held = true;
...
fn app_return_tutorial04_from_hut_to_tunnel(
        ...
        .is_some_and(|object| object.action.name.starts_with("Scale"))
    {
        ...
        let elevator_x = app.engine.test_object_snapshot(elevator_case).position.x;
...
fn app_take_tutorial04_flint_from_hut(
    ...
    let flint = app
        ...
        .test_value();
    AppVirtualKeyboard::new(app).tap(VirtualKeyCode::KeyD);
    advance_app_until(
        app,
...
fn app_tutorial04_blast_next_gold_face(
    ...
    {
        let attached = app.engine.test_object_snapshot(clonk);
        ...
        .is_some_and(|object| object.position.x > face_x && object.action.name == "Walk")
    {
        ...
        let replacement_dig_vertical = VirtualKeyCode::KeyX;
        {
            let mut keyboard = AppVirtualKeyboard::new(app);
            keyboard.press(VirtualKeyCode::KeyZ);
            keyboard.press(replacement_dig_vertical);
        }
        for tick in 0..120 {
            if tick > 0
                && app
            ...
            if tick == 12 {
                let mut keyboard = AppVirtualKeyboard::new(app);
                keyboard.release(replacement_dig_vertical);
                keyboard.press(replacement_dig_vertical);
            }
            app.test_update();
        }
    ...
    if face_x < 414 && !attached {
        ...
        if let Some(incidental) = incidental {
            for _ in 0..2 {
                AppVirtualKeyboard::new(app).tap(VirtualKeyCode::KeyC);
                app.test_update();
                AppVirtualKeyboard::new(app).tap(VirtualKeyCode::KeyA);
                AppVirtualKeyboard::new(app).press(VirtualKeyCode::KeyZ);
                for _ in 0..12 {
    ...
    for _ in 0..180 {
        ...
        let action = clonk_now.action.name;
        if action.starts_with("Scale") && !previous_action.starts_with("Scale") {
            AppVirtualKeyboard::new(app).release(retreat_key);
