...
fn right_click_retains_the_last_move_target_until_the_next_refill() {
    ...
    main_assert_eq!(app.input_routing.live.ingame_mouse_target => Some(front));
    main_assert_eq!(app.input_routing.live.ingame_mouse_caption.cursor => IngameMouseCursorKind::Grab);
    ...
    main_assert_eq!(app.input_routing.live.ingame_mouse_target => Some(rear));
    app.test_right_button(ElementState::Pressed);
    app.test_right_button(ElementState::Released);
    ...
    let mut pointer = app.input_routing.live.ingame_pointer.test_value();
    ...
    app.test_right_button(ElementState::Released);
    main_assert_eq!(app.input_routing.live.ingame_mouse_target => None);
    let (_, player_commands, _) = commands.take_submitted_mouse_controls();
...
fn mouse_clicks_retain_selection_across_same_frame_ocf_mutation() {
    ...
    main_assert_eq!(app.input_routing.live.ingame_mouse_target => Some(target));
    main_assert_eq!(app.input_routing.live.ingame_mouse_caption.cursor => IngameMouseCursorKind::Select);
...
fn help_click_describes_ocf_all_target_without_commands_or_drag() {
    ...
    main_assert!(app.input_routing.live.ingame_mouse_help, "the HUD Help button enters Help mode");
    ...
    main_assert!(app.input_routing.live.ingame_mouse_help, "left-up keeps Help active");
...
fn mouse_fog_deleted_ignore_fow_target_suppresses_retained_double_get() {
    ...
    main_assert_eq!(app.input_routing.live.ingame_mouse_caption.cursor => IngameMouseCursorKind::Carryable);
    let mut commands = install_mouse_network_capture(&mut app);
...
fn fog_keeps_ignore_fow_target_and_jump_captions() {
    ...
    main_assert_eq!(
        app.input_routing.live.ingame_mouse_caption
            .caption
            .as_ref()
...
fn mouse_fog_origin_drag_into_visible_terrain_uses_release_cursor() {
    ...
    let release = app.input_routing.live.ingame_pointer.test_value();
...
fn mouse_fog_blocks_all_four_landscape_boundaries_even_when_disabled() {
    ...
    for pointer in pointers {
        ...
        app.input_routing.live.ingame_mouse_caption.cursor = IngameMouseCursorKind::Nothing;
...
fn physical_left_drag_vehicle_queues_push_to_and_control_target() {
    ...
    main_assert_eq!(app.input_routing.live.ingame_mouse_caption.cursor => IngameMouseCursorKind::VehiclePut);
    let put_world = app
        .rendering.graphics
...
fn mouse_hover_caption_waits_ten_stable_moves_and_clears_on_miss() {
    ...
    main_assert_eq!(app.input_routing.live.ingame_mouse_caption.cursor => IngameMouseCursorKind::Grab);
    main_assert_eq!(app.input_routing.live.ingame_mouse_caption.time_on_target => 0);
    main_assert!(app.input_routing.live.ingame_mouse_caption.caption.is_none());
    for _ in 1..INGAME_MOUSE_CAPTION_DELAY {
        move_to(&mut app, target_point);
    }
    main_assert_eq!(app.input_routing.live.ingame_mouse_caption.time_on_target => INGAME_MOUSE_CAPTION_DELAY - 1);
    main_assert!(app.input_routing.live.ingame_mouse_caption.caption.is_none());
    ...
    main_assert!(app.input_routing.live.ingame_mouse_caption.caption.is_none(), "the next unmatched native Move clears a non-kept caption");
}
...
fn ctrl_region_drags_show_put_and_vehicle_put_captions() {
    for vehicle_drag in [false, true] {
        ...
        main_assert!(app.input_routing.live.ingame_mouse_caption.caption.is_none(), "DragNone starts the moving drag but does not run DragMoving yet");
        app.test_update();

        main_assert_eq!(app.input_routing.live.ingame_mouse_caption.cursor => expected_kind);
...
fn group_put_caption_uses_remaining_live_selection() {
    ...
    main_assert_eq!(app.input_routing.live.ingame_mouse_caption.cursor => IngameMouseCursorKind::Put);
    main_assert_eq!(app.input_routing.live.ingame_mouse_caption.caption.as_ref().expect("remaining grouped selection has a caption").text => "PUT <2 widgets> INTO <Group container>");
}
...
fn world_origin_put_caption_uses_dragged_object_name() {
    ...
    main_assert!(app.input_routing.live.ingame_mouse_caption.caption.is_none(), "the threshold move only enters DragMoving");
    move_cursor(&mut app, target_point, "run DragMoving over world bin");

    main_assert_eq!(app.input_routing.live.ingame_mouse_caption.cursor => IngameMouseCursorKind::Put);
    main_assert_eq!(app.input_routing.live.ingame_mouse_caption.caption.as_ref().expect("world-origin put has a caption").text => "PUT <World parcel> INTO <World bin>");
}
...
fn help_cursor_gets_the_delayed_red_help_caption() {
    ...
    main_assert_eq!(app.input_routing.live.ingame_mouse_caption.cursor => IngameMouseCursorKind::Help);
    main_assert_eq!(app.input_routing.live.ingame_mouse_caption.time_on_target => 0);
    for _ in 1..INGAME_MOUSE_CAPTION_DELAY {
        move_to_target(&mut app);
    }
    ...
    main_assert_eq!(app.input_routing.live.ingame_mouse_caption.caption.as_ref().map(|caption| caption.text.as_str()) => Some(expected.as_str()));
}
...
fn options_gamepad_device_claim_switches_and_releases() {
    ...
    let mut app = new_classic_menu_app(640, 480);
    app.open_options_menu();
    main_assert_eq!(app.input_routing.live.gamepads.options_open_slot() => None);
    ...
    *app.startup.options_dialog.test_mut().controls_mut() = controls;
    ...
    app.process_options_dialog_actions(vec![OptionsDlgAction::SheetChanged(OptionsSheet::Gamepad)])
        .test_value();
    main_assert!(app.input_routing.live.gamepads.is_options_slot_live(GamepadSlot::new(0)));
    ...
    for set in [2, 1] {
        ...
        app.process_options_dialog_actions(vec![OptionsDlgAction::GamepadDeviceSelected(set)])
            .test_value();
        main_assert_eq!(app.input_routing.live.gamepads.options_open_slot() => GamepadSlot::from_index(set));
        for other in 0..3 {
            main_assert_eq!(app.input_routing.live.gamepads.is_options_slot_live(GamepadSlot::new(other as u8)) => other == set);
        }
    }

    app.process_options_dialog_actions(vec![OptionsDlgAction::GamepadDeviceSelected(1)])
        .test_value();
    main_assert_eq!(app.input_routing.live.gamepads.options_open_slot() => Some(GamepadSlot::new(1)));
...
fn options_key_capture_matches_classic_modal_and_production_input_routing() {
    ...
    main_assert_eq!(app.input_routing.gamepad_bindings.raw_key_for_set(2, ControlBindingId::Dig) => None);
    ...
    .test_value();
    main_assert!(app.dialogs.messages.is_empty());
    main_assert_eq!(app.input_routing.gamepad_bindings.raw_key_for_set(2, ControlBindingId::Dig) => input::legacy_gamepad_button_key(2, 5));
}
...
fn false_startup_config_never_polls_gamepad_manager() {
    ...
    main_assert!(!load_gamepads_enabled(Some(&paths)));
    main_assert_eq!(app.input_routing.gamepad_poll_count => 0);
...
fn chart_gamepad_high_close_respects_player_control_priority() {
    ...
    main_assert!(player.input_routing.gamepad_bindings.rebind_button(0, ControlBindingId::Left, slot.index(), physical.index(),));
...
fn sandbox_mouse_toggle_updates_registry_and_reflected_player_state() {
    ...
    main_assert!(app.input_routing.live.ingame_pointer.is_some());
    app.engine
...
fn assigned_secondary_mouse_uses_its_own_command_region_to_suppress_edge_pan() {
    ...
    main_assert_eq!(app.input_routing.live.ingame_pointer.map(|pointer| pointer.owner) => Some(secondary));
    main_assert!(app.input_routing.live.ingame_edge_scroll.is_none());
...
fn first_mouse_move_after_init_centers_before_edge_scroll() {
    ...
    main_assert!(app.input_routing.live.ingame_mouse_init_centered);
    main_assert_eq!(app.input_routing.live.ingame_viewport_mouse.expect("centered gameplay point is retained").position => retained_center);
    main_assert!(app.input_routing.live.ingame_edge_scroll.is_none());
    ...
    main_assert_eq!(app.input_routing.live.ingame_edge_scroll.expect("second edge move arms scrolling").edge.cursor => clonk_frontend::MouseCursorPhase::Left);
}
...
fn tick5_initializes_mouse_before_a_later_edge_move() {
    ...
    main_assert!(app.input_routing.live.ingame_mouse_init_centered);
    main_assert_eq!(app.input_routing.live.ingame_viewport_mouse.expect("Tick5 retains the centered mouse coordinate").position => retained_center);
    main_assert!(app.input_routing.live.ingame_edge_scroll.is_none());
    let before = app.engine.player(owner).test_value().viewports()[0].center;
...
fn button_and_wheel_moves_consume_mouse_init_centering() {
    ...
    main_assert!(app.input_routing.live.ingame_mouse_init_centered);
    main_assert_eq!(app.input_routing.live.ingame_viewport_mouse.expect("button Move retains the centered coordinate").position => retained_center);
    ...
    main_assert!(app.input_routing.live.ingame_mouse_init_centered);
    main_assert!(app.input_routing.live.ingame_viewport_mouse.is_none());
    let before = app.engine.player(owner).test_value().viewports()[0].center;
    move_cursor(
        &mut app,
        left,
...
fn assigned_observer_key_uses_production_dispatch_and_physical_gate() {
    ...
    owned.input_routing.runtime_key_config_cache = OnceLock::new();
    owned.input_routing.runtime_key_config_cache.set(Ok(parsed)).test_value();
    owned.input_routing.live.modifiers = ModifiersState::ALT;
...
fn mouse_left_double_on_solid_queues_dig_and_control_material_data() {
    ...
    let point = app
        .input_routing.live.ingame_pointer
        .map(ingame_pointer_world_pixel)
        .test_value();
...
fn mouse_jump_zone_click_queues_exact_jump_control() {
    ...
    main_assert!(matches!(
        app.input_routing.live.ingame_mouse_caption.cursor,
...
fn mouse_jump_zone_overrides_overlapping_crew_selection() {
    ...
    main_assert_eq!(app.input_routing.live.ingame_mouse_caption.cursor => IngameMouseCursorKind::JumpRight);
    let pointer = app.input_routing.live.ingame_pointer.test_value();
...
fn non_autostop_player_f1_release_falls_through_without_a_stuck_latch() {
    ...
    main_assert!(app.input_routing.live.pressed_engine_keys.contains(&VirtualKeyCode::F1));
