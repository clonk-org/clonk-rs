...
macro_rules! n2_fixture {
    ...
    (player_data {
        ...
        flags: $flags:expr,
        players: $players:expr,
        by_client: $by_client:expr $(,)?
    }) => {
    ...
    (player_data: $client_id:expr, $players:expr, by: $by_client:expr $(,)?) => {
        clonk_engine::PlayerInfoControlData {
            client_id: $client_id,
            players: $players,
            by_client: $by_client,
    ...
    (reference { $($field:ident $(: $value:expr)?),* $(,)? }) => {
    ...
    (scheduled_sync: $tick:expr, $controls:expr $(,)?) => {
        NetworkEvent::ScheduledSync {
            tick: $tick,
            controls: $controls,
        }
    };
    (peer_disconnected: $client_id:expr, $reason:expr $(,)?) => {
        NetworkEvent::PeerDisconnected {
            client_id: $client_id,
            reason: $reason,
        }
    };
    (player_info_event: $origin:expr, $request:expr, $by_host:expr $(,)?) => {
    ...
    (activation_request: $client_id:expr, $tick:expr, $waited_for:expr, $ping_ms:expr $(,)?) => {
    ...
    (client_remove: $client_id:expr, $reason:expr, $by_client:expr $(,)?) => {
    ...
    (eliminate: $player:expr, $by_client:expr $(,)?) => {
        clonk_engine::EliminatePlayerControlData {
            player: $player,
            by_client: $by_client,
        }
    };
    (remove_player: $player:expr, $disconnected:expr, $by_client:expr $(,)?) => {
    ...
    (join_player: $filename:expr, $at_client:expr, $info_id:expr, $source:expr, $by_client:expr $(,)?) => {
    ...
    (runtime_client: $client_id:expr, $status:expr, $control_ready:expr, $wait_ms:expr $(,)?) => {
...
struct N2SilentVoiceSource;

impl crate::voice_chat::VoiceFrameSource for N2SilentVoiceSource {
    fn drain_frames(&self) -> Vec<clonk_audio::VoiceInputFrame> {
        Vec::new()
    }
}

struct N2ConstantVoiceSource {
    sample: i16,
    level: f32,
}

impl crate::voice_chat::VoiceFrameSource for N2ConstantVoiceSource {
    fn drain_frames(&self) -> Vec<clonk_audio::VoiceInputFrame> {
        vec![n2_voice_input_frame(self.sample, self.level)]
    }
}

struct N2UnreachableVoiceSource;

impl crate::voice_chat::VoiceFrameSource for N2UnreachableVoiceSource {
    fn drain_frames(&self) -> Vec<clonk_audio::VoiceInputFrame> {
        unreachable!("this microphone never opens")
    }
}

struct N2VoiceFrames(std::cell::RefCell<Vec<clonk_audio::VoiceInputFrame>>);

impl N2VoiceFrames {
    fn new(frames: Vec<clonk_audio::VoiceInputFrame>) -> Self {
        Self(std::cell::RefCell::new(frames))
    }
}

impl crate::voice_chat::VoiceFrameSource for N2VoiceFrames {
    fn drain_frames(&self) -> Vec<clonk_audio::VoiceInputFrame> {
...
fn n2_enable_voice_activation(app: &mut GameApp) {
    let options = &mut app.test_audio_mut().options;
    options.voice_enabled = true;
    options.voice_activation_mode = crate::settings::VoiceActivationMode::VoiceActivated;
}

fn n2_send_event(events: &network::NetworkEventSender, event: NetworkEvent) {
    events.send(event).test_value();
}

fn n2_send_voice(voice: &network::TestVoiceChannels, frame: clonk_network::VoiceFrame) {
    voice.send_inbound(frame).test_value();
}

fn n2_native(bytes: &[u8]) -> clonk_engine::LegacyCString {
    clonk_engine::LegacyCString::from_bytes(bytes.to_vec()).test_value()
}

fn n2_netdlg_metrics() -> clonk_frontend::startup_netdlg::NetDlgFontMetrics {
    clonk_frontend::startup_netdlg::NetDlgFontMetrics {
...
fn n2_netdlg(width: i32, height: i32) -> clonk_frontend::startup_netdlg::NetDlgController {
...
fn n2_client_settings() -> ClientSettings {
    ClientSettings::new(SocketAddr::from(([127, 0, 0, 1], 11_112)), "Client")
}

fn n2_repository_paths() -> (tempfile::TempDir, EnvGuard, AppPaths) {
...
fn n2_empty_install_paths() -> (tempfile::TempDir, tempfile::TempDir, EnvGuard, AppPaths) {
    let root = tempdir();
    let user_data = tempdir();
    ...
    (root, user_data, guard, paths)
}

fn n2_pending_rejoin(deadline: Instant, next_attempt_at: Option<Instant>) -> PendingHostRejoin {
    PendingHostRejoin {
        settings: n2_client_settings(),
        deadline,
        next_attempt_at,
    }
}

fn n2_team(id: i32, player_ids: Vec<i32>, color: u32) -> clonk_engine::InitialNetworkTeam {
...
fn n2_team_metadata(
    distribution: clonk_engine::InitialNetworkTeamDistribution,
    team_colors: bool,
...
fn n2_saved_game(
    ...
    user_label: &str,
    engine_state: clonk_engine::EngineState,
) -> SavedGameFile {
    SavedGameFile {
...
fn n2_packed_player_fixture(
    filename: &str,
    player_txt: &[u8],
) -> (tempfile::TempDir, std::path::PathBuf) {
    let directory = tempdir();
    let player_path = directory.path().join(filename);
    let mut player_group = clonk_resources::MutableGroup::new(filename);
    player_group
        .add_file_with_metadata("Player.txt", player_txt.to_vec(), 1, false)
        .test_value();
    fs::write(&player_path, player_group.pack().test_value()).test_value();
    (directory, player_path)
}

fn n2_join_runtime_player(app: &mut GameApp, player_path: &Path) {
...
struct N2RetiredHostProfileFixture {
    _directory: tempfile::TempDir,
    player_path: std::path::PathBuf,
    app: GameApp,
    resource: clonk_engine::NetworkResourceCore,
    events: network::NetworkEventSender,
    commands: network::TestNetworkCommands,
}

fn n2_retired_host_profile_fixture(rejoin_policy: Option<bool>) -> N2RetiredHostProfileFixture {
    let (directory, player_path) = n2_packed_player_fixture(
        "HostRejoin.c4p",
        b"[Player]\nName=Host Rejoin\n[Preferences]\nColorDw=1193046\n",
    );
    ...
    app.netplay.rejoin_after_elimination_allowed = rejoin_policy;

    let wire_name = n2_native(player_path.as_os_str().as_encoded_bytes());
    let resource = n2_fixture!(player_resource: 17, wire_name.clone());
    app.players.infos.replace_snapshot(
        41,
        [n2_fixture!(player_data {
            client_id: 0,
            flags: clonk_engine::CLIENT_PLAYER_INFO_FLAG_INITIAL,
            players: vec![n2_fixture!(player {
                id: 41,
    ...
    let retiring = app.players.local_owner + 1;
    app.engine
    ...
    main_assert!(app.engine.execute_eliminate_player_control(&n2_fixture!(eliminate: retiring, 0)).expect("execute host elimination"));
    ...
    main_assert!(app.engine.player(retiring).is_none(), "the eliminated player retires after the 60-frame delay");

    N2RetiredHostProfileFixture {
...
fn push_to_talk_opens_capture_in_a_network_lobby() {
    ...
    app.netplay.control_clients.register(7, false, true);
...
fn network_lobby_voice_activation_uses_a_client_scoped_wire_identity() {
    ...
    main_assert!(app.voice_chat.capture_active());
    main_assert_eq!(app.voice_chat.capture_key() => None);
    let outbound = voice.try_recv_outbound().test_value();
    main_assert_eq!(outbound.player_id => crate::voice_chat::LOBBY_VOICE_PLAYER_ID);
    main_assert!(app.voice_chat.active_speakers(admitted_at).contains(&(0, crate::voice_chat::LOBBY_VOICE_PLAYER_ID)));
...
fn network_lobby_voice_plays_authenticated_clients_non_positionally() {
    ...
    for sequence in [0, 0, 1, 2, 3] {
        n2_send_voice(
            ...
            n2_voice_frame(
                remote_client,
                crate::voice_chat::LOBBY_VOICE_PLAYER_ID,
                3,
                sequence,
                2_000,
            ),
    ...
    main_assert_eq!(app.test_audio_ref().system.voice_stream_stats(stream_id).queued_frames => 0, "an inactive transition must remove already-queued lobby speech",);
...
#[test]
fn network_lobby_voice_plays_for_a_client_that_never_opted_in_to_its_microphone() {
    ...
    {
        let mut audio = app.test_audio_mut();
        audio.system = clonk_audio::AudioSystem::new_manual_with_resampling(
            8,
            clonk_audio::ResamplingMode::Linear,
        );
        // Voice activation with a zero threshold opens any microphone it is
        // allowed to, so a listener that leaked into capture would show here.
        audio.options.voice_enabled = false;
        audio.options.voice_activation_mode = crate::settings::VoiceActivationMode::VoiceActivated;
        audio.options.voice_activation_threshold = 0.0;
    }
    app.voice_chat = crate::voice_service::VoiceChatService::with_source_opener(move |_| {
    ...
    let stream_id =
        crate::voice_chat::voice_stream_id(remote_client, crate::voice_chat::LOBBY_VOICE_PLAYER_ID);
    main_assert_eq!(app.test_audio_ref().system.voice_stream_stats(stream_id).queued_frames => 4, "Voice.Enabled opts in the microphone; hearing the players who opted in needs no opt-in",);
    main_assert!(app
        .voice_chat
        .active_speakers(admitted_at)
        .contains(&(remote_client, crate::voice_chat::LOBBY_VOICE_PLAYER_ID)));
    main_assert_eq!(opens.get() => 0, "listening must never open the listener's own microphone",);
    main_assert!(!app.voice_chat.capture_active());
    main_assert!(voice.try_recv_outbound().is_none());
}
...
fn network_lobby_voice_rejects_unknown_clients_and_non_lobby_scopes() {
    ...
    app.netplay.manager = Some(manager);
    let known_client = 7;
    app.netplay.control_clients.register(known_client, false, true);
    let admitted_at = Instant::now();
    for (client_id, player_id) in [
        (8, crate::voice_chat::LOBBY_VOICE_PLAYER_ID),
        (known_client, 17),
    ] {
        n2_send_voice(&voice, n2_voice_frame(client_id, player_id, 1, 0, 2_000));
    }
...
fn voice_capture_does_not_cross_the_game_and_lobby_identity_boundary() {
    ...
    main_assert!(!app.voice_chat.capture_active(), "a held in-game capture must not silently become lobby speech",);
}
...
fn push_to_talk_and_remote_playback_cross_the_game_runtime_voice_seam() {
    ...
    main_assert_eq!(outbound.stream_epoch => 1);
    main_assert_eq!(outbound.sequence => 0);
    let local_activity = app.voice_chat.active_speakers(Instant::now());
    main_assert!(local_activity.contains(&(local_client, local_player)));
    main_assert_eq!(collect_speaking_overlay_objects(&app.snapshot, &local_activity) => vec![local_cursor],);
    ...
    actual_output.extend(mix_frames(&app.test_audio_ref().system, final_mix_frames));
    ...
    let last_active_frame = stereo_frames
        ...
        .test_value();
    main_assert!(stereo_frames[..=last_active_frame].windows(2).all(|pair| {
        (i32::from(pair[1][0]) - i32::from(pair[0][0])).abs() <= 128
            && (i32::from(pair[1][1]) - i32::from(pair[0][1])).abs() <= 128
    }));
    main_assert_eq!(app.engine.snapshot() => simulation_before_remote_voice, "voice playout state must not become observable to the simulation",);
...
#[test]
fn turning_voice_chat_off_in_game_closes_the_microphone_but_keeps_remote_speech_audible() {
    let opens = std::rc::Rc::new(std::cell::Cell::new(0));
    let observed = opens.clone();
    let local_client = 7;
    let (mut app, voice) = n2_classic_voice_app(local_client);
    app.test_audio_mut().system = clonk_audio::AudioSystem::new_manual_with_resampling(
        8,
        clonk_audio::ResamplingMode::Linear,
    );
    ...
    app.test_audio_mut().options.voice_activation_threshold = 0.0;
    app.voice_chat = crate::voice_service::VoiceChatService::with_source_opener(move |_| {
        observed.set(observed.get() + 1);
        Ok(N2SilentVoiceSource)
    });
    ...
    for sequence in 0..4 {
        n2_send_voice(
            &voice,
            n2_voice_frame(remote_client, remote_player, 1, sequence, 4_000),
        );
    }
    app.update_voice_chat_at(started_at);
    main_assert!(app.voice_chat.capture_active());
    let stream_id = crate::voice_chat::voice_stream_id(remote_client, remote_player);
    main_assert_eq!(app.test_audio_ref().system.voice_stream_stats(stream_id).queued_frames => 4,);

    app.test_audio_mut().options.voice_enabled = false;
    app.update_voice_chat_at(started_at + Duration::from_millis(20));

    main_assert!(
        !app.voice_chat.capture_active(),
        "turning voice chat off must still close the microphone",
    );
    main_assert_eq!(opens.get() => 1, "a listener must not reopen the microphone",);
    main_assert_eq!(app.test_audio_ref().system.voice_stream_stats(stream_id).queued_frames => 4, "turning voice chat off must not cut off a remote player mid-sentence",);
    let mut mixed =
        vec![0_i16; app.test_audio_ref().system.mixer().sample_rate() as usize / 50 * 2];
    app.test_audio_ref().system.mixer().mix_i16(&mut mixed);
    main_assert!(mixed.iter().any(|&sample| sample != 0));
    main_assert!(app
        .voice_chat
        .active_speakers(started_at + Duration::from_millis(20))
...
fn voice_volume_action_scales_remote_playback_gain_above_unity() {
    ...
    fn render_remote_peak(voice_volume: u8) -> i16 {
        ...
        .test_value();
        main_assert_eq!(app.test_audio_ref().options.voice_volume_percent() => i32::from(voice_volume),);
...
fn voice_activation_opens_the_microphone_on_speech_and_leaves_the_key_to_the_game() {
    let local_client = 7;
    let (mut app, mut voice) = n2_classic_voice_app(local_client);
    let local_player = app.players.local_owner;
    n2_enable_voice_activation(&mut app);
    {
        let mut audio = app.test_audio_mut();
        audio.options.voice_activation_threshold = 0.5;
    ...
    app.update_voice_chat();
    main_assert!(app.voice_chat.capture_active());
    main_assert_eq!(app.voice_chat.capture_key() => None, "a voice-activated capture is owned by no key",);
...
fn voice_activation_never_opens_a_microphone_the_player_did_not_opt_in_to() {
    ...
    main_assert_eq!(opens.get() => 0, "Voice.Enabled is the microphone opt-in; the activation mode is only how an opted-in \
         microphone is keyed",);
    main_assert!(!app.voice_chat.capture_active());
}
...
fn switching_back_to_push_to_talk_closes_a_voice_activated_capture() {
    ...
    app.test_audio_mut().options.voice_activation_mode =
        crate::settings::VoiceActivationMode::PushToTalk;
    app.update_voice_chat();
    main_assert!(!app.voice_chat.capture_active(), "no key holds this capture open, so nothing else would ever close it",);
}
...
fn capture_processing_follows_the_settings_without_reopening_the_microphone() {
    ...
    app.test_audio_mut().options.voice_automatic_gain_control = false;
    app.update_voice_chat();
    ...
    main_assert_eq!(opens.get() => 1, "and does it without closing and reopening the device",);
}
...
fn voice_activation_opens_the_exact_configured_input_device() {
    ...
    main_assert_eq!(*observed.borrow() => Some(selected));
}
...
fn clearing_the_live_network_session_drops_microphone_capture_without_another_tick() {
    ...
    main_assert_eq!(drops.get() => 1, "session teardown closes capture synchronously");
    main_assert!(!app.voice_chat.capture_active());
...
fn changing_voice_input_keeps_the_network_and_simulation_session() {
    ...
    app.test_audio_mut().options.voice_input_device = Some(second.clone());
    app.update_voice_chat_at(now + Duration::from_millis(20));
    let after_hotplug = voice.try_recv_outbound().test_value();

    main_assert_eq!(opens.borrow().as_slice() => &[Some(first), Some(second)]);
    main_assert_eq!(drops.get() => 1, "only the replaced stream is closed");
    main_assert_eq!((before_hotplug.stream_epoch, before_hotplug.sequence) => (1, 0));
    main_assert_eq!((after_hotplug.stream_epoch, after_hotplug.sequence) => (2, 0));
    main_assert!(app.netplay.manager.is_some(), "the game session remains connected");
    main_assert_eq!(app.snapshot => before, "voice never mutates lockstep state");
}
...
fn a_microphone_voice_activation_cannot_open_is_not_retried_every_tick() {
    ...
    main_assert_eq!(opens.get() => 1, "a failed open must not be retried per tick");

    app.window_active = false;
    app.update_voice_chat();
    app.window_active = true;
    app.update_voice_chat();
    main_assert_eq!(opens.get() => 2, "becoming eligible again is what clears the failure",);
}
...
fn direct_runtime_repairs_urls_truncated_by_the_old_rust_parser() {
    ...
    main_assert!(repair_rust_truncated_masterserver_urls(&path).expect("repair config"));

    let config = Config::load(path).test_value();
    main_assert_eq!(config.get_in(Some("Network"), "ServerAddress") => Some(OFFICIAL_LEAGUE_SERVER));
    main_assert_eq!(config.get_in(Some("Network"), "AlternateServerAddress") => Some(OFFICIAL_LEAGUE_SERVER));
}
...
fn reference_query_settings_use_cpp_configured_locale() {
    ...
    main_assert_eq!(load_network_advertiser_settings(Some(&paths)).language_charset => "RUSSIAN");
}
...
fn zero_ports_flow_to_disabled_app_network_services() {
    ...
    main_assert_eq!(load_network_startup_settings(Some(&paths)).1 => 0);
    main_assert_eq!(load_network_reference_port(Some(&paths)) => 0);
    main_assert_eq!(load_network_search_settings(Some(&paths)).discovery_port => 0);
    main_assert_eq!(
...
fn network_create_selects_a_scenario_before_binding_a_host() {
    ...
    main_assert!(app.netplay.manager.is_none());
    main_assert!(app.netplay.game_advertiser.is_none());
...
fn network_host_preparation_keeps_cpp_configured_participant_order() {
    ...
    main_assert_eq!(clonk_resources::encode_legacy_script_text(&preparation.group_maker) => Some(b"M\xc3\xa4ker".to_vec()));
    main_assert_eq!(clonk_resources::encode_legacy_script_text(&preparation.host_name) => Some(b"H\xc3\xa4st".to_vec()));
    main_assert_eq!(clonk_resources::encode_legacy_script_text(&preparation.host_nick) => Some(b"N\xc3\xa4ck".to_vec()));
    main_assert_eq!(
    ...
    main_assert_eq!(preparation.config.control_rate => 7);
    main_assert_eq!(preparation.config.control_mode => 1);
    main_assert_eq!(preparation.config.network_tcp_port => 12_345);
    main_assert_eq!(preparation.config.network_udp_port => 12_346);
    main_assert_eq!(preparation.config.max_load_file_size => 123_456);
    main_assert!(!preparation.config.no_runtime_join);
    main_assert!(!preparation.config.enable_upnp);

    let publication = preparation.league.test_ref();
    main_assert!(!publication.league_server_signup);
    main_assert_eq!(
        preparation
            .player_sources
            .iter()
...
fn command_line_definition_selection_is_published_to_network_clients() {
    ...
    main_assert!(dynamic_scenario.windows(expected_definitions.len()).any(|window| window == expected_definitions));

    let host_files = host.resource_files.clone();
    let join_data = n2_fixture!(join_data:
        2, snapshot.dynamic_tick, host.initial_status, snapshot.dynamic, snapshot.parameters
    );
    ...
    main_assert_eq!(client_definition_ids => vec!["BAS1".to_string(), "EXT1".to_string(), "EXT2".to_string(), "OBJS".to_string(),]);
    let mut host_definition_ids = Vec::new();
    prepared
        .claim_scenario()
        .test_value()
        .visit_definition_groups(|id, _| host_definition_ids.push(id.to_string()));
    main_assert_eq!(host_definition_ids => client_definition_ids);
}
...
fn packed_scenario_alias_is_skipped_as_an_external_definition() {
    ...
    main_assert_eq!(snapshot.parameters.game_resources[0].id => snapshot.parameters.scenario.id);
    main_assert_eq!(snapshot.parameters.game_resources[0].resource_type => clonk_network::HostResourceType::Scenario as u8);
    main_assert!(published_definition_wire_names(&prepared).is_empty());
    main_assert_eq!(prepared.definition_modules() => [path_as_legacy_text(&scenario_path)]);

    let scenario = prepared.claim_scenario().test_value();
    main_assert!(scenario.definition_resource_paths().is_empty());
    let mut definition_ids = Vec::new();
    scenario.visit_definition_groups(|id, _| definition_ids.push(id.to_string()));
    main_assert_eq!(definition_ids => ["SCEN"]);
}
...
fn packed_system_alias_becomes_a_repeated_definition_row() {
    ...
    let game_resources = &prepared
        ...
        .parameters
        .game_resources;
    main_assert_eq!(game_resources[0] => game_resources[1]);
    main_assert_eq!(game_resources[0].resource_type => clonk_network::HostResourceType::Definitions as u8);
    main_assert_eq!(published_definition_wire_names(&prepared) => vec![b"System.c4g".to_vec(), b"System.c4g".to_vec()]);
    main_assert_eq!(prepared.definition_modules() => ["System.c4g"]);

    let scenario = prepared.claim_scenario().test_value();
    main_assert_eq!(scenario.definition_resource_paths() => [system_path.clone(), system_path]);
    let mut definition_ids = Vec::new();
    scenario.visit_definition_groups(|id, _| definition_ids.push(id.to_string()));
    main_assert_eq!(definition_ids => ["SYSD"]);
}
...
fn packed_material_alias_removes_the_host_material_projection() {
    ...
    let game_resources = &prepared
        ...
        .parameters
        .game_resources;
    main_assert_eq!(game_resources[0] => game_resources[2]);
    main_assert_eq!(game_resources[2].resource_type => clonk_network::HostResourceType::Definitions as u8);
    main_assert!(prepared.material_resource_groups().is_empty());
    main_assert_eq!(prepared.definition_modules() => ["Material.c4g"]);
    ...
    main_assert_eq!(frozen_definition_paths => game_save_definition_paths(Some(&paths), &load_native_config_bytes(Some(&paths)),));
    ...
    app.netplay.mode = Some(NetworkMode::Host(
        n2_fixture!(host: 0, "Host".to_owned(), Some(prepared.clone())),
    ));
    ...
    main_assert_eq!(scenario.definition_resource_paths() => [material_path.clone(), material_path]);
    let mut definition_ids = Vec::new();
    scenario.visit_definition_groups(|id, _| definition_ids.push(id.to_string()));
    main_assert_eq!(definition_ids => ["MATD"]);
}
...
fn reference_state_survives_initial_and_final_advertiser_bind_failure() {
    ...
    main_assert!(app.netplay.game_advertiser.is_none());
    main_assert_eq!(app.netplay.advertised_game_reference.as_ref().expect("validated state survives bind failure").summary().state => "Lobby");
    ...
    main_assert!(app.netplay.game_advertiser.is_none());
    let retained = app.netplay.advertised_game_reference.test_ref();
    main_assert_eq!(retained.summary().state => "Running");
    main_assert!(!retained.summary().join_allowed);
    main_assert_eq!((retained.metadata().time, retained.metadata().frame) => (37, 91));
    ...
    main_assert!(app.netplay.game_advertiser.is_none());
    main_assert!(app.netplay.advertised_game_reference.is_none());
}
...
fn netpuncher_assignment_refreshes_the_retained_host_reference() {
    ...
    let updated = app.netplay.advertised_game_reference.test_ref();
    main_assert_eq!(updated.summary().netpuncher_ipv4 => 0xaabb_ccdd);
    main_assert_eq!(updated.metadata().netpuncher_ipv4 => 0xaabb_ccdd);
    main_assert_eq!(updated.summary().addresses => addresses);
    main_assert_eq!(updated.metadata().addresses => addresses);
    main_assert_eq!(updated.summary().tcp_addresses => vec!["198.51.100.7:11112".parse::<SocketAddr>().unwrap()]);
...
fn recoverable_route_diagnostic_keeps_the_classic_host_lobby_open() {
    ...
    install_test_classic_host_lobby(&mut app);
    app.netplay.mode = Some(NetworkMode::Host(
        n2_fixture!(host: 11_112, "Host".to_string(), None),
    ));
...
fn peer_protocol_error_logs_and_keeps_the_classic_lobby_open() {
    ...
    n2_send_event(
        &events,
        NetworkEvent::Error("failed to decode direct control packet".to_string()),
    );
...
fn network_diagnostics_are_visible_in_a_joined_client_lobby() {
    ...
    n2_send_event(
        &events,
        NetworkEvent::Error("network scheduler warning".to_string()),
    );
...
fn failed_client_connection_reaches_cleanup_and_keeps_classic_lobby_open() {
    ...
    app.netplay.pending_runtime_dynamic_request =
    ...
    n2_send_event(
        &events,
        n2_fixture!(peer_disconnected: failed_client_id, None),
    );
    ...
    main_assert!(app.classic_host_lobby_active());
    main_assert!(app.netplay.manager.is_some());
    main_assert!(app.netplay.pending_runtime_dynamic_request.is_none(), "the ordinary PeerConnectionFailed cleanup handler must run");
...
fn runtime_dynamic_completion_rejects_stale_and_cancelled_generations() {
    ...
    app.netplay.pending_runtime_dynamic_request = Some(pending);
    ...
    main_assert_eq!(app.netplay.pending_runtime_dynamic_request.as_ref().and_then(|request| request.save_generation) => Some(2));

    app.netplay.pending_runtime_dynamic_request = None;
    app.finish_runtime_dynamic_save(save_worker::RuntimeDynamicSaveCompletion {
        generation: 2,
        synchronized_control_tick: 4,
        dynamic_tick: 4,
    ...
    main_assert!(app.netplay.pending_runtime_dynamic_request.is_none());
}
...
fn fatal_worker_failure_in_network_lobby_restores_startup_error_log() {
    ...
    n2_send_event(
        &events,
        NetworkEvent::FatalError("network worker stopped unexpectedly".to_string()),
    );
    ...
    main_assert_eq!(engine_results.network_result_message => b"network worker stopped unexpectedly");
...
fn fatal_worker_failure_after_lobby_while_loading_changes_to_local_control() {
    ...
    n2_send_event(
        &events,
        NetworkEvent::FatalError("network worker stopped during start".to_string()),
    );
    ...
    let engine_results = app.engine.snapshot().round_results;
    main_assert_eq!(engine_results.network_result => Some(clonk_engine::RoundResultsNetworkResult::NetworkError));
    main_assert_eq!(engine_results.network_result_message => b"network worker stopped during start");
    main_assert_eq!(app.snapshot.round_results => engine_results);
}
...
fn post_go_client_preparation_failure_clears_session_and_presents_startup_error() {
    ...
    let snapshot = clonk_network::HostConfig::default()
        .initial_join_snapshot
        .test_value();
    let go = n2_fixture!(status: clonk_network::NETWORK_STATE_GO, 0, snapshot.dynamic_tick);
    app.netplay.pending_client_start_status = Some(go);
    app.netplay.pending_join_data = Some(n2_fixture!(join_data:
        3, snapshot.dynamic_tick, go, snapshot.dynamic, snapshot.parameters
    ));
    app.netplay.client_combined_scenario_path = Some(dir.path().join("MissingCombined3.c4s"));
...
fn fatal_worker_failure_during_running_round_changes_to_local_control() {
    ...
    app.netplay.manager = Some(manager);
    app.netplay.mode = Some(NetworkMode::Client(n2_client_settings()));
    app.netplay.control_clock = Some(NetworkControlClock::new(31, 4));
...
fn fatal_worker_failure_during_game_over_preserves_the_network_halt() {
    ...
    n2_send_event(
        &events,
        NetworkEvent::FatalError("host disconnected during evaluation".to_string()),
    );
    ...
    main_assert!(app.netplay.manager.is_none());
    main_assert!(!app.netplay.control_running, "ChangeToLocal must not resume beneath the game-over dialog");
    main_assert_ne!(app.netplay.offline_halt_count => 0, "the dialog-owned network hold transfers to local control");
    let engine_results = app.engine.snapshot().round_results;
    main_assert_eq!(engine_results.network_result => Some(clonk_engine::RoundResultsNetworkResult::NetworkError));
    main_assert_eq!(engine_results.network_result_message => b"host disconnected during evaluation");
    main_assert_eq!(app.snapshot.round_results => engine_results);
...
fn network_search_results_render_only_in_native_rows() {
    ...
    app.test_render(&mut frame);
    main_assert!(frame.iter().any(|byte| *byte != 0x4d));
    ...
    .test_value();
    main_assert_eq!(app.status_text => "Querying game infos…");
    frame.fill(0x6e);
    let error = app
        .render(&mut frame)
...
fn join_progress_lists_logical_routes_once_without_collapsing_dial_attempts() {
    ...
    main_assert_eq!(settings.server_addresses => attempts, "the network worker must retain every scoped dial attempt");
    main_assert_eq!(
    ...
    main_assert_eq!(startup_network_connect_targets(&tcp_only) => "TCP:[fe80::cafe]:11112", "C++ names only protocols for which NetIO.Connect accepted an attempt");
}
...
fn join_progress_names_target_and_dismisses_on_resolution() {
    ...
    main_assert_eq!(progress.focused_button() => Some(MessageDialogButton::Cancel));
    main_assert_eq!(progress.icon() => MessageDialogIcon::Standard(3));
    main_assert_eq!(progress.size() => MessageDialogSize::Regular);
    main_assert!(app.status_text.is_empty());
...
fn a_reference_join_names_the_game_rather_than_its_transport_addresses() {
    ...
    app.activate_network_reference_join(n2_fixture!(reference {
...
fn escape_aborts_inflight_join_and_keeps_network_dialog() {
    ...
    app.netplay.pending_join = Some(ClientSettings::new(address, "Player"));
    let (sender, receiver) = mpsc::channel();
...
fn network_connection_progress_cancel_interrupts_inflight_transport() {
    ...
    app.netplay.pending_join = Some(cancellable_settings(
        second_server,
        second_mesh_tcp,
        second_mesh_udp,
    ));
...
fn network_reference_projects_five_lines_and_native_status_order() {
    ...
    main_assert_eq!(row.title => "Rage on Ada");
    main_assert_eq!(row.details => "2/4 players - Goals: Gold, Elimination - Game is running - 01:02:03");
    main_assert_eq!(row.extra_lines => ["Engine version: LegacyClonk 4.9.11.0 [362]", "Comment: Bring a friend", "Player: Ada, Bob",]);
    main_assert_eq!(
        row.status_icons =>
        [
    ...
    main_assert_eq!(row.row_icon => NetDlgRowIcon::Scenario(7));
    main_assert_eq!(row.address.as_deref() => Some("127.0.0.1:11112"));
    main_assert!(row.joinable);
    ...
    main_assert!(!alternate.status_icons.contains(&NetDlgStatusIcon::OfficialServer));
}
...
fn network_message_modal_freezes_search_events_and_expiry_until_close() {
    ...
    let mut app = new_classic_menu_app(800, 600);
    attach_l040_network_dialog(&mut app);
    let nearly_expired_reference = n2_fixture!(reference {
...
fn the_periodic_masterserver_requery_keeps_the_reply_it_is_refreshing() {
    ...
    let replied = app
        ...
        .masterserver_entry()
        .clone();
    main_assert_eq!(replied.details => "3 game(s) found.");
...
fn network_direct_query_ids_preserve_selection_across_out_of_order_results() {
    ...
    let first = n2_fixture!(reference {
...
fn network_version_mismatch_defers_to_runtime_join_policy() {
    ...
    let mut dialog = n2_netdlg(800, 600);
...
fn network_no_runtime_join_requires_yes_and_retains_the_exact_reference() {
    ...
    main_assert_eq!(modal.message() => "The game has started already and runtime join is not allowed! Try joining anyway?");
    main_assert_eq!(modal.buttons() => clonk_frontend::message_dialog::MessageDialogButtons::YES_NO);
    main_assert_eq!(modal.focused_button() => Some(clonk_frontend::message_dialog::MessageDialogButton::Yes));
    main_assert!(app.netplay.pending_join.is_none());
    app.finish_message_dialog(clonk_frontend::message_dialog::MessageDialogResult::No)
        .test_value();
    main_assert!(app.netplay.pending_join.is_none());
    ...
    let settings = app.netplay.pending_join.test_ref();
    main_assert_eq!(settings.server_addresses => [global_tcp, private_udp]);
...
fn client_join_flow_uses_cpp_reference_build_regardless_of_rust_version() {
    ...
    let mut network_dialog = n2_netdlg(800, 600);
    ...
    main_assert_eq!(settings.server_addresses => [global_tcp, private_udp]);
    main_assert_eq!(settings.logical_server_addresses => [global_tcp, private_udp], "reference joins retain the C++ progress routes separately");
    main_assert_eq!(settings.netpuncher_address.as_deref() => Some("puncher.invalid:11115"));
    main_assert_eq!(settings.netpuncher_game_ids.ipv4 => 0x1122_3344);
    main_assert_eq!(settings.netpuncher_game_ids.ipv6 => 0x5566_7788);
    ...
    main_assert_eq!(dialog.controller.icon() => InputDialogIcon::LOCKED);
    main_assert!(dialog.controller.text().is_empty());
...
fn client_join_flow_wrong_password_reprompts_without_rebuilding_attempts() {
    ...
    main_assert!(app.startup_network.connection.is_none());
    main_assert_eq!(app.netplay.pending_join.as_ref().expect("same join remains pending").server_addresses => attempts);
...
fn client_join_flow_submitted_password_is_frozen_for_the_worker() {
    ...
    drop(closed);
    app.netplay.pending_join = Some(ClientSettings::new(address, "Player").with_join_attempts([
    ...
    main_assert_eq!(app.netplay.pending_join.as_ref().expect("settings stay frozen while the worker starts").password.as_bytes() => b"typed secret");
    main_assert!(app.startup_network.connection.is_some());
    for _ in 0..100 {
        app.poll_startup_network_connection().test_value();
...
fn gamepad_enabled_uses_native_false_and_defaults_true() {
    main_assert!(configured_gamepads_enabled(b""));
    main_assert!(configured_gamepads_enabled(b"[General]\nGamepadEnabled=invalid\n"));
...
fn startup_group_maker_snapshot_preserves_configured_native_bytes() {
    main_assert_eq!(configured_process_group_maker(b"").as_bytes() => b"");
    main_assert_eq!(configured_process_group_maker(b"[General]\nName=\"M\\201ker\"\nName=Ignored\n").as_bytes() => b"M\x81ker");
}

#[test]
fn dirty_axis_calibration_updates_cpp_keys_without_rewriting_other_bytes() {
    let source = b"[Vendor]\nOpaque=\x80\xff\n[Gamepad2]\nVendorKey=keep\nAxis4Min=7\n";
    main_assert_eq!(update_dirty_gamepad_axis_calibration_config(source, &GamepadBindings::default()).expect("clean calibration leaves config untouched") => source);
    ...
    main_assert!(updated.windows(2).any(|bytes| bytes == b"\x80\xff"));
    main_assert_eq!(clonk_app_netplay::configured_native_value(&updated, "Gamepad2", "VendorKey").expect("unrelated gamepad value survives").as_bytes() => b"keep");
    main_assert_eq!(clonk_app_netplay::configured_native_value(&updated, "Gamepad2", "Axis4Min").expect("minimum persisted").as_bytes() => b"12");
    main_assert_eq!(clonk_app_netplay::configured_native_value(&updated, "Gamepad2", "Axis4Max").expect("full u32 maximum persisted").as_bytes() => b"4294967295");
    main_assert_eq!(clonk_app_netplay::configured_native_boolean(&updated, "Gamepad2", "Axis4Calibrated") => Some(true));
}
...
fn startup_dialog_fade_preserves_ordered_native_text_at_scaled_output() {
    ...
    for step in 1..=STARTUP_DIALOG_FADE_STEPS {
        ...
        if step == 1 {
            main_assert_eq!(plan.batches.len() => 2, "outgoing then incoming layers");
            main_assert!(plan.batches.iter().all(|batch| batch.logical_layer.is_some()));
            main_assert!(plan.batches.iter().all(|batch| !batch.text.is_empty()));
            main_assert!(plan.batches[0].fonts.is_some(), "outgoing retains its fonts");
            main_assert!(plan.batches[0].text.iter().all(|command| command.color[3] <= 230));
            main_assert!(plan.batches[1].text.iter().all(|command| command.color[3] <= 26));
        }
...
fn synchronized_player_file_policy_matches_native_suppression_matrix() {
    ...
    for status in [PlayerStatus::Eliminated, PlayerStatus::Surrendered] {
        main_assert_eq!(policy(status, false, 7, false, Some(4)) => SynchronizedPlayerFilePolicy::Skip);
    }
    main_assert_eq!(policy(PlayerStatus::Active, true, 7, false, Some(4)) => SynchronizedPlayerFilePolicy::Skip);
    for (league, max_players) in [(true, Some(4)), (false, Some(0)), (false, Some(-1))] {
        main_assert_eq!(policy(PlayerStatus::Active, false, 8, league, max_players) => SynchronizedPlayerFilePolicy::BlockedRemote);
    }
    for max_players in [None, Some(1)] {
        main_assert_eq!(policy(PlayerStatus::Active, false, 8, false, max_players) => SynchronizedPlayerFilePolicy::Persist {local_control: false});
    }
}
...
fn synchronized_player_file_remote_gates_leave_profile_untouched() {
    ...
    main_assert_eq!(app.offline_local_client_id() => 0, "the fixture must classify client 8 as remote");

    app.engine.set_max_players(4);
    app.netplay.is_league = true;
    main_assert!(!app.persist_synchronized_local_player_files());
    main_assert_eq!(fs::read(&profile_path).unwrap() => sentinel);
...
fn developer_console_latches_no_input_and_reflects_pending_network_pause() {
    ...
    app.netplay.runtime_status_barrier = Some(RuntimeNetworkStatusBarrier {
        status: n2_fixture!(status: clonk_network::NETWORK_STATE_PAUSE, 0, 0),
        local_reached: false,
        actual_control_tick: None,
    });
    main_assert!(app.developer_console_view_model().halted, "IsPaused becomes true as soon as a network status barrier is pending");
    app.netplay.runtime_status_barrier = None;
    main_assert!(!app.developer_console_view_model().halted);
}
...
fn developer_console_player_and_net_menus_use_native_controls() {
    ...
    main_assert!(app.developer_console_net_menu_entries().iter().all(|entry| entry.menu_enabled), "native keeps the rows enabled and rejects their callback");
    main_assert!(!app.developer_console_kick_client(7).expect("non-control host ignores kick"));
    main_assert!(commands.take_submitted_client_removes().is_empty());
    app.engine.set_control_host(true);
    main_assert!(app.developer_console_kick_client(7).expect("control host queues kick"));
    main_assert_eq!(
        commands.take_submitted_client_removes() =>
        vec![n2_fixture!(client_remove:
            7,
            clonk_engine::LegacyCString::from_bytes(b"kicked from host menu".to_vec())
    ...
    main_assert!(app.developer_console_quit_player(player, true).expect("queue console player quit"));
    main_assert_eq!(
        commands.take_submitted_internal_player_scripts() =>
        vec![(
            tick,
            clonk_engine::ControlPacket::EliminatePlayer(n2_fixture!(eliminate: player, 0)),
        )]
    );
...
fn developer_console_offline_join_and_quit_use_control_cadence() {
    ...
    main_assert_eq!(app.developer_console_join_players(&[player_path.to_path_buf()], true).expect("join selected offline player") => 1);
    main_assert_eq!(app.engine.snapshot().players.len() => before + 1);

    let player = app.players.local_owner;
    app.engine.set_control_rate(3);
    app.snapshot = app.engine.test_tick();
    main_assert!(app.developer_console_quit_player(player, true).expect("queue local CID_EliminatePlayer"));
    main_assert_ne!(
    ...
    app.test_update();
    app.test_update();
    main_assert_ne!(app.engine.player(player).map(clonk_engine::Player::status) => Some(clonk_engine::PlayerStatus::Eliminated));
    app.test_update();
    main_assert_eq!(app.engine.player(player).map(clonk_engine::Player::status) => Some(clonk_engine::PlayerStatus::Eliminated));
}
...
fn developer_console_network_join_uses_join_local_player_route() {
    ...
    let resource = n2_fixture!(player_resource: 7 << 16, wire_name.clone());
    let observer = thread::spawn(move || commands.complete_initial_client_join(vec![resource]));

    main_assert_eq!(app.developer_console_join_players(&[player_path.to_path_buf()], true).expect("console network player joins") => 1);
    drop(app.netplay.manager.take());
    ...
    main_assert_eq!(publications[0].source_path.as_path() => player_path);
    main_assert_eq!(publications[0].wire_name => wire_name);
    main_assert_eq!(publications[0].group_maker.as_bytes() => b"Console maker");
    main_assert_eq!(updates.len() => 1);
    main_assert_eq!(updates[0].client_id => 7);
    main_assert_eq!(updates[0].flags => clonk_engine::CLIENT_PLAYER_INFO_FLAG_ADD_PLAYERS);
...
fn developer_console_unpacked_c4f_child_has_native_mother_group() {
    ...
    main_assert_eq!(physical => outer);
    main_assert_eq!(children => vec!["Inner.c4f", "Game.c4s"]);

    let standalone = directory.path().join("Standalone.c4s");
    fs::create_dir(&standalone).test_value();
    main_assert_eq!(scenario_logical_storage(&standalone).unwrap() => (standalone, Vec::new()));
}
...
fn folder_live_save_replays_native_prefix_and_writes_packed_children() {
    ...
    main_assert_eq!(fs::read(destination.join("Prefix.txt")).unwrap() => b"committed prefix");
    main_assert!(!destination.join("Remove.txt").exists());
    main_assert_eq!(fs::read(destination.join("Untouched.dat")).unwrap() => b"keep me");
    main_assert!(!destination.join("After.txt").exists());
    let child_path = destination.join("SectNight.c4g");
    main_assert!(child_path.is_file(), "folder children stay packed files");
    let child = Group::open(&child_path).test_value();
    main_assert_eq!(child.maker() => Some("Folder maker"));
    main_assert_eq!(child.read_file("Scenario.txt").unwrap() => b"[Head]\nTitle=Night\n");
    main_assert!(fs::read_dir(directory.path()).unwrap().all(|entry| !entry.unwrap().file_name().to_string_lossy().contains("lc-rewrite")));
}
...
fn network_set_pre_send_applies_matching_local_target_fps_and_flash_without_fatal_boundary() {
    ...
    main_assert_eq!(app.netplay.control_clock.expect("network clock").target_fps() => 38);
    main_assert_eq!(app.runtime_flash_message => unchanged_flash);

    app.engine
        .call_scenario_script_function("Match", Vec::new())
        .test_value();
    app.apply_engine_network_target_fps_requests().test_value();
    let clock = app.netplay.control_clock.test_value();
    main_assert_eq!(clock.target_fps() => 76);
    main_assert_eq!(clock.control_presend() => 1, "setter does not recalculate inline");
    let flash = app.runtime_flash_message.test_ref();
    main_assert_eq!(flash.text => "TargetFPS: 76");
    main_assert_eq!(flash.remaining_draws => 26);
    main_assert_eq!(flash.y => app.runtime_flash_y());
    ...
    app.apply_engine_network_target_fps_requests().test_value();
    main_assert_eq!(app.netplay.control_clock.expect("network clock").target_fps() => 38);
    main_assert_eq!(runtime_flash_text(&app) => Some("TargetFPS: 38"));
}
...
fn runtime_client_pause_and_go_drive_to_targets_before_acknowledging() {
    ...
    let mut app = new_running_sandbox_app();
    main_assert_eq!(app.engine.frame() => 0, "fixture starts on a cadence boundary");
    let (events, mut commands) = install_running_network_stub(&mut app, 7, 0, 2);
    let pause = n2_fixture!(status: clonk_network::NETWORK_STATE_PAUSE, 1, 2);

    n2_send_event(&events, NetworkEvent::StatusRequested(pause));
    app.test_network_events();
    main_assert!(app.netplay.control_running, "receipt alone must not halt control");
    main_assert_eq!(app.netplay.pending_client_start_status => None);
    main_assert!(commands.take_framed_status_acknowledgements().is_empty());

    queue_empty_ready_tick(&app, &events);
    app.test_update();
    main_assert_eq!((app.engine.frame(), app.expected_network_control_tick()) => (1, 1));
    app.test_update();
    queue_empty_ready_tick(&app, &events);
    app.test_update();
    main_assert_eq!((app.engine.frame(), app.expected_network_control_tick()) => (3, 2));
    main_assert!(app.netplay.control_running, "tick target is not a cadence boundary yet");
    app.test_update();
    app.test_update();

    main_assert_eq!((app.engine.frame(), app.expected_network_control_tick()) => (4, 2));
    main_assert!(!app.netplay.control_running);
    main_assert_eq!(commands.take_framed_status_acknowledgements() => vec![(pause, 4)]);
    app.test_update();
    ...
    app.test_network_events();
    main_assert!(app.netplay.control_running, "Go drives out of committed Pause");
    ...
    app.test_update();
    app.test_update();
    main_assert_eq!((app.engine.frame(), app.expected_network_control_tick()) => (8, 4));
    main_assert!(!app.netplay.control_running, "Go also waits halted for commit");
    main_assert_eq!(commands.take_framed_status_acknowledgements() => vec![(go, 8)]);

    n2_send_event(&events, NetworkEvent::StatusCommitted(go));
    app.test_network_events();
    main_assert!(app.netplay.control_running);
    queue_empty_ready_tick(&app, &events);
    app.test_update();
    main_assert_eq!((app.engine.frame(), app.expected_network_control_tick()) => (9, 5));
}
...
fn ordinary_lobby_join_go_waits_for_the_current_ready_tick_before_acknowledging() {
    ...
    app.netplay.client_start_barrier = ClientStartBarrier::from_join_data_status(lobby);
    main_assert_eq!(app.netplay.client_start_barrier.status_requested(go) => Some(go));
...
fn a_status_without_a_runtime_reach_condition_supersedes_the_pending_barrier() {
    ...
    n2_send_event(&events, NetworkEvent::StatusRequested(pause));
    app.test_network_events();
    main_assert_eq!(app.netplay.runtime_status_barrier.map(|pending| pending.status) => Some(pause));

    let lobby = n2_fixture!(status: clonk_network::NETWORK_STATE_LOBBY, 1, -1);
    n2_send_event(&events, NetworkEvent::StatusRequested(lobby));
    app.test_network_events();
    main_assert_eq!(app.netplay.runtime_status_barrier => None);

    for _ in 0..4 {
        main_assert_eq!(app.check_runtime_network_status_reached() => RuntimeStatusReachOutcome::NotReached);
    }
    main_assert!(commands.take_runtime_status_commands().is_empty());
}
...
fn runtime_client_adopts_a_recorded_status_its_barrier_never_applied() {
    ...
    main_assert_eq!(app.check_runtime_network_status_reached() => RuntimeStatusReachOutcome::NotReached);
    main_assert_eq!(app.netplay.runtime_status_barrier.map(|pending| pending.status) => Some(retargeted));
    main_assert!(app.netplay.control_running, "the adopted target is still ahead");
    main_assert!(commands.take_framed_status_acknowledgements().is_empty());
    ...
    main_assert_eq!((app.engine.frame(), app.expected_network_control_tick()) => (8, 4));
    main_assert!(!app.netplay.control_running);
    main_assert_eq!(commands.take_framed_status_acknowledgements() => vec![(retargeted, 8)]);
    app.test_update();
    app.sec1_timer().test_value();
    main_assert!(commands.take_framed_status_acknowledgements().is_empty(), "the adopted barrier is acknowledged once");
}
...
fn runtime_client_barrier_follows_an_acknowledgement_already_on_the_wire() {
    ...
    let mut app = new_running_sandbox_app();
    let (events, mut commands) = install_running_network_stub(&mut app, 7, 0, 2);
    let pause = n2_fixture!(status: clonk_network::NETWORK_STATE_PAUSE, 1, 0);
    n2_send_event(&events, NetworkEvent::StatusRequested(pause));
    app.test_network_events();
    app.test_update();
    main_assert_eq!(commands.take_framed_status_acknowledgements() => vec![(pause, 0)]);
    ...
    main_assert_eq!(app.check_runtime_network_status_reached() => RuntimeStatusReachOutcome::NotReached);
    main_assert_eq!(app.netplay.runtime_status_barrier => Some(RuntimeNetworkStatusBarrier {status: pause, local_reached: true, actual_control_tick: Some(0),}));
    main_assert!(!app.netplay.control_running);
    main_assert!(commands.take_framed_status_acknowledgements().is_empty());
...
fn runtime_host_commit_executes_sync_at_the_actual_overshoot_tick() {
    ...
    n2_send_event(&events, NetworkEvent::StatusRequested(pause));
    for tick in 0..=2 {
        n2_send_event(&events, n2_fixture!(ready_tick: tick, Vec::new()));
    }
    ...
    main_assert_eq!(status_commands => vec![network::TestRuntimeStatusCommand::Reached {status: pause, actual_control_tick: 3,}]);

    n2_send_event(
        &events,
        n2_fixture!(scheduled_sync:
            3,
            vec![NetworkControl::Message(message_control(
                MESSAGE_TYPE_NORMAL,
                -1,
                -1,
                b"overshoot-sync",
...
fn runtime_host_pause_retarget_and_go_report_each_local_reach_once() {
    ...
    let pause = *pause;
    main_assert_eq!(pause.state => clonk_network::NETWORK_STATE_PAUSE);
    main_assert_eq!(pause.target_tick => 1);
    main_assert!(app.netplay.control_running);

    app.test_update();
    app.test_update();
    main_assert_eq!((app.engine.frame(), app.expected_network_control_tick()) => (2, 1));
    main_assert!(!app.netplay.control_running);
    main_assert_eq!(commands.take_runtime_status_commands() => vec![network::TestRuntimeStatusCommand::Reached {status: pause, actual_control_tick: 1,}]);
    app.sec1_timer().test_value();
    main_assert!(commands.take_runtime_status_commands().is_empty(), "local reach is sent once");

    let retargeted = clonk_network::NetworkStatus {
        target_tick: 3,
        ..pause
    };
    n2_send_event(&events, NetworkEvent::StatusRequested(retargeted));
    app.test_network_events();
    main_assert!(app.netplay.control_running);
    main_assert!(commands.take_runtime_status_commands().is_empty());
    queue_empty_ready_tick(&app, &events);
    ...
    main_assert_eq!((app.engine.frame(), app.expected_network_control_tick()) => (6, 3));
    main_assert!(!app.netplay.control_running);
    main_assert_eq!(commands.take_runtime_status_commands() => vec![network::TestRuntimeStatusCommand::Reached {status: retargeted, actual_control_tick: 3,}]);

    n2_send_event(&events, NetworkEvent::StatusCommitted(retargeted));
    app.test_network_events();
    main_assert!(!app.netplay.control_running);
    ...
    let go = *go;
    main_assert_eq!(*reached_go => go);
    main_assert_eq!(*actual_control_tick => 3);
    main_assert_eq!(go.state => clonk_network::NETWORK_STATE_GO);
    main_assert_eq!(go.target_tick => 3);
    main_assert_eq!((app.engine.frame(), app.expected_network_control_tick()) => (6, 3));
    main_assert!(!app.netplay.control_running);
    main_assert!(app.netplay.ticks.ready.contains_key(&3));
    n2_send_event(&events, NetworkEvent::StatusCommitted(go));
    app.test_network_events();
    main_assert!(app.netplay.control_running);
    app.test_update();
    main_assert_eq!((app.engine.frame(), app.expected_network_control_tick()) => (7, 4));
    main_assert!(!app.netplay.ticks.ready.contains_key(&3));
}
...
fn message_board_change_mode_and_execute_match_native_faders() {
    ...
    main_assert!(board.change_mode(MessageBoardMode::SingleLine, line_height));
    let first_frame = board.advance_frame(line_height, false);
    main_assert_eq!(first_frame.screen_fader => 90);
    main_assert_eq!(board.screen_fader => 95);
    main_assert!(board.empty);

    board.enqueue("first".to_string());
    board.enqueue("second".to_string());
    let _ = board.advance_frame(line_height, false);
    main_assert_eq!(board.fader => line_height - 1);
    main_assert_eq!(board.current_line().as_deref() => Some("second"));
    main_assert_eq!(board.screen_fader => 75);

    board.line_count = 7;
    main_assert!(board.change_mode(MessageBoardMode::Continuous, line_height));
    main_assert_eq!(board.mode => MessageBoardMode::Continuous);
    main_assert_eq!(board.line_count => 7);
    main_assert_eq!(board.back_scroll => -1);
    main_assert_eq!(board.fader => 0);
    let unchanged = board.clone();
    let _ = board.advance_frame(line_height, false);
    main_assert_eq!(board.mode => unchanged.mode);
    main_assert_eq!(board.screen_fader => unchanged.screen_fader);

    main_assert!(!board.change_mode(MessageBoardMode::Hidden, line_height));
    let _ = board.advance_frame(line_height, false);
    main_assert_eq!(board.screen_fader => 100);
    main_assert_eq!(board.back_scroll => -1);
    main_assert!(board.change_mode(MessageBoardMode::SingleLine, line_height));
    main_assert!(board.empty, "hidden-to-single keeps the native empty transition");
}
...
fn msgboard_command_uses_runtime_lines_but_persists_only_a_bool() {
    ...
    main_assert_eq!(overlay.mode => MessageBoardMode::Continuous);
    main_assert!(overlay.log_lines.ends_with(&["one".to_string(), "two".to_string(), "three".to_string(), "four".to_string(),]));
    ...
    main_assert_eq!(reloaded.mode => MessageBoardMode::SingleLine, "the bool-typed config cannot retain the runtime line count");
    ...
    main_assert_eq!(hidden_reloaded.mode => MessageBoardMode::Hidden);
...
fn running_set_comment_is_direct_host_effect() {
    ...
    app.flush_deferred_config();
    main_assert_eq!(Config::load(paths.config_file()).expect("load updated runtime config").get_in(Some("Network"), "Comment") => Some("live runtime comment"));
    main_assert_eq!(app.netplay.advertised_game_reference.as_ref().expect("updated running reference").metadata().comment.as_bytes() => b"live runtime comment");
    main_assert_eq!(latest_message_board_logical_entry(&app).as_deref() => Some(clonk_frontend::game_option_buttons::COMMENT_CHANGED_LOG),);
    main_assert!(commands.take_submitted_decided_controls().is_empty());
    ...
    main_assert!(message_board_logical_entries(&client).iter().all(|line| line != clonk_frontend::game_option_buttons::COMMENT_CHANGED_LOG));
}
...
fn running_network_client_cannot_set_maxplayer_with_stale_engine_host_state() {
    ...
    let mut app = new_state_only_running_sandbox_app();
    main_assert!(app.engine.is_control_host(), "fixture intentionally starts with stale local-host state");
    let (_events, mut commands) = install_running_network_stub(&mut app, 7, 0, 2);

    app.process_running_chat_text("/set maxplayer 23");

    main_assert!(commands.take_submitted_decided_controls().is_empty(), "an actual network client must not synthesize a host-authored Set");
}
...
fn running_dispatches_controls_modes_and_custom_commands() {
    ...
    main_assert_eq!(decided.len() => 1);
    main_assert_eq!(decided[0].0 => app.local_control_submission_tick());
    main_assert!(!decided[0].2, "an active running host queues CDT_Decide");
    main_assert_eq!(clonk_network::LegacyControlSet::from_control_packet(&decided[0].1) => Some(n2_fixture!(control_set: 1, 0, 0)));
    app.process_running_chat_text("/set maxplayer 9");
    app.process_running_chat_text("/set faircrew off");
    let decided = commands.take_submitted_decided_controls();
    main_assert_eq!(decided.len() => 2);
    main_assert!(decided.iter().all(|(_, _, sync)| !sync));
    main_assert_eq!(
        decided
            .iter()
            ...
            .collect::<Vec<_>>() =>
        vec![
            n2_fixture!(control_set: 2, 9, 0),
            n2_fixture!(control_set: 5, -1, 0),
        ]
    );
    ...
    main_assert_eq!(
        ...
        vec![
            n2_fixture!(client_update: clonk_engine::CLIENT_UPDATE_ACTIVATE, 7, 1, 0),
            n2_fixture!(client_update: clonk_engine::CLIENT_UPDATE_ACTIVATE, 7, 0, 0),
            n2_fixture!(client_update: clonk_engine::CLIENT_UPDATE_SET_OBSERVER, 7, 0, 0),
        ]
    );
    ...
    main_assert_eq!(*tick => app.local_control_submission_tick());
    main_assert_eq!(script.target_object => clonk_engine::SCRIPT_SCOPE_CONSOLE);
    main_assert_eq!(script.script.as_bytes() => b"return 1");

    main_assert!(app.engine.add_message_board_command(
    ...
    main_assert_eq!(*tick => app.local_control_submission_tick());
    main_assert_eq!(custom.command.as_bytes() => b"probe");
    main_assert_eq!(custom.argument.as_bytes() => b" exact tail");
    ...
    main_assert!(commands.take_runtime_status_commands().is_empty());
    let (_snapshot, reference) = default_exact_host_reference();
    main_assert_eq!(reference.summary().control_mode => 0);
    app.netplay.advertised_game_reference = Some(reference);
    app.process_running_chat_text("/centralctrl");
    main_assert_eq!(
        commands.take_runtime_status_commands() =>
        vec![
            network::TestRuntimeStatusCommand::Change(
                n2_fixture!(status: clonk_network::NETWORK_STATE_GO, 1, 0)
            ),
            network::TestRuntimeStatusCommand::Reached {
                status: n2_fixture!(status: clonk_network::NETWORK_STATE_GO, 1, 0),
                actual_control_tick: 0,
            },
        ]
    );
    main_assert_eq!(app.netplay.advertised_game_reference.as_ref().expect("running reference survives control-mode publication").summary().control_mode => 1);
    app.netplay.is_league = true;
...
fn network_chart_tracks_running_network_sandbox_and_toggles_as_singleton() {
    let mut app = new_running_sandbox_app();
    main_assert!(app.netplay.stats.is_some(), "running initialization owns live stats");
    ...
    app.engine
        ...
        .test_value();
    app.snapshot = app.engine.snapshot();
    main_assert!(app.snapshot.objects.iter().any(|object| object.status == clonk_engine::ObjectStatus::Inactive));
    let expected_object_count = app
        ...
        .count() as f32;
    let object_sample_time = app.netplay.stats.test_ref().object_count_graph().end_time();
    ...
    let second_counter = app.netplay.stats.test_ref().second_counter();
    app.mode = AppMode::Loading;
    ...
    main_assert!(object_graph.end_time() - object_graph.start_time() >= 3);
    ...
    main_assert!(app.netplay.stats.is_some(), "closing presentation retains sampling");

    app.return_to_menu_for_relaunch();
    main_assert!(app.netplay.stats.is_none(), "game teardown drops live stats");
    main_assert!(app.dialogs.chart.is_none());
}
...
fn chart_toggle_respects_reachable_native_key_priorities() {
    ...
    player_escape
        .local_controls
        .remove(player_escape.players.local_owner);
    player_escape.local_controls.initialize(LocalControlInit {
        owner: player_escape.players.local_owner,
        preferred_set: 1,
        prefers_mouse: false,
    ...
    main_assert!(player_escape.bindings.rebind_for_set(1, ControlBindingId::Left, VirtualKeyCode::Escape,));
...
fn control_mode_targets_native_current_tick_after_cadence_consumption() {
    ...
    let expected = n2_fixture!(status: clonk_network::NETWORK_STATE_GO, 1, 0);
    main_assert_eq!(commands.take_runtime_status_commands() => vec![network::TestRuntimeStatusCommand::Change(expected)]);
}

#[test]
fn frozen_runtime_sync_executes_immediately_but_pending_barrier_queues() {
    let disable_debug = || NetworkControl::Set(n2_fixture!(control_set: 1, 0, 0));
    ...
    frozen.netplay.host_reference_paused = true;
    frozen.netplay.control_running = false;
    n2_send_event(
        &events,
        n2_fixture!(scheduled_sync: 0, vec![disable_debug()]),
    );

    frozen.test_network_events();
    main_assert!(!frozen.engine.debug_mode());
    main_assert!(frozen.netplay.sync.scheduled.is_empty());
    ...
    transitioning.netplay.host_reference_paused = true;
    transitioning.netplay.control_running = false;
    transitioning.netplay.runtime_status_barrier = Some(RuntimeNetworkStatusBarrier {
        status: n2_fixture!(status: clonk_network::NETWORK_STATE_GO, 0, 0),
        local_reached: true,
    ...
    main_assert!(transitioning.engine.debug_mode());
    main_assert!(transitioning.netplay.sync.scheduled.contains_key(&0));
}
...
fn message_control_sound_uses_global_cooldown_before_per_client_mute() {
    ...
    let first = app.execute_message_control_with_sound_at(
        ...
        |_, name| {
            attempts += 1;
            main_assert_eq!(name => "Ping");
            true
        },
    );
    main_assert!(first.sound_attempted && first.sound_played);
    ...
    main_assert!(!other_sender.sound_attempted);
    ...
    main_assert!(!muted.sound_attempted);
    ...
    main_assert!(!consumed.sound_attempted);
...
fn message_control_host_system_and_inactive_attention_match_cpp() {
    ...
    main_assert!(!app.execute_message_control(message_control(MESSAGE_TYPE_SYSTEM, -1, -1, b"forged", 7,)).displayed);
    main_assert!(app.execute_message_control(message_control(MESSAGE_TYPE_SYSTEM, -1, -1, b"host notice", 0,)).displayed);
    main_assert!(!app.take_user_attention_request());

    let alert = app.execute_message_control(message_control(MESSAGE_TYPE_ALERT, -1, -1, b"", 7));
    main_assert!(alert.attention_requested);
    main_assert!(app.take_user_attention_request());

    let mention =
        app.execute_message_control(message_control(MESSAGE_TYPE_NORMAL, 7, -1, b"hi aLi!", 7));
    main_assert!(mention.attention_requested);
    main_assert!(app.take_user_attention_request());
    ...
    main_assert!(!rejected_first_match.attention_requested);
    main_assert!(!app.take_user_attention_request());
}
...
fn portrait_crew_label_decodes_native_info_name_for_presentation() {
    ...
    main_assert!(app.engine.definition_portrait_graphics_image(&definition_id).is_some(), "fixture definition must have a portrait so current=None is observable");
    ...
    let overlay = players
        ...
        .test_value();
    main_assert_eq!(overlay.info_name.as_deref() => Some("Ren\u{e9}"));
    main_assert_eq!(overlay.label => "Ren\u{e9}");
    main_assert!(
    ...
    main_assert_eq!(clonk_script::c4_string_bytes(&app.engine.crew_object_info(crew).expect("crew info remains live").name) => b"Ren\xe9");
}
...
fn viewport_selection_preserves_authoritative_local_order_slots_and_elimination() {
    ...
    main_assert_eq!(viewports.len() => 3);
    main_assert_eq!(
        viewports
            .iter()
            .map(|viewport| (
    ...
    main_assert_eq!(collect_viewport_inputs(&snapshot).expect("live cursor supersedes a deleted slot focus")[0].focus.expect("cursor focus").id => focus);
    inherited.cursor = None;
    inherited.crew = vec![focus];
    snapshot.players = vec![inherited];
    main_assert_eq!(collect_viewport_inputs(&snapshot).expect("first crew member supplies the local slot focus")[0].focus.expect("crew focus").id => focus);
    ...
    main_assert_eq!(eliminated_views.len() => 1);
    main_assert_eq!(eliminated_views[0].owner => local_owner);
    main_assert_eq!(eliminated_views[0].center => Vector2::new(123, 456));
    main_assert_eq!(eliminated_views[0].zoom => 1.75);
...
fn viewport_selection_uses_cpp_keyboard_layout_order_across_joins() {
    ...
    main_assert_eq!(initial => vec![keyboard1.id, keyboard2.id]);
    ...
    main_assert_eq!(joined => vec![keyboard1.id, keyboard3.id, keyboard2.id]);
    main_assert_eq!(joined.iter().copied().filter(|owner| initial.contains(owner)).collect::<Vec<_>>() => initial,);
    ...
    for _ in 0..8 {
        main_assert_eq!(viewport_owners(&snapshot) => all_keyboards);
    }
    snapshot.players.reverse();
    snapshot.hud.local_players.reverse();
    main_assert_eq!(viewport_owners(&snapshot) => all_keyboards, "unique control layouts make slot assignment independent of source order",);
}
...
fn pending_preflight_retains_the_exact_network_tick() {
    ...
    main_assert!(gate.take_exact_if_ready(tick, |_| false).is_none());
    main_assert_eq!(gate.take_exact_if_ready(tick, |_| true) => Some(controls), "the retained tick executes once its preflight becomes ready");
}
...
fn client_join_data_replaces_authoritative_control_registries() {
    ...
    let mut app = new_state_only_menu_app(320, 200);
    app.netplay.control_clients.register(99, true, false);
    app.netplay.client_activity.mark_activated(99, 123);
    app.players.infos.apply(netplay_player_info_data(
        99,
        vec![n2_fixture!(player { id: 3 })],
    ));
    ...
    snapshot.parameters.player_infos = clonk_network::PlayerInfoListSnapshot {
        ...
        clients: vec![clonk_network::ClientPlayerInfosSnapshot {
            client_id: 0,
            flags: 0,
            players: vec![n2_fixture!(player { id: 12 })],
        }],
    };
    let join_data = n2_fixture!(join_data:
        7, 23, host_config.initial_status, snapshot.dynamic, snapshot.parameters
    );
    n2_send_event(&event_tx, NetworkEvent::JoinData(join_data.clone()));

    app.test_network_events();

    main_assert_eq!(app.netplay.pending_join_data => Some(join_data));
    main_assert!(!app.netplay.control_clients.contains(99));
    main_assert_eq!(app.netplay.client_activity.last_frame => BTreeMap::from([(0, 0), (7, 0)]));
    let host = app.netplay.control_clients.state(0).test_value();
    main_assert!(host.activated);
    main_assert!(host.lobby_ready);
    let local = app.netplay.control_clients.state(7).test_value();
    main_assert_eq!(local.name.as_bytes() => b"Joining client");
    main_assert_eq!(local.nick.as_bytes() => b"Joiner");
    main_assert!(app.players.infos.get(3).is_none());
    main_assert!(app.players.infos.get(12).is_some());
    let admitted = app
        .players.infos
        .admit_request(
        ...
        .test_value();
    main_assert_eq!(admitted.players[0].id => 41);
}
...
fn synchronized_player_derivation_registers_the_latest_core_before_event_drain() {
    ...
    main_assert_eq!(resources.derivation_target(root.id) => Some(second.id));
    main_assert_eq!(resources.status(second.id) => Some(&AdmissionResourceState::Complete {path: PathBuf::from("Alice.c4p"), removed: false, local: true,}));
    ...
    main_assert_eq!(resources.derivation_target(remote.id) => Some(remote_derived.id));
    main_assert_eq!(
...
fn main_menu_player_join_uses_active_network_max_players() {
    ...
    main_assert_eq!(conditions.max_players => 1);
...
fn offline_runtime_join_player_local_no_network() {
    ...
    let mut app = new_synthetic_running_sandbox_app();
    let player_file = n2_install_startup_player(&mut app, player_path);
    main_assert!(app.netplay.manager.is_none());
    main_assert!(app.netplay.mode.is_none());
    let before_players = app.engine.snapshot().players.len();
    let before_info_ids = app.players.infos.client_info_ids(0);
    ...
    let info = app.players.infos.get(*joined_info_id).test_value();
    main_assert_eq!(info.filename.as_bytes() => clonk_script::c4_string_bytes(player_path.to_string_lossy().as_ref()));
    main_assert_eq!(info.name.as_bytes() => clonk_script::c4_string_bytes(&player_file.name));
    main_assert_eq!((info.color, info.original_color) => (player_file.normalized_preferred_color(), player_file.normalized_preferred_color(),));
    main_assert_eq!(info.resource => None);
    main_assert_eq!(info.flags & (clonk_engine::PLAYER_INFO_FLAG_HAS_RESOURCE | clonk_engine::PLAYER_INFO_FLAG_JOINED) => clonk_engine::PLAYER_INFO_FLAG_JOINED);

    let snapshot = app.engine.snapshot();
    main_assert_eq!(snapshot.players.len() => before_players + 1);
    let joined = snapshot
        .players
        .iter()
    ...
    main_assert!(recorded_controls.iter().any(|packet| {
        matches!(
            packet,
    ...
    for path in [&malformed, &missing] {
        ...
        let before_failure_infos = app.players.infos.retained_rows_snapshot();
        ...
        main_assert_eq!(app.players.infos.retained_rows_snapshot() => before_failure_infos, "{path:?} admits nothing");
    }
}
...
fn retired_local_player_releases_profile_and_preferred_controls_for_runtime_rejoin() {
    ...
    let mut app = new_synthetic_running_sandbox_app();
    n2_install_startup_player(&mut app, player_path);
    let before_info_ids = app.players.infos.client_info_ids(0);
    ...
    main_assert!(app.engine.execute_eliminate_player_control(&n2_fixture!(eliminate: retired_owner, 0)).expect("execute host elimination"));
    for _ in 0..60 {
        app.test_update();
    }

    main_assert!(app.engine.player(retired_owner).is_none());
    let retired_info = app.players.infos.get(retired_info_id).test_value();
    main_assert_ne!(retired_info.flags & clonk_engine::PLAYER_INFO_FLAG_REMOVED => 0, "automatic retirement must release the profile from FileInUse");
    main_assert!(app.available_runtime_player_files().iter().any(|entry| { entry.file == player_path.to_string_lossy() }));
    let info_ids_before_rejoin = app.players.infos.client_info_ids(0);
    ...
    main_assert_eq!(rejoined_control.set => original_control.set);
    main_assert_eq!(rejoined_control.mouse => original_control.mouse);
}
...
fn active_network_client_runtime_join_publishes_before_add_request() {
    ...
    let mut app = new_running_sandbox_app();
    app.players.local_name = "Exact maker".to_string();
    ...
    app.netplay.mode = Some(NetworkMode::Client(client_settings));
    app.netplay.control_clients.register(7, true, false);
    ...
    n2_join_runtime_player(&mut app, &player_path);
    main_assert_eq!(app.engine.snapshot().players => before_players, "the client waits for the host's synchronized player-info echo before mutating players");
    main_assert_eq!(
        app.netplay.admission_resources.complete_path(expected_resource.id) =>
    ...
    main_assert_eq!(publications[0].wire_name => wire_name.clone());
    main_assert_eq!(publications[0].group_maker.as_bytes() => b"Exact maker");
    main_assert_eq!(player_infos.len() => 1);
    main_assert_eq!(player_infos[0].client_id => 7);
    main_assert_eq!(player_infos[0].flags => clonk_engine::CLIENT_PLAYER_INFO_FLAG_ADD_PLAYERS);
    let player = &player_infos[0].players[0];
    main_assert_eq!(player.id => 0);
    main_assert_eq!(player.name.as_bytes() => b"Runtime");
    main_assert_eq!(player.filename => wire_name);
    main_assert_eq!(player.color => 0x65_43_21);
    main_assert_eq!(player.original_color => 0x65_43_21);
    main_assert_eq!(player.resource => Some(expected_resource));
    main_assert!(acknowledgements.is_empty());
}
...
fn active_network_client_directory_runtime_join_uses_published_standalone_for_local_admission() {
    ...
    n2_join_runtime_player(&mut app, &player_path);
    main_assert_eq!(
        app.netplay.admission_resources.complete_path(resource.id) =>
...
fn active_network_host_directory_runtime_join_uses_published_standalone_for_local_admission() {
    ...
    app.players.local_name = "Exact host maker".to_string();
    app.netplay.control_clients.register(0, true, false);
    app.players.infos.replace_snapshot(40, []);
...
fn deactivated_network_client_can_request_runtime_player_join() {
    ...
    app.netplay.manager = Some(manager);
    app.netplay.mode = Some(NetworkMode::Client(n2_client_settings()));
    app.netplay.control_clients.register(7, false, false);
...
fn active_network_host_runtime_join_publishes_admits_and_queues_join() {
    ...
    main_assert_eq!(order => vec!["publish", "player-info", "join-player"]);
    main_assert_eq!(publications.len() => 1);
    main_assert_eq!(publications[0].source_path => player_path);
    main_assert_eq!(publications[0].wire_name => wire_name.clone());
    main_assert_eq!(publications[0].group_maker.as_bytes() => b"Exact host maker");
    let [info] = player_infos.as_slice() else {
        panic!("expected one authoritative PlayerInfo");
    };
    main_assert_eq!((info.client_id, info.by_client) => (0, 0));
    main_assert_eq!(info.flags => clonk_engine::CLIENT_PLAYER_INFO_FLAG_ADD_PLAYERS);
    let [player] = info.players.as_slice() else {
        panic!("expected one admitted player");
    };
    main_assert_eq!(player.id => 41);
    main_assert_eq!(player.name.as_bytes() => b"Host Runtime");
    main_assert_eq!(player.resource.as_ref() => Some(&expected_resource));
    main_assert_eq!(joins.len() => 1);
    main_assert_eq!(joins[0].0 => tick);
    main_assert_eq!(joins[0].1.at_client => 0);
    main_assert_eq!(joins[0].1.info_id => 41);
    main_assert_eq!(joins[0].1.filename.as_bytes() => player_path.as_os_str().as_encoded_bytes());
    main_assert_eq!(joins[0].1.source => clonk_engine::JoinPlayerSource::Resource(expected_resource));
}
...
fn inactive_remote_client_readds_retired_profile_after_activation_go() {
    ...
    let resource = n2_fixture!(resource {
        resource_type: clonk_network::HostResourceType::Player as u8,
        id: 7 << 16,
        loadable: true,
    });
    ...
    main_assert!(app.engine.execute_eliminate_player_control(&n2_fixture!(eliminate: retiring, 0)).expect("execute remote player elimination"));
    let players_before_tick = app.player_info_ids_by_player();
    for _ in 0..60 {
        app.snapshot = app.engine.test_tick();
    }
    ...
    main_assert_eq!((info.client_id, info.by_client) => (7, 0));
    main_assert!(join_players_on_echo.is_empty());
    let [player] = info.players.as_slice() else {
        panic!("expected the remote client's retained player row");
    };
    main_assert_eq!(player.id => 41);
    main_assert_ne!(player.flags & clonk_engine::PLAYER_INFO_FLAG_REMOVED => 0, "the remote client must receive the released profile before rejoining");
    app.netplay.control_clients.register(7, false, false);
    let expected_resource = resource.clone();

    n2_send_event(
        &event_tx,
        n2_fixture!(player_info_event:
            7,
            n2_fixture!(player_update:
                7,
    ...
    main_assert_eq!((info.client_id, player.id) => (7, 42));
    main_assert!(join_players_on_echo.is_empty(), "the inactive client must wait for synchronized activation");

    let frame = i32::try_from(app.engine.frame()).test_value();
    n2_send_event(
        &event_tx,
        n2_fixture!(activation_request: 7, frame, true, 0),
    );
    app.test_network_events();
    ...
    main_assert_eq!(activation => n2_fixture!(client_update: clonk_engine::CLIENT_UPDATE_ACTIVATE, 7, 1, 0));
    let tick = app.local_control_submission_tick();
    app.netplay.control_running = false;
    n2_send_event(
        &event_tx,
        n2_fixture!(scheduled_sync: tick, vec![NetworkControl::ClientUpdate(activation)]),
    );
    app.test_network_events();
    main_assert!(!app.netplay.control_clients.is_activated(7));
    main_assert!(app.netplay.sync.scheduled.contains_key(&tick));
    main_assert!(commands.take_submitted_join_players().is_empty());

    n2_send_event(
        &event_tx,
        NetworkEvent::StatusCommitted(
            n2_fixture!(status: clonk_network::NETWORK_STATE_GO, 1, i32::try_from(tick).test_value()),
        ),
    );
    app.test_network_events();
    ...
    main_assert_eq!(*join_tick => tick);
    main_assert_eq!((join.at_client, join.info_id) => (7, 42));
    main_assert_eq!(join.by_client => 0);
    main_assert_eq!(join.source => clonk_engine::JoinPlayerSource::Resource(expected_resource));
}
...
fn active_network_host_readmits_its_own_retired_profile_at_runtime() {
    ...
    let N2RetiredHostProfileFixture {
        _directory,
        player_path,
        mut app,
        resource,
        events: event_tx,
        commands,
    } = n2_retired_host_profile_fixture(None);
    let retired = app.players.infos.get(41).test_value();
    main_assert_ne!(retired.flags & clonk_engine::PLAYER_INFO_FLAG_REMOVED => 0, "retirement releases the profile before the host readmits it");
    ...
    main_assert!(!app.status_text.starts_with("Unable to join player"), "the readmitted profile must not be refused: {}", app.status_text);
    ...
    main_assert_eq!(player.id => 42, "the readmitted profile gets a fresh ID");
    main_assert_eq!(joins.len() => 1);
    main_assert_eq!(joins[0].1.info_id => 42);
    ...
    main_assert_ne!(
        app.players.infos.get(42).test_value().flags
    ...
    n2_join_runtime_player(&mut app, &player_path);
    let broadcast = direct_wait.recv_timeout(Duration::from_secs(1));
    main_assert!(!app.status_text.starts_with("Unable to join player"), "the profile's second readmission must not be refused: {}", app.status_text);
    broadcast.test_value();
    app.test_network_events();
    drop(app.netplay.manager.take());
    ...
    main_assert_eq!(player.id => 43, "the second rejoin gets another fresh ID");
    main_assert_eq!(joins.len() => 1);
    main_assert_eq!(joins[0].1.info_id => 43);
}
...
fn rejoin_after_elimination_policy_reads_its_network_config_key() {
    ...
    main_assert!(app.rejoin_after_elimination_allowed());
    ...
    app.netplay.rejoin_after_elimination_allowed = Some(false);
    main_assert!(!app.rejoin_after_elimination_allowed());
}
...
fn active_network_host_barring_rejoins_refuses_its_own_retired_profile() {
    ...
    let N2RetiredHostProfileFixture {
        ...
        events: event_tx,
        commands,
    } = n2_retired_host_profile_fixture(Some(false));
    ...
    n2_join_runtime_player(&mut app, &player_path);
    main_assert!(app.status_text.starts_with("Unable to join player"), "the eliminated profile must be refused: {}", app.status_text);
    drop(app.netplay.manager.take());

    // The resource is published before admission runs, as in C++, but nothing
    // is broadcast and no player is queued to join.
    let (order, _publications, player_infos, joins) = command_observer.test_join();
    main_assert_eq!(order => vec!["publish"]);
    main_assert!(player_infos.is_empty());
    main_assert!(joins.is_empty());
    main_assert_eq!(app.players.infos.player_count() => 1, "only the retired history row remains");
}
...
fn active_network_host_runtime_join_assigns_team_before_broadcast() {
    ...
    app.players.team_assignment = Some(NetworkTeamAssignmentState::from_prepared_host(
        n2_team_metadata(
            ...
            false,
            vec![
                n2_team(1, vec![1], 0x00f4_0000),
                n2_team(2, Vec::new(), 0x0000_c800),
            ],
        ),
    ));
    let (manager, event_tx, commands) = NetworkManager::test_stub_with_commands();
    ...
    main_assert_eq!(player.original_color => 0x0012_3456);
    main_assert_ne!(player.color => player.original_color, "native skips the alternate while current still equals original and enters its process-random fallback");
    main_assert_eq!(app.players.host_local_alternate_colors.get(&17) => Some(&0x0065_4321), "a successful local runtime join extends the persistent sidecar");
    main_assert!(app.players.host_local_info_ids.contains(&2));
    let teams = app.players.team_assignment.test_mut().teams_mut();
...
fn client_join_data_submits_an_empty_initial_player_info_for_an_observer() {
    ...
    main_assert_eq!(commands.take_player_info_updates() => vec![n2_fixture!(player_update: 7, clonk_engine::CLIENT_PLAYER_INFO_FLAG_INITIAL, Vec::new())]);
}
...
fn startup_host_auth_players_stay_with_their_connection_and_cancel_cleanly() {
    ...
    main_assert!(first_commands.receive_league_player_auth().complete(Ok(
    ...
    app.show_main_menu();
    main_assert!(app.netplay.pending_league_player_auth.is_none());
    main_assert!(app.startup_network.connection.is_none());
...
fn client_rejects_a_combined_scenario_without_network_game_flag() {
    ...
    main_assert_eq!(validate_client_network_scenario(&scenario) => Err("retrieved scenario is not marked as a network game".to_string()));
}
...
fn host_player_info_request_queues_authoritative_direct_broadcast() {
    ...
    main_assert_eq!((info.client_id, info.by_client) => (3, 0));
    let [player] = info.players.as_slice() else {
        panic!("expected one admitted player");
    };
    main_assert_eq!(player.id => 1);
    main_assert!(app.players.infos.get(1).is_some());
}
...
fn same_client_add_echo_normalizes_and_issues_only_its_direct_snapshot() {
    ...
    app.players.infos.replace_snapshot(0, []);
    let resources = [
        n2_fixture!(player_resource: 17, clonk_engine::LegacyCString::from_bytes(b"First.c4p".to_vec()).test_value()),
        n2_fixture!(player_resource: 18, clonk_engine::LegacyCString::from_bytes(b"Second.c4p".to_vec()).test_value()),
    ];
    for resource in &resources {
        app.netplay.admission_resources.register_lobby_resource(resource);
        app.netplay.admission_resources.mark_complete(
            resource.id,
    ...
    main_assert_eq!(controls.len() => 2);
    main_assert_eq!(controls[0].1.iter().map(|player| player.id).collect::<Vec<_>>() => vec![1]);
    main_assert_eq!(controls[1].1.iter().map(|player| player.id).collect::<Vec<_>>() => vec![1, 2]);
    ...
    main_assert_eq!(first_joins.iter().map(|(_, join)| join.info_id).collect::<Vec<_>>() => vec![1]);
    main_assert_ne!(
        app.players.infos.get(1).unwrap().flags
            & clonk_engine::PLAYER_INFO_FLAG_HAS_RESOURCE =>
        0,
    ...
    main_assert_eq!(second_joins.iter().map(|(_, join)| join.info_id).collect::<Vec<_>>() => vec![2]);
    main_assert_ne!(app.players.infos.get(1).unwrap().flags & clonk_engine::PLAYER_INFO_FLAG_HAS_RESOURCE => 0);
}
...
fn stale_add_echo_joins_its_snapshot_without_normalizing_later_replacement() {
    ...
    app.netplay.control_clients.register(3, true, false);
    app.players.infos.replace_snapshot(0, []);
    let resource = |id, filename: &[u8]| n2_fixture!(player_resource: id, clonk_engine::LegacyCString::from_bytes(filename.to_vec()).test_value());
    ...
    for request in [
        ...
        n2_send_event(&event_tx, n2_fixture!(player_info_event: 3, request, false));
    }

    app.test_network_events();
    let controls = commands.take_preexecuted_player_infos();
    main_assert_eq!(controls.len() => 2);
    main_assert_eq!(controls[0].1[0].resource.as_ref() => Some(&first_resource));
    main_assert_eq!(controls[1].1[0].resource.as_ref() => Some(&replacement_resource));
    main_assert_eq!(app.players.infos.get(1).unwrap().resource.as_ref() => Some(&replacement_resource));
    ...
    app.test_network_events();
    let first_joins = commands.take_submitted_join_players();
    main_assert_eq!(first_joins.len() => 1);
    main_assert_eq!(first_joins[0].1.source => clonk_engine::JoinPlayerSource::Resource(first_resource));
    main_assert_eq!(
        app.players.infos.get(1).unwrap().resource.as_ref() =>
...
fn preexecuted_join_snapshot_is_client_scoped_for_duplicate_info_id() {
    ...
    app.broadcast_and_preexecute_player_info(
        n2_fixture!(player_data: 3, vec![target], by: 0),
        false,
        true,
    )
    .test_value();
    ...
    main_assert_eq!(captured.player_type => clonk_engine::PLAYER_INFO_TYPE_SCRIPT);
    main_assert_eq!(captured.resource.as_ref() => Some(&target_resource));
}
...
fn host_direct_remote_player_info_refreshes_join_data_for_later_consumers() {
    ...
    main_assert_eq!(authoritative.players[0].id => 1);
    let latest = published.last().test_value();
    main_assert_eq!(published.len() => 1);
    main_assert_eq!(latest.parameters.player_infos.last_player_id => 1);
    let [client] = latest.parameters.player_infos.clients.as_slice() else {
        panic!("runtime client row is retained in JoinData");
    };
    main_assert_eq!((client.client_id, client.flags) => (3, 0));
    let [player] = client.players.as_slice() else {
        panic!("runtime player row is retained in JoinData");
    };
    main_assert_eq!(player.id => 1);
    main_assert_eq!(player.name.as_bytes() => b"Runtime Remote");
    main_assert_eq!(published.last() => app.netplay.host_join_snapshot.as_ref());
}
...
fn host_synchronized_player_info_refreshes_join_data() {
    ...
    let latest = published.last().test_value();
    main_assert_eq!(latest.parameters.player_infos.last_player_id => 1);
    main_assert_eq!(latest.parameters.player_infos.clients[0].client_id => 4);
    main_assert_eq!(latest.parameters.player_infos.clients[0].players[0].id => 1);
    main_assert_eq!(published.last() => app.netplay.host_join_snapshot.as_ref());
}
...
fn client_direct_player_info_does_not_rebalance_random_teams_or_echo_updates() {
    ...
    n2_send_event(
        &event_tx,
        NetworkEvent::DirectControl(NetworkControl::PlayerInfo(n2_fixture!(player_data:
            3,
            vec![
                set_control_test_player(10, 1, 0),
                set_control_test_player(20, 1, 0),
    ...
    main_assert_eq!(teams.teams[0].player_ids => vec![10, 20, 30]);
    main_assert!(teams.teams[1].player_ids.is_empty());
    main_assert_eq!(app.players.infos.get(10).unwrap().team => 1);
...
fn host_authored_script_player_info_assigns_a_runtime_host_team() {
    ...
    main_assert_eq!((player.id, player.team) => (2, 2));
    main_assert_eq!(player.color => 0x0000_c800);
    let teams = app.players.team_assignment.test_mut().teams_mut();
    main_assert_eq!(teams.teams[0].player_ids => vec![1]);
    main_assert_eq!(teams.teams[1].player_ids => vec![2]);
}
...
fn running_host_direct_script_info_queues_one_synchronized_join() {
    ...
    main_assert_eq!(
        ...
        vec![(
            tick,
            n2_fixture!(join_player:
                clonk_engine::LegacyCString::default(),
                0,
                41,
                clonk_engine::JoinPlayerSource::Embedded(Vec::new()),
                0
            ),
        )]
    );
...
fn offline_create_script_player_joins_through_player_info_control_path() {
    let mut app = new_state_only_running_sandbox_app();
    let existing_player_info_id = app.engine.test_player(app.players.local_owner).player_info_id();
    main_assert!(existing_player_info_id > 0);
    app.engine
    ...
    app.engine
        ...
        .test_value();
    main_assert_eq!(app.engine.snapshot().players.len() => before, "CreateScriptPlayer must not join synchronously inside the VM call");
    ...
    let infos = app
        ...
        .collect::<Vec<_>>();
    main_assert_eq!(infos.len() => 1, "script PlayerInfo is admitted exactly once");
    let info = infos[0];
    main_assert_eq!(info.id => existing_player_info_id + 1, "PlayerInfo admission reserves IDs already owned by live low-level players");
    main_assert_eq!(info.player_type => clonk_engine::PLAYER_INFO_TYPE_SCRIPT);
    main_assert_eq!((info.color, info.original_color, info.team) => (0x445566, 0x445566, 2));
    main_assert_eq!(info.extra_data => *b"__AI");
    main_assert_eq!(
        info.flags
    ...
    let runtime = app
        ...
        .test_value();
    main_assert_eq!(runtime.name => "Bot");
    main_assert_eq!(runtime.team => Some(2));
    main_assert_eq!(runtime.color => Some(clonk_engine::RgbColor::new(0x44, 0x55, 0x66)));
    main_assert!(app.snapshot.players.iter().any(|player| player.player_info_id == info.id), "post-control app snapshot includes the joined player");
}
...
fn client_direct_cpp_sync_check_desync_continues_running_round_locally() {
    ...
    app.netplay.manager = Some(manager);
    app.netplay.mode = Some(NetworkMode::Client(n2_client_settings()));
    app.netplay.control_clock = Some(NetworkControlClock::new(37, 4));
    app.netplay.control_clients = ControlClientRegistry::default();
    app.netplay.control_clients.register(0, true, false);
    app.netplay.control_clients.register(local_client, true, false);
    ...
    n2_send_event(
        &event_tx,
        NetworkEvent::DirectControl(NetworkControl::SyncCheck(remote)),
    );
    ...
    main_assert_eq!(app.status_text => "Network desync detected; disconnected from host");
    main_assert_eq!(app.engine.snapshot().round_results.network_result => Some(clonk_engine::RoundResultsNetworkResult::NetworkError));
    main_assert_eq!(app.engine.snapshot().round_results.network_result_message.as_slice() => &b"Network: Synchronization loss!"[..]);
    main_assert_eq!(app.snapshot.round_results => app.engine.snapshot().round_results, "the presentation snapshot exposes the verdict immediately");
    main_assert!(
        app.test_audio_ref()
            .loaded_sounds
            .keys()
    ...
    main_assert!(app.snapshot.audio.is_empty(), "the immediate UI cue is not replayed through engine audio");
}
...
fn host_ignores_remote_sync_check_without_desync_side_effects() {
    ...
    main_assert!(app.netplay.manager.is_some());
    main_assert!(matches!(app.netplay.mode, Some(NetworkMode::Host(_))));
    main_assert_eq!(app.status_text => status_before);
    main_assert!(app.engine.snapshot().round_results.network_result.is_none());
    main_assert!(app.snapshot.round_results.network_result.is_none());
...
fn synchronized_raw_player_control_counts_before_byte_narrowing() {
    ...
    main_assert_eq!((player.control_count(), player.action_count()) => (1, 1));
    main_assert_eq!(app.netplay.executing_ready_tick => None);
}
...
fn synchronized_player_command_executes_stack_data_and_count() {
    ...
    main_assert_eq!(commands[0].name => "Wait");
    main_assert_eq!(commands[0].target => None);
    main_assert_eq!(commands[0].tx => Some(12));
    main_assert_eq!(commands[0].ty => Some(-7));
    main_assert_eq!(commands[0].data => CommandData::Integer(23));
    let player = app.engine.test_player(owner);
    main_assert_eq!((player.control_count(), player.action_count()) => (1, 1));
    main_assert_eq!(app.netplay.executing_ready_tick => None);
}
...
fn synchronized_custom_command_executes_only_while_game_is_running() {
    ...
    app.apply_ready_controls(11, vec![command.clone()])
        .test_value();
    main_assert_eq!(app.engine.game_tick_delay_ms() => initial_delay);

    app.mode = AppMode::Running;
    app.apply_ready_controls(12, vec![command]).test_value();
    main_assert_eq!(app.engine.game_tick_delay_ms() => 10);
    ...
    app.apply_ready_controls(
        13,
        vec![
            NetworkControl::Set(n2_fixture!(control_set: 1, 0, 7)),
            disabled_speed,
        ],
    )
    .test_value();
    main_assert_eq!(app.engine.game_tick_delay_ms() => 10, "DisableDebug removes /speed before the following packet executes");
    main_assert_eq!(app.netplay.executing_ready_tick => None);
}
...
fn synchronized_message_board_answer_executes_only_for_the_owning_client() {
    ...
    main_assert!(app.engine.active_message_board_input().is_some());
    ...
    main_assert!(app.engine.active_message_board_input().is_some(), "synchronized execution does not own local dialog closure");
    ...
    let (_, answer) = commands
        ...
        .pop()
        .test_value();
    main_assert_eq!(answer.by_client => 0, "manager stamps the local client ID");
    ...
    main_assert_eq!(target.local_vars.get("callback_answer") => Some(&Value::String("q\"\\z".to_string().into())));
    main_assert_eq!(target.local_vars.get("callback_count") => Some(&Value::Int(1)));
    main_assert_eq!(app.netplay.executing_ready_tick => None);
}
...
fn host_control_rate_set_changes_both_clocks_and_keeps_batch_order() {
    ...
    let initial_pressed = app.engine.test_player(app.players.local_owner).control.pressed_coms;
    ...
    app.apply_ready_controls(
        ...
        vec![
            ...
            NetworkControl::Set(n2_fixture!(control_set: 0, 1, 0)),
        ],
    )
    .test_value();

    main_assert_ne!(app.engine.player(app.players.local_owner).expect("local player").control.pressed_coms => initial_pressed);
    main_assert_eq!(app.engine.control_rate() => 5);
    main_assert_eq!(app.netplay.control_clock.map(NetworkControlClock::control_rate) => Some(5));
    main_assert_eq!(app.netplay.control_clock.and_then(|clock| clock.tick_for_frame(4)) => None);
    main_assert_eq!(app.netplay.control_clock.and_then(|clock| clock.tick_for_frame(5)) => Some(37));

    for (delta, expected) in [(i32::MAX, 20), (i32::MIN, 1)] {
        app.apply_ready_controls(
            38,
            vec![NetworkControl::Set(n2_fixture!(control_set: 0, delta, 0))],
        )
        .test_value();
        main_assert_eq!(app.engine.control_rate() => expected);
        main_assert_eq!(app.netplay.control_clock.map(NetworkControlClock::control_rate) => Some(expected));
    }
    main_assert_eq!(app.netplay.executing_ready_tick => None);
}
...
fn host_parameter_sets_update_live_game_state_without_boundaries() {
    ...
    for (value_type, data) in [(2, 37), (3, 4), (4, -1), (5, 777)] {
        app.apply_ready_controls(
            9,
            vec![NetworkControl::Set(n2_fixture!(control_set:
                value_type, data, 0,
            ))],
        )
    ...
    main_assert_eq!(app.netplay.max_players => 37);
    main_assert_eq!(app.engine.max_players() => Some(37));
    main_assert_eq!(app.engine.team_distribution() => 4);
    main_assert!(app.engine.team_colors());
    main_assert!(app.engine.use_fair_crew());
    main_assert_eq!(app.engine.fair_crew_strength() => 777);
    ...
    app.apply_ready_controls(
        10,
        vec![NetworkControl::Set(n2_fixture!(control_set: 5, -1, 0))],
    )
    .test_value();
    main_assert!(!app.engine.use_fair_crew());
    main_assert_eq!(app.engine.fair_crew_strength() => 0);

    app.engine.set_fair_crew_forced(true);
    app.apply_ready_controls(
        11,
        vec![NetworkControl::Set(n2_fixture!(control_set: 5, 999, 0))],
    )
    .test_value();
    main_assert!(!app.engine.use_fair_crew());
    main_assert_eq!(app.engine.fair_crew_strength() => 0);
    ...
    app.apply_ready_controls(
        12,
        vec![NetworkControl::Set(n2_fixture!(control_set: 2, 99, 0))],
    )
    .test_value();
    main_assert_eq!(app.engine.max_players() => Some(37));
...
fn network_host_distribution_reassigns_and_publishes_full_team_state() {
    ...
    app.execute_control_set(n2_fixture!(control_set: 3, 2, 0));

    main_assert_eq!(app.players.infos.get(20).unwrap().team => 2);
    main_assert_eq!(app.engine.team_distribution() => 2);
    let assignment = app.players.team_assignment.as_ref().test_value().teams();
    main_assert_eq!(assignment.teams[0].player_ids => vec![10]);
    main_assert_eq!(assignment.teams[1].player_ids => vec![20]);
    let snapshot = app.netplay.host_join_snapshot.as_ref().test_value();
    main_assert_eq!(snapshot.parameters.teams.teams[0].player_ids => vec![10]);
    main_assert_eq!(snapshot.parameters.teams.teams[1].player_ids => vec![20]);
    main_assert_eq!(snapshot.parameters.player_infos.clients[0].players[0].team => 2);
    let (player_infos, snapshots) = commands.take_team_control_updates();
    main_assert_eq!(player_infos.len() => 1);
    main_assert_eq!(snapshots.len() => 1);
    main_assert_eq!(snapshots[0] => *snapshot);
}
...
fn generated_distribution_rebuilds_default_teams_and_publishes() {
    ...
    app.execute_control_set(n2_fixture!(control_set: 3, 3, 0));

    let assignment = app.players.team_assignment.as_ref().test_value().teams();
    main_assert_eq!(assignment.team_distribution => clonk_engine::InitialNetworkTeamDistribution::Random);
    main_assert_eq!(assignment.last_team_id => 2);
    main_assert_eq!(
        assignment
            .teams
            .iter()
    ...
    let assigned_team = app.players.infos.get(20).test_value().team;
    main_assert!((1..=2).contains(&assigned_team));
    main_assert_eq!(assignment.teams.iter().find(|team| team.id == assigned_team).unwrap().player_ids => vec![20]);
    main_assert_eq!(app.engine.team_distribution() => 3);
    let snapshot = app.netplay.host_join_snapshot.as_ref().test_value();
    main_assert_eq!(snapshot.parameters.teams.team_distribution => 3);
    main_assert_eq!(snapshot.parameters.teams.teams.len() => 2);
    let (player_infos, snapshots) = commands.take_team_control_updates();
    let [player_info] = player_infos.as_slice() else {
        panic!("expected one regenerated PlayerInfo packet");
    };
    main_assert_eq!(player_info.client_id => 3);
    main_assert_eq!(player_info.flags & (clonk_engine::CLIENT_PLAYER_INFO_FLAG_ADD_PLAYERS | clonk_engine::CLIENT_PLAYER_INFO_FLAG_UPDATED) => 0);
    main_assert_eq!(player_info.players[0].team => assigned_team);
    main_assert_eq!(snapshots => vec![snapshot.clone()]);
}
...
fn invalid_host_team_distribution_is_a_silent_no_op() {
    ...
    let before_teams = app
        .players.team_assignment
        .as_ref()
        .test_value()
        .teams()
        .clone();
    let before_infos = app.players.infos.retained_rows_snapshot();
    let before_snapshot = app.netplay.host_join_snapshot.clone();

    app.execute_control_set(n2_fixture!(control_set: 3, 9, 0));

    main_assert_eq!(app.status_text => "unchanged");
    main_assert_eq!(app.engine.team_distribution() => before_distribution);
    main_assert_eq!(app.players.team_assignment.as_ref().unwrap().teams() => &before_teams);
    main_assert_eq!(app.players.infos.retained_rows_snapshot() => before_infos);
    main_assert_eq!(app.netplay.host_join_snapshot => before_snapshot);
    main_assert_eq!(commands.take_team_control_updates() => (Vec::new(), Vec::new()));
}
...
fn network_host_team_colors_broadcasts_attributes_and_full_join_data() {
    ...
    app.execute_control_set(n2_fixture!(control_set: 4, 1, 0));

    main_assert!(app.engine.team_colors());
    main_assert!(app.players.team_assignment.as_ref().unwrap().teams().team_colors);
    main_assert_eq!(app.players.infos.get(20).unwrap().color => 0x0000_00f4);
    let snapshot = app.netplay.host_join_snapshot.as_ref().test_value();
    main_assert_eq!(snapshot.parameters.teams.team_colors => 1);
    main_assert_eq!(snapshot.parameters.teams.teams.len() => 2);
    main_assert_eq!(snapshot.parameters.player_infos.clients[0].players[0].color => 0x0000_00f4);
    let (player_infos, snapshots) = commands.take_team_control_updates();
    main_assert_eq!(player_infos.len() => 1);
    main_assert_eq!(snapshots.len() => 1);
    main_assert_eq!(snapshots[0] => *snapshot);
...
fn wire_known_zero_color_and_name_conflicts_resolve_and_publish_team_colors() {
    for conflict_kind in ["color", "name"] {
        ...
        main_assert!(app.engine.team_colors(), "{conflict_kind}");
        main_assert!(app.players.team_assignment.as_ref().unwrap().teams().team_colors, "{conflict_kind}");
        let first = app.players.infos.get(20).test_value();
        let second = app.players.infos.get(21).test_value();
        if conflict_kind == "color" {
            main_assert_ne!(first.color => second.color);
        } else {
            ...
            main_assert!(!effective_name(first).eq_ignore_ascii_case(effective_name(second)));
        }
        let snapshot = app.netplay.host_join_snapshot.as_ref().test_value();
        main_assert_eq!(snapshot.parameters.teams.team_colors => 1);
        let (player_infos, snapshots) = commands.take_team_control_updates();
        main_assert_eq!(player_infos.len() => 1, "{conflict_kind}");
        main_assert_eq!(snapshots.as_slice() => std::slice::from_ref(snapshot), "{conflict_kind}");
    }
}
...
fn offline_host_reassigns_but_replay_only_changes_the_distribution_flag() {
    ...
    offline.players.team_assignment = Some(NetworkTeamAssignmentState::from_prepared_host(
        metadata.clone(),
    ));
    offline
        .players.infos
        .replace_snapshot(20, [packet.clone()]);
    offline.execute_control_set(n2_fixture!(control_set: 3, 2, 0));
    main_assert_eq!(offline.players.infos.get(20).unwrap().team => 2);

    let mut replay = new_state_only_running_sandbox_app();
    replay.engine.set_control_host(false);
    replay.players.team_assignment = None;
    replay.players.infos.replace_snapshot(20, [packet]);
    replay.execute_control_set(n2_fixture!(control_set: 3, 3, 0));
    main_assert_eq!(replay.engine.team_distribution() => 3);
    main_assert_eq!(replay.players.infos.get(20).unwrap().team => 1);
}
...
fn client_player_info_rechecks_prepared_team_memberships_before_activation() {
    ...
    n2_send_event(
        &event_tx,
        NetworkEvent::DirectControl(NetworkControl::PlayerInfo(netplay_player_info_data(
            3,
            vec![set_control_test_player(20, 2, 0)],
        ))),
    );
    ...
    let teams = &app
        ...
        .test_value()
        .team_registry;
    main_assert!(teams[0].player_ids.is_empty());
    main_assert_eq!(teams[1].player_ids => vec![20]);
}
...
fn client_player_info_generates_missing_autogenerated_teams_before_activation() {
    ...
    snapshot.parameters.teams = clonk_app_netplay::join_team_list_snapshot(metadata);
    n2_send_event(
        &event_tx,
        NetworkEvent::JoinData(n2_fixture!(join_data:
            7,
            snapshot.dynamic_tick,
            host_config.initial_status,
            snapshot.dynamic,
    ...
    n2_send_event(
        ...
        NetworkEvent::DirectControl(NetworkControl::PlayerInfo(netplay_player_info_data(
            3,
            vec![set_control_test_player(20, 4, 0)],
        ))),
    );
    ...
    let teams = &app
        .netplay.pending_join_data
        .test_ref()
        .parameters
        .teams
        .teams;
    main_assert_eq!(teams.iter().map(|team| team.id).collect::<Vec<_>>() => vec![1, 2, 3, 4]);
    main_assert!(teams[..3].iter().all(|team| team.player_ids.is_empty()));
    main_assert_eq!(teams[3].player_ids => vec![20]);
}

#[test]
fn offline_player_info_control_rechecks_teams_before_joining_unjoined_script_player() {
    let mut app = new_state_only_synthetic_crew_running_sandbox_app();
    let existing_info_id = app.engine.test_player(app.players.local_owner).player_info_id();
    ...
    app.apply_ready_controls(
        7,
        vec![NetworkControl::PlayerInfo(n2_fixture!(player_data {
            client_id: 0,
    ...
    let teams = app.players.team_assignment.test_ref().teams();
    main_assert_eq!(teams.teams[0].player_ids => vec![existing_info_id]);
    main_assert_eq!(teams.teams[1].player_ids => vec![bot_info_id]);
    let bot = app
        .engine
        .players()
        .find(|player| player.player_info_id() == bot_info_id)
        .test_value();
    main_assert_eq!(bot.team() => Some(2));
}
...
fn mutable_set_parameters_round_trip_and_legacy_absence_preserves_bootstrap() {
    ...
    main_assert!(restored.fair_crew_forced());
    main_assert!(!restored.allow_debug());
    main_assert_eq!(restored.control_rate() => 7);
    ...
    main_assert!(seeded.fair_crew_forced());
    main_assert!(!seeded.allow_debug());
    main_assert_eq!(seeded.control_rate() => 9);
}
...
fn non_host_host_gated_control_sets_are_synchronized_no_ops() {
    for value_type in [0, 2, 3, 4, 5] {
        ...
        let (manager, _event_tx) = NetworkManager::test_stub();
        app.netplay.manager = Some(manager);
        app.netplay.control_clock = Some(NetworkControlClock::new(0, 3));
        app.engine.set_control_rate(3);
        app.netplay.control_clients.register(3, false, false);
        let before = (
        ...
        app.apply_ready_controls(
            4,
            vec![
                NetworkControl::Set(n2_fixture!(control_set: value_type, 99, 7)),
                NetworkControl::ClientUpdate(
                    n2_fixture!(client_update: clonk_engine::CLIENT_UPDATE_ACTIVATE, 3, 1, 0),
                ),
            ],
        )
        .unwrap_or_else(|error| {
    ...
    for by_client in [0, 7] {
        ...
        app.apply_ready_controls(
            ...
            vec![
                ...
                NetworkControl::Set(n2_fixture!(control_set: 1, 123, by_client)),
            ],
        )
        .test_value();
        main_assert_ne!(app.engine.player(app.players.local_owner).expect("local player").control.pressed_coms => initial_pressed);
...
fn immediate_control_sets_execute_for_host_and_non_host_disable_debug() {
    ...
    n2_send_event(
        &event_tx,
        NetworkEvent::DirectControl(NetworkControl::Set(n2_fixture!(control_set: 4, 1, 0))),
    );
    n2_send_event(
        &event_tx,
        NetworkEvent::DirectControl(NetworkControl::Set(n2_fixture!(control_set: 1, 0, 7))),
    );
...
fn host_direct_elimination_closes_one_viewport_then_falls_back_silently() {
    ...
    n2_send_event(
        ...
        n2_fixture!(ready_tick:
            ...
            vec![NetworkControl::RemovePlayer(remove)]
        ),
    );
    app.test_update();
...
fn non_control_host_direct_elimination_returns_success_without_queuing() {
    ...
    main_assert_eq!(result => Some(Value::Int(1)));
    main_assert!(app.engine.take_pending_remove_player_controls().is_empty());
    main_assert!(app.engine.player(player).is_some());
}
...
fn remove_player_control_is_host_only_and_propagates_disconnected() {
    ...
    app.apply_ready_controls(
        0,
        vec![NetworkControl::RemovePlayer(n2_fixture!(remove_player:
            17, true, 3
        ))],
    )
    .test_value();
    main_assert!(app.engine.player(17).is_some());
    main_assert_eq!(
        app.players.infos
            .get(7)
    ...
    main_assert_eq!(app.players.infos.get(7).expect("info retained").game_part_frame => -99);

    app.apply_ready_controls(
        1,
        vec![
            NetworkControl::RemovePlayer(n2_fixture!(remove_player: 17, true, 0)),
            NetworkControl::RemovePlayer(n2_fixture!(remove_player: 18, false, 0)),
        ],
    )
    .test_value();

    main_assert!(app.engine.player(17).is_none());
    main_assert!(app.engine.player(18).is_none());
    let disconnected = app.players.infos.get(7).test_value();
    main_assert_ne!(disconnected.flags & clonk_engine::PLAYER_INFO_FLAG_REMOVED => 0);
    main_assert_ne!(disconnected.flags & clonk_engine::PLAYER_INFO_FLAG_DISCONNECTED => 0);
    main_assert_eq!(disconnected.game_part_frame => 0);
    let ordinary = app.players.infos.get(8).test_value();
    main_assert_ne!(ordinary.flags & clonk_engine::PLAYER_INFO_FLAG_REMOVED => 0);
    main_assert_eq!(ordinary.flags & clonk_engine::PLAYER_INFO_FLAG_DISCONNECTED => 0);
    main_assert_eq!(ordinary.game_part_frame => 0);
    let published = commands.take_published_join_snapshots();
    ...
    let published_players = &published
        ...
        .clients[0]
        .players;
    main_assert_ne!(published_players[0].flags & clonk_engine::PLAYER_INFO_FLAG_DISCONNECTED => 0);
    main_assert_eq!(published_players[1].flags & clonk_engine::PLAYER_INFO_FLAG_DISCONNECTED => 0);
    main_assert_eq!(published_players.iter().map(|info| info.game_part_frame).collect::<Vec<_>>() => vec![0, 0]);
}
...
fn synchronized_activation_admits_later_remote_player_info() {
    ...
    n2_send_event(
        &event_tx,
        NetworkEvent::DirectControl(NetworkControl::ClientJoin(
    ...
    let joins = commands.take_submitted_join_players();
    main_assert_eq!(joins.len() => 1);
    main_assert_eq!((joins[0].1.at_client, joins[0].1.info_id) => (3, 41));
}
...
fn host_activation_request_submits_cpp_eligible_synchronized_update() {
    ...
    n2_send_event(
        &event_tx,
        n2_fixture!(activation_request: 3, frame, true, 25),
    );
...
fn classic_host_lobby_activation_request_has_no_presentation_child() {
    ...
    app.netplay.mode = Some(NetworkMode::Host(host_network_settings()));
    app.netplay.control_clients.register(3, false, false);
    n2_send_event(&event_tx, n2_fixture!(activation_request: 3, -1, true, 25));
...
fn classic_host_lobby_status_commit_has_no_presentation_child() {
    ...
    let lobby = n2_fixture!(status: clonk_network::NETWORK_STATE_LOBBY, 0, 0);
    n2_send_event(&event_tx, NetworkEvent::StatusCommitted(lobby));

    app.test_network_events();

    main_assert_eq!(app.netplay.runtime_committed_status => Some(lobby));
    main_assert!(!app.netplay.control_running);
    main_assert!(app.lobby.classic_host.is_some());
    main_assert_eq!(app.mode => AppMode::Menu);
...
fn network_client_activity_uses_player_presence_and_strict_frame_age() {
    ...
    main_assert!(activity.deactivation_candidates([0, 3], [], 0, 500).is_empty());
    main_assert_eq!(activity.deactivation_candidates([0, 3], [], 0, 501) => vec![3]);

    activity.mark_activated(3, 700);
    main_assert!(activity.deactivation_candidates([0, 3], [], 0, 1_200).is_empty());
    main_assert_eq!(activity.deactivation_candidates([0, 3], [], 0, 1_201) => vec![3]);

    main_assert!(activity.deactivation_candidates([0, 3], [3], 0, 10_000).is_empty());
}
...
fn reaching_go_on_a_savegame_frame_does_not_backdate_a_client_into_inactivity() {
    ...
    app.netplay.mode = Some(NetworkMode::Host(host_network_settings()));
    app.netplay.control_clients.register(3, true, false);
    app.netplay.client_activity.mark_activated(3, 0);
...
fn control_rate_two_client_executes_scheduled_activation_at_the_control_tick() {
    ...
    app.netplay.mode = Some(NetworkMode::Client(n2_client_settings()));
    app.netplay.control_clients.register(3, false, false);
    app.netplay.control_clock = Some(NetworkControlClock::new(0, 2));
    app.engine.initialize_network_control_timing(
        clonk_engine::NetworkControlTiming::new(0, 2).test_value(),
    );

    for tick in 0..3 {
        n2_send_event(&event_tx, n2_fixture!(ready_tick: tick, Vec::new()));
        app.test_update();
        if tick < 2 {
            app.test_update();
        }
    }
    main_assert_eq!(app.engine.frame() => 5);
    main_assert_eq!(app.expected_network_control_tick() => 3);

    let update = n2_fixture!(client_update: clonk_engine::CLIENT_UPDATE_ACTIVATE, 3, 1, 0);
    n2_send_event(
        &event_tx,
        n2_fixture!(scheduled_sync:
            3,
            vec![NetworkControl::ClientUpdate(update.clone())]
        ),
    );

    app.test_update();
    main_assert_eq!(app.engine.frame() => 6);
    main_assert!(!app.netplay.control_clients.is_activated(3));
    main_assert!(commands.take_executed_client_updates().is_empty());

    n2_send_event(&event_tx, n2_fixture!(ready_tick: 3, Vec::new()));
    app.test_update();

    main_assert_eq!(app.engine.frame() => 7);
    main_assert!(app.netplay.control_clients.is_activated(3));
    main_assert_eq!(commands.take_executed_client_updates() => vec![update]);
}
...
fn synchronized_client_remove_prunes_and_rechecks_teams_without_client_host_cascade() {
    ...
    app.players.team_assignment = None;
    app.netplay.control_clients.register(3, true, false);
    let metadata =
    ...
    app.apply_ready_controls(
        0,
        vec![NetworkControl::ClientRemove(n2_fixture!(client_remove:
            3,
            clonk_engine::LegacyCString::default(),
            0
        ))],
    ...
    main_assert_ne!(retained.flags & clonk_engine::PLAYER_INFO_FLAG_REMOVED => 0);
    main_assert_ne!(retained.flags & clonk_engine::PLAYER_INFO_FLAG_DISCONNECTED => 0);
    main_assert!(app.players.infos.get(8).is_none());
    main_assert_eq!(app.engine.teams()[0].player_ids => vec![9]);
    let unaffected = app.players.infos.get(9).test_value();
    main_assert_eq!(unaffected.color => retained_color);
    main_assert_eq!(unaffected.forced_name.as_bytes() => b"Alias");
    main_assert_eq!(unaffected.league_projected_gain => 5);
    main_assert!(commands.take_broadcast_player_infos().is_empty());
}
...
fn synchronized_client_remove_without_player_info_skips_the_part_cascade() {
    ...
    let retained = app.players.infos.get(77).test_value();
    main_assert_eq!(retained.color => 0x0000_f400);
    main_assert_eq!(retained.league_projected_gain => 5);
    main_assert_eq!(app.players.team_assignment.as_ref().unwrap().teams().teams[0].player_ids => vec![77],);
    main_assert!(commands.take_broadcast_player_infos().is_empty());
}
...
fn network_host_without_control_host_still_runs_client_part_updates() {
    ...
    main_assert_eq!(app.players.infos.get(40).unwrap().league_projected_gain => -1,);
    let broadcasts = commands.take_broadcast_player_infos();
    let [update] = broadcasts.as_slice() else {
        panic!("expected one gain-reset packet, got {broadcasts:?}");
    };
    main_assert_eq!(update.client_id => 4);
    main_assert_eq!(update.players[0].league_projected_gain => -1);
}
...
fn client_host_socket_loss_continues_the_running_round_locally() {
    ...
    app.players.infos.apply(netplay_player_info_data(
        0,
        vec![n2_fixture!(player: remote_info, clonk_engine::PLAYER_INFO_FLAG_JOINED)],
    ));
    ...
    n2_send_event(
        &event_tx,
        n2_fixture!(peer_disconnected: 0, Some("connection lost".to_string())),
    );
    app.test_network_events();
    ...
    main_assert_eq!(app.engine.sync_check(local_client).control_tick => control_tick_before);
    main_assert_eq!(app.engine.control_rate => 1);
    main_assert_eq!(app.netplay.max_players => 9);
...
fn client_follows_an_announced_host_restart_back_to_the_lobby() {
    ...
    main_assert_eq!(app.mode => AppMode::Menu, "the round the host abandoned is torn down, not continued locally");
    main_assert!(app.netplay.manager.is_none());
    main_assert!(app.netplay.pending_host_rejoin.is_some(), "the rejoin stays armed until the connection resolves");
    main_assert_eq!(
...
fn client_follows_a_session_preserving_restart_into_the_lobby() {
    ...
    main_assert!(app.netplay.manager.is_some(), "the session the host kept up is the one this client keeps");
    main_assert!(app.startup_network.connection.is_none(), "the restart must not launch a new client admission");
    main_assert!(app.netplay.pending_host_rejoin.is_none(), "a preserved session has no reconnect to arm");
...
fn restarted_client_acknowledges_join_data_before_reaffirming_player_info() {
    ...
    n2_send_event(&event_tx, NetworkEvent::HostRestartLobby);
    app.test_network_events();
    main_assert!(app.netplay.pending_round_restart_join_data);
...
fn an_oversized_rejoin_window_is_clamped() {
    ...
    let deadline = app.netplay.pending_host_rejoin.test_ref().deadline;
    main_assert!(
        deadline
    ...
    main_assert!(app.netplay.pending_host_rejoin.is_some(), "a zero window declines to re-arm; it does not disarm what is already armed");
}
...
fn cancelling_the_reconnect_dialog_abandons_the_rejoin() {
    ...
    let mut app = new_classic_menu_app(800, 600);
    app.netplay.pending_host_rejoin = Some(n2_pending_rejoin(
    ...
    main_assert!(app.startup_network.connection.is_none());
    main_assert!(app.netplay.pending_host_rejoin.is_none(), "Cancel must end the rejoin, not just the attempt in flight");
...
fn a_window_that_closes_mid_attempt_reports_one_failure() {
    ...
    main_assert!(app.netplay.pending_host_rejoin.is_none());
...
fn an_armed_rejoin_survives_the_hosts_rebind_window_and_then_gives_up() {
    ...
    main_assert!(app.startup_network.connection.is_none());
    let scheduled = app.netplay.pending_host_rejoin.test_ref();
    ...
    main_assert!(app.netplay.pending_host_rejoin.is_none(), "the rejoin stops at the window the host named");
    main_assert_eq!(app.startup.view => StartupView::NetworkGame);
}
...
fn a_rejoin_reuses_the_live_join_settings_rather_than_rebuilding_them() {
    ...
    let (manager, event_tx) = NetworkManager::test_stub_for_client_id(local_client as u32);
    app.netplay.manager = Some(manager);
    app.netplay.mode = Some(NetworkMode::Client(settings.clone()));
    app.netplay.control_clock = Some(NetworkControlClock::new(31, 4));
    ...
    n2_send_event(&event_tx, n2_fixture!(peer_disconnected: 0, None));
    app.test_network_events();

    let relaunched = app.netplay.pending_join.test_ref();
    main_assert_eq!(relaunched.password => password);
    main_assert_eq!(relaunched.compatibility_build => 42);
    main_assert_eq!(relaunched.logical_server_addresses => settings.logical_server_addresses);
}
...
fn dropping_to_local_control_abandons_an_armed_rejoin() {
    ...
    n2_send_event(
        &event_tx,
        NetworkEvent::FatalError("worker failed".to_string()),
    );
    app.test_network_events();
    app.poll_startup_network_connection().test_value();
    ...
    main_assert!(app.netplay.manager.is_none());
    main_assert!(app.netplay.pending_host_rejoin.is_none(), "a round that dropped to local control is no longer following the host");
...
fn an_announced_restart_alone_does_not_disturb_the_running_round() {
    ...
    main_assert!(app.netplay.pending_host_rejoin.is_some(), "the intent is recorded");
...
fn a_client_waiting_in_the_lobby_also_follows_an_announced_restart() {
    ...
    main_assert!(app.netplay.pending_host_rejoin.is_some());
}
...
fn a_rejoin_disarms_when_it_resolves_or_the_player_leaves() {
    ...
    let armed = || {
        n2_pending_rejoin(
            Instant::now() + Duration::from_secs(30),
            Some(Instant::now() + HOST_REJOIN_RETRY_INTERVAL),
        )
    };
    app.netplay.pending_host_rejoin = Some(armed());
    let (manager, _events) = NetworkManager::test_stub_for_client_id(7);
    let (sender, receiver) = mpsc::channel();
    sender
    ...
    main_assert!(app.netplay.pending_host_rejoin.is_none(), "a resolved rejoin must not keep its deadline running");

    app.netplay.pending_host_rejoin = Some(armed());
    app.return_to_menu();

    main_assert!(app.netplay.pending_host_rejoin.is_none(), "leaving the round abandons the rejoin with it");
}
...
fn client_non_host_peer_loss_keeps_the_network_session() {
    ...
    app.netplay.control_clients.register(0, true, false);
    app.netplay.control_clients.register(local_client, true, false);
    app.netplay.control_clients.register(peer_client, true, false);

    n2_send_event(
        &event_tx,
        n2_fixture!(peer_disconnected:
            peer_client as u32,
            Some("peer transport lost".to_string())
        ),
    );
    app.test_network_events();
    ...
    main_assert!(matches!(app.mode, AppMode::Running));
    main_assert!(app.engine.player(peer_player).is_some());
}
...
fn removing_local_network_client_changes_to_local_control() {
    ...
    app.apply_ready_controls(
        ...
        vec![NetworkControl::ClientRemove(n2_fixture!(client_remove:
            3,
            clonk_engine::LegacyCString::from_bytes(b"Removed".to_vec())
                .expect("valid reason"),
            0
        ))],
    )
...
fn running_stale_lobby_state_cannot_execute_scheduled_sync_immediately() {
    ...
    n2_send_event(
        ...
        n2_fixture!(scheduled_sync:
            1,
            vec![NetworkControl::Set(n2_fixture!(control_set: 2, 23, 0))]
        ),
    );

    app.test_network_events();

    main_assert_ne!(app.engine.max_players() => Some(23), "post-GO sync must not execute through the frozen-lobby path");
    main_assert_eq!(app.netplay.sync.take_exact(1).len() => 1);
}
...
fn final_go_applies_lifecycle_sync_before_active_client_sweep() {
    ...
    app.netplay.control_clock = Some(NetworkControlClock::new(3, 2));
    app.engine.initialize_network_control_timing(
        clonk_engine::NetworkControlTiming::new(0, 2).test_value(),
    );
    for _ in 0..6 {
    ...
    app.netplay.control_running = false;
    app.netplay.control_clients.register(3, false, false);
    app.netplay.control_clients.register(4, true, false);
    app.players.infos.apply(netplay_player_info_data(
        3,
        vec![n2_fixture!(player {
            id: 31,
    ...
    n2_send_event(
        ...
        n2_fixture!(scheduled_sync:
            ...
            vec![
                ...
                NetworkControl::ClientRemove(n2_fixture!(client_remove:
                    4,
                    clonk_engine::LegacyCString::default(),
                    0
                )),
    ...
    n2_send_event(
        &event_tx,
        NetworkEvent::StatusCommitted(n2_fixture!(status: clonk_network::NETWORK_STATE_GO, 1, 3)),
    );

    app.test_network_events();

    main_assert!(app.netplay.control_clients.is_activated(3));
    main_assert!(!app.netplay.control_clients.contains(4));
    main_assert!(app.players.infos.get(41).is_none());
    let joins = commands.take_submitted_join_players();
    main_assert_eq!(joins.len() => 1);
...
fn network_update_uses_join_control_tick_and_cpp_control_rate() {
    ...
    let (manager, event_tx) = NetworkManager::test_stub();
    app.netplay.manager = Some(manager);
    app.netplay.control_clock = Some(network::NetworkControlClock::new(9, 2));
    main_assert_eq!(app.engine.frame() => 0);

    n2_send_event(&event_tx, n2_fixture!(ready_tick: 9, Vec::new()));
    app.test_update();
    ...
    app.test_update();
    main_assert_eq!(app.engine.frame() => 2, "stalled control frame is retried");
    main_assert_eq!(app.netplay.control_clock.map(|clock| clock.current_tick()) => Some(10));

    n2_send_event(&event_tx, n2_fixture!(ready_tick: 10, Vec::new()));
    app.test_update();
...
fn blocked_save_finalizer_does_not_block_two_peer_control_or_input_progress() {
    ...
    n2_send_event(
        ...
        n2_fixture!(ready_tick: 0, vec![NetworkControl::Player {
            owner: host.players.local_owner,
            event: ControlEvent::Press(ControlButton::Right),
        }]),
    );
    n2_send_event(
        &client_events,
        n2_fixture!(ready_tick: 0, vec![NetworkControl::Player {
            owner: client.players.local_owner,
            event: ControlEvent::Press(ControlButton::Right),
        }]),
    );
    host.test_update();
    ...
    main_assert_ne!(host.engine.player(host.players.local_owner).expect("host player").control.pressed_coms & (1 << clonk_engine::COM_RIGHT) => 0);
    main_assert_ne!(client.engine.player(client.players.local_owner).expect("client player").control.pressed_coms & (1 << clonk_engine::COM_RIGHT) => 0);
...
fn joined_lobby_non_roster_network_batch_keeps_cached_player_raster() {
    ...
    n2_send_event(
        &event_tx,
        NetworkEvent::HostPingMeasured { round_trip_ms: 12 },
    );
    ...
    let host = lobby
        ...
        .test_value();
    main_assert!(!host.connected);
    main_assert_eq!(host.ping_ms => None);
    main_assert_eq!(host.resource_progress => None);
    main_assert!(
    ...
    let host = lobby
        ...
        .test_value();
    main_assert!(host.connected);
    main_assert_eq!(host.ping_ms => Some(12));
    main_assert_eq!(host.resource_progress => Some(80));
    main_assert!(
        lobby.roster_rows.iter().any(|row| {
            matches!(
                row,
    ...
    main_assert_eq!(lobby.controller.rows() => lobby.roster_rows.as_slice());
    ...
    n2_send_event(
        &event_tx,
        NetworkEvent::ResourceLoadFailed { resource_id: 89 },
    );
    app.test_network_events();
    main_assert_ne!(
        app.lobby.session
    ...
    main_assert_eq!(lobby.roster_rows => lobby.controller.rows());
}
...
fn initial_network_client_registry_keeps_the_local_client_name() {
    let (manager, _event_tx) = NetworkManager::test_stub();
    let mode = NetworkMode::Host(n2_fixture!(host: 11_112, "Named Host".to_string(), None));
...
fn locally_authored_join_uses_filename_instead_of_embedded_data() {
    ...
    n2_send_event(
        ...
        n2_fixture!(ready_tick:
            tick,
            vec![
                NetworkControl::PlayerInfo(n2_fixture!(player_data: 0, vec![info], by: 0)),
    ...
    let joined = app
        ...
        .test_value();
    main_assert_eq!(joined.name => "Local Tyler");
    main_assert_eq!((joined.score, joined.total_playing_time) => (42, 99));
    main_assert_eq!(app.local_controls.owner_for_set(1) => Some(joined.id), "the joined file's missing Control field defaults to Keyboard2");
}
...
fn network_cursor_menu_converts_exact_press_coms_before_submission() {
    ...
    for (raw, mapped, wire_com) in [
        ...
        main_assert_eq!(submitted => vec![(owner, ControlEvent::Command {command: mapped, kind: CommandKind::Press,}, tick,)]);
        ...
        main_assert_eq!(packet.player => owner);
        main_assert_eq!(packet.command => wire_com);
        main_assert_eq!(packet.data => 0);
    }
}
...
fn network_progressing_cursor_menu_submits_show_text_before_execution() {
    ...
    main_assert_eq!(event => ControlEvent::Command {command: ControlCommand::MenuShowText, kind: CommandKind::Press,});
    ...
    main_assert_eq!(packet.command => 42);
    main_assert!(app.engine.cursor_object_menu(owner).is_some_and(|(_, menu)| menu.text_progressing), "submission alone must not reveal local-length text");
...
fn runtime_network_role_requires_consistent_manager_identity_and_mode() {
    ...
    main_assert_eq!(app.runtime_network_role() => RuntimeNetworkRole::Offline);
    app.netplay.mode = Some(NetworkMode::Host(host_network_settings()));
    main_assert_eq!(app.runtime_network_role() => RuntimeNetworkRole::Offline);

    let (host_manager, _events) = NetworkManager::test_stub();
    app.netplay.manager = Some(host_manager);
    app.netplay.mode = None;
    main_assert_eq!(app.runtime_network_role() => RuntimeNetworkRole::Ambiguous);
    app.netplay.mode = Some(NetworkMode::Client(client_network_settings()));
    main_assert_eq!(app.runtime_network_role() => RuntimeNetworkRole::Ambiguous);
    app.netplay.mode = Some(NetworkMode::Host(host_network_settings()));
    main_assert_eq!(app.runtime_network_role() => RuntimeNetworkRole::Host);

    let (client_manager, _events) = NetworkManager::test_stub_for_client_id(3);
    app.netplay.manager = Some(client_manager);
    app.netplay.mode = None;
    main_assert_eq!(app.runtime_network_role() => RuntimeNetworkRole::Ambiguous);
    app.netplay.mode = Some(NetworkMode::Host(host_network_settings()));
    main_assert_eq!(app.runtime_network_role() => RuntimeNetworkRole::Ambiguous);
    app.netplay.mode = Some(NetworkMode::Client(client_network_settings()));
    main_assert_eq!(app.runtime_network_role() => RuntimeNetworkRole::Client);
}
...
fn game_execution_retires_runtime_dynamic_after_its_control_tick() {
    ...
    configure_runtime_network_role(&mut app, RuntimeNetworkRole::Host);
    app.netplay.host_join_snapshot = Some(
        clonk_network::HostConfig::default()
            .initial_join_snapshot
            .test_value(),
    );
    app.netplay.control_clock = Some(NetworkControlClock::new(0, 1));

    app.execute_network_before_control_prepare();
    main_assert_eq!(
        app.netplay.host_join_snapshot
            .as_ref()
            .test_value()
            .dynamic
            .resource_type =>
    ...
    main_assert_eq!(snapshot.dynamic_tick => -1);
}
...
fn saved_game_reapplies_current_player_info_identity_and_preferences() {
    ...
    let save = n2_saved_game(&app, &scenario, "current player info wins", engine_state);

    app.apply_loaded_game(save).test_value();

    let player = app.engine.test_player(owner);
    main_assert_eq!(clonk_script::c4_string_bytes(player.name()) => b"League Ren\xe9");
    main_assert_eq!(player.at_client_name() => "Local");
    main_assert!(!player.is_script_player());
    main_assert!(!player.no_elimination_check());
    main_assert_eq!(player.control_style_preferences() => (false, false));
}
...
fn saved_game_promotes_unjoined_takeover_info_before_recreation_filter() {
    ...
    saved_player.no_elimination_check = true;
    let save = n2_saved_game(&app, &scenario, "unjoined takeover restore", engine_state);

    app.apply_loaded_game(save).test_value();

    let player = app.engine.test_player(owner);
    main_assert_eq!(player.player_info_id() => saved_info_id);
    main_assert_eq!(player.name() => "Current takeover");
    main_assert!(player.no_elimination_check());
    main_assert_eq!(app.players.infos.recreation_info_ids() => vec![saved_info_id]);
}
...
fn network_restore_routes_associated_info_away_from_plain_join_queue() {
    ...
    main_assert_eq!(recreation => vec![(3, 7)]);
    main_assert_eq!(plain_joins.len() => 1);
    main_assert_eq!(plain_joins[0].info_id => 92);
    let resumed = player_infos.get(7).test_value();
    main_assert_eq!((resumed.color, resumed.team) => (restore.color, restore.team));
    main_assert_ne!(resumed.flags & clonk_engine::PLAYER_INFO_FLAG_JOINED => 0);
}
...
fn ordinary_network_savegame_starts_associated_profile_wait_after_go_commit() {
    ...
    .test_value();
    let mut app = new_running_sandbox_app();
    let provisional_owner = app.players.local_owner;
    ...
    app.netplay.control_clients.replace_snapshot([n2_fixture!(client {
        client_id: 0,
        activated: true,
        name: native(b"Current client"),
    })]);
    let resource = n2_fixture!(player_resource: 17, native(b"Alice.c4p"));
    let script_resource = n2_fixture!(player_resource: 19, native(b"ScriptResource.c4p"));
    let current = n2_fixture!(player {
        id: 91,
        savegame_player: 7,
        name: native(b"Alice"),
        flags: clonk_engine::PLAYER_INFO_FLAG_HAS_RESOURCE,
    ...
    let script_current = n2_fixture!(player {
        id: 92,
        savegame_player: 9,
    ...
    let embedded_script_current = n2_fixture!(player {
        id: 93,
        savegame_player: 11,
        ...
        flags: clonk_engine::PLAYER_INFO_FLAG_HAS_RESOURCE,
        player_type: clonk_engine::PLAYER_INFO_TYPE_SCRIPT,
        resource: Some(n2_fixture!(player_resource: 20, native(b"StaleScriptResource.c4p"))),
    });
    app.players.infos = ControlPlayerInfoRegistry::default();
    app.players.infos.replace_snapshot(
        script_current.id,
    ...
    main_assert!(app.netplay.blocking_resource_wait.is_none());

    app.handle_status_committed(n2_fixture!(status: clonk_network::NETWORK_STATE_GO, 0, 0))
        .test_value();

    main_assert!(app.netplay.blocking_resource_wait.as_ref().is_some_and(|wait| {
    ...
    let staged_script = app.players.infos.get(11).test_value();
    main_assert!(staged_script.resource.is_none());
    main_assert_eq!(staged_script.flags & clonk_engine::PLAYER_INFO_FLAG_HAS_RESOURCE => 0);
    ...
    main_assert_eq!((user.player_info_id(), user.wealth(), user.score()) => (7, 19, 31));
    main_assert!(app.engine.player(3).is_none());

    app.poll_loading().test_value();
    main_assert_eq!(app.engine.players().filter(|player| player.player_info_id() == 7).count() => 1);
    let ready_tick = app.expected_network_control_tick();
    n2_send_event(&events, n2_fixture!(ready_tick: ready_tick, Vec::new()));
    app.test_network_events();
    main_assert!(app.netplay.ticks.ready.contains_key(&ready_tick));
    ...
    main_assert_eq!((player.player_info_id(), player.wealth(), player.score()) => (7, 19, 31));
    let script = app.engine.test_player(3);
    main_assert_eq!((script.player_info_id(), script.wealth(), script.score()) => (9, 23, 37));
    main_assert!(script.is_script_player());
    let embedded = app.engine.test_player(4);
    main_assert_eq!((embedded.player_info_id(), embedded.wealth(), embedded.score()) => (11, 29, 41));
    main_assert!(embedded.is_script_player());
}
...
fn change_to_local_during_savegame_recreation_retains_completed_profile_files() {
    ...
    let native =
        |bytes: &[u8]| clonk_engine::LegacyCString::from_bytes(bytes.to_vec()).test_value();
    let completed = n2_fixture!(player_resource: 31, native(b"Complete.c4p"));
    let pending = n2_fixture!(player_resource: 32, native(b"Pending.c4p"));
    let mut app = new_running_sandbox_app();
    ...
    app.netplay.admission_resources.register_lobby_resource(&completed);
    app.netplay.admission_resources
        .mark_complete(completed.id, completed_path.clone());
    app.netplay.admission_resources.register_lobby_resource(&pending);

    app.change_network_control_to_local(7);

    main_assert_eq!(app.netplay.admission_resources.complete_path(completed.id) => Some(completed_path.as_path()));
    main_assert!(matches!(app.netplay.admission_resources.status(pending.id), Some(AdmissionResourceState::Unavailable(AdmissionResourceUnavailable::TransferFailed))));
    let retained_client = app.netplay.control_clients.state(7).test_value();
    main_assert_eq!(retained_client.name.as_bytes() => b"Local survivor");
    main_assert_eq!(retained_client.nick.as_bytes() => b"Survivor nick");
}
...
fn ordinary_network_savegame_recreates_associated_user_player_with_local_control() {
    ...
    let native =
        |bytes: &[u8]| clonk_engine::LegacyCString::from_bytes(bytes.to_vec()).test_value();
    let resource = n2_fixture!(player_resource: 17, native(b"Alice.c4p"));
    let installed_script_resource =
        n2_fixture!(player_resource: 19, native(b"InstalledScript.c4p"));
    let unavailable_filename_resource = n2_fixture!(resource {
        resource_type: clonk_network::HostResourceType::Player as u8,
        id: 21,
        loadable: false,
    ...
    let filename_current = n2_fixture!(player {
        id: 92,
        savegame_player: 8,
    ...
    let filename_restore = n2_fixture!(player {
        id: 8,
        flags: clonk_engine::PLAYER_INFO_FLAG_JOINED
            | clonk_engine::PLAYER_INFO_FLAG_NO_SCENARIO_INIT,
        player_type: clonk_engine::PLAYER_INFO_TYPE_USER,
    });
    let missing_filename_current = n2_fixture!(player {
        id: 93,
        savegame_player: 12,
    ...
    let missing_filename_restore = n2_fixture!(player {
        id: 12,
        flags: clonk_engine::PLAYER_INFO_FLAG_JOINED
            | clonk_engine::PLAYER_INFO_FLAG_NO_SCENARIO_INIT,
        player_type: clonk_engine::PLAYER_INFO_TYPE_USER,
    });
    let missing_script_current = n2_fixture!(player {
        id: 9,
        savegame_player: 9,
    ...
    let missing_script_restore = n2_fixture!(player {
        id: 9,
        flags: clonk_engine::PLAYER_INFO_FLAG_JOINED
            | clonk_engine::PLAYER_INFO_FLAG_NO_SCENARIO_INIT,
        player_type: clonk_engine::PLAYER_INFO_TYPE_SCRIPT,
    ...
    let script_current = n2_fixture!(player {
        ...
        flags: clonk_engine::PLAYER_INFO_FLAG_HAS_RESOURCE,
        player_type: clonk_engine::PLAYER_INFO_TYPE_SCRIPT,
        resource: Some(n2_fixture!(player_resource: 18, native(b"ScriptPlr-10.c4p"))),
    });
    let script_restore = n2_fixture!(player {
        id: 10,
        flags: clonk_engine::PLAYER_INFO_FLAG_JOINED
            | clonk_engine::PLAYER_INFO_FLAG_NO_SCENARIO_INIT,
        player_type: clonk_engine::PLAYER_INFO_TYPE_SCRIPT,
    ...
    let player = app.engine.test_player(2);
    main_assert_eq!((player.player_info_id(), player.wealth()) => (7, 19));
    main_assert_eq!((player.name(), player.score()) => ("Alice current", 31));
    main_assert_eq!(player.at_client() => clonk_engine::PlayerAtClient::HOST);
    main_assert_eq!(player.at_client_name() => "Current client");
    ...
    main_assert_eq!((filename_player.player_info_id(), filename_player.name(), filename_player.wealth(), filename_player.score(),) => (8, "Bob current", 23, 37));
    let filename_info = app.players.infos.get(8).test_value();
    main_assert!(filename_info.resource.is_none());
    main_assert_eq!(filename_info.flags & clonk_engine::PLAYER_INFO_FLAG_HAS_RESOURCE => 0);
    let installed_script = app.engine.test_player(4);
    main_assert_eq!((installed_script.player_info_id(), installed_script.name(), installed_script.score(),) => (9, "Installed script player", 43));
    main_assert!(installed_script.is_script_player());
    let script_player = app.engine.test_player(5);
    main_assert_eq!((script_player.player_info_id(), script_player.name(), script_player.wealth(), script_player.score(),) => (10, "Current script player", 29, 41));
    main_assert!(script_player.is_script_player());
    ...
    main_assert!(app.engine.player(6).is_none());
    main_assert!(app.players.infos.get(12).is_some_and(|info| { info.flags & clonk_engine::PLAYER_INFO_FLAG_REMOVED != 0 }));
    main_assert!(app.engine.snapshot().round_results.players.iter().any(|result| result.player_info_id == 12 && result.score_old == 0));
    main_assert_eq!(
        app.engine
    ...
    let script_info = app.players.infos.get(10).test_value();
    ...
    main_assert!(script_info.filename.is_empty());
    main_assert!(script_info.resource.is_none());
    main_assert_eq!(script_info.flags & clonk_engine::PLAYER_INFO_FLAG_HAS_RESOURCE => 0);
    main_assert!(app.local_controls.assignment(2).is_some());
    main_assert!(app.local_controls.assignment(3).is_some());
    main_assert_eq!(app.engine.snapshot().hud.local_players => vec![2, 3]);
    main_assert_eq!(app.players.local_owner => 2);
    main_assert!(app.finish_recording().is_none());
    ...
    main_assert!(record.open_child("Recreate-10.c4p").expect("recorded embedded script profile").exists("Player.txt"));
}
...
fn regular_network_scenario_recreates_fileless_script_player_without_runtime_data() {
    ...
    main_assert!(player.is_script_player());
    main_assert_eq!(player.player_info_id() => 7);
    main_assert!(app.engine.snapshot().hud.local_players.is_empty());
    main_assert!(app.local_controls.assignment(10).is_none());
}
...
fn dragon_rock_network_restore_makes_script_npcs_hostile_to_joined_users() {
    ...
    let user_info_id = 2;
    let current_user = n2_fixture!(player {
        id: user_info_id,
    ...
    .test_value();
    app.apply_join_player_control(n2_fixture!(join_player:
        native(b"NetworkUser.c4p"),
        3,
        user_info_id,
        clonk_engine::JoinPlayerSource::Embedded(packed_player),
        0
    ))
    .test_value();
    ...
    main_assert_eq!((user.team(), script.team()) => (Some(1), Some(2)));
    main_assert!(user.is_hostile_towards(10));
    main_assert!(script.is_hostile_towards(user.id()));
}
...
fn network_savegame_finalization_does_not_rerun_scenario_initialize() {
    ...
    main_assert_ne!(restored_gravity => 77);
    install_probe(&mut savegame);
    savegame.finalize_network_loaded_scenario(true).test_value();
    main_assert_eq!(savegame.engine.physics().gravity => restored_gravity, "C4Game::InitGameFinal skips Script.Initialize for savegames");
    ...
    main_assert_eq!(fresh.engine.physics().gravity => 77);
}
...
fn runtime_join_into_committed_pause_finishes_initialization_without_starting_control() {
    ...
    app.netplay.client_start_barrier = ClientStartBarrier::from_join_data_status(pause_reference);
    main_assert_eq!(app.netplay.client_start_barrier.local_initialized_at(0) => Some(pause));
    app.netplay.pending_client_start_status = Some(pause_reference);
    ...
    events
        ...
        .test_value();
    app.process_network_events().test_value();
    main_assert!(app.netplay.ticks.ready.contains_key(&0));

    app.handle_status_committed(pause).test_value();

    main_assert_eq!(app.mode => AppMode::Running);
    main_assert!(app.scenario_lifecycle.loading.is_none());
    main_assert!(app.netplay.pending_client_start_status.is_none());
    main_assert!(app.netplay.host_reference_paused);
    main_assert!(!app.netplay.control_running);
    main_assert!(app.netplay.ticks.ready.contains_key(&0));
}
...
fn deferred_paused_runtime_join_finishes_after_player_resources_arrive() {
    ...
    app.netplay.runtime_committed_status = Some(pause);
...
fn deferred_runtime_join_waits_for_the_commit_matching_its_latest_status() {
    ...
    for (committed_state, committed_tick, prepared_state, prepared_tick) in [
        ...
        app.netplay.runtime_committed_status = Some(committed);
        ...
        app.process_network_events().test_value();
        main_assert!(
            app.netplay.sync.scheduled.contains_key(&1),
...
fn replacement_status_with_same_target_requires_a_fresh_runtime_join_commit() {
    ...
    main_assert!(app.netplay.runtime_committed_status.is_none());
    let prepared = app
        .scenario_lifecycle.loading
        .as_mut()
...
fn runtime_join_chase_retarget_reopens_a_reached_loading_barrier() {
    ...
    app.netplay.manager = Some(network);
    app.netplay.mode = Some(NetworkMode::Client(n2_client_settings()));
    app.netplay.control_clock = Some(NetworkControlClock::new(23, 1));
    app.mode = AppMode::Loading;
    ...
    app.netplay.client_start_barrier = ClientStartBarrier::from_join_data_status(reference);
    main_assert_eq!(app.netplay.client_start_barrier.local_initialized_at(23) => Some(reached));
    app.netplay.pending_client_start_status = Some(reference);
    let mut prepared = test_prepared_go(2, true, false, true, Vec::new(), Vec::new(), Vec::new());
    ...
    main_assert_eq!(app.netplay.pending_client_start_status => Some(chase));
...
fn runtime_network_client_join_loading_reaches_running_render() {
    ...
    let join_data = n2_fixture!(join_data:
        7,
        23,
        n2_fixture!(status: clonk_network::NETWORK_STATE_GO, 2, -1),
        dynamic,
        parameters
    );
    ...
    main_assert_eq!(initial.client_id => 7);
    ...
    let prepared = app
        ...
        .and_then(|loading| loading.prepared_go.as_ref())
        .test_value();
    main_assert!(prepared.network_runtime_join);
    main_assert_eq!(prepared.runtime_join_players.len() => 1);
    let acknowledgements = commands.take_framed_status_acknowledgements();
    main_assert_eq!(acknowledgements.len() => 1);
    main_assert_eq!(acknowledgements[0].0.state => go.state);
    main_assert_eq!(acknowledgements[0].0.control_mode => go.control_mode);
    main_assert_eq!(acknowledgements[0].0.target_tick => go.target_tick);

    n2_send_event(&event_tx, NetworkEvent::StatusCommitted(go));
    app.process_network_events().test_value();
    main_assert!(matches!(app.mode, AppMode::Running));
    main_assert!(app.netplay.control_running);
    ...
    main_assert!(rendered_bytes > 32, "rendered frame was effectively blank");
}
...
fn runtime_join_combined_save_recreates_players_in_save_player_info_order() {
    ...
    main_assert_eq!((first_player.wealth(), first_player.score()) => (1111, 111));
    main_assert_eq!((second_player.wealth(), second_player.score()) => (2222, 222));
    main_assert_eq!(first_player.at_client() => clonk_engine::PlayerAtClient::HOST);
    main_assert_eq!(second_player.at_client() => clonk_engine::PlayerAtClient::HOST);
    main_assert_eq!(first_player.at_client_name() => "Current client");
    main_assert_eq!(second_player.at_client_name() => "Current client");
    main_assert!(app.engine.players().all(|player| player.player_info_id() != 33), "the missing current client's whole SavePlayerInfos packet is skipped");
    main_assert_eq!(
        app.engine
            .players()
    ...
    main_assert_eq!(second_player.message_board_queries()[0].target => Some(object), "runtime object pointers denumerate against the loaded object graph");
    let restored_state = app.engine.capture_state();
    main_assert_eq!(restored_state.crew_info_rosters[&5][0].name => "Veteran", "the root embedded .c4p group and nested crew roster were loaded");
    main_assert_eq!(app.local_controls.assignment(7).expect("first local control").set => 0, "the first SavePlayerInfos row claims its preferred control set");
...
fn saved_raw_mouse_control_survives_a_failed_restore_preference_gate() {
    ...
    let save = n2_saved_game(
        &app,
        &scenario,
        "loaded mouse survives InitControl gate",
        engine_state,
    );
...
fn debug_key_gates_remaps_and_native_priority_body() {
    ...
    main_assert!(!denied.engine.debug_mode());
    ...
    main_assert!(missing_resources.engine.debug_mode());
    main_assert_eq!(runtime_flash_text(&missing_resources) => Some("[Undefined: IDS_CTL_DEBUGMODE]: [Undefined: IDS_CTL_ON]"));

    let mut disabled_binding = n2_app_with_key_config(
        new_running_sandbox_app(),
        b"[Keys]\nDbgModeToggle=None\n",
        "parse disabled debug binding",
    );
    disabled_binding.test_modifiers(ModifiersState::CONTROL);
    disabled_binding.test_key(VirtualKeyCode::F5, ElementState::Pressed);
    main_assert!(!disabled_binding.engine.debug_mode());
    main_assert!(disabled_binding.runtime_flash_message.is_none());

    let mut debug_collision = n2_app_with_key_config(
        new_running_sandbox_app(),
        b"[Keys]\nDbgModeToggle=G\nDbgShowVtxToggle=G\n",
        "parse same-priority debug collision",
    );
    debug_collision.engine.set_allow_debug(false);
    debug_collision.test_key(VirtualKeyCode::KeyG, ElementState::Pressed);
    main_assert_eq!(runtime_flash_text(&debug_collision) => Some("No debug mode!"));
    ...
    main_assert!(modified_player.engine.debug_mode());

    let mut context_priority = n2_app_with_key_config(
        new_running_sandbox_app(),
        b"[Keys]\nDbgModeToggle=R\n",
        "parse context/debug collision",
    );
    context_priority
    ...
    main_assert!(!context_priority.engine.debug_mode());
    main_assert!(context_priority.runtime_flash_message.is_none());

    let mut chat_priority = n2_app_with_key_config(
        new_running_sandbox_app(),
    ...
    main_assert!(!chat_priority.running_chat_active());

    let mut modified_chat_fallthrough = n2_app_with_key_config(
        new_running_sandbox_app(),
        b"[Keys]\nDbgModeToggle=Ctrl+Alt+G\n",
        "parse modified chat/debug collision",
    );
    ...
    main_assert!(modified_chat_fallthrough.engine.debug_mode());
    main_assert!(modified_chat_fallthrough.running_chat_active());

    let mut vote_priority = n2_app_with_key_config(
        new_running_sandbox_app(),
        b"[Keys]\nDbgModeToggle=Return\n",
        "parse vote/debug collision",
    );
    vote_priority
        .push_message_dialog(
            clonk_frontend::message_dialog::MessageDialogState::new(
    ...
    let mut game_over_priority = n2_app_with_key_config(
        new_game_over_keyboard_app(),
        b"[Keys]\nDbgModeToggle=Return\n",
        "parse game-over/debug collision",
    );
    game_over_priority.test_key(VirtualKeyCode::Enter, ElementState::Pressed);
    main_assert!(!game_over_priority.engine.debug_mode());
    main_assert!(game_over_priority.running_chat_active());
    ...
    let mut later_collision = n2_app_with_key_config(
        new_running_sandbox_app(),
        b"[Keys]\nDbgShowVtxToggle=G\nGameSpeedUp=G\n",
        "parse denied-debug/later-global collision",
    );
    later_collision.test_key(VirtualKeyCode::KeyG, ElementState::Pressed);
    main_assert!(!later_collision.engine.debug_mode());
    main_assert_eq!(later_collision.frame_skip => 2);
    main_assert!(later_collision.full_speed);
    main_assert_eq!(runtime_flash_text(&later_collision) => Some("Speed: 2x"));

    let mut rebound = n2_app_with_key_config(
        new_running_sandbox_app(),
        b"[Keys]\nDbgModeToggle=G\nDbgShowVtxToggle=H\n",
        "parse rebound debug keys",
    );
    ...
    let mut player_collision = n2_app_with_key_config(
        new_running_sandbox_app(),
        b"[Keys]\nKbd1Key1=G\nDbgModeToggle=G\n",
        "parse player/debug collision",
    );
    player_collision.test_key(VirtualKeyCode::KeyG, ElementState::Pressed);
    main_assert!(!player_collision.engine.debug_mode());
    main_assert!(player_collision.runtime_flash_message.is_none());

    let mut global_collision = n2_app_with_key_config(
        new_running_sandbox_app(),
        b"[Keys]\nSoundToggle=G\nDbgModeToggle=G\n",
        "parse earlier-global/debug collision",
    );
    ...
    main_assert_eq!(global_collision.test_audio_ref().options.sound_enabled => !sound_enabled);
    main_assert!(!global_collision.engine.debug_mode());
    main_assert!(global_collision.runtime_flash_message.is_none());
...
fn renderer_config_loads_native_defaults_and_graphics_values() {
    ...
    main_assert!(defaults.show_player_hud_always);
    main_assert!(defaults.splitscreen_dividers);
    main_assert!(defaults.fire_particles);
    main_assert!(defaults.pxs_gfx);
    ...
    main_assert!(!disabled.show_player_hud_always);
    main_assert!(!disabled.splitscreen_dividers);
    main_assert!(!disabled.fire_particles);
    main_assert!(!disabled.pxs_gfx);
    ...
    main_assert!(enabled.show_player_hud_always);
    main_assert!(enabled.splitscreen_dividers, "any nonzero divider integer follows C++ truthiness");
    main_assert!(enabled.fire_particles);
    main_assert!(enabled.pxs_gfx);
    ...
    .test_value();
    main_assert!(load_display_flags(Some(&paths)).splitscreen_dividers);
}
...
fn client_info_ack_marker_needs_a_host_and_an_unready_net_client() {
    let state = |status| n2_fixture!(runtime_client: 7, status, true, 0);
    for unready in [
        ...
        main_assert!(GameApp::runtime_client_row_unacknowledged(true, Some(&state(unready))));
        main_assert!(!GameApp::runtime_client_row_unacknowledged(false, Some(&state(unready))), "a client never renders the host-only marker");
    }
    main_assert!(!GameApp::runtime_client_row_unacknowledged(true, Some(&state(clonk_network::RemoteBarrierState::Ready))));
    main_assert!(!GameApp::runtime_client_row_unacknowledged(true, None), "the local row has no C4Network2Client to interrogate");
}
...
fn running_chat_netgetscen_saves_client_scenario_resource_like_cpp() {
    ...
    let host_config = clonk_network::HostConfig::default();
    let mut snapshot = host_config.initial_join_snapshot.test_value();
    snapshot.parameters.scenario = n2_fixture!(resource {
    ...
    app.netplay.pending_join_data = Some(n2_fixture!(join_data:
        7, 0, host_config.initial_status, snapshot.dynamic, snapshot.parameters
    ));

    // The resource has not arrived yet: C++ returns false and the command is
    // reported as unknown.
    main_assert!(app.save_joined_scenario_resource().is_none());

    app.netplay.admission_resources.resources.insert(
        9,
    ...
    app.netplay.manager = Some(host_network);
    app.netplay.mode = Some(NetworkMode::Host(host_network_settings()));
    app.process_running_chat_text("/netgetscen");
    main_assert!(!install_root.path().join("Remote.c4s").exists(), "the network host has the resource already");

    let (client_network, _client_events) = NetworkManager::test_stub_for_client_id(7);
    app.netplay.manager = Some(client_network);
    app.netplay.mode = Some(NetworkMode::Client(n2_client_settings()));
    app.process_running_chat_text("/netgetscen");
    ...
    main_assert_eq!(fs::read(&destination).expect("saved scenario") => b"packed scenario bytes");
    ...
    main_assert_eq!(app.save_joined_scenario_resource().as_deref() => Some(destination.as_path()));
    main_assert_eq!(fs::read(&destination).expect("overwritten scenario") => b"newer scenario bytes");
}
...
fn network_status_overlay_displays_live_protocol_rate_samples() {
    ...
    app.netplay.manager.test_ref().set_test_local_addresses([
        clonk_network::NetworkAddress::new(
            clonk_network::NetworkProtocol::Tcp,
            SocketAddr::from(([127, 0, 0, 1], 11_112)),
        ),
    ...
    let network = app.netplay.manager.test_ref();
    ...
    let protocols = text
        ...
        .test_value();
    main_assert!(protocols.contains(" i") && protocols.contains(" o") && protocols.contains(" bc"), "{protocols}");
    main_assert!(!protocols.contains("i0 o0 bc0"), "the overlay must show the sampled rates, not zero: {protocols}");
...
fn net_allow_join_toggle_key_toggles_the_live_host_gate() {
    ...
    app.test_key(VirtualKeyCode::F10, ElementState::Pressed);
    main_assert_eq!(app.netplay.runtime_join_allowed => Some(true));
    main_assert_eq!(app.runtime_flash_message.as_ref().map(|message| message.text.as_str()) => Some(labels.runtime_join_free.as_str()));
    app.test_key(VirtualKeyCode::F10, ElementState::Released);
    app.test_key(VirtualKeyCode::F10, ElementState::Pressed);
    acknowledgement.test_join();

    main_assert_eq!(app.netplay.runtime_join_allowed => Some(false));
    main_assert!(
...
fn runtime_client_list_maps_native_lifecycle_readiness_and_wait() {
    ...
    let state = |status, control_ready, wait_ms| n2_fixture!(runtime_client: 7, status, control_ready, wait_ms);
    for lifecycle in [
        clonk_network::RemoteBarrierState::Joining,
        clonk_network::RemoteBarrierState::Chasing,
        clonk_network::RemoteBarrierState::NotReady,
    ] {
        main_assert_eq!(GameApp::runtime_client_row_network_state(false, Some(&state(lifecycle, true, -12)),) => (RuntimeClientStatusIcon::Loading, Some(-12)));
    }
    main_assert_eq!(
...
fn network_status_collector_uses_native_client_next_control_baselines() {
    ...
    app.netplay.control_clock = Some(NetworkControlClock::new(44, 4));
    app.netplay.manager.test_ref().set_test_runtime_client_states([
        n2_fixture!(runtime_client:
            7, clonk_network::RemoteBarrierState::Ready, false, -12
        ),
        n2_fixture!(runtime_client:
...
fn runtime_f4_toggles_only_live_network_dialog_and_consumes_edges() {
    for (role, opens) in [
        ...
        main_assert!(after_release.pressed_engine_keys.is_empty(), "role {role:?}: the release clears the physical held-key latch");
        after_release
            .pressed_engine_keys
...
fn runtime_client_list_host_actions_submit_native_controls() {
    ...
    activate
        ...
        .test_value();
    main_assert_eq!(activate_commands.take_submitted_client_updates() => vec![n2_fixture!(client_update: clonk_engine::CLIENT_UPDATE_ACTIVATE, 7, 0, 0)]);

    let mut kick = new_running_sandbox_app();
    let (_events, mut kick_commands) = install_running_network_stub(&mut kick, 0, 40, 4);
    kick.netplay.control_clients
        .replace_snapshot([message_client(0, b"Host"), message_client(7, b"Remote")]);
    ...
    main_assert_eq!(
        ...
        vec![n2_fixture!(client_remove:
            7,
            clonk_engine::LegacyCString::from_bytes(b"kicked from client list".to_vec())
                .expect("fixture reason"),
            0
        )]
    );
...
fn f4_control_rate_dropdown_waits_for_authoritative_echo() {
    ...
    main_assert_eq!(app.netplay.control_clock.map(NetworkControlClock::control_rate) => Some(4));
    main_assert_eq!(
    ...
    main_assert_eq!(set => n2_fixture!(control_set: 0, 3, 0));

    app.apply_ready_controls(decided[0].0, vec![NetworkControl::Set(set)])
        .test_value();
    main_assert_eq!(app.engine.control_rate() => 7);
    main_assert_eq!(app.netplay.control_clock.map(NetworkControlClock::control_rate) => Some(7));
...
fn runtime_client_list_wheel_precedes_running_player_control() {
    ...
    main_assert!(matches!(submitted[0].1, ControlEvent::RawPlayerControl {command: clonk_engine::COM_WHEEL_DOWN, data: 0,}));
}
...
fn standalone_client_info_routes_wheel_and_keyboard_to_overflow() {
    ...
    let initial = app
        ...
        .test_value();
    main_assert!(initial.max_scroll > 0);
    ...
    let wheeled = app
        ...
        .and_then(|dialog| dialog.info_scroll_metrics(preferred, &fonts.text))
        .test_value();
    main_assert!(wheeled.scroll_y > initial.scroll_y);
    ...
    let ended = app
        ...
        .and_then(|dialog| dialog.info_scroll_metrics(preferred, &fonts.text))
        .test_value();
    main_assert_eq!(ended.scroll_y => ended.max_scroll);
...
fn runtime_client_list_status_refreshes_only_on_the_one_second_timer() {
    ...
    app.netplay.manager
        ...
        .set_test_runtime_client_states([n2_fixture!(runtime_client:
            7, clonk_network::RemoteBarrierState::Joining, true, -12
        )]);
    ...
    app.netplay.control_clock = Some(clock);
    ...
    let initial_remote = app
        ...
        .test_value();
    main_assert_eq!((initial_remote.status, initial_remote.wait_ms) => (clonk_frontend::runtime_client_list::RuntimeClientStatusIcon::Loading, Some(-12)));

    app.netplay.control_clock = Some(NetworkControlClock::new(50, 2));
    app.netplay.manager
        .test_ref()
        .set_test_runtime_client_states([n2_fixture!(runtime_client:
    ...
    let waiting_remote = app
        ...
        .find(|row| row.client_id == 7)
        .test_value();
    main_assert_eq!((waiting_remote.status, waiting_remote.wait_ms) => (clonk_frontend::runtime_client_list::RuntimeClientStatusIcon::NetWait, Some(9)));

    app.netplay.manager
        .test_ref()
        .set_test_runtime_client_states([n2_fixture!(runtime_client:
            7, clonk_network::RemoteBarrierState::Removing, true, 4
        )]);
    ...
    let removing_remote = app
        ...
        .find(|row| row.client_id == 7)
        .test_value();
    main_assert_eq!((removing_remote.status, removing_remote.wait_ms) => (clonk_frontend::runtime_client_list::RuntimeClientStatusIcon::Kick, Some(4)));
}
...
fn save_to_slot_writes_native_c4group_savegame() {
    ...
    app.players.infos.apply(netplay_player_info_data(
        ...
        vec![n2_fixture!(player {
            id: player_info_id,
            flags: clonk_engine::PLAYER_INFO_FLAG_JOINED,
            game_number: app.players.local_owner,
    ...
    main_assert_eq!(player_infos.clients.len() => 1);
    main_assert_eq!(player_infos.clients[0].players.len() => 1);
    let restore_plan = runtime_join_save::set_as_live_save_restore_infos(
        &app.netplay.control_clients.snapshot(),
        &player_infos,
        false,
    ...
    main_assert_eq!(saved.read_file("Source.bin").expect("copied source entry") => b"copied source sentinel");
    main_assert!(!saved.exists("Stale.txt"));
    for component in [
        "Parameters.txt",
        "Scenario.txt",
        "Game.txt",
        ...
        main_assert!(saved.exists(component), "missing native {component}");
    }
    for stale in ["Title.bmp", "Icon.bmp", "DescDE.rtf", "TitleUS.txt"] {
        main_assert!(!saved.exists(stale), "stale component survived: {stale}");
    }
    let scenario = String::from_utf8(saved.read_file("Scenario.txt").test_value()).test_value();
    main_assert!(scenario.contains("SaveGame=1\r\n"));
    main_assert!(scenario.contains("NoInitialize=1\r\n"));
    main_assert!(scenario.contains("Icon=11\r\n"));
    let game =
        clonk_engine::parse_initial_network_game_data(&saved.read_file("Game.txt").test_value());
    main_assert_eq!(game.frame => 37);
    main_assert_eq!(game.time => 12);
    ...
    main_assert_eq!((reader.info().width, reader.info().height) => (SAVE_THUMBNAIL_WIDTH, SAVE_THUMBNAIL_HEIGHT));
    main_assert!(!slot.with_extension("png").exists(), "native slot must not write a sidecar thumbnail");
...
fn network_quicksave_latency_report() {
    ...
    host.players.infos.apply(netplay_player_info_data(
        0,
        vec![n2_fixture!(player {
            id: player_info_id,
            flags: clonk_engine::PLAYER_INFO_FLAG_JOINED,
            game_number: host.players.local_owner,
...
fn native_save_rejects_a_joined_player_without_a_runtime_section() {
    ...
    main_assert!(error.to_string().contains("exact save has joined SavePlayerInfos IDs without matching Game.txt Player<ID> sections: 313"), "{error:#}");
    main_assert!(!destination.exists());
}
...
fn network_cleanup_records_cpp_network_error_bytes_before_clearing() {
    ...
    let engine_results = app.engine.snapshot().round_results;
    main_assert_eq!(engine_results.network_result => Some(clonk_engine::RoundResultsNetworkResult::NetworkError));
    main_assert_eq!(engine_results.network_result_message => b"Network: host Andr\xe9 disconnected!");
    main_assert_eq!(app.snapshot.round_results => engine_results, "cleanup presentation sees the network verdict before teardown");
}
...
fn a_lockstep_stall_announces_itself_once_after_a_grace_period() {
    ...
    main_assert!(app.runtime_flash_message.is_none(), "a stall must not be announced the instant it begins");

    app.announce_network_stall(start + Duration::from_millis(1_400))
        .test_value();
    main_assert!(app.runtime_flash_message.is_none(), "1.4s is still inside the grace period");

    app.announce_network_stall(start + Duration::from_millis(1_600))
        .test_value();
    main_assert!(app.runtime_flash_message.is_some(), "a stall lasting past the grace period must be announced");

    app.runtime_flash_message = None;
    app.announce_network_stall(start + Duration::from_millis(5_000))
        .test_value();
    main_assert!(app.runtime_flash_message.is_none(), "one notice per stall, not one per frame");

    // Clearing the stall arms the next one.
    app.netplay.stall_since = None;
    ...
    app.announce_network_stall(resumed + Duration::from_millis(1_600))
        .test_value();
    main_assert!(app.runtime_flash_message.is_some(), "a fresh stall must be announced again");
}
...
fn a_long_catch_up_still_draws_at_the_render_floor() {
    ...
    main_assert!(outcome.skip_redraw, "the floor must not fire before the backlog of undrawn frames reaches it");
...
fn configured_max_resource_search_recursion_reaches_client_candidates() {
    let load = |body: Option<&str>| {
        let (_root, _user_data, _guard, paths) = n2_empty_install_paths();
    ...
    // non-numeric value.
    main_assert_eq!(load(None) => (1, 1));
    main_assert_eq!(load(Some("[Network]\nName=Tester\n")) => (1, 1));
    ...
    main_assert_eq!(config.bootstrap_local_candidates.max_search_recursion() => 3);
    main_assert_eq!(
...
fn configured_network_work_path_controls_resource_staging_directory() {
    let staging = |body: Option<&str>| {
        ...
        main_assert_eq!(client => cache.join(&name));
        main_assert!(directory.starts_with(&cache));
        name
    };

    // The native default survives an absent config and an absent/empty key.
    main_assert_eq!(staging(None) => "Network");
    main_assert_eq!(staging(Some("[Network]\nName=Tester\n")) => "Network");
    main_assert_eq!(staging(Some("[Network]\nWorkPath=\n")) => "Network");
...
fn configured_thread_pool_count_builds_runtime_with_requested_workers() {
    ...
    main_assert_eq!(workers(None) => 8);
    main_assert_eq!(workers(Some("[General]\nName=Tester\n")) => 8);
    ...
    main_assert_eq!(network_runtime_worker_threads() => 3);
    set_network_runtime_worker_threads(0);
    main_assert_eq!(network_runtime_worker_threads() => DEFAULT_NETWORK_RUNTIME_WORKER_THREADS);
    main_assert_eq!(DEFAULT_NETWORK_RUNTIME_WORKER_THREADS => 8);
...
fn debug_classic_host_client_arguments_apply_cpp_lobby_ports() {
    ...
    if !DEBUG_CLASSIC_SHORTCUTS {
        ...
        let host = parse("/host");
        main_assert_eq!(host.network_active => None);
        main_assert_eq!(host.tcp_port => None);
        let client = parse("/client:0");
        main_assert_eq!(client.direct_join => None);
        return;
    }

    let host = parse("/host");
    main_assert_eq!(host.network_active => Some(true));
    main_assert_eq!(host.lobby_timeout => Some(None));
    main_assert_eq!(host.tcp_port => Some(11_112));
    main_assert_eq!(host.udp_port => Some(11_113));
    main_assert_eq!(host.master_server_signup => Some(false));
    main_assert_eq!(host.league_server_signup => Some(false));
    // `/host` does not set a join address; it is the host.
    main_assert_eq!(host.direct_join => None);
    // Case-insensitive, like SEqualNoCase.
    main_assert_eq!(parse("/HOST").tcp_port => Some(11_112));
    ...
    for (index, tcp, udp) in [
        ...
        main_assert_eq!(client.network_active => Some(true));
        main_assert_eq!(client.direct_join.as_deref() => Some("localhost"));
        main_assert_eq!(client.lobby_timeout => Some(None));
        main_assert_eq!(client.tcp_port => Some(tcp), "client {index} TCP");
        main_assert_eq!(client.udp_port => Some(udp), "client {index} UDP");
        // Unlike `/host`, signup state is left alone.
        main_assert_eq!(client.master_server_signup => None);
        main_assert_eq!(client.league_server_signup => None);
    }
    main_assert_eq!(parse("/CLIENT:1").tcp_port => Some(11_116));
    ...
    for value in ["", "abc", "0x2"] {
        ...
        main_assert_eq!(client.tcp_port => Some(11_114), "/client:{value}");
    }
    // A trailing suffix keeps the numeric prefix.
    main_assert_eq!(parse("/client:3rd").tcp_port => Some(11_120));
}
...
fn network_host_own_join_binds_the_local_presentation_to_its_player() {
    ...
    app.remove_local_control_assignment(app.players.local_owner);
    ...
    let info_id = 1;
    app.players.infos.apply(n2_fixture!(player_data:
        0,
        vec![n2_fixture!(player {
            id: info_id,
    ...
    app.apply_join_player_control(n2_fixture!(join_player:
        LegacyCString::from_bytes(b"Host.c4p".to_vec()).expect("valid legacy filename"),
        0,
        info_id,
    ...
    let joined = app
        ...
        .test_value();
    main_assert_eq!(app.engine.snapshot().hud.local_players => vec![joined], "the joined host player is the only local player");
    main_assert_eq!(app.players.local_owner => joined, "local presentation must follow the host's own joined player number");
}
...
fn network_client_routes_player_targeted_sound_only_to_its_local_player() {
    ...
    for client in [0, 3, 7] {
        app.netplay.control_clients.register(client, true, false);
    }
    ...
    for (info_id, at_client, name) in [
        ...
        app.apply_join_player_control(n2_fixture!(join_player:
            LegacyCString::from_bytes(format!("Player{info_id}.c4p").into_bytes())
                .expect("valid player filename"),
            at_client,
            info_id,
            clonk_engine::JoinPlayerSource::Embedded(packed_player.clone()),
            0
        ))
        .test_value();
    }
    ...
    let join_sounds = app
        ...
        .missing_sounds
        .iter()
        .filter_map(|request| request.strip_prefix("request::join"))
        .map(ToOwned::to_owned)
        .collect::<Vec<_>>();
    main_assert_eq!(
        join_sounds =>
        vec![local_player.to_string()],
        "InitializePlayer audio must use the join's authoritative local assignment; all attempts: {:?}",
        app.test_audio_ref().missing_sounds
    );
    main_assert_eq!(app.engine.snapshot().hud.local_players => vec![local_player], "remote players must not pass player-targeted sound's local-client gate");
    ...
    app.test_audio_mut()
        .missing_sounds
        .remove("request::warning_lowoxygen");
    app.engine
    ...
    main_assert!(
        !app
            .test_audio_ref()
            .missing_sounds
            .contains("request::warning_lowoxygen"),
...
fn an_observer_is_not_a_voice_source_and_a_multi_player_client_speaks_as_its_selected_one() {
    ...
    main_assert!(crate::voice_chat::authenticated_selected_voice_crew(&app.snapshot, local_client, local_player,).is_some());
    ...
    app.engine
        ...
        .set_at_client(clonk_engine::PlayerAtClient::new(local_client));
    app.snapshot = app.engine.snapshot();
    main_assert!(app.snapshot.players.iter().any(|player| player.id == observer && player.cursor.is_none()), "the observer holds no crew");
    main_assert!(crate::voice_chat::authenticated_selected_voice_crew(&app.snapshot, local_client, observer,).is_none(), "an observer has no position to mix from");
    main_assert!(app.local_voice_identity().is_none(), "and with no network attached this client is no local source either");
    ...
    .test_value();
    main_assert_eq!(second_source.id => second_cursor, "the claimed player is mixed from its own crew, not another local player's");
    ...
    .test_value();
    main_assert_ne!(first_source.id => second_source.id, "two local players are two independently positioned sources");

    // Ownership is still revalidated: the same player claimed by a different
    // client is not a source at all.
    main_assert!(crate::voice_chat::authenticated_selected_voice_crew(&app.snapshot, local_client + 1, second_player,).is_none());
}
...
fn muting_a_participant_discards_their_voice_locally_for_the_session() {
    ...
    let now = Instant::now();
    main_assert!(voice.accept_authorized_remote_frame(&frame, now).is_none());
    main_assert!(voice.remote_streams.is_empty());
    ...
    main_assert!(voice.accept_authorized_remote_frame(&restarted, now).is_none());

    // Another participant is unaffected.
    main_assert!(!voice.is_client_muted(noisy_client + 1));
