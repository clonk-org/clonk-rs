...
/// `C4FullScreen` gives its carrier window (C4FullScreen.cpp:474-480) and
/// every other caption built from the engine name. C++ spells it
/// "LegacyClonk"; the port is its own product, so it presents `PRODUCT_NAME`
/// everywhere a person can read it. Wire identities that stock peers and the
/// masterserver parse (`Game=LegacyClonk`, the league user agent) are not
/// captions and keep the C++ spelling.
pub const ENGINE_CAPTION: &str = PRODUCT_NAME;
...
pub const CONSOLE_CAPTION: &str = "Clonk Rust Console";
