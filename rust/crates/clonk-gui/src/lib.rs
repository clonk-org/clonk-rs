...
fn intern_image_data(
    ...
    height: u32,
    pixels: Arc<[u8]>,
) -> (Arc<[u8]>, clonk_graphics::GpuTextureId) {
    ...
    if pixels.len() > IMAGE_DATA_INTERNER_MAX_ENTRY_BYTES {
        return (pixels, clonk_graphics::GpuTextureId::fresh());
    }
    ...
    let gpu_texture_id = clonk_graphics::GpuTextureId::fresh();
    interner
        .entries
        .entry(key)
        .or_default()
...
impl ImageData {
    ...
    pub fn gpu_texture_id(&self) -> clonk_graphics::GpuTextureId {
        self.gpu_texture_id
    }

    pub fn gpu_texture_resource(&self) -> clonk_graphics::GpuTextureResource {
        clonk_graphics::GpuTextureResource::immutable_rgba(
            self.gpu_texture_id,
            self.width,
            self.height,
            Arc::clone(&self.pixels),
