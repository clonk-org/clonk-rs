...
/// One 8-bit-in / 8-bit-out gamma lookup per colour channel.
...
fn ramp_channel(c1: u8, c2: u8, c3: u8) -> [u8; 256] {
    let (c1, c2) = (i64::from(c1), i64::from(c2));
    let c3 = i64::from(c3) + 1; // "adjust clr3-value" (StdDDraw2.cpp:242)
    let size: i64 = 256;
    let size1 = size - 1;
    let denom = size1 * size1 / 512;
    let mut ramp16 = [0i64; 256];
    for i in 0..size / 2 {
        let i2 = size / 2 - i;
        let bend = (2 * c2 - c1 - c3) * 2 * i * i2;
        ramp16[i as usize] = (((c1 * i2 + c2 * i) * size1 + bend) / denom).clamp(0, 0xffff);
        ramp16[(i + size / 2) as usize] =
            (((c2 * i2 + c3 * i) * size1 + bend) / denom).clamp(0, 0xffff);
    }
    let mut out = [0u8; 256];
    for (entry, v) in out.iter_mut().zip(ramp16) {
        // MinGamma clamp, then 16-bit -> 8-bit framebuffer rounding.
        let v = v.clamp(0x100, 0xffff) as f32;
        *entry = (v / 257.0).round() as u8;
    }
    out
}
...
impl GammaRamp {
    ...
    pub fn standard() -> Self {
        let lut = ramp_channel(0x00, 0x80, 0xff);
        Self {
            red: lut,
            green: lut,
            blue: lut,
