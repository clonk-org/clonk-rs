...
use lc_script::{Engine, HostCallerStrictness, LocalCells, RuntimeError, Script, Value};

fn caller_strictness_code() -> i32 {
    match lc_script::caller_strictness() {
...
fn reentrant_vm_restores_the_outer_host_caller_context() {
    ...
    outer.register_host_function("OuterProbe", |_args| {
        if lc_script::caller_strictness() != HostCallerStrictness::Strict(2) {
        ...
        inner.register_host_function("InnerProbe", |_args| {
            if lc_script::caller_strictness() != HostCallerStrictness::NonStrict {
...
fn arrow_bridge_can_preserve_the_suspended_script_caller() {
    ...
    outer.register_method_dispatch(Arc::new(|_args| {
        if lc_script::caller_strictness() != HostCallerStrictness::Strict(3) {
