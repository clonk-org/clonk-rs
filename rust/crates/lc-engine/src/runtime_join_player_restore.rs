...
fn parse_message_board_query(
    ...
    let bytes = value.map(lc_script::c4_string_bytes)?;
    let mut source = skip_horizontal(&bytes).strip_prefix(b"(")?;
    let comma = source.iter().position(|byte| *byte == b',')?;
    let target = parse_i32_prefix(&lc_script::c4_string_from_bytes(&source[..comma]))
    ...
    let uppercase =
        parse_i32_prefix(&lc_script::c4_string_from_bytes(skip_horizontal(source)))? != 0;
    Some(MessageBoardQuery::new(
        target,
        lc_script::c4_string_from_bytes(&prompt),
        uppercase,
    ))
}
                ...
                external_player_paths.get(&source.info.id)
            {
                ...
                let entry = root_entries
                    ...
                    .ok_or_else(|| RuntimeJoinPlayerRestoreError::MissingPlayerGroup {
                        ...
                        filename: lc_script::c4_string_from_bytes(filename),
                    })?;
