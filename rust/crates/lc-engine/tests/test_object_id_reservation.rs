...
fn explicit_ids_should_not_conflict_with_auto_assigned_ids() {
    ...
    engine.register_definition(simple_definition("COWB")).unwrap();
    engine.register_definition(simple_definition("BUSH")).unwrap();

    // Create initial objects with auto-assigned IDs (like crew members)
    let cowb1 = engine.spawn_object(SpawnConfig::new("COWB".to_string())).unwrap();
    let cowb2 = engine.spawn_object(SpawnConfig::new("COWB".to_string())).unwrap();
    ...
    let result = engine.spawn_object(
        SpawnConfig::new("BUSH".to_string()).with_id(ObjectId::new(1))
    );
...
fn scenario_should_reserve_explicit_ids_before_spawning() {
    ...
    engine.register_definition(simple_definition("COWB")).unwrap();
    engine.register_definition(simple_definition("BUSH")).unwrap();
    engine.register_definition(simple_definition("TRPR")).unwrap();
    ...
    let obj_150 = engine.spawn_object(
        SpawnConfig::new("BUSH".to_string()).with_id(ObjectId::new(150))
    ).unwrap();
    ...
    let auto_obj = engine.spawn_object(SpawnConfig::new("COWB".to_string())).unwrap();
    ...
    let obj_1 = engine.spawn_object(
        SpawnConfig::new("TRPR".to_string()).with_id(ObjectId::new(1))
    ).unwrap();
