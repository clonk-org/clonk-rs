...
#[test]
fn regicide_assigns_unset_players_to_its_loaded_teams_before_scenario_init() {
    // Regicide's Teams.txt defines Left=1 and Right=2. Its InitializePlayer
    // immediately passes the callback's iTeam to _PLO::InitPlayer; zero leaves
    // the crew trapped and logs "No Team specified" (Regicide Script.c:34-40;
    // PlayerObject.c4d/Script.c:9-19). C++ assigns the player-info team before
    // C4Player::Init/ScenarioInit (C4PlayerInfo.cpp:810-816).
    let mut engine = load_installed_scenario("Knights.c4f/Regicide.c4s", 0);
    let first = join_local_player(&mut engine, "Regicide first default team");
    let first_team = engine
        .player(first)
        .and_then(|player| player.team())
        .expect("the first Regicide player receives a loaded team");
    ...
    let first_state = engine
        ...
        .expect("the first Regicide player remains live");
    assert!(
        first_state.cursor.is_some(),
        "Regicide must leave the joined player with a usable crew cursor"
    );
    assert!(
        first_state.crew.len() >= 3,
        "Regicide retains its three starting KNIG crew (and may add the team's KING)"
    );
    assert!(
        first_state.crew.iter().all(|&crew| {
            engine.object_snapshot(crew).is_some_and(|object| {
                object.status.is_active() && object.container.is_none()
            })
        }),
        "the assigned team's JoinTeam path must release every KNIG instead of trapping them in _PLO"
    );

    let second = join_local_player(&mut engine, "Regicide second default team");
    let second_team = engine
        .player(second)
        .and_then(|player| player.team())
        .expect("the second Regicide player receives a loaded team");
    assert_ne!(
        second_team, first_team,
        "Regicide balances unset joins across its two loaded teams"
    );
}
