...
use lc_engine::{Engine, ObjectId, COM_DIG, COM_DOWN, COM_LEFT, COM_RIGHT, COM_THROW, COM_UP};
...
fn tutorial02_virtual_player_completes_the_real_tutorial_route() -> Result<(), Box<dyn Error>> {
    ...
    player.wait_until("LOAM opens its real construction menu", 10, |engine| {
        engine
            ...
            .is_some_and(|(_, menu)| menu.identification == lc_script::Value::C4Id("LMMS".into()))
    })?;
    player.wait_until(
