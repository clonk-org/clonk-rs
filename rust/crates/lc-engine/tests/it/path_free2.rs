...
use lc_engine::{Definition, Engine, EngineError, Landscape, SpawnConfig};
use lc_script::Value;
...
fn path_free2_native_integer_slots_apply_legacy_falsy_conversion() {
    fn script_engine(source: &str) -> lc_script::Engine {
        let mut engine = lc_script::Engine::new();
        lc_engine::compat::register_host_functions(&mut engine);
