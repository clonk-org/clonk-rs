...
impl PlayerPropertiesController {
    ...
    pub fn handle_text_input(&mut self, text: &str) -> Vec<PlayerPropertiesAction> {
        ...
        let mut bytes = lc_script::c4_string_bytes(&self.player.name);
        for character in text.chars().filter(|character| !character.is_control()) {
            let encoded = lc_script::c4_string_bytes(&character.to_string());
            if bytes.len() + encoded.len() > PLAYER_NAME_MAX_BYTES {
                break;
            }
            bytes.extend(encoded);
        }
        self.player.name = lc_script::c4_string_from_bytes(&bytes);
        self.validation_error = None;
        Vec::new()
    }
...
fn truncate_c4_name(value: &str) -> String {
    ...
    for character in value.chars() {
        let size = lc_script::c4_string_bytes(&character.to_string()).len();
...
mod tests {
    ...
    fn name_edit_is_initially_focused_and_limited_to_thirty_c4_bytes() {
        ...
        assert_eq!(lc_script::c4_string_bytes(state.name()).len(), 29);
        assert!(!state.name().ends_with('Z'));
        state.set_name("123456789012345678901234567890more");
        assert_eq!(lc_script::c4_string_bytes(state.name()).len(), 30);
    }
