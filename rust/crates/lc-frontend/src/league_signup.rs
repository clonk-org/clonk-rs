...
impl EditState {
    ...
    fn insert_user_text(&mut self, input: &str) -> bool {
        ...
        for character in input.chars() {
            ...
            let Some(encoded) = lc_resources::encode_legacy_script_text(&character.to_string())
            else {
                continue;
            };
            if encoded
...
fn truncate_legacy_text(text: &str, limit: usize) -> String {
    ...
    for character in text.chars() {
        let Some(encoded) = lc_resources::encode_legacy_script_text(&character.to_string()) else {
            continue;
        };
