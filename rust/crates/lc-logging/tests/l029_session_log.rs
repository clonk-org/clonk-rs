...
fn l029_session_log_is_overwritten_and_verbose_tees_to_stderr() {
    if let Ok(mode) = env::var(CHILD_MODE) {
        ...
        lc_logging::init_verbose_with_file(mode == "verbose", &log_path)
        ...
        let err = lc_logging::init_verbose_with_file(false, &unused_path)
...
fn unique_temp_dir() -> PathBuf {
    let nonce = SystemTime::now()
        ...
        .as_nanos();
    env::temp_dir().join(format!("lc-logging-l029-{}-{nonce}", process::id()))
}
