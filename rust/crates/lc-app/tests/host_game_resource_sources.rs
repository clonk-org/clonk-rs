...
fn selected_definition_wire_name_retains_native_high_bytes() {
    ...
    let selected_module = lc_script::c4_string_from_bytes(expected_wire_name);
    assert_eq!(
        lc_script::c4_string_bytes(&selected_module),
        expected_wire_name
    );
...
#[cfg(not(unix))]
fn path_bytes(path: &Path) -> Vec<u8> {
    lc_script::c4_string_bytes(path.to_string_lossy().as_ref())
}
