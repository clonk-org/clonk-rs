...
use lc_app::ConfiguredClientPlayers;
use lc_engine::{
...
fn associate_offline_savegame_players(
    ...
    restore: &lc_network::PlayerInfoListSnapshot,
...
fn associate_offline_savegame_player_info(
    mut player_info: lc_engine::PlayerInfoControlData,
    ...
    let player_info = packets
        .pop()
        .map(
            |(client_id, flags, players)| lc_engine::PlayerInfoControlData {
                client_id,
                flags,
                players,
...
mod tests {
    ...
    use lc_network::{ClientPlayerInfosSnapshot, PlayerInfoListSnapshot};
