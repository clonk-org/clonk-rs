...
use lc_network::JoinDataEnvelope;
use thiserror::Error;
...
pub enum ClientNetworkScenarioError {
    ...
    OpenScenario {
        path: PathBuf,
        #[source]
        source: lc_resources::GroupError,
    },
    ...
    #[error(transparent)]
    Combine(#[from] lc_resources::NetworkScenarioError),
}
...
pub fn compose_client_network_scenario(
    ...
    let scenario = lc_resources::Group::open(&resources.scenario.path).map_err(|source| {
        ClientNetworkScenarioError::OpenScenario {
            path: resources.scenario.path.clone(),
            source,
        }
    })?;
    let dynamic = lc_resources::Group::open(&resources.dynamic.path).map_err(|source| {
        ClientNetworkScenarioError::OpenDynamic {
            path: resources.dynamic.path.clone(),
            source,
        }
    })?;
    lc_resources::combine_network_scenario(&scenario, &dynamic, output_filename, maker)
        .map_err(Into::into)
}
...
mod tests {
    ...
    fn cpp_client_combines_completed_scenario_and_dynamic_paths() {
        ...
        let mut scenario = lc_resources::MutableGroup::new("Scenario.c4s");
        scenario
        ...
        let mut dynamic = lc_resources::MutableGroup::new("Dynamic.c4s");
        dynamic
        ...
        let combined = lc_resources::Group::from_memory("Combined2.c4s".into(), packed)
    ...
    fn join_data(
        ...
        let empty_player_infos = || PlayerInfoListSnapshot {
            last_player_id: 0,
            clients: Vec::new(),
        };
        JoinDataEnvelope {
