...
const DEFAULT_RES_X: u32 = 960;
const DEFAULT_RES_Y: u32 = 540;
...
mod tests {
    ...
    fn display_options_apply_config_parses_values() {
        ...
        cfg.set_in(Some("Graphics"), "ResX", "1280");
        cfg.set_in(Some("Graphics"), "ResY", "720");
...
impl DisplayOptions {
    ...
    pub fn actual_size(&self) -> (u32, u32) {
        ...
        let width = ((self.base_width as f32) * self.scale).round().max(min);
        let height = ((self.base_height as f32) * self.scale).round().max(min);
    ...
    pub fn record_actual_size(&mut self, width: u32, height: u32) {
        ...
        let new_width = ((width as f32) / scale).round().clamp(min, max);
        let new_height = ((height as f32) / scale).round().clamp(min, max);
    ...
    pub fn persist_if_dirty(&mut self, paths: &AppPaths) {
        ...
        config.set_in(Some("Graphics"), "ResX", self.base_width.to_string());
        config.set_in(Some("Graphics"), "ResY", self.base_height.to_string());
    ...
    fn apply_config(&mut self, config: &Config) {
        if let Some(raw) = config.get_in(Some("Graphics"), "ResX") {
        ...
        if let Some(raw) = config.get_in(Some("Graphics"), "ResY") {
            if let Ok(parsed) = raw.trim().parse::<i32>() {
                if parsed > 0 {
