...
use lc_engine::scenario::ScenarioLoaderHead;
use lc_resources::{compress_c4group_image, merge_extracted_group_entries, Group, MutableGroup};
...
pub(crate) fn convert_classic_record_stream(
    ...
    let stream = lc_network::decode_classic_record_stream(&compressed)
...
fn persist_directory_record(
    origin_path: &Path,
    initial: &Group,
    files: Vec<lc_network::ClassicRecordStreamFile>,
...
mod tests {
    ...
    use lc_engine::{LegacyCString, RCT_FRAME};
    ...
    fn classic_record_stream_rewrites_a_packed_origin_like_cpp() {
        ...
        let mut raw = lc_network::encode_league_stream_file_chunk(&initial_name, &initial)
