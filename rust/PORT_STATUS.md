...
| host-fn backlog | ShiftContents/GrabObjectInfo semantics partial; HORS action-start target-zero; BlastObject unported (see findobject-ocf); DoDamage/DoEnergy silently no-op on foreign targets (C++ damages them — BlastObjects' living-target hits; Punch routes energy through the same scope write, so its LastEnergyLossCausePlayer kill tracing is not carried either); nil-target AddEffect never ran Fx*Start/Timer for System.c4g ShakeEffect in the Goldrush headless probe (Distance/SetViewOffset were unreachable until 2026-07-03) |
