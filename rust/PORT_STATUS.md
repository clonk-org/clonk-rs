...
| **effects** | partial (timer semantics DONE 2026-06-10) | DONE: C++ modulo timer (monotonic iTime, zero interval never fires, verbatim iIntervall/iTime incl. AddEffect default 0), kill on C4Fx_Execute_Kill and on elapsed timerless intervals (Stop callback runs), FxStart C4Fx_Start_Deny (dead without Stop), list order ascending by |priority| (C4Effect.cpp:80-94). Open: builtin fire/helper effects (Splash/Smoke/Explosion/BubbleOut), the Fx*Effect `Check()` priority-conflict chain (Effect_Deny/Annul), TempRemove/TempReadd, command-target def resolution for Fx* callbacks (cross-def effects call the host def script). | `effect.rs`, `lib.rs` |
