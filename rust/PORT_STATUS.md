...
| **effects** | partial (35%, 195 vs 921) | Builtin fire effect (300+ LOC) + helper effects (Splash/Smoke/Explosion/BubbleOut) missing. No `Check()` priority-conflict, no `TempRemove/TempReadd`. Timer FIXED to the C++ modulo model (2026-06-09): monotonic `iTime`, fires on `iTime % iIntervall == 0`, zero interval never fires; kill-when-no-timer-fn (C4Effect.cpp:355-357) still needs the Fx* dispatch site. Dispatch infra exists but never invoked for builtins. | `effect.rs`, `lib.rs:5175+,5272+` |
