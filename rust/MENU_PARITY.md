...
| Rust generic startup status route | no C++ screen; `GameApp::reject_generic_startup_status` | **Fail-fast** | Arbitrary Rust status is retained for diagnostics but never rendered. Menu presentation checks every `StartupView` before cache replay or base composition, and the scale-native main-menu text pass independently checks the same typed, logged boundary. Table-driven all-view, cache-preservation, native-pass and post-connect regressions cover the route. Individual producers still need their classic child dialog/progress states for completion. |
...
| New Player / Properties form | `C4StartupPlrPropertiesDlg` | **Fail-fast** | Current actions retain diagnostic status and presentation returns the typed, logged startup-status boundary before drawing. Implement name, colors, control set, mouse, movement mode, portrait and validation. |
| Portrait selector | `C4PortraitSelDlg` | **Missing** | Location combo, image grid, flags, OK/Cancel. |
| Crew mode and crew detail list | `SetCrewMode`, crew list classes | **Fail-fast** | Current route retains diagnostic status and presentation returns the typed, logged startup-status boundary before drawing. Implement participation, stats, portraits and sorting. |
...
| Update button and update controller | `C4UpdateDlg` | **Fail-fast** | Rust retains the unported-update diagnostic and presentation returns the typed, logged startup-status boundary before drawing. Implement lookup/wait, redirect, available/no-update/error and apply/log transitions. The reusable `C4DownloadDlg` screen is tracked separately below. |
...
|---|---|---|---|
| Game over/evaluation | `C4GameOverDlg` | **Partial** | The eight unconditional core/dialog/icon resources (`CStdFont/Endeavour.ttf`, `GUICaption.png`, `GUIButton.png`, `GUIButtonDown.png`, processed `GUIButtonHighlight.png`, `GUIIcons.png`, `Player.png` and the HUD `Score.png` source) now have one complete ordered fail-fast preflight before viewport collection, cached-frame replay or pixel output; production app paths pass only the proven classic bundle and cannot enter the generic fallback. Later unported branches still need a recursive resource inventory for `GUIIcons2.png`, `Crew.png`, `GUIScroll.png` and dynamic definition/player images. Global/per-player custom evaluation, two-team lists, team/league rows, live network result/streaming updates and exact host/film button policy also remain. |
| Game-over chat input | `C4GameOverDlg.cpp:317-321`; `C4MessageInput` | **Fail-fast** | Exact bare Return/F2 and Shift+Return reach typed, logged All/Allies boundaries before any evaluation button can activate; Logo is outside the C++ modifier mask and keypad Enter is not silently aliased on the macOS SDL route. Exact Alt/Alt+Shift probes first reach a separate typed mnemonic boundary because SDL derives a letter from the key name and visible localized buttons outrank the global Say binding (for example Return can activate `&Restart`). The recursive edit screen, history, completion, paste/clipboard context popup, submit/cancel lifecycle and network transport remain unported. The shared command-language descendants are inventoried below; notably `/chart` opens the tabbed `C4ChartDialog` and league `/kick` queues a `C4VoteDialog` with Yes/No dispatch. |
| Game-over focus and keyboard lifecycle | `C4GUI::Dialog`, `C4GameOverDlg` | **Missing** | The representable initial no-focus state keeps arrows and Space inert even after pointer hover, releases are consumed, and exact Tab/Shift+Tab reach typed forward/backward traversal boundaries. Exact Alt/Alt+Shift keys reach the higher-priority mnemonic boundary; other Ctrl-modified Tab and modified Escape combinations remain inert, while bare Escape retains End. True none/list/button focus ownership, focus-versus-pointer visual highlight separation, traversal, localized mnemonics, button keyboard press/release visuals and activation remain missing. |
