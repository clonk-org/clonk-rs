...
|---|---|---|---|
| Main six-button screen | `C4StartupMainDlg.cpp:38-122`; `rust/crates/lc-frontend/src/startup_main_menu.rs` | **Partial** | The classic path has strong pixel/input coverage, but the app can still select BitmapFont, optional-image, blue-pane and solid-button substitutes. It is not complete while those paths are reachable. |
| Main screen font/resource failure boundary | `C4StartupMainDlg`; startup graphics resources | **Missing** | `lc-app/src/main.rs` and `startup_main_menu.rs` currently render substitutes when classic fonts/images are unavailable. Log the missing resource and return an error before rendering. |
...
| Rust editor scenario/controller route | no C++ startup-list equivalent | **Missing** | `ScenarioKind::Editor`, the touch route, `StartupMenuAction::EditEntry`, and the external-editor dispatch remain reachable; they do not fail fast. Reject the route with a logged error until the real developer UI exists. |
...
| Folder-map view (`FolderMap.txt`) | `C4StartupScenSelDlg.cpp:47-404,1016-1023`, `C4MapFolderData`; shipped `content/Western.c4f/FolderMap.txt` | **Missing** | A reachable shipped folder is silently rendered as the ordinary book. Port its background, scenario buttons, overlays, access graphics and map info pane, or fail before drawing it. |
...
| Abort/restart/no | `C4GameDialogs.cpp:33-128`, `C4AbortGameDialog` | **Partial** | A menu-shaped approximation is reachable. Until the exact dialog, halt state, restart policy and cancel semantics exist, this action must log and fail instead of rendering the approximation. |
| Mouse hit testing/scrollbars/tooltips | `C4Menu` | **Partial** | Keyboard navigation works; pointer/scroll behavior incomplete. |
| In-game/script menu font and sheet boundary | `C4GUI::Resource`, `C4Menu::Draw`; `lc-frontend/src/ingame_menu.rs`, `lc-app/src/main.rs` | **Missing** | Missing base sheets/fonts currently select generic frames/fonts in several paths. Validate all required resources and return an error before any menu/HUD fallback is drawn. |
...
| Script-menu resource/font fallback boundary | `C4Menu::Draw`, `C4GUI::Resource`; `ingame_menu.rs` | **Missing** | Even strong style-specific renderers are not complete while app rendering can substitute generic fonts, frames or sheets. Preflight and fail before drawing every style. |
...
| Loader/progress/log screen | `C4LoaderScreen.cpp:126-177`; `lc-app/src/main.rs` | **Partial** | A reachable Rust approximation draws, but scenario title, log region, progress/process bars and copyright layout do not match. |
| Loader generic-fallback boundary | `C4LoaderScreen` | **Missing** | Boot can render the inaccurate pane without an error. Until the exact loader lands, log the boundary and return an error instead of drawing it. |
| Object/global game messages (`C4GameMessage`) | `C4GameMessage.cpp:38-231` | **Partial** | Rust draws text in a generic font/dark frame. Port global/object positioning, target visibility, lifetime/fade, wrapping, color, portrait, decoration, flags and per-player clipping; fail before generic rendering when assets are absent. |
...
| Rust-only F5 quick save | no C++ menu equivalent; C++ uses `C4MainMenu::ActivateSavegame` | **Missing** | The active shortcut bypasses the classic ten-slot flow. Remove/guard it or prove an oracle-equivalent route. |
| Rust-only F9 quick load | conflicts with C++ `C4Game.cpp:3373-3374` | **Missing** | C++ F9 takes a screenshot; Rust quick-loads. Restore screenshot behavior and keep loading in an oracle-backed menu. |
...
| Rust synthetic menu sandbox | no C++ player-facing equivalent; `lc-app/src/main.rs` | **Missing** | A synthetic scenario/menu route remains in the binary. Remove it from player-facing dispatch or reject it with a logged error. |
