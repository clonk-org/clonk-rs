...
| Scenario rename | `ScenListItem::KeyRename`, `Entry::RenameTo` | **Complete** | F2 opens a selected-all inline edit; Escape/empty/same-title abort, valid commits regenerate the type extension, move ordinary or nested packed entries, replace `Title.txt`, rediscover/resort, reselect and refocus. Collisions and storage failures remain modal and nonfatal. |
