...
| Screen or recursive child | C++ authority | Rust status | Remaining work |
|---|---|---|---|
| Credits page/chrome | `C4StartupAboutDlg` | **Partial** | The seven developer TextWindows keep independent scroll offsets, clip partial rows, route wheel input and implement fixed-pin arrow repeat plus captured track/thumb dragging with classic pressed facets and sounds. Common-dialog traversal/tooltips remain. |
...
| Bottom lobby game-option strip | `C4GameOptionButtons`; `game_option_buttons.rs` | **Partial** | Exact Host/Client contexts, focus, sounds and countdown/forced locks are app-wired. Internet performs live masterserver Start/End with enable rollback and league-off on disable; password set/clear updates host admission and the advertised reference transactionally; comment persists, invalidates, logs and plays Connect; Fair Crew emits `CID_Set(C4CVT_FairCrew)` and updates on its synchronized echo; Record remains local/config-backed. League is intentionally display-only/disabled in the C++ lobby. Remaining lifecycle gaps are responsive Retry/Abort/cancellation while Start/End waits, applying a dynamic Start response, and initializing the puncher when signup is enabled live. |
...
| Player row context root | `C4PlayerInfoListBox.cpp:490-533` | **Complete** | The lobby always opens the root, including empty unauthorized roots, with exact conditional Take Over, Remove and New Color ordering, permissions, localized tooltips, icons and accelerators. Nested Take Over enumeration remains separately tracked below. |
...
| Free-savegame player group | `C4PlayerInfoListBox.cpp:1112-1143` | **Partial** | Component presentation and the Take Over root entry are complete; nested takeover enumeration and restoration remain. |
