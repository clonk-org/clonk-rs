...
| Sound | sound sheet controls | **Fail-fast** | Typed entry boundary precedes cache/pixels. Process-global startup/loading F3 and Ctrl+F3 now mutate the distinct `FEMusic`/`FESamples` analogues with no in-game flash on every startup root; bare F3 also retains the Options dialog's higher-priority route. The Sound sheet checkbox model/render, volume sliders/test sound, direct checkbox callbacks and config-save lifecycle remain. |
