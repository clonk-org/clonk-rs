...
| Timed flash-message overlay | `C4GraphicsSystem::FlashMessage`, `DrawFlashMessage` | **Missing** | Port timing, placement below the upper board/multiple viewports and background invalidation. Recursive producers include observer/gamepad prompts, runtime-join state, speed changes, debug toggles, control-rate feedback (`C4Control.cpp:143`), fair-crew activation (`C4Control.cpp:226,229`), script `TargetFPS` (`C4Script.cpp:5705`) and adaptive `PreSend`/`TargetFPS` feedback (`C4GameControlNetwork.cpp:444`). Generic startup/running status text is not a substitute. |
