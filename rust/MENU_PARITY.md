...
| Dialog style | `C4MN_Style_Dialog` | **Partial** | Four direct shipped creation sites fan out through Hazard, LastWill, and Western dialogue helpers. Production uses the classic variable-height layout, portrait column, empty-title rule, pointer targets, selection/progress behavior, and decoration path; unresolved requested images or decoration sheets fail fast. Remaining: real-scenario render fixtures, exact rank/index/object-rank symbols, and the optional extra footer. |
...
| Rank/Indexed/ObjRank/Object/TextSpec/Color symbols | `AddMenuItem` image grammar | **Partial** | All recipes and arguments survive `AddMenuItem`; all 75 shipped definition portraits are recursively inventoried/loaded in group order, named Western portraits resolve case-insensitively, TextSpec fallback color is honored, and current Set/GetPortrait names are retained. Remaining: exact indexed/rank/object-rank rendering, fallback presentation definition, cached-at-add snapshots, icon grammar, exact portrait-name validation/random choice, persisted crew portraits, and current-vs-permanent portrait state. |
| Menu decoration | `SetMenuDecoration` | **Partial** | `SetByDef` snapshots callback-derived color/borders and all eight `FrameDeco*` facets immediately; Dialog rendering applies packed alpha, tiled/truncated edges, protruding corners, and captured client margins. Remaining: real LastWill/Western render fixtures across resolutions/scales. |
