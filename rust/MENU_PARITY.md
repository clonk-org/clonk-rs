...
| Dialog style | `C4MN_Style_Dialog` | **Partial** | Four direct shipped creation sites fan out through Hazard, LastWill, and Western dialogue helpers. Production uses the classic variable-height layout, portrait column, empty-title rule, pointer targets, selection/progress behavior, and decoration path; unresolved requested images or drawable decoration sheets fail fast. Remaining: real-scenario render fixtures, exact rank/index/object-rank symbols, and the optional extra footer. |
