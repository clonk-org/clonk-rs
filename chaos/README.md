...
**Report-only.** `docs/PERFORMANCE.md` ("Baseline collection") requires ~20
comparable samples before a timing becomes a blocking threshold, so `verify`
prints deltas and does not fail on them. Two things *do* fail, because neither is
...
`slow-cpu-only` and `bad-link-only` isolate each. The first run of this suite
found that dial-up alone costs the healthy players almost nothing while a
Pi-class machine alone costs them ~10 s of drift over an 11 s session — the
opposite of the intuition that led to the work.
...
- **The host's uplink is not shared between peers.** Each participant has its own
  ...
  where a larger game degrades first. Modelling it is the next thing this harness
  needs.
...
- The transport-level view (`clonk_network::sim`) and the session view
  ...
  `ControlCoordinator` over a modelled link.
